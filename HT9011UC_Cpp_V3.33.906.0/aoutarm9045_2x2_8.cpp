// =============================================================================
//  aoutarm9045_2x2_8.cpp  --  Out-Arm per-site VARIANT (2x2_8, 2-row 2-col 8-picker)
//
//  Faithful translation of golden aoutarm9045_2x2_8.cpp (BCB6, Big5/cp950).
//  Translator: AI(W6.2c-OUT-2x2_8) 20260627
//  Translation wave: W6.2c (out-arm per-site-config variant: 2x2_8).
//
//  ROLE: the per-site out-arm shuttle-pick / place-to-auto state machines for the
//  2x2_8 layout.  This file owns (golden names VERBATIM):
//    GetNowShuttleMode_2x2_8, OutArmZNeedDown_2x2_8, CheckOutArmXYPitch_2x2_8,
//    MoveOutArmToShuttleIncludeZ_9045_2x2_8, DoPickFromShuttle_9045_2x2_8,
//    DoOutArm_9045_2x2_8.
//
//  ENGINE DISPATCH (LIVE variant): the out-arm engine aoutarm9045.cpp routes
//  iInArmType==e9045_2x2_8_Hot -> DoOutArm_9045_2x2_8() and
//  DoPickFromShuttle_9045_2x2_8(int) via the already-active dispatch ladders.
//  The file layout suffix == the callee suffix (ENUM->CALLEE: 2x2_8_Hot -> 2x2_8).
//
...
//  CP950 source.  The DoOutArm_9045_2x2_8 case-1/5/10/50/100, case-1140->1150,
//  case-2040->2050, case-3100->3300, case-3301->3500 fall-throughs are preserved
//  EXACTLY.  DoPickFromShuttle case-10->case-200 fall-through preserved EXACTLY.
//
...
//
//  INCLUDE REWRITE (mirror of the golden header set, translated subset).  Golden
//  golden set: MachineDefine.h ; aoutarm9045_2x2_8.h ; aArmHeader.h ; MyKitSuck.h ;
...
//==============================================================================
#ifndef OUTARM_2x2_8_FWD_GUARD
#define OUTARM_2x2_8_FWD_GUARD
...
#endif // OUTARM_2x2_8_FWD_GUARD

//==============================================================================
//  GetNowShuttleMode_2x2_8  -- golden aoutarm9045_2x2_8.cpp:37
//  輸出臂 x y 軸移到 shuttle 1   Ver : 2003_07
//==============================================================================
int GetNowShuttleMode_2x2_8(int iSht)
{
...
//==============================================================================
//  OutArmZNeedDown_2x2_8  -- golden aoutarm9045_2x2_8.cpp:94
//==============================================================================
bool OutArmZNeedDown_2x2_8(int iSht)
{
    if(InitialOutArmNeedSuck(iSht)==false)
        return false;

    ptrOutSHT=(iSht==0)?&FRCarryKit:&BRCarryKit;
    int iMode=GetNowShuttleMode_2x2_8(iSht);
    ...
    for(int i=0; i<OutArmSuck.iShtRow; i++)
    {
        for(int j=0; j<OutArmSuck.iShtCol; j++)
        {
            ...
            {
                SetOutArmNeedDestory(i, iPickJ, i, j);
            }
...
//------------------------------------------------------------------------------
//  CheckOutArmXYPitch_2x2_8  -- golden aoutarm9045_2x2_8.cpp:130
//------------------------------------------------------------------------------
bool CheckOutArmXYPitch_2x2_8(int *iX, int *iY, int iSht, int iMovePitchX, int iMovePitchY)
{
    int iMode=GetNowShuttleMode_2x2_8(iSht);
    int iModeRow=iMode%100;
    int iModeCol=iMode/100;
    ...
    else if(USE_OUT_ARM_Y_PITCH==iXYPitchVariable)                              //基準為第二隻吸嘴  //JerryYang 20251218 : IN/OUT ARM支援不同模組
    {
        ...
        if(iModeCol==0)
        {
            ...
            else                                                                //Aa --> Aa
            {
                ...
                {
                    *iX=*iX+(dMovePitchX*0-TestIF.dSiteXPitch*0.5);
                }
            }
        }
        else if(iModeCol==100)
    ...
    else                                                                        //基準為第三隻吸嘴
    {
        ...
        if(iSht==1)                                                             //Sam 20250217 : 修正 2X2 8 吸嘴模式
             *iX=*iX-dMovePitchX;                                               //Sam 20250424 : 修正 2X2 8 吸嘴模式
    ...
    *iX=*iX+GetOutArmXToShuttleOffset_9045(iSht, iOffsetPos);
    ...
    if(OutArmZNeedDown_2x2_8(iSht)==false)
        return false;
    return true;
}
//==============================================================================
//  MoveOutArmToShuttleIncludeZ_9045_2x2_8  -- golden aoutarm9045_2x2_8.cpp:303
//==============================================================================
bool MoveOutArmToShuttleIncludeZ_9045_2x2_8(int iSht, bool bZDown)
{
    ...
    if(CheckOutArmXYPitch_2x2_8(&iXPos, &iYPos, iSht, iMovePitchX, iMovePitchY)==false && bZDown==true)
        return false;
...
//==============================================================================
//  DoPickFromShuttle_9045_2x2_8  -- golden aoutarm9045_2x2_8.cpp:369
//  輸出臂從 Shuttle 1 吸取 IC 流程動作   Ver : 2003_07
//==============================================================================
    ...
    switch(Task)
    {
        case 1:
            if(ArmSpeed[OutArm].dWaitOnSH!=0)                                   //kevin 20210525 out arm 起動在shuttle 上等待 時間
            {
                ...
                {
                    if(MoveOutArmToShuttleIncludeZ_9045_2x2_8(iSht))            //移到 Shuttle 上等待
                    {
        ...
        case 10:
            ...
            Task=200;
        case 200:
            if(MoveOutArmToShuttleIncludeZ_9045_2x2_8(iSht, true))              //kevin 20210717 change function
            {
...
//==============================================================================
//  DoOutArm_9045_2x2_8  -- golden aoutarm9045_2x2_8.cpp:631
//==============================================================================
...
//                        #endif
        ...
        case 1150:
            ...
            if(MoveOutArmToShuttleIncludeZ_9045_2x2_8(iWhichShuttle))
            {
                IsCatchTrayReadySupplyNewTray();

                if(OutArmSuck.iXStep!=1 || OutArmSuck.iYStep!=1)
                {
                    iBackMode1=GetNowShuttleMode_2x2_8(0);
                }
...
//#endif
                ...
                if(OutArmSuck.iXStep!=1 || OutArmSuck.iYStep!=1)
                {
                    iMode1=GetNowShuttleMode_2x2_8(0);
...
//                if(WhichAutoNeedTray()==0)                                      //JerryYang 20170731 (wei) Out arm要讓位才不會hang up
        ...
        case 1200:
            ...
            if(DoPickFromShuttle_9045_2x2_8(0))
            {
...
//                        #endif
        ...
        case 2050:
            ...
            if(MoveOutArmToShuttleIncludeZ_9045_2x2_8(iWhichShuttle))
            {
                ...
                if(OutArmSuck.iXStep!=1 || OutArmSuck.iYStep!=1)
                {
                    iBackMode2=GetNowShuttleMode_2x2_8(1);
                }
                Task=2100;
            }
...
//#endif
                ...
                if(OutArmSuck.iXStep!=1 || OutArmSuck.iYStep!=1)
                {
                    iMode2=GetNowShuttleMode_2x2_8(1);
        ...
        case 2200:
            ...
            if(DoPickFromShuttle_9045_2x2_8(1))
            {
                PreSetOutAdditionalFlag();                                                                              //Steven 20220614 : Fixed for out arm additional function
...
//#endif
        ...
        case 5300:
            if(iPickFinish==0x01)
            {
                flag3=MoveOutArmToShuttleIncludeZ_9045_2x2_8(1, false);
            }
            else
            {
                flag3=MoveOutArmToShuttleIncludeZ_9045_2x2_8(0, false);
            }
