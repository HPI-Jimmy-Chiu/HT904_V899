...
## 20260819 清晨 II — FW-BinSelUnlock（255ae7a）：28 個消費端 gate 全解

- 兩線並行（檔案不重疊）：主迴圈解 Command.cpp 11 塊（含 SetOSBIN 去
  #else 替代、3 個 Sender-ignored NULL 替代）；代理解 SECSGEM
  uHGemHT9045_EC 17 區塊 147 條 SetECDataPointer（語句零觸碰、EOL 保持）。
...
- ~~(i) SIGURD 溫控鏈補洞~~ **已收（fd1bb48，FW-CMD-E）**：ChangeTempMode
  落新檔 MainTempMode.cpp（173/177 行 ACTIVE、四點真缺 gate、非 MainCalcCore
...
## 20260825 V — census 重量＋下一個大標的裁決（主迴圈自做，唯讀）

一整夜的解鎖波之後重跑 `tools/census/census.py --detail`（20260825 03:5x），
把「接下來做什麼」放回數字上，而不是憑順手。

**完成度（單位＝golden code 行，分母標明）**

| 軸 | 總行數 | 缺 | 已翻 | % |
|---|---|---|---|---|
| 非表單 | 336,509 | 13,388 | 323,121 | **96.0%** |
| 表單 | 261,862 | 225,959 | 35,903 | **13.7%** |
| 全案 | 598,371 | 239,347 | 359,024 | **60.0%** |

`mirrored but INCOMPLETE`＝35 檔、60,125 行（有鏡射檔但沒翻完的部分；
表單缺口的其餘 ~166k 行是**連鏡射檔都還沒有**的檔案，兩者不要混為一談）。

**本夜十五波對行數的貢獻幾乎為零**，這是誠實的話：QWKEY2-6／G24／BARCODE2-4
都是解鎖與註解，BARCODE1＋QWKEY1＋QWKEY6 才是實體翻譯（約 +1.2k 行）。
解鎖波的價值在「讓已翻的碼真的會執行」，不在 census 數字——**兩者不要互相冒充**。

**下一個大標的：差點選錯，記下來**

缺口最大的單一檔是 `cContact.cpp`（22,324 行、103 個方法缺 102），
佔 `mirrored but INCOMPLETE` 的 37%。純看 census 會直接選它——**但它不在
計畫書 §4 的佇列裡，而且是對的**。實測 golden cContact.cpp（22,761 行、
93 個 `TfContact::` 方法）：`MotorMove` 77 次、`Teach` 62 次、`DoContact`
21 次——這是接觸高度**教導**表單，會下真機動作，屬 §4 佇列段
...
## 20260827 I — FW-TRAYMAP-W34：本波的價值不在數量，在「證明沒有了」

### 交付

**5 支 file-scope 自由函式 / 59 golden 行**——
**45.5%（5/11 支自由函式）／28.6%（59/206 行）**。
成員普查**不變**：仍是 42/150 支（28.0%）、355/6,040 行（5.9%），本波沒有新增成員本體。

`iWhichTrayNeedDeviceCount` :2789-2800／`CheckNeedDeviceCount` :2802-2812／
`InitialMultileTrayIDKeyence3Task` :4586-4589／`Crc_16_create` :6443-6464／
`BlockToString` :6466-6475。

### 真正的產出：`TfTrayMappingForm` 的唯讀表面已窮盡

agent **讀完剩下 84 支 ≤60 行的本體全文**，外加分類 24 支較大的，結論是
**108 支剩餘成員方法全部真的該 gated**——每一支都踩到機台動作／持久化／對外命令／
警報／零-port／呼叫 gated 本體其中之一。**沒有殘留的唯讀方法。**

另外兩條路也量過並關閉：
- **header inline 本體**：golden 類別本體（`.h:149-749`）**恰好 4 支**，W32 已全數交付。
- **非成員定義**：golden 檔 7,268 行裡只有 6,040 行是成員 span。用括號配對掃完
  196 個 top-level 定義，找出 `TfRFID::*`（17 支/576 行）、
  `cLineScanRemainICYieldRecord::*`（11/110）、`cDatabaseJson::*`（7/99），
  以及**11 支 file-scope 自由函式（206 行）**——最後那個族群就是本波的批次。

分母獨立重量（新的字元狀態機，不繼承前波）：raw regex **161 → 剝後 150 定義、0 宣告**，
span 6,040 行，**與 W32 banner 逐位元組相同**，該數字確認。

### 避開一個會炸 build 的重複符號陷阱

四個候選自由函式——`InitialTrayIDTask`／`InitialTrayID2Task`／`InitialTrayMapTask`／
`InitialCoverTrayIDTask`——**已經在 `acatchtray_shims.cpp:232-235` 定義為 offline no-op**，
`acatchtray.h:55-57,66` 有 extern 宣告，**20+ 個 live caller**。
翻譯它們會是**重複符號、直接 build 破，不是進度**。主迴圈已開檔逐行驗證。

**這是類別名衝突的同一形狀、往下一層**：W32 遇到 `class TfTrayMapping` 被
`acatchtray_shims.h:219` 佔用（故改名 `TfTrayMappingForm`），W34 遇到的是**函式層級**的同一件事。

### 修正 W32 的 gate 理由，但**不推翻 gate**

W32 把 `cbSimulationAOICommandClick` 歸在 (G-1)「對外 TCP」。**那個理由是錯的**——
本體只有三個記憶體賦值，沒開任何 socket。

但**真正的理由更強**：那是**模式切換**。`TfAOI::bSimulationAOICommand` 是
`TfAOILaserScan.cpp` 裡 **15 個站點**用來決定「假裝發還是真的發 AOI 命令」的旗標——
**離線未勾選的 checkbox 會讓每個 AOI 都走真實模式**。
理由換成真的那個，**gate 保持關閉**（policy 擋的，不是缺符號擋的）。

`DoMoveIn`／`DoMoveOut` 是同一形狀（本體看起來乾淨，馬達在被呼叫者裡）。
agent 自己點出：**它的 regex 交叉檢查把這三支全標成「乾淨」——這正是必須讀完整本體的理由。**
（與 W33 的四支 `sb_AutoOffset*Click` 同型，那次是 `spbSaveClick` 寫檔 + `fMain->Start()`。）

### 記錄但不修的分歧

`iCoverTrayIDTask`：golden 是 `[iKeyenceTotalFunction]={1,1,...}`（**13 個元素、全 1**），
port 是 `[64]={0}`（**64 個元素、全 0**）——`acatchtray_shims.cpp:151`。
**長度與初值都不同**，是真實的初始狀態差異（主迴圈已開檔驗證）。
那個陣列是 **shim 的財產**，不是這個 facade 的，所以記錄不修。

### 兩個刻意偏離

- **(D-9)** 加了 golden 沒有的宣告。golden 不需要，因為它的 caller 全在同一個 TU；
  port 不能把 `TfRFID` 留在這個 TU，所以**在這裡宣告可以擋住未來那一波定義第二份**。
- **(D-10)** `iMultileTrayIDKeyence3Task`（golden `:4584`）給 internal linkage，
  沿用 W32 的 `iCCDConntectionOkTask` 先例與理由。

### 其他排除

- `DoCoverTrayID_NFC` :6044-6104（**六個 SECS `EventReport`**）。
- `ShouldParseAutoBinLabelList` :51-56 是 `static`，它唯一的 3 個 caller 都在
  G-6 省略掉的 `ClientSocket_Keyence1Read` 裡——**交付它會為不可達的碼新增一個
  `-Wunused-function`**。G-6 解掉之後它就免費了。

### 驗收

全新 dir、最後一次整併之後量：**Debug 與 Release 各 137/142**，失敗集合逐項相同
且等於常駐五項。`-fsyntax-only` 0 error，**唯一的 warning 是那個既有的、
主迴圈裁決保留的 `-Wdelete-non-virtual-dtor`（`fTrayMapping.cpp:316`），無新增**。

append-only：`+210/-0`（.h）／`+151/-0`（.cpp），零刪除，無其他檔被動。
**備份 md5 與 `git show HEAD` 逐字相同**（`13a7f96c…`／`2494ebe2…`）。
兩檔 bare LF、零孤立 CR、零 U+FFFD、無 BOM。
**沒有新的 archive 相依**；新增的唯一外部資料符號 `bNeedCCDTrayDeviceCount`
（`cmydef.h:4188`）來自 `ht9045_globals`，本來就在清單裡。

### 下一批已量好（不是猜的）

同一個 golden 檔裡的兩個零-port 輔助類別：
`cDatabaseJson`（`.h:29-45`／`.cpp:6123-6227`，7 支/99 行）與
`cLineScanRemainICYieldRecord`（`.h:47-67`／`.cpp:6229-6348`，11 支/110 行 + 3 支 header inline），
翻完可解 (G-5b)。

**兩個先決條件已先講明**：
1. `cJSON` 在 port **存在**，但 golden 宣告 `cDatabaseJson : public uBasicPickPlace`
   ——**要先解那個基底**。
2. `GetTotalNumDir`／`GetFileNameWithDir` 呼叫 `MyForceDirectories`，
   **那會建目錄**——是 **(G-2) 寫入路徑**，不是純路徑組裝。

### 批次大小 5 而不是 30-50

派工給的目標是 30-50 支。agent 的說明：**在不製造重複符號、也不推翻安全 gate 的前提下
無法達到那個數字**，所以照「寧可少而正確」只取可驗證正確的部分。
**這是對的取捨**——本波真正的交付是「證明沒有了」這個結論，而不是行數。

### 🔖 RESUME（20260827 凌晨，已被檔尾那則取代）
...
## 20260827 IV — 一個註解裡的 `{` 讓 census 憑空造出 813 行假缺口，而我照著它派了一整波

### 發生了什麼

跑完 census（03:34，25 秒）後，我照 `mirrored but INCOMPLETE` 表選標的。
`Command.cpp` 是**最大的未 gated 非表單缺口**：43 支、813 行、`gated_funcs=0`。
形狀也漂亮——幾乎全是 `Get*`／`*Strings`，正是戰役要的唯讀方向。
我列出 36 支唯讀的派工，另外 7 支（`WriteHandlerTestArmEncoder`／`WriteHandlerTestArmEP`
／`WriteChkSetup`／`UploadProdLog`／三支 `Set*`）標為安全關鍵、佇列不翻。

agent 回來說：**這 36 支早在 20260817 的 `FW-3 Wave A`（`3fee669`）就翻完了，
本波零變更。** 它附了 git 序列（Wave A→F 加 `FW-CMD-C`，`Command.cpp` 已達 164/164）
與 `git merge-base --is-ancestor ece9626 HEAD` → YES。

我開檔驗了它指名的兩處，**它是對的**：`Command.cpp:3026` 是
`void TfMain::GetCZSiteMap(bool bSendGPIB)` 的真本體、`:3734` 是
`AnsiString TfMain::GetSiteState()`，連 golden 行號區間註解都與我給的一字不差。

**但它給的解釋是錯的**：它說「census 分母舊了，重跑就好」。我在派工前 25 分鐘
才跑過 census，而 census **仍然**說缺 43 支。問題不在過期，在 census 本身。

### 根因：`functions()` 數括號時不剝註解

```
port Command.cpp:1228   //2013.01.24 Q_Q TSMC GPIB COMMAND Part 2. {
```

一個 `{` 藏在 `//` 註解裡。`census.py` 的 `functions()` 用括號平衡切 span，
**但它不剝註解與字串字面值**，所以深度永遠多 1：
`TfMain::PERSITETemperatureStrings` 的 span 從 `1218..1249` 變成
**`1218..5219`（4,002 行）**，一口氣吞掉後面 61 支定義。
被吞的名字不會進 `pnames`，於是 census 把它們判成「golden 有、port 沒有」＝未翻譯。

`defn_probe` **有**認出 `GetSiteState` 那一行——不是探針失敗，是 span 覆蓋。
**整個失敗是靜默的。**

而那個 `{` 是**忠實翻譯的產物**：golden 自己就這樣寫。

### 量清楚邊界（A/B：同一個 `functions()`，差別只有先不先剝註解）

| 側 | 受影響檔數 | 少抽定義 | 有實際後果的 |
|---|---|---|---|
| port | **3** | 82 | **只有 `Command.cpp`**（`cJSON.c`／`ainarm9045_2x8_8.cpp` 的 `miss_fn` 都是 0） |
| golden | 9 | 281 | `main.cpp` 239→382、`cContact.cpp` 103→139、`Magazine.cpp` 21→50… |

**分母沒被污染**：`gcode` 是 `code_lines()` 另外算的，不經 span。
被扭曲的只有分子裡的 per-file 缺口清單。

**修正後的三軸**（把 `Command.cpp` 的 813 行假缺口扣掉）：
非表單 缺 13,973 → **13,160**，95.8% → **96.1%**（/336,509 golden code 行）；
全案 缺 230,543 → **229,730**，61.5% → **61.6%**（/598,371）。
表單 17.3%（/261,862）不受影響。
⚠ golden 側 span 膨脹可能讓其他檔的 `missing_lines` 也略有偏差，本次**沒有**逐檔修正，
只確認分母與量級無誤。

### 處置：**不修 census**，改加一支旁證工具

`tools/census/span_sanity.py`（新檔）。同一份文字跑兩次 census 自己的 `functions()`，
一次原樣、一次先把註解與字串換成等長空白（行號不變），差額就是損害。
單檔模式會列出**被吞的定義名與行號**。

**為什麼不直接修 `functions()`**：
1. 記憶裡記著上一次「修好」census 的嘗試算出過 **parked > gcode 與 35,289 行的假 credit**。
2. 改 `functions()` 會同時改變 golden 與 port 的所有 span，**歷史數字全部失去可比性**。
3. 凌晨無人看顧，安全預設是「保持現行行為 → 可逆 → 樹維持編得起來」。

所以 census 仍是權威量尺，`span_sanity.py` 是選標的前的過篩器。

### 這件事我早就寫過，然後照樣踩了

記憶裡有一條就叫 **「census 的『缺』不是待辦清單」**，另一條叫
**「抽 golden 函式範圍的兩個陷阱：註解裡的 `}` 騙過括號配對；失敗都是靜默的」**。
兩條都命中，而我在選標的時一條都沒想起來。
**知道一個陷阱和在正確的時刻想起它，是兩件不同的事**——所以這次的處置是工具，
不是又一條要記得的紀律。

### 我自己的兩個錯，一個更正

1. **第一次量的時候我是對的，然後我否定了自己。** 派工前我用自己寫的 regex 比對函式名，
   得到「golden 有而 port 沒有：**0 支**」。那是**正確答案**。我因為它與 census 的 43 支
   不合，就判定「我的比對方式錯了」，改用 census 的邏輯重算，然後拿錯的答案去派工。
   **與權威工具不合時，不代表你錯。**
2. **中途我產出過一張完全無效的表**：拿 `defn_probe` 的命中數當「定義數」，
   得出 `csystem.cpp` 有 19,816 個定義這種荒謬數字（它是候選探針，對大量非定義行回報命中）。
   當場作廢重做，改用上面那個 A/B。**寫在這裡是因為那張表當時看起來很有說服力。**

### 這一波的淨產出

**翻譯 0 行**——因為沒有東西要翻。真正的產出是：
一個假缺口被消除、一支防它再發生的工具、以及三軸數字的修正。
...
### CORRECTION（同日稍晚）：run2 的 Release 側其實**過了**，而我的 Debug 探針很可能是暖啟動

上面那段把四次逾時歸因於「機器負載」，**這個框架是錯的，或至少不完整**。
run2 的 Release 側後來跑完並且是 **GREEN**：

```
103/142 Test #103: dfm2rc_fidelity ..................   Passed  396.21 sec
R_TOTAL=5   R_EXTRA=(空)   R_ABSENT=(空)   R_VERDICT=GREEN
```

所以 run2 是 **Debug RED（逾時 600.02s）／Release GREEN（通過 396.21s）**，
而不是「兩側都被負載打死」。

**這反過來揭穿了我那兩個直接探針的一個混淆**：我量到 Debug **350s** 比 Release **470s** *快*，
但在 gate 內是**反過來的**。最可能的原因是——
**Debug 探針是在 run1 Debug 逾時被砍之後才跑的**，
`build_btq1g/dfm2rc_regen_fidelity` 已經被那次部分產生過，等於**暖啟動**；
而 gate 內每次都是冷啟動。
佐證：Release 探針（470s）與 Release 在 gate 內冷跑（396.21s）**是同一個量級**，
只有 Debug 那一組出現「探針遠快於 gate」的落差。

**修正後的結論**：
- **Debug 側本來就比 Release 慢**（與 `dualgate2` 檔頭記的 565.94s vs 355.37s 一致），
  而它在**冷啟動**下已經超出 600 秒的預算。**這不是單純的機器負載，是預算本身不夠。**
- **Release 側目前仍在預算內**（冷跑 396.21s，餘裕約 34%）。
- 使用者裁決因此更明確了：**要放寬的是那個 600 秒上限**（Debug 冷跑需要 >600s），
  或是讓 build 目錄不被端點防護掃描以壓低整體時間。**我仍然沒有動任何一個。**

**方法論的教訓**（比這次的數字更耐久）：
**在被砍掉的執行之後量同一件事，量到的可能是那次留下的殘骸幫你加速的結果。**
要量冷啟動，就得先確認產出目錄是空的——我沒有做這一步，於是得到一個看起來乾淨、
實際上不可比的數字，還用它推翻了一個其實成立的假設。
這與同一天稍早那個「用 `dualgate2` 檔頭的重疊條件數字外推 Debug≈750s」是同一族錯誤：
**基準不對，後面的推理再嚴謹也沒有用。**
