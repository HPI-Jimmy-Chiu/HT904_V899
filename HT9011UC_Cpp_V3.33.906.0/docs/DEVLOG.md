...
- **下一步：W6.4 index/tester**(atester.cpp + atester_32Site.cpp + atester_ProcessCount.cpp + cContact 的 iIndexTask)；或 W6.5 shuttle/carry(acarry.cpp)；或 W6.2b in/out site variants(25+26) pipeline。同套 scaffold 縫。
...
- ⚠ Big5 go-forward 規則持續(cp950/gloss，0 U+FFFD)。96 譯出源檔、branch 26 commits。
