...
- ▶ **下一步：W6.4b**(atester_32Site/atester_ProcessCount/aTester_Front/aTester_Rear + cContact iIndexTask 下壓 SM)；或 W6.5 shuttle/carry(acarry.cpp)；或 W6.2b in/out site variants(25+26) pipeline。
...
- **W6.4 INDEX/TESTER STAR**：✅ atester.cpp 引擎已完成(DoTestY/DoTestHeadMotor 核心 SM ACTIVE+pump、decode 消費者、5 oracle)；剩 W6.4b＝atester_32Site/atester_ProcessCount/aTester_Front/aTester_Rear(Front↔Rear 互依不可拆，現 shim COMPLETE) + cContact iIndexTask 下壓 SM(22761 行 VCL form) + GetTesterResult/ProcessTestResult tester-comm 本體(W5)。
...
| atester.cpp | DoTestHeadMotor 下壓/test/torque/EP/load-cell case 樹(golden 6232-8655：case 30..600/12000../14000../122100../142../1500../10000../20000.. 多臂下壓 SM) 整塊 `#if 0`(case 1-21 oracle window ACTIVE、cursor 走到 gated case 於 default 終端 park) | W7 | MOT[] Z/Y 移動+Suck/Socket bin 寫入+EP DAQ+Tester 交握，需 home |
| atester.cpp | iIndexTask@cContact 下壓 SM 交握(DoTestHeadMotor 從 case 21 後進入)；cContact.cpp 是 22761 行 VCL form 整檔未翻 | W7 | cContact TfContact VCL form 未翻；atester 永不直接 deref iIndexTask(僅透過 gated case 樹) |
| atester.cpp | DoCheckSocketHasIC/DoInterFaceErrorStep/CheckIndexStatus/CheckPlaceOutShuttle/DoSetupTest/GetTempICResult/CheckAndRecodrEP/CheckAndRecodrTorque/NewCheckAndRecodeTorque/IndexCheckOneByOne/IndexCheck4Site/CheckSocketSensor/DoIndexSocketCheck/DoHalfView*VerifyRTC/SendSiteMapToRTC/DoTesterSidePush/DoAllPassVerifyRTC/DoTestHeadMotorLoadCell/SaveTorque/CheckInitialStartDelayInSocket/.. 本體 `#if 0`+active stub(保 cursor+golden default) | W7 | MOT[]/Sen[]/Socket/EP DAQ/COM2-RTC/torque/VCL 本體需 home |
