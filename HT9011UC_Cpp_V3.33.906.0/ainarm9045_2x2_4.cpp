// =============================================================================
//  ainarm9045_2x2_4.cpp  --  In-Arm per-site VARIANT (2x2_4, dual-row 2x2 Quad-site)
//
//  Faithful translation of golden ainarm9045_2x2_4.cpp (3032 lines, BCB6, Big5).
//  Translator: AI(W6.2c-2x2_4) 20260626
//  Translation wave: W6.2c (in-arm per-site-config variant: 2x2_4).  Shares the
//  enum family e9045_2x2_4_13 / e9045_2x2_4_14 / e9045_2x2_4_12 (the dispatch enum
//  for THIS file's DoInArm_9045_2x2_4() is e9045_2x2_4_13; helper naming mixes
//  _2x2_4 / _2x2_4_13 -- both are golden, preserved verbatim).  Sibling of the
//  already-translated leaves ainarm_SearchPickPlate / ainarm_SearchPlacePlate and
//  the engine ainarm9045.cpp.
//
//  ROLE: the per-site in-arm pick/place state machines for the dual-row 2x2 layout.
//  The engine's dispatch ladder DoInArm_9045() / DoInArm_9045_SuckerMap() routes
//  iInArmType==e9045_2x2_4_13 here (un-gated at the integrate stage).  This file owns:
//    SetInOutArmParameter_2x2_4, GetNowSiteKitMode_2x2_4, InArmZNeedDown_2x2_4_13,
//    CheckXYPitch_2x2_4, MoveInArm2XYToShuttle_9045_2x2_4_13,
//    DoPlaceToHotPlate_9045_2x2_4_13, DoInArmPlaceToShuttle_9045_2x2_4_13,
//    DoInArm_9045_2x2_4_SuckerMap, DoInArm_9045_2x2_4,
//    DoInArmTryPickFromHotPlate_9045_2x2_4, GetShuttleState_2x2_4,
//    SetInArmHasDropToShuttle_2x2_4, and the file-scope const array XPHSuckToSht_2x2_13.
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula,
//  fall-through and goto/label (IN_ARM_SHUTTLE_2x2_4 / IN_ARM_TRY_PICK_HOTPLATE_2X2_4)
...
//  ainarm9045 engine.  No body is #if 0-gated.
//
...
//  DoInArmTryPickFromHotPlate_9045_2x2_4 keeps its golden #else (real) path
...
#include "ainarm9045.h"             // engine: GetInArmToShuttleOffset_9045 / InArmNeedCheckOffset / MoveInArmXYToShuttle_9045 / GetShuttleCol / GetVariableYInShuttleData / ...
...
#include "cmydef.h"                 // global scalar universe + IC consts + enums (eInArmType / picker enum / Tempture_* / rsm* / CC_* / iXpitchMaxX3 / iXpitchMaxX7 / In_Shuttle_Auto_Latch / bInSHLtcFin)
#include "common.h"
#include "mycylin.h"
...
#include "aHotPlateSubstrate.h"     // InArmSuck/FLCarryKit/BLCarryKit/OutArmSuck/FTestSuck/BTestSuck + ainarm2 cursors/timers + engine shims + PickFromHPList + ptrInSHT(Backup) + CopyInitSuck/OutArmSuckBackup + Pitch helpers
...
//==============================================================================
// golden ainarm9045_2x2_4.cpp:32  (file-scope const array, defined in this TU)
const int XPHSuckToSht_2x2_13[3][8]={{0, 2, 1, 3, 4, 5, 6, 7},
                                     {0, 2, 1, 3, 4, 5, 6, 7},
                                     {0, 2, 1, 3, 4, 5, 6, 7}};

//==============================================================================
// golden ainarm9045_2x2_4.cpp:37  (forward decl, file-local)
bool DoInArmTryPickFromHotPlate_9045_2x2_4(bool bOneTimeCheckAll, bool bShowErrorMessage);
//==============================================================================
// golden ainarm9045_2x2_4.cpp:39
...
//            CopyInitSuck(&InArmSuckBackup , &InArmSuck , 1, 1, 1, 3);
        ...
        InArmSuck .SetPickerCount(2, 2, 2, 2, 3, 0, 0);
        OutArmSuck.SetPickerCount(2, 2, 2, 2, 3, 0, 0);
    }
...
//            CopyInitSuck(&InArmSuckBackup , &InArmSuck , 1, 1, 1, 2);
            ...
            CopyInitSuck(&OutArmSuckBackup, &OutArmSuck, 1, 3, 1, 0);
            CopyInitSuck(&OutArmSuckBackup, &OutArmSuck, 0, 5, 0, 1);
            CopyInitSuck(&OutArmSuckBackup, &OutArmSuck, 1, 5, 1, 1);

            CopyInitSuck(&OutArmSuckBackup, &OutArmSuck, 0, 0, 0, 3);
            CopyInitSuck(&OutArmSuckBackup, &OutArmSuck, 1, 0, 1, 3);
            CopyInitSuck(&OutArmSuckBackup, &OutArmSuck, 0, 1, 0, 5);
            CopyInitSuck(&OutArmSuckBackup, &OutArmSuck, 1, 1, 1, 5);
        }
        else
        {
            //使用Ab, Ac
            CopyInitSuck(&OutArmSuckBackup, &OutArmSuck, 0, 1, 0, 0);
            CopyInitSuck(&OutArmSuckBackup, &OutArmSuck, 1, 1, 1, 0);
            ...
            CopyInitSuck(&OutArmSuckBackup, &OutArmSuck, 0, 3, 0, 3);
            CopyInitSuck(&OutArmSuckBackup, &OutArmSuck, 1, 3, 1, 3);
        }
        InArmSuck .SetPickerCount(2, 2, 2, 2, 1, 0, 0);
        OutArmSuck.SetPickerCount(2, 2, 2, 2, 1, 0, 0);
    }
    else
    {
        if(USE_PICKER_COUNT==ep16Picker)
...
//            CopyInitSuck(&InArmSuckBackup , &InArmSuck , 1, 1, 1, 2);
        ...
        InArmSuck .SetPickerCount(2, 2, 2, 2, 2, 0, 0);
        OutArmSuck.SetPickerCount(2, 2, 2, 2, 2, 0, 0);
    }
}
//==============================================================================
// golden ainarm9045_2x2_4.cpp:143
int GetNowSiteKitMode_2x2_4(int iSht, bool bPlace)
{
...
//==============================================================================
// golden ainarm9045_2x2_4.cpp:244
void InArmZNeedDown_2x2_4_13(int iSht, bool bPlace)
{
    if(InitialInArmNeedSuck(iSht, bPlace)==false)
        return;

    int iSuckRow, iSuckCol, iShtCol;
    int iMode   =GetNowSiteKitMode_2x2_4(iSht, bPlace);
...
//==============================================================================
// golden ainarm9045_2x2_4.cpp:281
void CheckXYPitch_2x2_4(int *iX, int *iY, int iSht, bool bPlace, int iMovePitchX, int iMovePitchY)
{
    int iMode       =GetNowSiteKitMode_2x2_4(iSht, bPlace);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
...
//==============================================================================
// golden ainarm9045_2x2_4.cpp:381
bool MoveInArm2XYToShuttle_9045_2x2_4_13(int iSht, bool IncludeZ, bool bPlace)
{
    ...
    CheckXYPitch_2x2_4(&iXPos, &iYPos, iSht, bPlace, iMovePitchX, iMovePitchY);
...
//==============================================================================
// golden ainarm9045_2x2_4.cpp:463
bool DoPlaceToHotPlate_9045_2x2_4_13()
{
            ...
            if(MoveInArmZToPlateSafe(Task))
            {
                for(int i=0; i<2; i++)
                {
                    for(int j=0; j<4; j++)
                    {
                        ...
                        if(TestIF.iSiteMap[i][j2]!=0 &&                         //KevinCheng 20250912 : 0 --> i 吸嘴改為AxEx後的路徑
...
//==============================================================================
// golden ainarm9045_2x2_4.cpp:728
bool DoInArmPlaceToShuttle_9045_2x2_4_13()
{
    IN_ARM_SHUTTLE_2x2_4:
        ...
        case 1000:
            if(InSHT1InLF())
            {
                ...
                if(bCanFreeShuttle==false)
                {
                    ...
                    (IniConfig.bP56TrayArmWaitAtColorTrack ||                   //Steven 20240516 : Tray Arm等待位置改到Color
...
//==============================================================================
// golden ainarm9045_2x2_4.cpp:1616
void DoInArm_9045_2x2_4_SuckerMap()
{
    for(int i=0; i<2; i++)
    {
        for(int j=0; j<8; j++)
...
//==============================================================================
// golden ainarm9045_2x2_4.cpp:1645
void DoInArm_9045_2x2_4()
{
    int &Task=iArmTask;
...
//            break;
        ...
        case 15000:                                                             //ChungHung 20120206 Hotplate check
            ...
            if(DoInArmTryPickFromHotPlate_9045_2x2_4(bOneTimeHotPlateCheckAll, bTryPickFromHotPlateShowError))
            {
                for(int i=0; i<2; i++)
                {
                    for(int j=0; j<4; j++)
...
// golden ainarm9045_2x2_4.cpp:2580
bool DoInArmTryPickFromHotPlate_9045_2x2_4(bool bOneTimeCheckAll, bool bShowErrorMessage)
{
    IN_ARM_TRY_PICK_HOTPLATE_2X2_4:
...
//    static bool bSuckDuplicateErr[2][4]={{false, false, false, false}, {false, false, false, false}};
    ...
    switch(Task)
    {
        ...
        case 350:
            ...
            Task=400;
            goto IN_ARM_TRY_PICK_HOTPLATE_2X2_4;
        case 400:
            for(int i=0; i<2; i++)
            {
                for(int j=0; j<4; j++)
...
//
// SUBSTRATE GAP (report for Integrate): the verbatim golden body calls
// ptrInSHT->ArmDownSideAllTypeIC(...), the twin of TMyKitSuck::ArmUpSideAllTypeIC.
// Golden mykitsuck.h:335 declares BOTH (ArmUpSideAllTypeIC :334 / ArmDownSideAllTypeIC :335),
// but the ported TMyKitSuck in aHotPlateSubstrate.h only carries ArmUpSideAllTypeIC (:263)
// -- ArmDownSideAllTypeIC was NOT brought across.  This TU is the first ACTIVE consumer
// (sibling ainarm9045_2x8_32.cpp uses it too but its whole body is #if 0-gated, so the gap
// was latent).  Per concurrency discipline I do NOT edit the shared substrate header; the
// faithful golden body is preserved VERBATIM under #if 0 // TODO(W7) below, and an ACTIVE
// stub keeps this TU compiling standalone.  Integrate: add
//   bool ArmDownSideAllTypeIC(int IC_TYPE, int iOffset, int iCol);  // golden mykitsuck.h:335
// to TMyKitSuck (next to ArmUpSideAllTypeIC @ aHotPlateSubstrate.h:263) + its predicate
// body, then flip the #if 0 -> #if 1 to restore the golden flow.
...
#else
// ACTIVE offline stub (W7 gate): returns 0 (the golden "no left-side action" default)
// until TMyKitSuck::ArmDownSideAllTypeIC lands in the substrate.  Same signature/symbol.
int GetShuttleState_2x2_4(int iSht, bool bPick)
{
...
#endif
//==============================================================================
// golden ainarm9045_2x2_4.cpp:3008
