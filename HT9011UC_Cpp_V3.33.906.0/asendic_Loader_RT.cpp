// =============================================================================
//  asendic_Loader_RT.cpp  --  Loader ROUND-TRIP (auto-retest tray recycle) SM
//
//  Translated from: HT9011UC_Code_V3.33.906.0_20260618/asendic_Loader_RT.cpp
//                   (904 lines of code + 1 trailing blank = 905 decoded lines;
//                    count re-derived from the cp950 decode in this pass)
...
//  VERBATIM.  ALL 12 golden top-level functions are translated -- golden
//  asendic_Loader_RT.cpp :33, :38, :172, :177, :339, :345, :472, :477, :681,
//  :686, :847, :852 (6 Init* + 6 Do*).  Nothing in this file is #if 0.
//
//  RELATIONSHIP TO asendic_Empty.cpp -- MEASURED, NOT ASSUMED
//  ----------------------------------------------------------
//  A W7-L1 planner measured Loader_RT as a VARIANT of asendic_Empty.cpp (all
//  functions pair by role, but only 54.6% identical at line level), and as
//  pairing ZERO functions with asendic_Loader.cpp.  asendic_Empty.cpp was read
//  for the porting CONVENTIONS only; every line below was transcribed from
//  asendic_Loader_RT.cpp's own golden, and the halves that differ from Empty are
//  exactly where the tray-recycle behaviour differs.  Two examples that a
//  copy-from-Empty would have got wrong, called out because they are the kind of
//  slip that survives review:
//    * DoLoadNewLoaderTrayToCar case 1 tests SnLoaderTrayHasTray_ART on BOTH
//      sides of the bForKyecBu3RunART conditional (golden :54-55), where the
//      Empty twin tests a single sensor.
//    * DoUnLoadNewLoaderToStack case 400 carries a THREE-WAY customer split
//      (CC_KYEC_LEE / bUseSCKART / the ASE @e02009 report, golden :788-837) that
//      has no counterpart in the Empty twin at all.
//
...
//    (1) DoLoaderTrayToFront case 1   -> case 100 (golden :207-208)
//    (2) DoLoaderTrayToFront case 100 -> case 200 (golden :314-315) -- note this
//        one is reached ONLY when none of case 100's four early exits fired.
//    (3) DoLoaderTrayToRear  case 1   -> case 100 (golden :358-359)
//  There are NO other fall-throughs in this file.
//
//  INCLUDES (BCB6 original vs this tree)
//  -------------------------------------
//  BCB6 pulled: MachineDefine.h, asendic_Loader_RT.h, asendic.h, MyMotor.h,
//  mycylin.h, main.h, mymessbox.h, note.h, mysensor.h, myswitch.h, acatchtray.h,
//  cprod.h, OCR.h, OCRInsp.h, cMyDB.h, uHGemHT9045.h, cmydef.h, SCK_ART.h,
//  cpublic.h, common.h.  Replaced by the same substitution table
//  asendic_Loader.cpp documents, plus:
//    asendic_Loader.h  (CheckLoaderICFloating -- golden reaches it through
//                       asendic.h's god-include, which this tree's asendic.h
//                       subset does not reproduce; it is the third edge of the
//                       Loader <-> Loader_RT cycle, golden :190)
//    FormsFacade.h     (fSCKART->iTesterType / SetLotStatus / iLOTSTATUS_W)
//  Not needed offline: OCR.h / OCRInsp.h / cMyDB.h (no OCR/DB symbol in this
//  file's body); mymessbox.h and note.h (ShowMyMessage / ShowErrorMessage now
//  live in canary_support.h).
//
//  SOFT_SIMULTE is NOT defined: the #ifndef SOFT_SIMULTE (real-machine) branches
//  are compiled, the #ifdef SOFT_SIMULTE branches are not.  Both arms verbatim.
//
...
//---------------------------------------------------------------------------
#include "asendic.h"
#include "asendic_Loader.h"     // CheckLoaderICFloating (golden :190)
