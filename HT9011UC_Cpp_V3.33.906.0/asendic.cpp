// =============================================================================
//  asendic.cpp  --  tray-lifter / Auto-stack cylinder state machines + the
//                   tray-move belt surface + the small free-function substrate
//                   every asendic_* sibling sits on.
//
//  Translated from: HT9011UC_Code_V3.33.906.0_20260618/asendic.cpp
//                   (1581 decoded lines; the whole file is landed -- nothing is
//                    #if 0, nothing is a stub.  Line count and every golden
//                    citation below were re-derived from the cp950 decode in
//                    THIS pass.)
//  Translation wave: W906-W7-L1 Wave 3 (asendic.cpp substrate -- L1a + L1b)
//  Translator: AI(W906-W7-L1-Wave3) 20260802
//
//  WHAT THIS LANDING REPLACES
//  --------------------------
//  Until this wave this file shipped SIM bodies (CylinderUp/Middle/Lower drove
//  Cylinder[] and returned true; TrayMoveIn/Out/TrayCylinMoveIn/Out were empty;
//  TrayMoveStatus was a hardwired `return 0`), and AutoCylinderUp/Middle/Lower
//  did not exist here at all -- they were three `{ return true; }` no-op stubs in
//  acatchtray_shims.cpp.  All of that is gone: every body below is the golden
//  state machine.
//
//  AI(W906-W7-L1-W3fixA) 20260802 -- WIRING FOLLOW-UP: TWO OF THOSE BODIES WERE
//  LANDED BUT NOT REACHED, AND NOW ARE
//  --------------------------------------------------------------------------
//  Wave 3 translated DoLoaderTrayFeed / InitDoLoaderTrayFeedTask (golden :1514-
//  1537) and already owned RecordAutoCleanOutStartEnd (golden :61-84), but left
//  csystem.cpp's two surviving redirects in place:
//      #define DoLoaderTrayFeed            W7C2_DoLoaderTrayFeed
//      #define RecordAutoCleanOutStartEnd  W7C1_RecordAutoCleanOutStartEnd
//  csystem.cpp is golden's ONLY caller of the DoLoaderTrayFeed pair and the owner
//  of the END half of the RecordAutoCleanOutStartEnd START/END pair, so those two
//  landings shipped as UNREACHABLE code (`nm -u` across every production and test
//  archive: refs=0 for DoLoaderTrayFeed and InitDoLoaderTrayFeedTask; refs=1 for
//  RecordAutoCleanOutStartEnd, and that one reference was the START call in
//  asendic_Loader.cpp -- so the timer latched and nothing read it).  Both
//  redirects are now DELETED and csystem.cpp includes asendic.h, so all three
//  bodies below are genuinely reached.  Rationale and the measured behaviour
//  delta for each are recorded at the deletion sites in csystem.cpp.
//  STILL HAS NO PRODUCTION CALLER, DELIBERATELY: InitDoLoaderTrayFeedTask.
//  Golden's only call site is csystem.cpp:7814, inside the DoTrayFeedProcess mode
//  ladder this tree still gates `#if 0 // TODO(W7)`; the W7-U wave that un-gates
//  it supplies the call.  Do NOT invent a production call here to make a refs
//  count look better.  Its only caller in this tree is
//  tests/test_w6_6_csystem_cycle.cpp's O5 wiring pin, which uses it for what it
//  is -- a cursor reset -- to keep the pin re-runnable and state-neutral.
//
//  THE BEHAVIOUR INVERSION THIS CAUSES -- READ BEFORE DEBUGGING A "STALL"
//  ---------------------------------------------------------------------
//  The stubs "always succeeded instantly".  The real bodies do NOT succeed on a
//  default-constructed Cylinder[] entry, and that is FAITHFUL, not a porting
//  slip.  Mechanism, re-derived from the decoded golden in this pass:
//    * TMyCylinder's ctor (mycylin.cpp) leaves Enable=false, OnSenEnable=false,
//      OffSenEnable=false -- so OnStatus() and OffStatus() BOTH return false.
//    * AutoCylinderMidIsOn (golden :529-560) returns TRUE when
//      Cylinder[CylinderNameMid].Enable==false ("Steven 20250312 : Add
//      protection"), so AutoCylinderUp case 100 (golden :661-666) accepts the
//      disabled mid cylinder and advances 1 -> 50 -> 100 -> 200 -> 201.
//    * But AutoCylinderUp case 201's non-ART guard (golden :720, "JerryYang
//      20250314 : fix non ART AUTO 上升誤ALARM") tests the RAW
//      `Cylinder[CylinderNameMid].OnStatus()`, which is false for the same
//      disabled cylinder -> `Task=1; return false;`.  The SM therefore cycles
//      1 -> 50 -> 100 -> 200 -> 201 -> 1 forever and NEVER returns true.  It does
//      not even alarm: case 200 re-arms CylinderAlarmTime[0] to 10 s on every
//      pass, so the case-201 watchdog can never expire.
//    * AutoCylinderMiddle has the same never-true outcome by a DIFFERENT route:
//      its case 100 non-ART guard (golden :849-850) needs
//      Cylinder[CylinderName].OnStatus()==true, which a disabled cylinder cannot
//      produce, so it never leaves case 100 and instead repeats the
//      "Lifter Middle error" ShowMyMessage every OnAlarmTime.
//    * AutoCylinderLower DOES converge on the untouched default, because its
//      case 100 reads the Ifor-20200506 bOffStatus/bMidOnStatus pair (golden
//      :954-970) which substitute true/false when the sensors are disabled.
//  The correct fixture for any test that walks an Auto lifter path is therefore
//  "configure the cylinders the way a real machine configures them" (Enable +
//  the sensor enables + Sim IO addresses), NOT a weakened assertion.
//
//  iLifterTask / iAutoTask ROW STRIDE -- A GOLDEN DEFECT, DELIBERATELY MIRRORED
//  ---------------------------------------------------------------------------
//  Golden defines `int iLifterTask[3][10];` (:28) and
//  `int iAutoTask[3][MAX_UNLOAD_TRAY];` (:35, MAX_UNLOAD_TRAY==12), while golden
//  main.cpp:9136-9137 extern-declares BOTH as `[3][7]`.  The row strides
//  disagree, so main.cpp:9796-9841's 42 QueueTaskList registrations address the
//  wrong storage for every row above 0 (`&iLifterTask[1][0]` under a stride of 7
//  lands on the definition's `[0][7]`).  The definitions below are golden
//  VERBATIM and this file deliberately does NOT publish a "corrected"
//  declaration anywhere: reconciling the two would be a behaviour change
//  disguised as a header cleanup.  Handed to the W7-U main.cpp wave.
//
//  INCLUDES (BCB6 original vs this tree)
//  -------------------------------------
//  BCB6 pulled: MachineDefine.h, asendic.h, MyMotor.h, mycylin.h, main.h,
//  mymessbox.h, note.h, mysensor.h, myswitch.h, acatchtray.h, cprod.h, OCR.h,
//  OCRInsp.h, cMyDB.h, cmydef.h, common.h.  Substitutions used here, matching
//  the table the landed asendic_* siblings document:
//    main.h / mymessbox.h / note.h / cMyDB.h -> canary_support.h
//                                (ShowMyMessage / ShowErrorMessage / K_SKIP /
//                                 RecordProcess / LastSet / DUMMY / __FUNC__)
//    OCR.h / OCRInsp.h           -> not needed: the only OCR symbols this file
//                                reads are INSTALL_OCR / INSTALL_OCR_YMot
//                                (cmydef.h) and CosFunction.bTrayOCR
//                                (CosFunction.h via cprod.h).
//    acatchtray.h                -> not needed: no acatchtray symbol is
//                                referenced by this file's body.
//  Added for this tree:
//    Config.h        -- IniConfig (golden reaches it through main.h)
//    common.h        -- MySleepEx (golden common.h:260)
//    asendic_Loader_RT.h -- InitAutoLoaderTask / InitUnLoadNewLoaderTrayTask /
//                       DoUnLoadNewLoaderToStack / DoLoaderTrayToFront, the four
//                       functions DoLoaderTrayFeed calls.  Golden reaches them
//                       through asendic.h's god-include of the sibling headers,
//                       which this tree's asendic.h subset does not reproduce.
//    BoolToStr       -- golden SysUtils.hpp; this tree's home is
//                       acatchtray_shims.{h,cpp}.  Declared locally (rather than
//                       including that header) because acatchtray_shims.h also
//                       carries default arguments for the AutoCylinder* family
//                       and asendic.h is the file that now owns those.
//
//  SOFT_SIMULTE is NOT defined in this tree (MachineType.h:48 is commented out),
//  so every `#ifndef SOFT_SIMULTE` real-machine arm below is compiled and every
//  `#ifdef SOFT_SIMULTE` arm is not.  Both arms are transcribed verbatim.
//  DEBUG_AutoZMotor is likewise NOT defined (MachineType.h:42 commented out), so
//  the two RogerYang-20250512 Auto3Z debug blocks (golden :745-753 and :780-787)
//  are preserved verbatim but compile out -- which is why this file needs no
//  fMain / NewRecordProcess dependency.
//
...
#include "Config.h"                 // IniConfig (golden reaches it via main.h)
#include "canary_support.h"         // ShowMyMessage / ShowErrorMessage / K_SKIP /
                                    // RecordProcess / LastSet / DUMMY / __FUNC__
#include "asendic_Loader_RT.h"      // the four functions DoLoaderTrayFeed calls

// golden SysUtils.hpp BoolToStr -- real body in acatchtray_shims.cpp.  Declared
// here instead of including acatchtray_shims.h: that header also declares the
// AutoCylinder* family WITH default arguments, and asendic.h now owns those, so
// pulling it in would give parameter 4 two default arguments in one TU.
AnsiString BoolToStr(bool B, bool UseBoolStrs = false);

// =============================================================================
//  BODY -- VERBATIM golden asendic.cpp:23-1581 (1559 lines, zero diff).
...
//------------------------------------------------------------------------------
//==> Eastsun 20260512 F012 整合 (CheckCylinderCanMove (a-side asendic.cpp L1447))
//---------------------------------------------------------------------------
bool CheckCylinderCanMove(int iAxis)    //Ifor 20211005 add Tray 載盤上升下降前判斷是否有異常
{
    bool bCanMove=true;
    #ifndef SOFT_SIMULTE
    int iDisplayPos[6] ={MMTrayY,MMEmpty,MMColor,MMAuto1,MMAuto2,MMAuto3};
    AnsiString asErrorCode[6]=   {"WAR0916","WAR1016","WAR1416","WAR1116","WAR1216","WAR1316"};
    if(LastSet.iRealDummy!=DUMMY)
    {
        if(Sen[SnLoader_Detect+iAxis].Enable==true && Sen[SnLoader_Detect+iAxis].IsOff()==false)
        {
            bCanMove=false;
            ShowErrorMessage(asErrorCode[iAxis], K_SKIP, iDisplayPos[iAxis]);
        }
    }
    #endif
    return bCanMove;
