// =============================================================================
//  ainarm9045_2x2_8_Hot.cpp  --  In-Arm per-site VARIANT (2x2_8_Hot, 2x2 8-picker Hot)
//
//  Faithful translation of golden ainarm9045_2x2_8_Hot.cpp (3974 lines, BCB6, Big5).
//  Translator: AI(W6.2c-2x2_8_Hot) 20260626
//  Translation wave: W6.2c batch-4 (in-arm per-site-config variant: 2x2_8_Hot;
//  2 physical rows x 2 site columns driven through 8 pickers, with Hot-mode
//  HotPlate place/pick).  Sibling of the already-translated leaves
//  ainarm_SearchPickPlate / ainarm_SearchPlacePlate and the engine ainarm9045.cpp.
//
//  ROLE: the per-site in-arm pick/place state machines for the 2x2_8_Hot layout.
//  The engine's dispatch ladder DoInArm_9045() / DoInArm_9045_SuckerMap() routes
//  iInArmType==e9045_2x2_8_Hot here (un-gated at the integrate stage).  This file
//  owns the full set of 2x2_8_Hot helpers + the two engine entry points
//  DoInArm_9045_2x2_8_Hot / DoInArm_9045_2x2_8_Hot_SuckerMap.
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula,
//  fall-through and goto/label (IN_ARM_SHUTTLE_2x2_4_Hot) is transcribed
...
//  HAL + aHotPlateSubstrate + atester_shims + canary_support + ainarm9045 engine.
...
//  SOFT_SIMULTE is NOT defined and does NOT appear in this file.  Numeric
//  semantics (int vs double, integer division, double pitch math) kept EXACT.
//
//  SUBSTRATE EXTERNS reported for the serial Integrate phase (NOT added here):
//    - bool RowCanDualSite();                 golden ainarm2.cpp:1554 / ainarm2.h.
//      Used by GetPlaceHotPlate_4/_8/_8All.   Offline-safe: golden body returns
//      false in the only reachable branches over Sim HAL.  Forward-declared
//      below under an #ifndef guard so this TU compiles standalone.
//    - void TMyKitSuck::SetNullIcToHasNullIc(); golden MyKitSuck.h:280 /
//      MyKitSuck.cpp:340.  Used 4x on FLCarryKit/BLCarryKit.  Member of an
//      existing class (aHotPlateSubstrate.h TMyKitSuck) so it CANNOT be forward-
//      declared from here; the Integrate phase adds it to the class ONCE.
//
...
#include "ainarm9045.h"             // engine: GetInArmToShuttleOffset_9045 / InArmNeedCheckOffset / MoveInArmXYToShuttle_9045 / GetShuttleCol / DoInArmPickFromLoadStage_9045 / ...
#include "ainarm_SearchPickPlate.h" // shared HP-pick externs (HasHotReadyIC_9045 / ArmRow1AllHasIC / GetHeaterWaitTime / MoveInArmXYPickHotPlate_9045 / InArmNeedCheckHotPlateOffset / ...)
#include "ainarm_SearchPlacePlate.h"// shared HP-place externs (SearchPlateToPlace / DoPlaceToHPSwapData / DoPlaceToHPBackupData / Row2CanPutHP / ArmRow1NotICForPlaceToPlate / GetHotPlateYHalfPos / GetVariableYInHotPlateData / CheckHasSpaceToPlace_9045 / HPPlaceLog)
#include "csystem.h"                // CheckInArmSuckICFallDownToHasNullIC / DoInArmAutoSiteMapping / CheckInArmDestroyICFail / AutoSiteMappingCheckShuttle / DoSiteMappingCHK / IsInArmCleanOutFinish / IsInArmOneCycleFinish predicates
#include "Motor/mymotor.h"          // MOT[], InArmContinuousMove_9045, TTrayMotor
...
#include "cprod.h"                  // Prod / TestIF / TestIF_File / ArmSpeed / LastSet / InArmOffSet
#include "cpublic.h"                // CosFunction
#include "atester.h"                // dwStartShuttle1Soak / dwStartShuttle2Soak / bOneTimeWait
#include "aHotPlateSubstrate.h"     // InArmSuck/FLCarryKit/BLCarryKit/OutArmSuck + ptrInSHTBackup + ainarm2 cursors/timers + engine shims + PickFromHPList
...
#include "acatchtray_shims.h"        // InitProcessSingleMotorTask / MoveInArm2XYToWait / InShtInLF / InSHT1InLF / InSHT2InLF
#include "acarry.h"                 // IsFLCarrKitAllHasIC / IsBLCarrKitAllHasIC
#include "ainarm9045_2x4_16.h"      // DoInArmPickFromHotPlate_9045
#include "ainarm9045_2x4_16_shims.h"// InArmAddSpeedDisplay / AdjustShuttlePlaceOrder_AutoSiteMapping / MyTickCount / iInposLed
#include "canary_support.h"         // ShowErrorMessage / ShowMyMessage / RecordProcess / LastSet / __FUNC__ / K_RETRY / Tempture_Hot

//------------------------------------------------------------------------------
// SUBSTRATE EXTERN forward-declaration (REPORTED for Integrate; #ifndef-guarded
// so it cannot ODR-collide with the sibling that finally homes it).
//   golden: ainarm2.cpp:1554 / ainarm2.h
//------------------------------------------------------------------------------
#ifndef ROWCANDUALSITE_DECLARED
#define ROWCANDUALSITE_DECLARED
bool RowCanDualSite();
#endif
//------------------------------------------------------------------------------
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:35
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:41
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:102
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:224
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:275
...
//------------------------------------------------------------------------------
// golden ainarm9045_2x2_8_Hot.cpp:407
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:495
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:576
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:613
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:649
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:691
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:733
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:775
...
// golden ainarm9045_2x2_8_Hot.cpp:809
...
// golden ainarm9045_2x2_8_Hot.cpp:850
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:884
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:989
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:1630
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:1711
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:1823
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:1896
...
//==>
// golden ainarm9045_2x2_8_Hot.cpp:1977
...
//Steven 20180409 (Jou) : Add 2x2 support X=12
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:2066
...
//------------------------------------------------------------------------------
// golden ainarm9045_2x2_8_Hot.cpp:2155
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:2287
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:2299
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:3232
...
//==============================================================================
// golden ainarm9045_2x2_8_Hot.cpp:3275
