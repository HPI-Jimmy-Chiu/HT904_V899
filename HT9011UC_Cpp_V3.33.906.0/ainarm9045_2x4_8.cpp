// =============================================================================
//  ainarm9045_2x4_8.cpp  --  In-Arm per-site VARIANT (2x4_8, dual-shuttle 2x4)
//
//  Faithful translation of golden ainarm9045_2x4_8.cpp (3867 lines, BCB6, Big5).
//  Translator: AI(W6.2c-2x4_8) 20260626
//  Translation wave: W6.2c batch-4 (in-arm per-site-config variant: 2x4_8; the
//  2-row x 4-col picker that feeds the 8/10/12/16-column shuttle, two-arm 32-site
//  family).  Sibling of the already-translated in-arm variants (1x3_4 / 1x2_2 /
//  ...) and the engine ainarm9045.cpp.
//
//  ROLE: the per-site in-arm pick/place state machines for the 2x4_8 layout.
//  The engine's dispatch ladder DoInArm_9045() / DoInArm_9045_SuckerMap() routes
//  iInArmType==e9045_2x4_8 here (un-gated at the Integrate stage).  This file owns:
//    SetInOutArmParameter_2x4_8, GetNowSiteKitMode_2x4_8, InArmZNeedDown_2x4_8,
//    CheckXYPitch_2x4_8, MoveInArm2XYToShuttle_9045_2x4_8,
//    DoPlaceToHotPlate_9045_2x4_8, DoInArmPlaceToShuttle_9045_2x4_8,
//    DoInArm_9045_2x4_8_SuckerMap, DoInArm_9045_2x4_8,
//    DoInArmTryPickFromHotPlate_9045_2x4_8, GetShuttleState_2x4_8,
//    SetInArmHasDropToShuttle_2x4_8, plus the file-scope const table
//    XPHSuckToSht_2x4_8[3][8].
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula,
//  fall-through and goto/label (IN_ARM_SHUTTLE_2x4_8 /
//  IN_ARM_TRY_PICK_HOTPLATE_2X4_8) is transcribed VERBATIM from the golden CP950.
//  All state-machine case bodies are ACTIVE -- they touch MOT[]/Suck/Sensor/
...
//  DoInArmTryPickFromHotPlate_9045_2x4_8 (golden :3112 / :3246 / :3364 / :3480)
//  keep their golden #else (real) path active, reproduced verbatim.  Numeric
//  semantics (int vs double, integer division, double pitch math) kept EXACT.
//
//  SUBSTRATE NEEDS (reported for the serial Integrate agent; NONE NEW required):
//    All cross-module / substrate / form symbols referenced here already resolve
//    over the committed substrate (verified): InArmSuck/OutArmSuck/OutArm2Suck/
//    FLCarryKit/BLCarryKit/FTestSuck/BTestSuck/ptrInSHT(Backup) +
//    GetShuttleCol/MoveInArm*/CheckInArm*/DoPlaceToHP* + engine offset/pitch
//    helpers + tRotate + PickFromHPList + HotPlateForm/TrayForm/fObserver/
//    fContact/fYieldMonitoring/fMain (FormsFacade) + global bool bUseTwoArm32Site
//    (cmydef.h, NOT the absent Prod.fInArmSuckUse_TwoArm32Site member) +
//    USE_OUT_SORT_ARM/eartUninstall (cmydef.h/MachineType.h).
//
...
#include "ainarm_SearchPlacePlate.h"// shared HP-place externs (SearchPlateToPlace / MoveInArmXYToHotPlatePlace / DoPlaceToHPSwapData / GetPlaceToHotPlateSuckCol / Row2CanPutHP / GetHotPlateColStep / GetVariableXInHotPlateData / iPlaceHP / HPPlaceLog)
...
#include "cmydef.h"                 // global scalar universe + IC consts + enums (eInArmType / picker enum / Tempture_* / rsm* / CC_* / bUseTwoArm32Site / USE_OUT_SORT_ARM)
#include "common.h"
#include "mycylin.h"
...
#include "aHotPlateSubstrate.h"     // InArmSuck/FLCarryKit/BLCarryKit/OutArmSuck/OutArm2Suck/FTestSuck/BTestSuck + ainarm2 cursors/timers + engine shims + PickFromHPList + tRotate + ptrInSHT(Backup)
#include "FormsFacade.h"            // HotPlateForm / TrayForm / fContact / fObserver / fYieldMonitoring / fMain offline stand-ins / DUMMY
...
const int XPHSuckToSht_2x4_8[3][8]={{0, 1, 2, 3, 4, 5, 6, 7},
                                    {0, 2, 1, 3, 4, 6, 5, 7},
                                    {0, 1, 2, 3, 4, 5, 6, 7}};

//==============================================================================
bool DoInArmTryPickFromHotPlate_9045_2x4_8(bool bOneTimeCheckAll, bool bShowErrorMessage);
//==============================================================================
...
//        CopyInitSuck(&InArmSuckBackup , &InArmSuck , 1, 7, 1, 7);
    ...
    if(TestIF.iTestMode==_10Site2X5)
        ...
        iShtCol=8;
    InArmSuck .SetPickerCount(2, 4, 2, iShtCol, 1, 0, 0);
    OutArmSuck.SetPickerCount(2, 4, 2, iShtCol, 1, 0, 0);

    if(USE_OUT_SORT_ARM!=eartUninstall)                                         //RogerYang 20250515 add for 9046AU
    {
        OutArm2Suck.SetPickerCount(1, 2, 2, iShtCol, 1, 2, 2);                  //暫時for 2x4_8 pickstep=2
    }
}
//==============================================================================
...
int GetNowSiteKitMode_2x4_8(int iSht, bool bPlace)
{
    int iX=HAS_TESTING_IC, k;
        ...
        if(TestIF.iAutoClean_Function && bUse8Picker==false && bRunAutoClean)
        {
            ...
            else                                                                //if(InArmSuck.iXStep==4)
            {
                if(InArmSuck.Item[1][0]>=HAS_IC)
                    ...
                    return (iShuttleRowKit==1 || iShuttleRowKit==3)?21000:21001;
                else                                                            //if(InArmSuck.Item[1][3]>=HAS_IC)
                    return (iShuttleRowKit==1 || iShuttleRowKit==3)?21100:21101;
            }
        }
        else
        {
    ...
    else
    {
        for(int i=0; i<FLCarryKit.iMaxRow; i++)                                 //Steven 20241206 : InArmSuck.iShtRow -->　FLCarryKit.iMaxRow
        {
            for(int j=0; j<FLCarryKit.iMaxCol; j++)                             //Steven 20241206 : InArmSuck.iShtCol -->　FLCarryKit.iMaxCol
            {
                k=j;
                if(iSht==0)
                    ptrInSHTBackup.SetItemData(i, j, (FLCarryKit.Item[i][k] || FTestSuck.Item[i][k])?iX:0);             //kevin 20220928 change shuttle for autoclean pick data   //ChungHung 20131120 AutoClean use Hotplate1
                else
                    ptrInSHTBackup.SetItemData(i, j, (BLCarryKit.Item[i][k] || BTestSuck.Item[i][k])?iX:0);             //kevin 20220928 change shuttle for autoclean pick data  //ChungHung 20131120 AutoClean use Hotplate1
            }
        }

        if(InArmSuck.iXStep==1)
        {
            if(ptrInSHTBackup.Item[0][0] || ptrInSHTBackup.Item[0][1] || ptrInSHTBackup.Item[0][2] || ptrInSHTBackup.Item[0][3])
                return 0;
            else
                return 1;
        }
        else if(InArmSuck.iXStep==2)
        {
            if(ptrInSHTBackup.Item[0][0]==iX || ptrInSHTBackup.Item[0][1]==iX)
                ...
                return 11000;
            else if(ptrInSHTBackup.Item[1][0]==iX || ptrInSHTBackup.Item[1][1]==iX)
                return 10001;
            else                                                                //if(ptrInSHTBackup.Item[1][2]==iX || ptrInSHTBackup.Item[1][3]==iX)
                return 11001;
        }
        else                                                                    //if(InArmSuck.iXStep==4)
        {
...
//==============================================================================
void InArmZNeedDown_2x4_8(int iSht, bool bPlace)
{
    if(InitialInArmNeedSuck(iSht, bPlace)==false)
        return;

    int iSuckRow, iSuckCol, iShtCol;
    int iMode   =GetNowSiteKitMode_2x4_8(iSht, bPlace);
...
//==============================================================================
void CheckXYPitch_2x4_8(int *iX, int *iY, int iSht, bool bPlace, int iMovePitchX, int iMovePitchY)
{
    int iMode       =GetNowSiteKitMode_2x4_8(iSht, bPlace);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
...
//==============================================================================
bool MoveInArm2XYToShuttle_9045_2x4_8(int iSht, bool IncludeZ, bool bPlace)
{
    ...
    CheckXYPitch_2x4_8(&iXPos, &iYPos, iSht, bPlace, iMovePitchX, iMovePitchY);
...
//==============================================================================
bool DoPlaceToHotPlate_9045_2x4_8()
{
    int &Task=iInArmPlaceToHotPlateTask;
    int ip, ix, iy, j2;
    int iStepHP=4;
    bool flag=true;
    bAmbientHotPlate=(TestIF_File.bEnablePreciserHotPlate &&
                        SPIL_FOR_QLE==1 && USE_PRECISER==2);                    //Kevincheng 20260525 : 渠梁常溫模式使用hotplate
...
//            break;
        ...
        case 501:
            ...
            if((LastSet.iTemperature==Tempture_Hot ||
                bAmbientHotPlate) &&                                            //Kevincheng 20260525 : 渠梁常溫模式使用hotplate
...
//==============================================================================
bool DoInArmPlaceToShuttle_9045_2x4_8()
{
    ...
    switch(Task)
    {
        ...
        case 400:                                                               //kevin 20130115 判斷關site是否吸ic
            ...
            if(flag==true)
            {
                ...
                {
                    if(bAmbientHotPlate )                                       //Kevincheng 20260525 : 渠梁常溫模式使用hotplate
                    {
                        Task=1000;
                    }
                    else
        ...
        case 1000:
            if(LastSet.iTemperature==Tempture_Hot ||
                bAmbientHotPlate)                                               //Kevincheng 20260525 : 渠梁常溫模式使用hotplate   //ChungHung 20141023 add for ATK QA mode after QA Clean out direct send to Shuttle
            {
...
//            break;
        ...
        case 1550:
            ...
            if(DoInArmPickFromHotPlate_9045())
            {
                ...
                if(bAmbientHotPlate)                                            //Kevincheng 20260525 : 渠梁常溫模式使用hotplate
                {
                    InArmSuck.iWhichSht = InArmSuck.iWhichShtPickFor32;
                    InArmSuck.iWhichKit = InArmSuck.iWhichKitPickFor32;
                }
                InitInArmPlaceToShuttleTask();
                Task=2000;
            }
        ...
        case 15000:                                                             //ChungHung 20120206 Hotplate check
            ...
            if(DoInArmTryPickFromHotPlate_9045_2x4_8(bOneTimeHotPlateCheckAll, bTryPickFromHotPlateShowError))
            {
                for(int i=0; i<2; i++)
                {
                    for(int j=0; j<4; j++)
...
bool DoInArmTryPickFromHotPlate_9045_2x4_8(bool bOneTimeCheckAll, bool bShowErrorMessage)
{
    IN_ARM_TRY_PICK_HOTPLATE_2X4_8:
    ...
    switch(Task)
    {
        ...
        case 350:
            if(HotPlateForm.XDivision==6)
            {
                ...
                Task=400;
                goto IN_ARM_TRY_PICK_HOTPLATE_2X4_8;
            }
            else
            {
                for(i=0; i<MAX_ARM_Row; i++)
                {
                    for(j=0; j<MAX_ARM_Col; j++)
                    {
                        ...
                        else if(HotPlateForm.XDivision==12)                     //Steven 20260304 : 12x16 Hot Plate
                        {
                            if(bPitchOver12000)
                                ix=iPlacePlateX[0]+j;
                            else if(iPlacePlateX[0]==8)
                                ix=iPlacePlateX[0]+j;
                            else
                                ix=iPlacePlateX[0]+j*2;
                        }
...
{
    AnsiString Str;
    //ChungHung 20111230 add Shuttle必須放滿
    //若InArmSuck對應之Shuttle上已有IC 則只清除InArmSuck上的IC不進行資料轉移
