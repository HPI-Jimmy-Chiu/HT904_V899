// =============================================================================
//  ainarm9045_1x4_2.cpp  --  In-Arm per-site VARIANT (1x4_2 / e9045_1x4_2_14)
//
//  Faithful translation of golden ainarm9045_1x4_2.cpp (2669 lines, BCB6, Big5).
//  Translator: AI(W6.2c-INARM-1x4_2) 20260626
//  Translation wave: W6.2c (in-arm per-site-config variant: 1x4_2).  Sibling of
...
//  DoInArm_9045() / DoInArm_9045_SuckerMap() ladder for iInArmType==e9045_1x4_2_14
//  (un-gated at the serial Integrate stage; this Translate agent touches only its
//  own .cpp/.h, never a shared file).
//
//  ROLE: the per-site in-arm pick/place state machines for the 1x4_2 layout.
//  This file owns (golden file:line):
//    SetInOutArmParameter_1x4_2                 :32
//    GetNowSiteKitMode_1x4_2                    :99
//    InArmZNeedDown_1x4_2                        :175
//    CheckXYPitch_1x4_2                          :228
//    MoveInArm2XYToShuttle_9045_1x4_2            :354
//    DoPlaceToHotPlate_9045_1x4_2_14            :436
//    DoInArmPlaceToShuttle_9045_1x4_2_14        :702
//    DoInArm_9045_1x4_2_SuckerMap               :1591  (engine dispatch symbol)
//    DoInArm_9045_1x4_2                          :1669  (engine dispatch symbol)
//    GetShtNeedActive_1x4_2_14                  :2529
//    GetNeedActive_1x4_2_14                      :2549
//    GetShuttleState_1x4_2_14                    :2568  (exported, golden .h)
//    GetNeedSuckActive_1x4_2_14                 :2610  (exported, golden .h)
//    SetInArmHasDropToShuttle_1x4_2             :2617  (exported, golden .h)
//  (NAME ASYMMETRY is golden: the dispatch FN is DoInArm_9045_1x4_2() but the
//   enum is e9045_1x4_2_14 and several helpers carry the "_14" suffix.  Preserved
//   VERBATIM for BCB6 reverse-lookup.)
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula and
//  fall-through is transcribed VERBATIM from the golden CP950.  This file has NO
//  active goto/label, NO #ifdef/#if preprocessor branches and NO SOFT_SIMULTE
//  block (verified).  All SM case bodies are ACTIVE -- they touch
//  MOT[]/Suck/Sensor/predicates/FormsFacade, all of which resolve over the Sim
//  HAL + aHotPlateSubstrate + atester_shims + acatchtray_shims + canary_support +
//  ainarm9045 engine + ainarm9045_2x4_16_shims.  No body is #if 0-gated.
//
//  VCL/Borland conversions: AnsiString (1-based, .sprintf) via vclcompat;
//  __fastcall/__property/__published/PACKAGE NOT present in this file (verified
//  0 occurrences); __FUNC__ via the aArmHeader shim.  Numeric semantics (int vs
//  double, integer division, double pitch math) kept EXACT.
//
//  SUBSTRATE NEEDS (reported for the serial Integrate agent; NOT added here):
//    none new -- every external symbol this variant derefs is already present in
//    the W6.2c substrate (verified): DoCheckAutoSiteMappingPosition
//    (aHotPlateSubstrate.h:686), Zteach facade (aHotPlateSubstrate.h:680),
//    DoInArmPickFromHotPlate_9045 (ainarm_SearchPickPlate.h:22),
//    InitInArmPickFromHotPlateTask*/InitInArmTryPickFromHotPlateTask*
//    (aHotPlateSubstrate.h / ainarm9045_2x4_16_shims.h), enums e9045_1x4_2_14 /
//    e9045_1x4_4_13 (MachineType.h:1014+).
//
...
#include "ainarm9045.h"             // engine: GetInArmToShuttleOffset_9045 / InArmNeedCheckOffset / MoveInArmXYToShuttle_9045 / GetShuttleCol / GetInArmPitchX/Y_9045 / InArmZNeedDown_9045 / ...
#include "ainarm_SearchPickPlate.h" // shared HP-pick externs (bInArmTryPickFromHotPlateFinish / DoInArmPickFromHotPlate_9045 / HasHotReadyIC_9045 / ...)
...
#include "atester.h"                // soak/wait timers consumed by the SM
#include "aHotPlateSubstrate.h"     // InArmSuck/FLCarryKit/BLCarryKit/OutArmSuck/FTestSuck/BTestSuck + ainarm2 cursors/timers + engine shims + PickFromHPList + tRotate + ptrInSHT(Backup) + Zteach facade + DoCheckAutoSiteMappingPosition + InitInArm*Task
...
#include "acatchtray_shims.h"        // InitProcessSingleMotorTask / MoveInArm2XYToWait / InShtInLF / InSHT1InLF / CheckOneCycleAction
#include "ainarm9045_2x4_16_shims.h"// InitInArmTryPickFromHotPlateTask100 / MyTickCount / iInposLed
...
//        CopyInitSuck(&InArmSuckBackup , &InArmSuck , 1, 1, 1, 6);
    ...
    //        CopyInitSuck(&InArmSuckBackup , &InArmSuck , 1, 1, 1, 2);
        ...
        {
//            if(TestIF_File.dSiteXPitch>iXpitchMaxX2_MM)                       //Steven 20251024 : mark for 1x4_2_14
//            {
...
//                CopyInitSuck(&OutArmSuckBackup, &OutArmSuck, 0, 1, 0, 2);
//                CopyInitSuck(&OutArmSuckBackup, &OutArmSuck, 1, 1, 1, 2);
//                InArmSuck.SetPickerCount(1, 2, 1, 4, 3, 2, 2);                  //Jimmychiu 20250220:修正Inarm保持AG吸嘴
//                OutArmSuck.SetPickerCount(1, 2, 1, 4, 2, 2, 2);
//                return;
//            }
        }
    }
    InArmSuck.SetPickerCount(1, 2, 1, 4, 3, 2, 2);
    OutArmSuck.SetPickerCount(1, 2, 1, 4, 3, 2, 2);
}
//==============================================================================
...
int GetNowSiteKitMode_1x4_2(int iSht, int iKit, bool bPlace)
{
    ...
    else
    {
        for(int i=0; i<FLCarryKit.iMaxRow; i++)                                 //Steven 20241206 : InArmSuck.iShtRow -->　FLCarryKit.iMaxRow
        {
            ...
            {
                if(iSht==0)
                    ptrInSHTBackup.Item[i][j]=(FLCarryKit.Item[i][j] || FTestSuck.Item[i][j]?iX:0);                     //kevin 20220928 change shuttle for autoclean pick data   //ChungHung 20131120 AutoClean use Hotplate1
                else
                    ptrInSHTBackup.Item[i][j]=(BLCarryKit.Item[i][j] || BTestSuck.Item[i][j]?iX:0);                     //kevin 20220928 change shuttle for autoclean pick data  //ChungHung 20131120 AutoClean use Hotplate1
            }
        }
...
//==============================================================================
void InArmZNeedDown_1x4_2(int iSht, int iKit, bool bPlace)
{
    if(InitialInArmNeedSuck(iSht, bPlace)==false)
        return;

    int iSuckRow, iSuckCol, iShtCol;
    int iMode   =GetNowSiteKitMode_1x4_2(iSht, iKit, bPlace);
    ...
    for(int i=0; i<1; i++)
    {
        for(int j=0; j<4; j++)
        {
            ...
            if(iModeCol==100 && (j==0 || j==3) &&                               //吸嘴       ==>  蝦頭
               (iModeRow==2 || iModeRow==i))                                    // O X X O        O O X X
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
            else if(iModeCol==110 && (j==0 || j==3) &&                          //吸嘴       ==>  蝦頭
                    (iModeRow==2 || iModeRow==i))                               // O X X O        X X O O
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
            ...
            else if(iModeCol==201 && j==3 &&                                    //吸嘴       ==>  蝦頭
                    (iModeRow==2 || iModeRow==i))                               // X X X O        X O X X
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
            else if(iModeCol==210 && j==0 &&                                    //吸嘴       ==>  蝦頭
                    (iModeRow==2 || iModeRow==i))                               // O X X X        X X O X
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
...
//==============================================================================
void CheckXYPitch_1x4_2(int *iX, int *iY, int iSht, int iKit, bool bPlace, int iMovePitchX, int iMovePitchY)
{
    int iMode       =GetNowSiteKitMode_1x4_2(iSht, iKit, bPlace);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
...
//==============================================================================
bool MoveInArm2XYToShuttle_9045_1x4_2(int iSht, int iKit, bool IncludeZ, bool bPlace)
{
    ...
    {
        if(InArmSuck.iXStep==1)
            iMovePitchX=TestIF.dSiteXPitch*1.0;
        else
    ...
    CheckXYPitch_1x4_2(&iXPos, &iYPos, iSht, iKit, bPlace, iMovePitchX, iMovePitchY);
...
//==============================================================================
bool DoPlaceToHotPlate_9045_1x4_2_14()
{
...
//==============================================================================
bool DoInArmPlaceToShuttle_9045_1x4_2_14()
{
//    IN_ARM_SHUTTLE_1x4_2:
...
//==============================================================================
bool GetShtNeedActive_1x4_2_14(bool bPick,int iShtRow,int iShtCol,int iSuckRow,int iSuckCol)
{
    bool b1=false,b2=false,b3=false,bret=false;
    if(bPick==bAutoPick)
    {
        b1=ptrInSHT->Item[iShtRow][iShtCol]==HAS_CLEAN_IC;
        b2=ptrInSHT->Item[iShtRow][iShtCol]==HAS_NULL_CLEAN_IC;
        b3=InArmSuck.Item[iSuckRow][iSuckCol]==NULL_IC;
        bret=b3 && !(b1 || b2);
    }
    else
    {
        b1=ptrInSHT->Item[iShtRow][iShtCol]==NULL_IC;
        b2=InArmSuck.Item[iSuckRow][iSuckCol]==HAS_CLEAN_IC;
        b3=InArmSuck.Item[iSuckRow][iSuckCol]==HAS_NULL_CLEAN_IC;
        bret=b1 && !(b2 || b3);
    }
    return bret;
}
//==============================================================================
int GetNeedActive_1x4_2_14(bool bPick)
{
    bool b1=false,b2=false;
    b1=GetShtNeedActive_1x4_2_14(bPick,0,0,1,0);
    b2=GetShtNeedActive_1x4_2_14(bPick,0,1,1,3);
    if(b1 || b2)
    {
        return 1;
    }

    b1=GetShtNeedActive_1x4_2_14(bPick,0,2,1,0);
    b2=GetShtNeedActive_1x4_2_14(bPick,0,3,1,3);
    if(b1 || b2)
    {
        return 3;
    }
...
{
    InArmSuck.SetItemData(1, 1, HAS_NULL_CLEAN_IC);
    InArmSuck.SetItemData(1, 2, HAS_NULL_CLEAN_IC);
    return GetNeedActive_1x4_2_14(bPick);
}
//==============================================================================
void SetInArmHasDropToShuttle_1x4_2(int SHT,int Row,int Col)
{
    if(SHT==0)
    {
        ...
        else
        {
            if(Row==0 && Col==0)
            {
                bDropAtSht1NeedCheckVac[0][2]=true;
            }
            else if(Row==0 && Col==3)
            {
                bDropAtSht1NeedCheckVac[0][3]=true;
            }
        }
    }
    else
    {
        ...
        else
        {
            if(Row==0 && Col==0)
            {
                bDropAtSht2NeedCheckVac[0][2]=true;
            }
            else if(Row==0 && Col==3)
            {
                bDropAtSht2NeedCheckVac[0][3]=true;
            }
        }
    }
}
