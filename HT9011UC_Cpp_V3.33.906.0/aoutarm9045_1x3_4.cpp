// =============================================================================
//  aoutarm9045_1x3_4.cpp  --  Out-Arm per-site VARIANT (1x3_4, single-row 3-picker
//                             over the 4-wide out shuttle)
//
//  Faithful translation of golden aoutarm9045_1x3_4.cpp (1490 lines, BCB6, Big5).
//  Translator: AI(W6.2c-1x3_4) 20260627
//  Translation wave: W6.2c (out-arm per-site-config variant: 1x3_4).  This is a
//  LIVE variant: the out-arm engine dispatch ladder DoOutArm_9045() routes
//  iInArmType==e9045_1x3_4 to DoOutArm_9045_1x3_4(), and DoPickFromShuttle_9045()
//  routes to DoPickFromShuttle_9045_1x3_4(int).  Integration (serial, NOT here):
//  add this file to CMake ht9045_sm, remove the two static stubs in
//  aoutarm9045.cpp, un-gate the two extern decls.
//
//  ROLE: the per-site out-arm pick-from-shuttle / place-to-auto state machines
//  for the 1x3_4 layout.  This file owns (all golden file-local):
//    GetNowShuttleMode_1x3_4, OutArmZNeedDown_1x3_4, CheckOutArmXYPitch_1x3_4,
//    MoveOutArmToShuttleIncludeZ_9045_1x3_4, DoPickFromShuttle_9045_1x3_4,
//    DoOutArm_9045_1x3_4.
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula,
//  fall-through and commented-out block is transcribed VERBATIM from the golden
//  CP950.  No body is #if 0-gated (the engine helpers/globals it touches resolve
//  over the out-arm engine + aHotPlateSubstrate + shims; the not-yet-translated
//  out-arm engine helpers/globals are forward-declared below and REPORTED for the
//  serial Integrate phase -- they are link-time externs, not syntax errors).
//
//  VCL/Borland conversions: AnsiString (1-based) via vclcompat; no
//  __fastcall/__property/__published/PACKAGE in this file.  ZeroMemory comes from
//  <windows.h> pulled by MachineDefine.h (mirrors sibling aoutarm9045.cpp).
//  SOFT_SIMULTE is NOT defined -> the one #ifdef SOFT_SIMULTE block in
//  DoPickFromShuttle_9045_1x3_4 compiles OUT (golden behavior off-machine),
//  reproduced VERBATIM.  Numeric semantics (int vs double, integer division/
//  truncation, double pitch math) kept EXACT.
//
...
//   #include "AutoTeach.h" / "FixAICCD.h" / "Magazine.h" ; #pragma package(smart_init)
// =============================================================================
...
// NOTE: golden also #includes the base engine "aoutarm.h" (not yet translated in
// this tree).  Its exports used here -- TransferOutShuttleRatio / SetOutArmNeedDestory /
// SwapShuttleDataToOutArm / ShowOutputShuttleDataMiss / CheckTesterZ / PickFromShuttle /
// bOutShtwaitPick / OutArmPickShuttleAlarm / PorcessJAM0201OutArmPickUpErrorSkip /
// ErrorBinBoxDetect / iPickFromShuttle1Task / iPickFromShuttle2Task / OutArmTask /
// bOutArmManualStepPress -- are forward-declared in the guarded block below and REPORTED.
#include "ainarm9045.h"            // GetShuttleCol(int,int,bool=false) / bCheckYPitchRunHomeSen / bCheckYPitchHome
...
#include "FormsFacade.h"           // TrayForm / fMain / fFixAICCD offline stand-ins
#include "canary_support.h"        // ShowErrorMessage / ShowMyMessage / RecordProcess / LastSet / IniConfig
...
#include "ainarm9045_2x4_16_shims.h"// AutoTeachLoadTrayZ / MyTickCount
...
//  Magazine.cpp / mykitsuck.h), partially translated.
//
...
// -----------------------------------------------------------------------------
#ifndef AOUTARM9045_1X3_4_ENGINE_FWD
#define AOUTARM9045_1X3_4_ENGINE_FWD
extern TMyKitSuck *ptrOutSHT;                                                   // golden mykitsuck.h:381
extern bool bOutSuckShtDupErr[MAX_ARM_Row][MAX_ARM_Col];                        // golden aoutarm9045.h:8
extern int  iOutShtRetryCount;                                                  // golden aoutarm9045.h:9
extern int  iPickFromShuttle1Task;                                             // golden aoutarm.h:46
extern int  iPickFromShuttle2Task;                                             // golden aoutarm.h:47
extern int  OutArmTask;                                                        // golden aoutarm.h:14
extern bool bOutArmManualStepPress;                                            // golden aoutarm.h:12

void TransferOutShuttleRatio(int iShuttle, int *iXPos, int *iYPos, int Row, int Col); // golden aoutarm.h:105
void SetOutArmNeedDestory(int iRow, int iCol, int iShtRow, int iShtCol, bool bPlace=false); // golden aoutarm.h:171
bool SwapShuttleDataToOutArm(int iSht, int iShtRow, int iShtCol, int iSuckRow, int iSuckCol, int *iWitchErrBin); // golden aoutarm.h:126
void ShowOutputShuttleDataMiss();                                              // golden aoutarm.h:100
bool CheckTesterZ(int WhichShuttle);                                           // golden aoutarm.h:56
void PickFromShuttle(int ibwhichKit);                                          // golden aoutarm.h:68
bool bOutShtwaitPick();                                                        // golden aoutarm.h:131
int  OutArmPickShuttleAlarm(int iSht, bool bHasDuplicateErr, AnsiString ErrPart); // golden aoutarm.h:106
void PorcessJAM0201OutArmPickUpErrorSkip(int iSht, int iShtR, int iShtC, int iSuckR, int iSuckC); // golden aoutarm.h:107
bool ErrorBinBoxDetect(int iError);                                            // golden aoutarm.h:124
void SetOutArmHome();                                                          // golden aoutarm.h:97
int  GetVariableYOutShuttleData();                                             // golden aoutarm9045.h:23
int  GetOutArmToShuttleOffset_9045(int iSht, int iModeRow, int iModeCol, bool bPitch); // golden aoutarm9045.cpp:1211
int  GetOutArmYToShuttleOffset_9045(int iSht, int iOffsetPos);                 // golden aoutarm9045.cpp:1303
int  GetOutArmXToShuttleOffset_9045(int iSht, int iOffsetPos);                 // golden aoutarm9045.cpp:1290

bool CheckOutArmInitState();                                                   // golden aoutarm.h
bool IsCatchTrayReadySupplyNewTray();                                          // golden acatchtray.h
bool MoveOutArmXY_ToFix_Tray_Full(bool bMoveY=false);                          // golden aoutarm9045.h:41
int  CheckOutArmCleanOut(int Task=50);                                         // golden aoutarm9045.h:58
bool CheckOutArmToTask50(int iSht);                                            // golden aoutarm9045.h:49
bool CheckShuttleICPos(int WhichShuttle);                                      // golden aoutarm9045.h:57
void InitPickFromShuttle1Task();                                               // golden aoutarm.h:67
void InitPickFromShuttle2Task();                                               // golden aoutarm.h:72
void OutArmSubSpeed();                                                         // golden aoutarm9045.h:70
void OutArmSubSpeedDisplay();                                                  // golden aoutarm9045.h:128
void OutArmAddSpeed();                                                         // golden aoutarm9045.h:69
void OutArmAddSpeedDisplay();                                                  // golden aoutarm9045.h:127
void SetFixTrayFullIC();                                                       // golden aoutarm9045.h:73
bool DoFixTrayFullAlarm();                                                     // golden aoutarm9045.h:47
int  VerifyTrayStatus();                                                       // golden aoutarm9045.h:39
extern bool CheckRotateOutNotFinish();                                         // golden aRotateKIT.h:43
extern bool CheckPlaceToMagazineTray(int iWhichTray);                          // golden Magazine.h:148
extern bool DoPickFromMagazineBuffer();                                        // golden Magazine.h:152
int  SearchTrayToPlace_Magazine();                                             // golden aoutarm9045.h:63

extern bool bCarryControlOutarm1;                                              // golden aoutarm.h:94
extern bool bCarryControlOutarm2;                                              // golden aoutarm.h:95
extern int  iWhichAuto;                                                        // golden aoutarm.h:18
extern int  iOutRotateFinish;                                                  // golden aRotateKIT.h:19
extern int  iOutArmZTeachTask;                                                 // golden AutoTeach.h:235
extern TQPF_Timer hOutArmYpitchHomeTimer;                                      // golden ainarm2.h:213 (HTimer; offline TQPF_Timer -- same Off()/SetSecAndOn() surface, mirrors sibling aoutarm9045_1x1_1.cpp)
void InitPlaceToAutoTask();                                                    // golden aoutarm.h:66
#endif

// -----------------------------------------------------------------------------
//  fFixAICCD -- VCL form TfFixAICCD* (golden FixAICCD.h:152).  Belongs to the
//  FormsFacade offline stand-in surface (not yet present there).  REPORTED.
//  Minimal guarded offline stand-in so the two `.OutArmCycleCounterUpdate()`
//  call sites compile; gated behind the use predicates at runtime (USE_Fix_AI_CCD
...
//==============================================================================
// Provenance: golden aoutarm9045_1x3_4.cpp:35
int GetNowShuttleMode_1x3_4(int iSht)
{
...
//==============================================================================
// Provenance: golden aoutarm9045_1x3_4.cpp:78
bool OutArmZNeedDown_1x3_4(int iSht)
{
    if(InitialOutArmNeedSuck(iSht)==false)
        return false;

    ptrOutSHT   =(iSht==0)?&FRCarryKit:&BRCarryKit;
    int iMode   =GetNowShuttleMode_1x3_4(iSht);
...
//------------------------------------------------------------------------------
// Provenance: golden aoutarm9045_1x3_4.cpp:125
bool CheckOutArmXYPitch_1x3_4(int *iX, int *iY, int iSht, int iMovePitchX, int iMovePitchY)
{
    int iMode       =GetNowShuttleMode_1x3_4(iSht);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
    ...
    *iX=*iX+GetOutArmXToShuttleOffset_9045(iSht, iOffsetPos);
    ...
    if(OutArmZNeedDown_1x3_4(iSht)==false)
        return false;
    return true;
}
//==============================================================================
// Provenance: golden aoutarm9045_1x3_4.cpp:262
bool MoveOutArmToShuttleIncludeZ_9045_1x3_4(int iSht, bool bZDown)
{
...
//    if(bUseTwoArm32Site &&                                                      //kevin 20190322 add use two arm
...
//    }
    ...
    if(CheckOutArmXYPitch_1x3_4(&iXPos, &iYPos, iSht, iMovePitchX, iMovePitchY)==false && bZDown==true)
        return false;
...
//  Ver : 2003_07
//==============================================================================
// Provenance: golden aoutarm9045_1x3_4.cpp:345
    ...
    switch(Task)
    {
        case 1:
            if(ArmSpeed[OutArm].dWaitOnSH!=0)                                   //kevin 20210525 out arm 起動在shuttle 上等待 時間
            {
                ...
                {
                    if(MoveOutArmToShuttleIncludeZ_9045_1x3_4(iSht, false))     //移到 Shuttle 上等待
                    {
        ...
        case 10:
            ...
            Task=200;
        case 200:
            if(MoveOutArmToShuttleIncludeZ_9045_1x3_4(iSht, true))              //kevin 20210717 change function
            {
...
//==============================================================================
// Provenance: golden aoutarm9045_1x3_4.cpp:586
...
//#endif
        ...
        case 100:
            ...
            if(FirstEnter)
            {
//                if(TestIF.iTestMode==_6Site2X3N)                              //Jou 20150909 : For Add 32Site UPH, 讓Shuttle 2先回去補Input
//                {
...
//                else
            ...
            if(TestIF.iTestMode==_6Site2X3N &&                                  //Steven 20220425 : 2X3NN Mode
               ((LastSet.bUseTestSocket[0][0][0]==false && LastSet.bUseTestSocket[0][0][1]==false && LastSet.bUseTestSocket[0][0][2]==false) ||
                (LastSet.bUseTestSocket[1][0][0]==false && LastSet.bUseTestSocket[1][0][1]==false && LastSet.bUseTestSocket[1][0][2]==false)))
            {
                if(LastSet.bUseTestSocket[0][0][0]==false && LastSet.bUseTestSocket[0][0][1]==false && LastSet.bUseTestSocket[0][0][2]==false)
                    OldPos=0;
                else if(LastSet.bUseTestSocket[1][0][0]==false && LastSet.bUseTestSocket[1][0][1]==false && LastSet.bUseTestSocket[1][0][2]==false)
                    OldPos=1;
            }
        ...
        case 200:
            ...
            break;
        case 300:
            if(TestIF.iTestMode==_6Site2X3N)                                    //Jou 20150909 : For Add 32Site UPH, 讓Shuttle 2先回去補Input   0-->1
            {
                if(MoveOutArmToShuttleIncludeZ_9045_1x3_4(1, false))
...
//                        #endif
        ...
        case 1150:
            ...
            if(MoveOutArmToShuttleIncludeZ_9045_1x3_4(iWhichShuttle, false))
            {
                IsCatchTrayReadySupplyNewTray();

                if(OutArmSuck.iXStep!=1)
                {
                    iBackMode1=GetNowShuttleMode_1x3_4(0);
                }
        ...
        case 1100:
            ...
            {
                if(IniConfig.bA15_1ESDGiveWayFunction==true && USE_BU5_Function==true)   //Eastsun 20260521 整合
                {
...
//#endif
                ...
                if(OutArmSuck.iXStep!=1)
                {
                    iMode1=GetNowShuttleMode_1x3_4(0);
...
//                if(WhichAutoNeedTray()==0)                                      //JerryYang 20170731 (wei) Out arm要讓位才不會hang up
        ...
        case 1200:
            ...
            if(DoPickFromShuttle_9045_1x3_4(0))
            {
...
//                        #endif
        ...
        case 2050:
            ...
            if(MoveOutArmToShuttleIncludeZ_9045_1x3_4(iWhichShuttle, false))
            {
                ...
                if(OutArmSuck.iXStep!=1)
                {
                    iBackMode2=GetNowShuttleMode_1x3_4(1);
                }
                Task=2100;
            }
...
//#endif
                ...
                if(OutArmSuck.iXStep!=1)
                {
                    iMode2=GetNowShuttleMode_1x3_4(1);
        ...
        case 2200:
            ...
            if(DoPickFromShuttle_9045_1x3_4(1))
            {
                PreSetOutAdditionalFlag();                                                                              //Steven 20220614 : Fixed for out arm additional function
