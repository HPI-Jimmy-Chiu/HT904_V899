// =============================================================================
//  aoutarm9045_2x5_8.cpp  --  Out-Arm per-site VARIANT (2x5_8, two-row 5-col 8-pick)
//
//  Faithful translation of golden aoutarm9045_2x5_8.cpp (BCB6, Big5/cp950).
//  Translator: AI(W6.2c-2x5_8) 20260627
//  Translation wave: W6.2c-OUT (out-arm per-site-config variant: 2x5_8), batch-3 final.
//
//  ROLE: the per-site out-arm shuttle-pick / place-to-auto state machines for the
//  2x5_8 layout (10-site, "wei" comment).  This file owns (golden names VERBATIM):
//    GetNowShuttleMode_2x5_8, OutArmZNeedDown_2x5_8, CheckOutArmXYPitch_2x5_8,
//    MoveOutArmToShuttleIncludeZ_9045_2x5_8, DoPickFromShuttle_9045_2x5_8,
//    DoOutArm_9045_2x5_8.
//
//  ENGINE DISPATCH (LIVE variant): the out-arm engine aoutarm9045.cpp routes
//  iInArmType==e9045_2x5_8 -> DoOutArm_9045_2x5_8() and
//  DoPickFromShuttle_9045_2x5_8(int).  File layout suffix == callee suffix.
//
...
//  200->300, 300 special-5000 path, 1140->1150, 1150->1100, 2040->2050,
//  2050->2100, 3100->3300, 3300->3301, 3301->3500) are preserved EXACTLY.
//
//  NOTE (signature irregularities preserved VERBATIM):
//    * MoveOutArmToShuttleIncludeZ_9045_2x5_8 takes an EXTRA iKit parameter
//      (golden .cpp def names it `iKit`, .h declares it `iWhichKit`).
//    * The three pure helpers carry the _2x5_8 suffix WITHOUT the _9045_ infix
//      (GetNowShuttleMode_2x5_8 / OutArmZNeedDown_2x5_8 / CheckOutArmXYPitch_2x5_8).
//    * CheckOutArmXYPitch_2x5_8 carries an extra iKit parameter.
//
...
//
//  OFFLINE GATES (#if 0 // TODO(W7)): the two fAutoTeach UI references (case 200
//  iATOutArmWhichKit assign; case 500 IsRun()/DoNext()) and the fFixAICCD UI call
//  (case iOUTARM_SUCK OutArmCycleCounterUpdate) have no offline form stand-in;
//  gated out behind #if 0 // TODO(W7), bodies kept VERBATIM.
//
//  Big5: every Chinese comment decoded via cp950 and preserved as UTF-8.
//  Final gate: ZERO U+FFFD.
//
//  INCLUDE REWRITE (mirror of the golden header set, translated subset):
//    golden: MachineDefine.h ; aoutarm9045_2x5_8.h ; aArmHeader.h ; MyKitSuck.h ;
...
#include "MachineType.h"            // e9045_2x5_8 / ep16Picker / iXYPitch16Bd_Be / iXYPitchVariable / eBulkBox / eAboveCoveyor / eUnderCoveyor / CC_KYEC_LEE global enums
//---------------------------------------------------------------------------
#include "aoutarm9045_2x5_8.h"
#include "aArmHeader.h"             // __FUNC__ shim, RecordProcess, OutArm, etc.
#include "aoutarm9045.h"            // engine: GetOutArmPitch*_9045 / DoOutArmSuckPreOn / IsCheckOutArmDestroyActiveFinish / GetOutShuttleStatus_9045 / InitialOutArmNeedSuck / OutArmNeedCheckOffset / iOUTARM_SUCK / SearchTrayToPlace_9045 / VerifyFixTrayLink / DoOutArmPlaceToAuto_9045 / CheekNeedToDoOutArmAdditionalFunction / MoveOutArmXY_To_ESDSafePos / DoOutArmAdditionalFunction / DoOutArmAfterPlaceToAuto / InitPlaceToAutoTask / OutArmContinuousMove_9045 / MoveOutArmToAutoSafe_9045 / MoveOutArmXY_To_ESDSafePos / GetOutArmPitchX_9045 / GetOutArmPitchY_9045
#include "csystem.h"                // OutSHT1InRT / OutSHT2InRT / HasICUnderMachine / HasICUnderHotPlate / IsOutArmCleanOutFinish / AutoTrayReCheck / CheckOutArmSuckICFallDown / HasICUnderHotPlate
#include "Motor/mymotor.h"          // MOT[], OutArmContinuousMove_9045
#include "cmydef.h"                 // global scalar universe + IC consts + iInArmType + enums + bEnterOffset / bCheckShuttle1/2Flag / iCleanOut / IniConfig / USE_* / iXpitch* / Mxxx motor ids / iAutoIndex / iMMAuto / AseIcRecord / bNeedCheckOurArmDeviceSuck / bMagNeedChangeTray / bMagCatchTrayfalg / bChaneMagTrayflag / iAuto3MagazineIndex / iMagAtAuto / iWhichMag / iOutArmWhichAuto / bSortingAllBinTrayFinish / bPickSH1Flag / bPickSH2Flag / bOutarmAutoHigh / USE_Fix_AI_CCD / iCatchTrayControlManual / iOutArmiWhichKit
#include "cprod.h"                  // Prod / TestIF / TestIF_File / ArmSpeed / OutArmOffSet / iXTrayColor / LastSet
#include "cpublic.h"                // CosFunction
#include "common.h"
#include "mysensor.h"               // Sen[] (SnFixedTrayDetect)
#include "acatchtray.h"             // WhichAutoNeedTray
#include "aHotPlateSubstrate.h"     // OutArmSuck / FRCarryKit / BRCarryKit (TMyKitSuck) + cursors + SetMotorSpeed + DoOutArmPlaceToAuto_9045 / DoOutArmAdditionalFunction / DoOutArmAfterPlaceToAuto / InitPlaceToAutoTask / iWhichBuff / iWhichMag
...
//==============================================================================
#ifndef OUTARM_2x5_8_FWD_GUARD
#define OUTARM_2x5_8_FWD_GUARD
...
// shuttle-column remap helper (golden ainarm9045.h:44; that header is not pulled
// here to avoid the bCheckYPitch* / substrate clash, so declared standalone)
int  GetShuttleCol(int iSuckRow, int iSuckCol, bool bOutArm=false);            // golden ainarm9045.h:44 -- offline: identity column map
#endif // OUTARM_2x5_8_FWD_GUARD

//==============================================================================
//  GetNowShuttleMode_2x5_8  -- golden aoutarm9045_2x5_8.cpp:31
//==============================================================================
int GetNowShuttleMode_2x5_8(int iSht)                                           //Steven 20241225 : Fixed for 2x5
{
    ptrOutSHT   =(iSht==0)?&FRCarryKit:&BRCarryKit;
    int iKit    =(ptrOutSHT->LeftSideNoIC(OutArmSuck.iShtKitStep)==true)?1:0;
        ...
        else if(OutArmSuck.iXStep==2)
        {
            ...
            else
            {
                if(ptrOutSHT->Item[0][0]>=HAS_IC || ptrOutSHT->Item[0][1]>=HAS_IC)
                    ...
                    return 10101;
                else if(ptrOutSHT->Item[0][3]>=HAS_IC || ptrOutSHT->Item[0][4]>=HAS_IC)
                    return 11100;
                else
                    return 11101;
            }
...
//==============================================================================
//  OutArmZNeedDown_2x5_8  -- golden aoutarm9045_2x5_8.cpp:152
//==============================================================================
bool OutArmZNeedDown_2x5_8(int iSht)
{
    if(InitialOutArmNeedSuck(iSht)==false)
        return false;

    ptrOutSHT   =(iSht==0)?&FRCarryKit:&BRCarryKit;
    int iMode   =GetNowShuttleMode_2x5_8(iSht);
    ...
    for(int i=0; i<OutArmSuck.iPickRow; i++)
    {
        for(int j=0; j<OutArmSuck.iPickCol; j++)
        {
            ...
            {                                                                   //O O O X      O O O X X X X X
               ...
               (iModeRow==2 || iModeRow==i))                                    //X X O O      X X X O O X X X
            ...
            {                                                                   //O X O X      O O X X X X X X
                SetOutArmNeedDestory(i, j, i, iShtJ);
                    ...
                    (iModeRow==2 || iModeRow==i))                               //X O X Q      X X O X X Q X X
            {                                                                   //X O X Q      X X O X X Q X X
                SetOutArmNeedDestory(i, j, i, iShtJ);
            }
            else if(iModeCol==110 && (j==0 || j==2) &&                          //吸嘴    ==>  蝦頭 Kit 0
                    (iModeRow==2 || iModeRow==i))                               //Q X Q X      X X X X X X Q Q
            {                                                                   //Q X Q X      X X X X X X Q Q
                SetOutArmNeedDestory(i, j, i, iShtJ);
            }
            else if(iModeCol==111 && (j==1 || j==3) &&                          //吸嘴    ==>  蝦頭 Kit 1
                    (iModeRow==2 || iModeRow==i))                               //X O X O      X X X O O X X X
            ...
            {                                                                   //X X O X      X X O X X X X X
                SetOutArmNeedDestory(i, j, i, iShtJ);
                    ...
                    (iModeRow==2 || iModeRow==i))                               //X X X Q      X X X X X Q X X
            {                                                                   //X X X Q      X X X X X Q X X
                SetOutArmNeedDestory(i, j, i, iShtJ);
            }
            else if(iModeCol==210 && j==0 &&                                    //吸嘴    ==>  蝦頭 Kit 1
                    (iModeRow==2 || iModeRow==i))                               //Q X X X      X X X X X X Q X
            {                                                                   //Q X X X      X X X X X X Q X
                SetOutArmNeedDestory(i, j, i, iShtJ);
            }
            else if(iModeCol==211 && j==1 &&                                    //吸嘴    ==>  蝦頭 Kit 1
                    (iModeRow==2 || iModeRow==i))                               //X Q X X      X X X X X X X Q
            {                                                                   //X Q X X      X X X X X X X Q
                SetOutArmNeedDestory(i, j, i, iShtJ);
            }
            ...
            {                                                                   //X X O X      X X X O X X X X
                SetOutArmNeedDestory(i, j, i, iShtJ);
                    ...
                    (iModeRow==2 || iModeRow==i))                               //X X X O      X X X X O X X X
...
//------------------------------------------------------------------------------
//  CheckOutArmXYPitch_2x5_8  -- golden aoutarm9045_2x5_8.cpp:247
//------------------------------------------------------------------------------
bool CheckOutArmXYPitch_2x5_8(int *iX, int *iY, int iSht, int iKit, int iMovePitchX, int iMovePitchY)
{
    int iMode       =GetNowShuttleMode_2x5_8(iSht);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
    ...
    else if(USE_PICKER_COUNT==ep16Picker)                                       //基準為第四隻吸嘴
    {
        ...
        if(iModeCol==0)                                                         //Aa --> Aa
        {
            *iX=*iX+(dMovePitchX*1.0-TestIF.dSiteXPitch*2.0);
        }
        else if(iModeCol==1)                                                    //Aa --> Ad
        {
        ...
        else if(iModeCol==210)                                                  //Aa --> Ad
        {
            *iX=*iX+(dMovePitchX*1.0+TestIF.dSiteXPitch*1.0);
        }
        else if(iModeCol==211)                                                  //Ab --> Ae
        {
            *iX=*iX+(dMovePitchX*0.0+TestIF.dSiteXPitch*2.0);
        }
    }
    else if(USE_OUT_ARM_Y_PITCH==iXYPitchVariable)                              //基準為第二隻吸嘴  //JerryYang 20251218 : IN/OUT ARM支援不同模組
    {
        dMovePitchX=double(iMovePitchX)/3.0;                                    //Steven 20250218 : fixed for 2x5
        if(iModeCol==0)                                                         //Aa --> Aa
        {
            *iX=*iX+(dMovePitchX*1.0-TestIF.dSiteXPitch*2.0);
        }
        else if(iModeCol==10)                                                   //Ac --> Ad
        {
            *iX=*iX+(-dMovePitchX*1.0+TestIF.dSiteXPitch*1.0);
        }
        else if(iModeCol==100)                                                  //Aa --> Aa
        ...
        else if(iModeCol==210 ||                                                //沒有這項
                iModeCol==211 ||                                                //沒有這項
                iModeCol==212)                                                  //Ac --> Ad
        {
            *iX=*iX+(-dMovePitchX*1.0+TestIF.dSiteXPitch*1.0);
        }
        else if(iModeCol==213)                                                  //Ad --> Ae
        {
            *iX=*iX+(-dMovePitchX*2.0+TestIF.dSiteXPitch*2.0);
        }
    }
    else                                                                        //基準為第三隻吸嘴
    {
    ...
    *iX=*iX+GetOutArmXToShuttleOffset_9045(iSht, iOffsetPos);
    ...
    if(OutArmZNeedDown_2x5_8(iSht)==false)
        return false;
    return true;
}
//==============================================================================
//  MoveOutArmToShuttleIncludeZ_9045_2x5_8  -- golden aoutarm9045_2x5_8.cpp:440
//==============================================================================
bool MoveOutArmToShuttleIncludeZ_9045_2x5_8(int iSht, int iKit, bool bZDown)
{
    ...
    if(CheckOutArmXYPitch_2x5_8(&iXPos, &iYPos, iSht, iKit, iMovePitchX, iMovePitchY)==false && bZDown==true)
        return false;
...
//  Ver : 2003_07
//  DoPickFromShuttle_9045_2x5_8  -- golden aoutarm9045_2x5_8.cpp:497
//==============================================================================
    ...
    switch(Task)
    {
        case 1:
            if(ArmSpeed[OutArm].dWaitOnSH!=0)                                                                           //kevin 20210525 out arm 起動在shuttle 上等待 時間
            {
                ...
                {
                    if(MoveOutArmToShuttleIncludeZ_9045_2x5_8(iSht, iOutArmiWhichKit, false))                           //移到 Shuttle 上等待
                    {
        ...
        case 10:
            ...
            Task=200;
        case 200:
            if(MoveOutArmToShuttleIncludeZ_9045_2x5_8(iSht, iOutArmiWhichKit, true))                                    //kevin 20210717 change function
            {
#if 0 // TODO(W7): fAutoTeach form UI not available offline (no FormsFacade TfAutoTeach)
        ...
        case iOUTARM_SUCK:
            ...
            {
#if 0 // TODO(W7): fFixAICCD form UI not available offline (no FormsFacade TfFixAICCD)
...
//==============================================================================
//  DoOutArm_9045_2x5_8  -- golden aoutarm9045_2x5_8.cpp:785
//==============================================================================
...
//                        #endif
        ...
        case 1150:
            ...
            if(MoveOutArmToShuttleIncludeZ_9045_2x5_8(iWhichShuttle, iOutArmiWhichKit, false))
            {
                IsCatchTrayReadySupplyNewTray();
...
//            }
            ...
            if(DoPickFromShuttle_9045_2x5_8(0))
            {
...
//                        #endif
        ...
        case 2050:
            ...
            if(MoveOutArmToShuttleIncludeZ_9045_2x5_8(iWhichShuttle, iOutArmiWhichKit))
            {
                IsCatchTrayReadySupplyNewTray();
...
//            }
            ...
            if(DoPickFromShuttle_9045_2x5_8(1))
            {
                PreSetOutAdditionalFlag();                                                                              //Steven 20220614 : Fixed for out arm additional function
