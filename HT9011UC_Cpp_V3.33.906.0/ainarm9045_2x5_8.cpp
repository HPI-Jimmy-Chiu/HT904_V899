// =============================================================================
//  ainarm9045_2x5_8.cpp  --  In-Arm per-site VARIANT (2x5_8)
//
//  Faithful translation of golden ainarm9045_2x5_8.cpp (3529 lines, BCB6, Big5).
//  Translator: AI(W6.2c-2x5_8) 20260626
//  Translation wave: W6.2c batch-4 (in-arm per-site-config variant: 2x5_8; two
//  physical rows, 5-of-8 picker columns, dual-Kit 32-site shuttle layout).
//  Sibling of the already-translated leaves ainarm_SearchPickPlate /
//  ainarm_SearchPlacePlate and the engine ainarm9045.cpp.
//
//  ROLE: the per-site in-arm pick/place state machines for the 2x5_8 layout.
//  The engine's dispatch ladder DoInArm_9045() / DoInArm_9045_SuckerMap() routes
//  iInArmType==e9045_2x5_8 here (un-gated at the integrate stage).  This file owns:
//    SetInOutArmParameter_2x5_8, GetNowSiteKitMode_2x5_8, InArmZNeedDown_2x5_8,
//    CheckXYPitch_2x5_8, MoveInArm2XYToShuttle_9045_2x5_8,
//    DoPlaceToHotPlate_9045_2x5_8, DoInArmPlaceToShuttle_9045_2x5_8,
//    DoInArm_9045_2x5_8_SuckerMap, DoInArm_9045_2x5_8,
//    DoInArmTryPickFromHotPlate_9045_2x5_8, GetShuttleState_2x5_8 and the
//    file-owned const table XPHSuckToSht_2x5_8[3][8].
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula,
//  fall-through and goto/label (IN_ARM_SHUTTLE_2x5_8 / IN_ARM_TRY_PICK_HOTPLATE_2X5_8)
...
//  DoInArmTryPickFromHotPlate_9045_2x5_8 (golden :2830 :2934 :3039 :3143) keep
...
//
//  SUBSTRATE NEEDS: NONE new.  References only symbols already present in the
//  W6 substrate (verified): bUseTwoArm32Site (cmydef.h), ptrInSHTBackup /
//  SetShuttlefCanMoveL / SetInArmNeedDestory (aHotPlateSubstrate.h),
//  GetShuttleCol / InitialInArmNeedSuck / GetInArmToShuttleOffset_9045 /
//  SetClosedShtKitToHasNullIC_9045 (ainarm9045.h), iShuttleRowKit (cmydef.h).
//  bNeedAdjust2Time is a function-LOCAL bool (golden :910), NOT a substrate global.
//  Does NOT use Prod.fInArmSuckUse_TwoArm32Site nor SetNullIcToHasNullIc.
//
...
#include "ainarm9045.h"             // engine: GetInArmToShuttleOffset_9045 / GetShuttleCol / InitialInArmNeedSuck / SetClosedShtKitToHasNullIC_9045 / ...
...
#include "cmydef.h"                 // global scalar universe + IC consts + enums (eInArmType / picker enum / Tempture_* / rsm* / CC_* / bUseTwoArm32Site / iShuttleRowKit)
#include "common.h"
#include "mycylin.h"
...
#include "aHotPlateSubstrate.h"     // InArmSuck/FLCarryKit/BLCarryKit/OutArmSuck/FTestSuck/BTestSuck + ptrInSHTBackup + SetShuttlefCanMoveL + SetInArmNeedDestory + ainarm2 cursors/timers + engine shims
...
//==============================================================================
const int XPHSuckToSht_2x5_8[3][8]={{0, 1, 2, 5, 6, 7, 3, 4},
                                    {0, 2, 1, 5, 6, 3, 7, 4},
                                    {0, 1, 2, 5, 6, 7, 3, 4}};
//==============================================================================
bool DoInArmTryPickFromHotPlate_9045_2x5_8(bool bOneTimeCheckAll, bool bShowErrorMessage);
//==============================================================================
void SetInOutArmParameter_2x5_8()
{
    InArmSuck.SetPickerCount(2, 4, 2, 5, 1, 4, 3);
    OutArmSuck.SetPickerCount(2, 4, 2, 5, 1, 4, 3);
}
//==============================================================================
...
int GetNowSiteKitMode_2x5_8(int iSht, int iKit, bool bPlace)
{
    ...
    else
    {
        ...
        else if(InArmSuck.iXStep==2)                                            //Steven 20241206 : fixed for 2x5 auto clean
        {
            if(ptrInSHTBackup.Item[0][iShtCol0]==iX || ptrInSHTBackup.Item[0][iShtCol2]==iX)
                return 10000+iKitPos;
            else if(ptrInSHTBackup.Item[0][iShtCol1]==iX || ptrInSHTBackup.Item[0][iShtCol3]==iX)
                return 10100+iKitPos;
            else if(ptrInSHTBackup.Item[1][iShtCol0]==iX || ptrInSHTBackup.Item[1][iShtCol2]==iX)
                return 10001+iKitPos;
            else                                                                //if(ptrInSHTBackup.Item[1][iShtCol1]==iX || ptrInSHTBackup.Item[1][iShtCol3]==iX)
...
//==============================================================================
void InArmZNeedDown_2x5_8(int iSht, int iKit, bool bPlace)
{
    if(InitialInArmNeedSuck(iSht, bPlace)==false)
        return;

    int iSuckRow, iSuckCol, iShtCol;
    int iMode   =GetNowSiteKitMode_2x5_8(iSht, iKit, bPlace);
    ...
    for(int i=0; i<2; i++)
    {
        for(int j=0; j<4; j++)
        {
            ...
            else if(iModeCol==101 && (j==1 || j==3) &&                          //吸嘴    ==>  蝦頭 Kit 0
                    (iModeRow==2 || iModeRow==i))                               //X O X Q      X X O X X Q X X
            {                                                                   //X O X Q      X X O X X Q X X
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
            else if(iModeCol==110 && (j==0 || j==2) &&                          //吸嘴    ==>  蝦頭 Kit 1
...
//==============================================================================
void CheckXYPitch_2x5_8(int *iX, int *iY, int iSht, int iKit, bool bPlace, int iMovePitchX, int iMovePitchY)
{
    int iMode       =GetNowSiteKitMode_2x5_8(iSht, iKit, bPlace);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
...
//==============================================================================
bool MoveInArm2XYToShuttle_9045_2x5_8(int iSht, int iKit, bool IncludeZ, bool bPlace)
{
    ...
    CheckXYPitch_2x5_8(&iXPos, &iYPos, iSht, iKit, bPlace, iMovePitchX, iMovePitchY);
...
//==============================================================================
bool DoPlaceToHotPlate_9045_2x5_8()
{
...
//==============================================================================
bool DoInArmPlaceToShuttle_9045_2x5_8()
{
//    IN_ARM_SHUTTLE_2x5_8:
...
//            break;
        ...
        case 15000:                                                             //ChungHung 20120206 Hotplate check
            ...
            if(DoInArmTryPickFromHotPlate_9045_2x5_8(bOneTimeHotPlateCheckAll, bTryPickFromHotPlateShowError))
            {
                for(int i=0; i<2; i++)
                {
                    for(int j=0; j<4; j++)
...
bool DoInArmTryPickFromHotPlate_9045_2x5_8(bool bOneTimeCheckAll, bool bShowErrorMessage)
{
    IN_ARM_TRY_PICK_HOTPLATE_2X5_8:
    ...
    switch(Task)
    {
        ...
        case 350:
            if(HotPlateForm.XDivision==6)
            {
                ...
                Task=400;
                goto IN_ARM_TRY_PICK_HOTPLATE_2X5_8;
            }
            else
            {
                iXItem10Step=0;
...
int GetShuttleState_2x5_8(int iSht, bool bPick)
{
    if(bPick==bAutoPick)                                                        //Pick
    {
        if(ptrInSHT->ArmUpSideAllTypeIC(NULL_IC, 0, 3)==false)                  //左上有料
            ...
            return 2;
        else if(ptrInSHT->ArmUpSideAllTypeIC(NULL_IC, 3, 2)==false)             //右上有料
            return 3;
        else if(ptrInSHT->ArmDownSideAllTypeIC(NULL_IC, 3, 2)==false)           //右下有料
            return 4;
        else
            return 0;
    }
        ...
        if(bUse8Picker)
        {
            if(ptrInSHT->ArmUpSideAllTypeIC(NULL_IC, 0, 3) &&                   //左上無料
               ((bAutoCleanPlaceToSht==false && InArmSuck.RowHasDefineIC(0, NULL_IC)) ||                                //Steven 20221005 : Fixed for auto clean clean kit吸兩次
                ...
                return 2;
            else if(ptrInSHT->ArmUpSideAllTypeIC(NULL_IC, 3, 2) &&              //右上無料
                    ((bAutoCleanPlaceToSht==false && InArmSuck.RowHasDefineIC(0, NULL_IC)) ||                           //Steven 20221005 : Fixed for auto clean clean kit吸兩次
                     (bAutoCleanPlaceToSht==true  && InArmSuck.ArmUpSideNoIC()==false)))
                return 3;
            else if(ptrInSHT->ArmDownSideAllTypeIC(NULL_IC, 3, 2) &&            //右下無料
                    ((bAutoCleanPlaceToSht==false && InArmSuck.RowHasDefineIC(1, NULL_IC)) ||                           //Steven 20221005 : Fixed for auto clean clean kit吸兩次
                     (bAutoCleanPlaceToSht==true  && InArmSuck.ArmDownSideNoIC()==false)))
                return 4;
            else
