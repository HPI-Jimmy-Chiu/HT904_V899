...
// FTestIF -- GATE(G-FTestIF), see facade header. Golden `TFTestIF` (cTesterIF.h)
// has no facade anywhere in this port; only its ONE call shape
// (`FTestIF->ReadTestIFFile();`) is ever reached here, and both call sites are
// `#if 0`-gated (matching Automation/SCK_ART_Remainder.h gate #2 / Command.cpp
// gates #11-12), so `FTestIF` itself is never dereferenced by compiled code --
...
#if 0 // GATE(dep-fMain-edATCAmbientTemper)
            dbSetTemp=atof(fMain->edATCAmbientTemper->Text.c_str());
#endif // GATE(dep-fMain-edATCAmbientTemper)
        }
...
//---------------------------------------------------------------------------
// AI(W906-FW3-TempSet-WA) 20260820: SAFETY GATE (S3) -- bounds-check/decision
// logic stays ACTIVE; the two real `WriteIniData(...)` persist calls are
// gated individually.
//---------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
// AI(W906-FW3-TempSet-WA) 20260820: SAFETY GATE (S4) -- delegates to (S3),
// which self-gates its own writes; nothing else here touches disk/hardware.
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
// Read-only (ReadIniData only) -- no SAFETY GATE per task brief.
//------------------------------------------------------------------------------
