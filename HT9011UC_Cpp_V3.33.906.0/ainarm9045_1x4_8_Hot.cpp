// =============================================================================
//  ainarm9045_1x4_8_Hot.cpp  --  In-Arm per-site VARIANT (1x4_8_Hot,
//  1x4 layout, 8-picker high-temperature / HotPlate flow)
//
//  Faithful translation of golden ainarm9045_1x4_8_Hot.cpp (2662 lines, BCB6,
//  Big5).  Translator: AI(W6.2c-INARM-batch2) 20260626
//  Translation wave: W6.2c (in-arm per-site-config variant: 1x4_8_Hot).
//  Sibling of the already-translated leaves ainarm_SearchPickPlate /
//  ainarm_SearchPlacePlate and the engine ainarm9045.cpp.
//
//  ROLE: the per-site in-arm pick/place state machines for the 1x4 8-picker
//  high-temperature layout (exercises the HotPlate place/pick path).  The
//  engine's dispatch ladder DoInArm_9045() / DoInArm_9045_SuckerMap() routes
//  iInArmType==e9045_1x4_8_Hot here (un-gated at the integrate stage).  This
//  file owns (names VERBATIM from golden):
//    SetInOutArmParameter_1x4_8_Hot, GetNowSiteKitMode_1x4_8_Hot,
//    InArmZNeedDown_1x4_8_Hot, CheckXYPitch_1x4_8_Hot,
//    MoveInArm2XYToShuttle_9045_1x4_8_Hot, DoPlaceToHotPlate_9045_1x4_8_Hot,
//    DoInArmPlaceToShuttle_9045_1x4_8_Hot, DoInArm_9045_1x4_8_Hot_SuckerMap,
//    DoInArm_9045_1x4_8_Hot.
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula,
//  fall-through and goto/label (IN_ARM_SHUTTLE_1x4_8_Hot) is transcribed
//  VERBATIM from the golden CP950.  All SM case bodies are ACTIVE -- they touch
//  MOT[]/Suck/Sensor/predicates/FormsFacade, all of which resolve over the Sim
//  HAL + aHotPlateSubstrate + atester_shims + canary_support + ainarm9045
//  engine + ainarm_Search{Pick,Place}Plate leaves.  No body is #if 0-gated.
//
...
//  DoInArm_9045_1x4_8_Hot (fMain->Label3 caption) is compiled OUT, reproduced
//  verbatim.  Numeric semantics (int vs double, integer division, double pitch
//  math) kept EXACT.
//
//  Big5: every Chinese comment decoded via cp950 and preserved as UTF-8.
//  Final gate: ZERO U+FFFD.
// =============================================================================
// BCB6 ORIGINAL include block (mirrored as a comment for provenance, golden :1-30):
...
#include "ainarm9045_1x4_8_Hot.h"
#include "aArmHeader.h"             // __FUNC__ shim, RecordProcess, CheckHeaterOK, Zteach
#include "ainarm9045.h"             // engine: GetInArmToShuttleOffset_9045 / GetShuttleCol / InArmNeedCheckOffset / MoveInArmXYToShuttle_9045 / DoInArmPickFromLoadStage_9045 / DoInArmCheckShuttleFloating / DoInArmIonFanGiveWay / bCheckYPitch* ...
#include "ainarm9045_2x4_16.h"      // DoInArmPickFromHotPlate_9045
#include "ainarm_SearchPickPlate.h" // shared HP-pick externs (bInArmTryPickFromHotPlateFinish / HasHotReadyIC_9045 / InArmNeedCheckHotPlateOffset / GetHeaterWaitTime / TryHotClearRecBufMMPlate / ...)
#include "ainarm_SearchPlacePlate.h"// shared HP-place externs (SearchPlateToPlace / MoveInArmXYToHotPlatePlace / DoPlaceToHPSwapData / DoPlaceToHPBackupData / GetPlaceToHotPlateSuckCol / GetPlaceToHotPlateCol / Row2CanPutHP / GetHotPlateColStep / GetHotPlateYHalfPos / GetVariableXInHotPlateData / CheckHasSpaceToPlace_9045 / iPlaceHP / HPPlaceLog)
#include "csystem.h"                // IsInArmOneCycleFinish / IsInArmCleanOutFinish / DoInArmAutoSiteMapping / AutoSiteMappingCheckShuttle / DoSiteMappingCHK predicates
#include "Motor/mymotor.h"          // MOT[], InArmContinuousMove_9045
#include "cmydef.h"                 // global scalar universe + IC consts + enums (eInArmType / Tempture_* / rsm* / CC_* / SW[] / SwShuttleVibration*)
#include "common.h"
#include "mycylin.h"
#include "myswitch.h"               // SW[] (SwShuttleVibration1/2 .On())
#include "cprod.h"                  // Prod / TestIF / TestIF_File / ArmSpeed / ArmSpeed_File / LastSet / HotPlateForm / TrayForm
#include "cpublic.h"                // CosFunction
#include "aHotPlateSubstrate.h"     // InArmSuck/OutArmSuck/FLCarryKit/BLCarryKit/FTestSuck/BTestSuck/ptrInSHTBackup + InArmSuckBackup/OutArmSuckBackup + cursors/timers + engine shims + PickFromHPList + Init*Task + Move/Set helpers
...
#include "acatchtray_shims.h"       // MoveInArm2XYToWait / InitProcessSingleMotorTask (offline single-motor home)
#include "acarry_shims.h"           // InSHT1InLF / InSHT2InLF / InShtInLF / CheckOneCycleAction
#include "canary_support.h"         // ShowErrorMessage / ShowMyMessage / RecordProcess / LastSet / __FUNC__
//==============================================================================
// AI(W6.2c-INARM-batch2) 20260626: golden ainarm9045_1x4_8_Hot.cpp:32 (faithful translation, verbatim body)
...
//        CopyInitSuck(&InArmSuckBackup , &InArmSuck , 1, 7, 1, 7);
    ...
    OutArmSuck.SetPickerCount(2, 4, 1, 4, 1, 0, 0);
}
//==============================================================================
...
int GetNowSiteKitMode_1x4_8_Hot(int iSht, bool bPlace)
{
    ...
    int iUseRow=(iSht==1)?0:1;                                                  //RogerYang 20251231 : Row 1放SH1 Row 0放SH2

    if(bPlace==true)
    {
        if(InArmSuck.iXStep==1)
        {
            return (iSht==1)?0:1;                                               //Shuttle 2, 使用Row 0
        }
        else if(InArmSuck.iXStep==2)
        {
            if(iUseRow==0)                                                      //RogerYang 20251231 : Row 1放SH1 Row 0放SH2
            {
...
//==============================================================================
// AI(W6.2c-INARM-batch2) 20260626: golden ainarm9045_1x4_8_Hot.cpp:205 (faithful translation, verbatim body)
void InArmZNeedDown_1x4_8_Hot(int iSht, bool bPlace)
{
    if(InitialInArmNeedSuck(iSht, bPlace)==false)
        return;

    int iSuckRow, iSuckCol, iShtCol;
    int iMode   =GetNowSiteKitMode_1x4_8_Hot(iSht, bPlace);
    ...
    for(int i=0; i<2; i++)
    {
        for(int j=0; j<4; j++)
        {
            ...
            else                                                                //RogerYang 20251231 : Row 1放SH1 Row 0放SH2
            {
                iSuckRow=(iSht==0)?1:0;
            }
...
//==============================================================================
// AI(W6.2c-INARM-batch2) 20260626: golden ainarm9045_1x4_8_Hot.cpp:270 (faithful translation, verbatim body)
void CheckXYPitch_1x4_8_Hot(int *iX, int *iY, int iSht, bool bPlace, int iMovePitchX, int iMovePitchY)
{
    int iMode       =GetNowSiteKitMode_1x4_8_Hot(iSht, bPlace);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
...
//==============================================================================
// AI(W6.2c-INARM-batch2) 20260626: golden ainarm9045_1x4_8_Hot.cpp:409 (faithful translation, verbatim body)
bool MoveInArm2XYToShuttle_9045_1x4_8_Hot(int iSht, bool IncludeZ, bool bPlace)
{
    ...
    CheckXYPitch_1x4_8_Hot(&iXPos, &iYPos, iSht, bPlace, iMovePitchX, iMovePitchY);
...
//==============================================================================
// AI(W6.2c-INARM-batch2) 20260626: golden ainarm9045_1x4_8_Hot.cpp:495 (faithful translation, verbatim body)
bool DoPlaceToHotPlate_9045_1x4_8_Hot()
{
...
//            break;
        ...
        case 200:
            ...
            {
                PickFromHPList->UpdateHPSuckGroup(iPlacePlate[0], iPlacePlateY[0], iPlacePlateX[0], 0, InArmSuck.iWhichKit);                                    //Sam 20250904 : 修正 1X4 8高溫吸嘴模式
            }
...
//==============================================================================
// AI(W6.2c-INARM-batch2) 20260626: golden ainarm9045_1x4_8_Hot.cpp:797 (faithful translation, verbatim body)
bool DoInArmPlaceToShuttle_9045_1x4_8_Hot()
{
    IN_ARM_SHUTTLE_1x4_8_Hot:
    ...
    switch(Task)
    {
        case 1:
            ...
            {
                if(i==1 && InArmSuck.ArmDownSideNoIC())
                    continue;
            ...
            if(InArmSuck.Item[1][0]==HAS_NULL_IC &&                             //shuttle 1 使用後排 b d f h
               InArmSuck.Item[1][1]==HAS_NULL_IC &&
               InArmSuck.Item[1][2]==HAS_NULL_IC &&
               InArmSuck.Item[1][3]==HAS_NULL_IC)
            {
                ...
                Task=1500;                                                      //Sam 20240217 : 修正 InArm 只有上排有 IC 要跑到 Shuttle2
                return false;
            }
        ...
        case 1200:
            ...
            if(InArmSuck.ArmDownSideHaveRealIC()==false)
            {
                Task=1300;
        ...
        case 1500:
            ...
            if(InArmSuck.ArmUpSideHaveRealIC()==true)
            {
       ...
       case 1610:
        ...
        case 2500:
            ...
            if(InArmSuck.ArmDownSideHaveRealIC()==true)
            {
                bPlaceOtherShuttle=true;
...
//==============================================================================
// AI(W6.2c-INARM-batch2) 20260626: golden ainarm9045_1x4_8_Hot.cpp:1720 (faithful translation, verbatim body)
void DoInArm_9045_1x4_8_Hot_SuckerMap()
{
    for(int j=0; j<8; j++)
    {
        ...
        if(iShtCol<InArmSuck.iShtCol)
        {
            ...
            {
                Prod.fInArmSuck4x8[0][0][j]=LastSet.bUseTestSocket[1][0][iShtCol];
                Prod.fInArmSuck4x8[0][1][j]=LastSet.bUseTestSocket[0][0][iShtCol];                                      //Sam 20250212 : 修正 1X4 8 吸嘴模式

                Prod.fInArmSuck4x8[1][0][j]=LastSet.bUseTestSocket[0][0][iShtCol];
                Prod.fInArmSuck4x8[1][1][j]=LastSet.bUseTestSocket[1][0][iShtCol];
                Prod.iSiteMap[0][0][j]     =TestIF.iSiteMap[0][iShtCol];
                Prod.iSiteMap[1][0][j]     =TestIF.iSiteMap[0][iShtCol];
                if(TestIF.iTestMode==QualSite1X4)                               //RogerYang 20260102 : 1x4 8吸嘴模式，Row1沒有site資料，inarm會無法從hotplate取料 -> 把Row0的site copy給Row1
                {
                    Prod.iSiteMap[0][1][j]=TestIF.iSiteMap[0][iShtCol];
                    Prod.iSiteMap[1][1][j]=TestIF.iSiteMap[0][iShtCol];
                }
            }
        }
        else
...
//==============================================================================
// AI(W6.2c-INARM-batch2) 20260626: golden ainarm9045_1x4_8_Hot.cpp:1764 (faithful translation, verbatim body)
...
//    DoJudgeInputShuttleNeedChangeToNullIC();
    ...
    if(InArmSuck.iWhichKit!=0 && LastSet.iRunStartMode!=rsmAutoSiteMap)         //AI(ht9045-inarm-flow) 20260328 (RogerYang) : 1x4_8_Hot only uses kit0 (col 0-3); ASM may leave iWhichKit=1 causing DoPickLoaderOK to check col 4-7 (all false) → hangup
...
//            break;
        ...
        case 1550:
            ...
            if(BLCarryKit.UseSiteNoIC() ||                                      //Steven 20220113 : 避免IC冷掉, 慢點吸料
               InSHT2InLF()==false ||
