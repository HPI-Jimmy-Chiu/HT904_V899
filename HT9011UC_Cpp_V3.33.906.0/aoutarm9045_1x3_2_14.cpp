// =============================================================================
//  aoutarm9045_1x3_2_14.cpp  --  Out-Arm per-site VARIANT (1x3_2_14, single-row
//                                3-picker / 2-IC shuttle, close-site "_14" mode)
//
//  Faithful translation of golden aoutarm9045_1x3_2_14.cpp (1463 lines, BCB6, Big5).
//  Translator: AI(W6.2c-1x3_2_14) 20260627
//  Translation wave: W6.2c (out-arm per-site-config variant: 1x3_2_14).  This is
//  the LIVE callee for iInArmType==e9045_1x3_2_14: the out-arm engine dispatch
//  ladders route DoOutArm_9045_1x3_2_14() (aoutarm9045.cpp :487/:489) and
//  DoPickFromShuttle_9045_1x3_2_14(int) (aoutarm9045.cpp :623/:625) here.
//
//  ROLE: the per-site out-arm shuttle-pick / unload state machines for the
//  1x3_2_14 layout.  This file owns (all names VERBATIM from golden):
//    GetNowShuttleMode_1x3_2_14, OutArmZNeedDown_1x3_2_14,
//    CheckOutArmXYPitch_1x3_2_14, MoveOutArmToShuttleIncludeZ_9045_1x3_2_14,
//    DoPickFromShuttle_9045_1x3_2_14, DoOutArm_9045_1x3_2_14.
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula,
//  fall-through (the case 10->200, 1100..1235, 3100->3300, 3301->3500 fall-
//  throughs and the `if(Task!=N) break;` guards) are transcribed VERBATIM from
//  the golden CP950.  Numeric semantics (int vs double; integer divisions like
//  iMovePitchY/2; double dMovePitchX=double(iMovePitchX)/7.0 etc.) kept EXACT.
//
//  VCL/Borland conversions: AnsiString (1-based, ::sprintf) via vclcompat;
//  no __fastcall / __property / __published / PACKAGE in this file.  SOFT_SIMULTE
//  is NOT defined -> the single `#ifdef SOFT_SIMULTE` block (the chkInPickLoadError
//  simulate-drop in case iOUTARM_SUCK) is compiled OUT, reproduced verbatim.
//
...
//   #include "aoutarm9045_1x3_2_14.h"
...
#include "MachineType.h"            // machine globals / enums

#include "aoutarm9045_1x3_2_14.h"
#include "aArmHeader.h"             // __FUNC__ shim, RecordProcess, etc.
#include "aoutarm9045.h"            // out-arm engine: GetOutArmPitchX/Y_9045 / GetOutShuttleStatus_9045 / DoOutArmSuckPreOn / IsCheckOutArmDestroyActiveFinish / InitialOutArmNeedSuck / OutArmNeedCheckOffset / iOUTARM_SUCK / DoOutArmPlaceToAuto_9045 / DoOutArmAfterPlaceToAuto / CheekNeedToDoOutArmAdditionalFunction / DoOutArmAdditionalFunction / SearchTrayToPlace_9045 / VerifyFixTrayLink / MoveOutArmXY_To_ESDSafePos
#include "ainarm9045.h"             // GetShuttleCol
#include "Motor/mymotor.h"          // MOT[], OutArmContinuousMove_9045
#include "cmydef.h"                 // global scalar universe + IC consts + enums (iInArmType / picker enums / Tempture_* / CC_* / MAX_ARM_Row/Col / X_PITCH_COUNT / iXpitchMaxX3/X7 / iOutArm*Base / USE_* / iOutArmiWhichKit / bCheckShuttle1/2Flag / AseIcRecord / bEnterOffset / bSortingAllBinTrayFinish)
#include "mycylin.h"
#include "mysensor.h"               // Sen[]
#include "cprod.h"                  // Prod / TestIF / TestIF_File / ArmSpeed / LastSet / IniConfig
#include "atester.h"                // TestIF.iTestMode / iShuttleMode / dSiteXPitch
#include "aHotPlateSubstrate.h"     // TMyKitSuck / TMySucker + OutArmSuck / FRCarryKit / BRCarryKit grids/cursors
#include "FormsFacade.h"            // fMain (chkInPickLoadError) / TrayForm offline stand-ins
#include "atester_shims.h"          // HTimer (hOutArmYpitchHomeTimer type)
#include "acatchtray_shims.h"       // InitProcessSingleMotorTask
#include "canary_support.h"         // ShowErrorMessage / RecordProcess / LastSet / __FUNC__

// -----------------------------------------------------------------------------
//  SUBSTRATE EXTERNS not yet provided by the current cpp tree (REPORTED for the
//  serial Integrate phase -- this is the FIRST out-arm variant batch, so the
//  out-arm pick/place/tray/magazine free-function surface and the out-arm shuttle
//  pointer are not yet landed).  Forward-declared here behind #ifndef guards so
//  this TU compiles standalone and CANNOT ODR-collide once the owning module
//  lands.  Signatures are VERBATIM from golden headers.
// -----------------------------------------------------------------------------
#ifndef AOUTARM9045_PTROUTSHT_FWD
#define AOUTARM9045_PTROUTSHT_FWD
extern TMyKitSuck *ptrOutSHT;                                                   // golden mykitsuck.h:381
#endif

// ---- out-arm <-> shuttle geometry/offset helpers (golden aoutarm9045.h) -----
#ifndef AOUTARM9045_OUTSHT_GEOM_FWD
#define AOUTARM9045_OUTSHT_GEOM_FWD
...
void TransferOutShuttleRatio(int iShuttle, int *iXPos, int *iYPos, int Row, int Col);                                   //Steven 20141029 : Out Shuttle 須要做比例補償
...
// ---- out-arm shuttle-pick action / alarm surface (golden aoutarm9045.h) -----
#ifndef AOUTARM9045_OUTSHT_PICK_FWD
#define AOUTARM9045_OUTSHT_PICK_FWD
...
bool ErrorBinBoxDetect(int iError);                                            //kevin 20160906 error bin box判斷 異常或計數
...
// ---- out-arm move / state / catch-tray / magazine helpers -------------------
#ifndef AOUTARM9045_OUTSTATE_FWD
#define AOUTARM9045_OUTSTATE_FWD
...
bool CheckOutArmSuckICFallDown();                                              //JerryYang 20200422 修正殘料可能檢查不出來的問題, 檢查到有殘料要重新再確認直到把料吹掉
bool OutSHT1InRT();
bool OutSHT2InRT();
...
bool bCheckYPitchRunHomeSen(int iXYHome);                                      //kevin 20180822 (Steven) : y pitch 運行home
bool bCheckYPitchHome(int iXYHome);                                           //kevin 20180822 (Steven) : y pitch 去home 動作
void OutArmSubSpeed();
void OutArmAddSpeed();
...
// ---- out-arm engine globals (golden aoutarm9045.h / cmydef.h) ---------------
#ifndef AOUTARM9045_GLOBALS_FWD
#define AOUTARM9045_GLOBALS_FWD
extern int  iPickFromShuttle1Task;
extern int  iPickFromShuttle2Task;
extern int  OutArmTask;
extern int  iOutShtRetryCount;
extern int  iOutRotateFinish;
...
//==============================================================================
// golden aoutarm9045_1x3_2_14.cpp:31
int GetNowShuttleMode_1x3_2_14(int iSht)
{
    ...
    else                                                                        //JerryYang 20251124 : fix 1x3
    {
        if(ptrOutSHT->Item[0][0]>=HAS_IC)
            return 30000;
        else
...
//==============================================================================
// golden aoutarm9045_1x3_2_14.cpp:82
bool OutArmZNeedDown_1x3_2_14(int iSht)
{
    if(InitialOutArmNeedSuck(iSht)==false)
        return false;

    ptrOutSHT=(iSht==0)?&FRCarryKit:&BRCarryKit;
    int iMode=GetNowShuttleMode_1x3_2_14(iSht);
//    int iModeRow=iMode%100;
    int iModeCol=iMode/100;

    if(iModeCol==100)                                                           //全部一起
    {
    ...
    else if(iModeCol==300)                                                      //JerryYang 20251124 : fix 1x3
    {
        SetOutArmNeedDestory(0, 0, 0, 0);
    }
    else if(iModeCol==301)
    {
        SetOutArmNeedDestory(0, 1, 0, 2);
    }
    return true;
}
//------------------------------------------------------------------------------
// golden aoutarm9045_1x3_2_14.cpp:124
bool CheckOutArmXYPitch_1x3_2_14(int *iX, int *iY, int iSht, int iKit, int iMovePitchX, int iMovePitchY)
{
    int iMode       =GetNowShuttleMode_1x3_2_14(iSht);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
    ...
    if(iModeRow==0)                                                             //Row A
    {
        if(USE_OUT_Y_IS_AUTO_PITCH==true)
            *iY=*iY-iMovePitchY;
        else
            *iY=*iY;
    }
    ...
    else if(USE_OUT_ARM_Y_PITCH==iXYPitchVariable)                              //基準為第二隻吸嘴  //JerryYang 20251218 : IN/OUT ARM支援不同模組
    {
        ...
        {
            *iX=*iX+(dMovePitchX*1.0-TestIF.dSiteXPitch*1.0);                   //Steven 20251113 : for 1x3_14 close site Ab
        }
        else if(iModeCol==301)                                                  //Ad --> Ac
        {
    ...
    *iX=*iX+GetOutArmXToShuttleOffset_9045(iSht, iOffsetPos);
    ...
    if(OutArmZNeedDown_1x3_2_14(iSht)==false)
        return false;
    return true;
}
//==============================================================================
// golden aoutarm9045_1x3_2_14.cpp:257
bool MoveOutArmToShuttleIncludeZ_9045_1x3_2_14(int iSht, int iKit, bool bZDown)
{
    ...
    {
        if(OutArmSuck.iXStep==1)
            iMovePitchX=TestIF.dSiteXPitch;                                     //Steven 20240112 : fixed 1x3_2_14 out arm position
        else
    ...
    if(CheckOutArmXYPitch_1x3_2_14(&iXPos, &iYPos, iSht, iKit, iMovePitchX, iMovePitchY)==false && bZDown==true)
        return false;
...
//  Ver : 2003_07
//==============================================================================
// golden aoutarm9045_1x3_2_14.cpp:318
    ...
    switch(Task)
    {
        case 1:
            if(ArmSpeed[OutArm].dWaitOnSH!=0)                                                                           //kevin 20210525 out arm 起動在shuttle 上等待 時間
            {
                ...
                {
                    if(MoveOutArmToShuttleIncludeZ_9045_1x3_2_14(iSht, iOutArmiWhichKit, false))                        //移到 Shuttle 上等待
                    {
        ...
        case 10:
            ...
            Task=200;
        case 200:
            if(MoveOutArmToShuttleIncludeZ_9045_1x3_2_14(iSht, iOutArmiWhichKit, true))                                 //kevin 20210717 change function
            {
        ...
        case iOUTARM_SUCK:
            ...
            for(int i=0; i<OutArmSuck.iPickRow; i++)
            {
                for(int j=0; j<OutArmSuck.iPickCol; j++)
                {
                    ...
                    if(OutArmSuck.iXStep==1)                                    //AI(general) 20260324 (RogerYang) : for hangup
                    {
                        if(iOutArmiWhichKit==0)
                            iShtCol=(j==0)?0:1;
                        else
                            iShtCol=2;
                    }
                    ...
                    {
                        if(OutArmSuck.iXStep==3)                                                                                                                    //JerryYang 20251114 : fix
                        {
                            iPickKit=0;
                        }
                        iShtCol =GetShuttleCol(i, j+iPickKit, true);
                    }
...
//==============================================================================
// golden aoutarm9045_1x3_2_14.cpp:607
...
//#endif
        ...
        case 300:
            ...
            {
                if(MoveOutArmToShuttleIncludeZ_9045_1x3_2_14(1, false))
                {
...
//                        #endif
        ...
        case 1150:
            ...
            if(MoveOutArmToShuttleIncludeZ_9045_1x3_2_14(iWhichShuttle, iOutArmiWhichKit))
            {
                IsCatchTrayReadySupplyNewTray();
...
//                if(WhichAutoNeedTray()==0)                                      //JerryYang 20170731 (wei) Out arm要讓位才不會hang up
        ...
        case 1200:
            ...
            if(DoPickFromShuttle_9045_1x3_2_14(0))
            {
...
//#endif
        ...
        case 2200:
            ...
            if(DoPickFromShuttle_9045_1x3_2_14(1))
            {
                PreSetOutAdditionalFlag();                                                                              //Steven 20220614 : Fixed for out arm additional function
