// =============================================================================
//  ainarm9045_1x3_4.cpp  --  In-Arm per-site VARIANT (1x3_4, TSMC 1x3 Site)
//
//  Faithful translation of golden ainarm9045_1x3_4.cpp (2963 lines, BCB6, Big5).
//  Translator: AI(W6.2c-1x3_4) 20260626
//  Translation wave: W6.2c (in-arm per-site-config variant: 1x3_4; uses Ab/Ac/Ad
//  suckers, single physical row, 3-of-4 picker columns).  Sibling of the already-
//  translated leaves ainarm_SearchPickPlate / ainarm_SearchPlacePlate and the
//  engine ainarm9045.cpp.
//
//  ROLE: the per-site in-arm pick/place state machines for the 1x3_4 layout.
//  The engine's dispatch ladder DoInArm_9045() / DoInArm_9045_SuckerMap() routes
//  iInArmType==e9045_1x3_4 here (un-gated at the integrate stage).  This file owns:
//    SetInOutArmParameter_1x3_4, GetNowSiteKitMode_1x3_4, InArmZNeedDown_1x3_4,
//    CheckXYPitch_1x3_4, MoveInArm2XYToShuttle_9045_1x3_4,
//    DoPlaceToHotPlate_9045_1x3_4, DoInArmPlaceToShuttle_9045_1x3_4,
//    DoInArm_9045_1x3_4_SuckerMap, DoInArm_9045_1x3_4,
//    DoInArmTryPickFromHotPlate_9045_1x3_4, GetShuttleState_1x3_4.
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula,
//  fall-through and goto/label (IN_ARM_SHUTTLE_1x3_4 / IN_ARM_TRY_PICK_HOTPLATE_1x3_4)
...
//  DoInArmTryPickFromHotPlate_9045_1x3_4 keeps its golden #else (real) path
...
//   #include "ainarm9045_1x3_4.h"
//   //Frank 20160104 add for TSMC 1x3 Site, use Ab, Ac, Ad suckers
...
//==============================================================================
bool DoInArmTryPickFromHotPlate_9045_1x3_4(bool bOneTimeCheckAll, bool bShowErrorMessage);
//==============================================================================
...
//        CopyInitSuck(&OutArmSuckBackup, &OutArmSuck, 0, 7, 0, 7);
    ...
    {
        if(InArmSuck.iXStep==2)
        {
            //In Arm 使用吸嘴Aa, Ab, Ac
            //A  C  E  G                                                        //Steven 20220425 : 修改1x3放Shuttle順序
            //1  0  2  x

            //Out Arm 使用吸嘴Aa, Ab, Ac
            //A  C  E  G
            //0  1  2  x
        }
        else
    ...
    InArmSuck .SetPickerCount(1, 3, 1, 3, 1, 0, 0);
    OutArmSuck.SetPickerCount(1, 3, 1, 3, 1, 0, 0);
}
//==============================================================================
...
//         100 (c)    |         101  (b, d) =左右分兩次
//==============================================================================
int GetNowSiteKitMode_1x3_4(int iSht, bool bPlace)
{
...
//==============================================================================
void InArmZNeedDown_1x3_4(int iSht, bool bPlace)
{
    if(InitialInArmNeedSuck(iSht, bPlace)==false)
        return;

    int iSuckRow, iSuckCol, iShtCol;
    int iMode   =GetNowSiteKitMode_1x3_4(iSht, bPlace);
        ...
        for(int j=0; j<4; j++)
        {
                    ...
                    (iModeRow==2 || iModeRow==i))                               // O X O X        O O X X
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
                    ...
                    (iModeRow==2 || iModeRow==i))                               // X O X Q        X X O Q
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
                    ...
                    (iModeRow==2 || iModeRow==i))                               // X X O X        X X O X
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
                    ...
                    (iModeRow==2 || iModeRow==i))                               // X X X Q        X X X Q
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
        }
...
//==============================================================================
void CheckXYPitch_1x3_4(int *iX, int *iY, int iSht, bool bPlace, int iMovePitchX, int iMovePitchY)
{
    int iMode       =GetNowSiteKitMode_1x3_4(iSht, bPlace);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
...
//==============================================================================
bool MoveInArm2XYToShuttle_9045_1x3_4(int iSht, bool IncludeZ, bool bPlace)
{
    ...
    CheckXYPitch_1x3_4(&iXPos, &iYPos, iSht, bPlace, iMovePitchX, iMovePitchY);
...
//==============================================================================
bool DoPlaceToHotPlate_9045_1x3_4()
{
...
//==============================================================================
bool DoInArmPlaceToShuttle_9045_1x3_4()
{
    IN_ARM_SHUTTLE_1x3_4:
...
//==============================================================================
void DoInArm_9045_1x3_4_SuckerMap()
{
    for(int i=0; i<1; i++)
    {
        for(int j=0; j<4; j++)
...
//            break;
        ...
        case 15000:                                                             //ChungHung 20120206 Hotplate check
            ...
            if(DoInArmTryPickFromHotPlate_9045_1x3_4(bOneTimeHotPlateCheckAll, bTryPickFromHotPlateShowError))
            {
                for(int i=0; i<2; i++)
                {
                    for(int j=0; j<4; j++)
...
bool DoInArmTryPickFromHotPlate_9045_1x3_4(bool bOneTimeCheckAll, bool bShowErrorMessage)
{
    IN_ARM_TRY_PICK_HOTPLATE_1x3_4:
    ...
    switch(Task)
    {
        ...
        case 100:
            if(MoveInArmXYToHotPlatePlace(iTryPickHP, true))
            {
                ...
                if(i8PickerHPMode==iHPWideHP && HotPlateForm.XDivision==6)      //ChungHung Add 20101025 CC_ASE_CL HotPlate Offset 10mm  //Steven 20151117 : 2x2 8Picker at Hot mode
                {
                    Task=375;                                                   //ChungHung Add 20101025 CC_ASE_CL HotPlate Offset 10mm 一次放全部
                }
                else
                {
                    Task=350;
            ...
            for(i=0; i<MAX_ARM_Row; i++)
            {
                for(j=0; j<MAX_ARM_Col; j++)
                {
                    if(iPlaceHPOrder==0)
                    {
                        if(Row2CanPutHP(PlaceMode))
                        {
                            if(InArmSuck.Item[i][j]==HAS_TRY_SUCK_IC)           //wei 20170216 (jou) j*2 --> j
                                return false;
                        }
                        else
                        {
                            if(InArmSuck.Item[0][j]==HAS_TRY_SUCK_IC)           //wei 20170216 (jou) j*2 --> j
                                return false;
                        }
                    }
                    else
                    {
                        if(InArmSuck.Item[1][j]==HAS_TRY_SUCK_IC)               //wei 20170216 (jou) j*2 --> j
                            return false;
                    }
                }
            }
//            DestroyFlag=false;
            Task=400;
            goto IN_ARM_TRY_PICK_HOTPLATE_1x3_4;
//            break;
        case 400:
            for(i=0; i<MAX_ARM_Row; i++)
            {
