// =============================================================================
//  ainarm9045S_1x4_4.cpp  --  In-Arm per-site VARIANT (1x4_4_13, HT-9045S AutoClean)
//
//  Faithful translation of golden ainarm9045S_1x4_4.cpp (2822 lines, BCB6, Big5).
//  Translator: AI(W6.2c-INARM-batch5) 20260626
//  Translation wave: W6.2c batch-5 (S-family HT-9045S in-arm variant: 1x4_4_13).
//  The engine dispatch ladder routes iInArmType==e9045_1x4_4_13 to
//  DoInArm_9045S_1x4_4() / DoInArm_9045S_1x4_4_SuckerMap() (golden ainarm9045.cpp
//  :4567-4569 DoInArm and :4732-4734 SuckerMap).
//
//  IRREGULAR func name: this file defines DoInArm_9045S_1x4_4 (S in the func) even
//  though the enum is e9045_1x4_4_13.  Copy EXACTLY -- do NOT normalize.
//
//  This file owns (all _1x4_4_13-suffixed -> unique symbols):
//    XPHSuckToSht_1x4_13[2][8] (const map, def in .cpp; extern in .h),
//    SetInOutArmParameter_1x4_4_13, GetNowSiteKitMode_9045_1x4_4_13,
//    InArmZNeedDown_1x4_4_13, CheckXYPitch_9045_1x4_4_13,
//    MoveInArm2XYToShuttle_9045_1x4_4_13, DoPlaceToHotPlate_9045_1x4_4_13,
//    DoInArmPlaceToShuttle_9045_1x4_4_13, DoInArm_9045S_1x4_4_SuckerMap,
//    DoInArm_9045S_1x4_4, DoInArmTryPickFromHotPlate_9045_1x4_4_13.
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula, fall-
//  through and the goto/label control flow (labels IN_ARM_SHUTTLE_1x4_13 in
//  DoInArmPlaceToShuttle_9045_1x4_4_13 and IN_ARM_TRY_PICK_HOTPLATE_1x4_13 in
//  DoInArmTryPickFromHotPlate_9045_1x4_4_13) are transcribed VERBATIM from the
//  golden CP950.  The single #ifdef SOFT_SIMULTE block (golden :2542-2569) keeps
//  its #ifdef/#else/#endif verbatim; SOFT_SIMULTE is NOT defined -> the golden
//  #else (real) path stays ACTIVE.  No __fastcall in this file (count 0).
//
//  Cursor: DoInArm_9045S_1x4_4 opens with "int &Task=iArmTask;" (iArmTask is the
//  engine global) plus the file-statics iAdjustSpeed / bOne / iDisplaySpeed.
//  Numeric semantics (int vs double, integer division/truncation) kept EXACT.
//
...
//   #include "mysensor.h" / "cSortCT.h" / "note.h" / "main.h" / "csystem.h"
//   #include "uhome.h" / "mymessbox.h" / "cinitial.h" / "atester.h" / "aRotateKIT.h"
//   #include "fRotate.h" / "cOffSet.h" / "cMyDB.h" / "cObserver.h" / "cmydef.h"
//   #include "uYieldMonitoring.h" / "BarCode.h" / "InOutArmZteach.h" / "acarry.h"
...
#include "ainarm9045.h"             // engine: GetShuttleCol / GetInArmToShuttleOffset_9045 / SetShuttlefCanMoveL / iArmTask / iShuttleRowKit / InArmContinuousMove_9045 / GetVariableYInShuttleData / InitialInArmNeedSuck / InArmZNeedDown_9045 / ...
#include "ainarm_SearchPickPlate.h" // shared HP-pick externs (bInArmTryPickFromHotPlateFinish / HasHotReadyIC_9045 / iTryPickHP / ...)
#include "ainarm_SearchPlacePlate.h"// shared HP-place externs (SearchPlateToPlace / MoveInArmXYToHotPlatePlace / DoPlaceToHPSwapData / Row2CanPutHP / iPlaceHP / HPPlaceLog / GetVariableXInHotPlateData)
#include "csystem.h"                // CheckInArmSuckICFallDownToHasNullIC / DoInArmAutoSiteMapping / CheckInArmDestroyICFail predicates
#include "Motor/mymotor.h"          // MOT[] (.InitEmptyTray / .HasIC / .HasRealIC / .Tray / .fHasTray / .Led / .fCanMoveL), InArmContinuousMove_9045
#include "cmydef.h"                 // global scalar universe + IC consts + enums (eInArmType / picker enum / Tempture_* / rsm* / CC_* / iInArmShtXCenterPos / ...)
#include "common.h"
#include "mycylin.h"
...
#include "atester.h"                // bOneTimeWait / dwStart*Soak / TestIF.iTestMode
#include "aHotPlateSubstrate.h"     // InArmSuck/OutArmSuck/FLCarryKit/BLCarryKit/FTestSuck/BTestSuck + ptrInSHTBackup + cursors/timers + SetInArm_Unuse_SuckToNullICForHP + engine shims + Zteach facade
#include "FormsFacade.h"            // HotPlateForm / Zteach / TrayForm / fContact / fObserver / fMain / fYieldMonitoring offline stand-ins
...
#include "ainarm9045_2x4_16_shims.h"// InitInArm*Task* / AdjustShuttlePlaceOrder_AutoSiteMapping / MyTickCount / fYieldMonitoring / iInposLed
#include "canary_support.h"         // ShowErrorMessage / ShowMyMessage / RecordProcess / LastSet / __FUNC__

const int XPHSuckToSht_1x4_13[2][8]={{0, 4, 1, 4, 4, 5, 6, 7},
                                     {2, 4, 3, 4, 4, 5, 6, 7}};

//==============================================================================
bool DoInArmTryPickFromHotPlate_9045_1x4_4_13(bool bOneTimeCheckAll, bool bShowErrorMessage);
//==============================================================================
//AI(W6.2c-INARM-batch5) 20260626: golden ainarm9045S_1x4_4.cpp:38 (SetInOutArmParameter_1x4_4_13)
void SetInOutArmParameter_1x4_4_13()
{
    //QQQ 待處理
    InArmSuck.SetPickerCount(2, 2, 1, 4, 2, 0, 0);
    OutArmSuck.SetPickerCount(2, 2, 1, 4, 2, 0, 0);
}
//==============================================================================
//AI(W6.2c-INARM-batch5) 20260626: golden ainarm9045S_1x4_4.cpp:45 (GetNowSiteKitMode_9045_1x4_4_13)
int GetNowSiteKitMode_9045_1x4_4_13(int iSht, bool bPlace)                      //QQQ 待處理
{
...
//==============================================================================
//AI(W6.2c-INARM-batch5) 20260626: golden ainarm9045S_1x4_4.cpp:146 (InArmZNeedDown_1x4_4_13)
void InArmZNeedDown_1x4_4_13(int iSht, bool bPlace)                             //QQQ 待處理
{
    if(InitialInArmNeedSuck(iSht, bPlace)==false)
        return;

    int iSuckRow, iSuckCol, iShtCol;
    int iMode   =GetNowSiteKitMode_9045_1x4_4_13(iSht, bPlace);
    ...
    for(int i=0; i<2; i++)
    {
        for(int j=0; j<4; j++)
        {
            ...
            if(iModeCol==0 && (j==0 || j==2) &&                                 //吸嘴       ==>  蝦頭      //Steven 20240508 : 3 --> 2
               (iModeRow==2 || iModeRow==i))                                    // O O            O O X X
            {                                                                   // X X            X X X X
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
            else if(iModeCol==10 && (j==0 || j==2) &&                           //吸嘴       ==>  蝦頭      //Steven 20240508 : 3 --> 2
               (iModeRow==2 || iModeRow==i))                                    // X X            X X O O
            {                                                                   // O O            X X X X
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
            else if(iModeCol==100 && j==0 &&                                    //吸嘴       ==>  蝦頭
                    (iModeRow==2 || iModeRow==i))                               // O X            O X X X
            {                                                                   // X X            X X X X
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
            else if(iModeCol==110 && j==2 &&                                    //吸嘴       ==>  蝦頭      //Steven 20240508 : 1 --> 2
                    (iModeRow==2 || iModeRow==i))                               // X O            X O X X
            {                                                                   // X X            X X X X
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
            else if(iModeCol==110 && j==0 &&                                    //吸嘴       ==>  蝦頭
                    (iModeRow==2 || iModeRow==i))                               // X X            X X O X
            {                                                                   // O X            X X X X
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
            else if(iModeCol==111 && j==2 &&                                    //吸嘴       ==>  蝦頭      //Steven 20240508 : 1 --> 2
                    (iModeRow==2 || iModeRow==i))                               // X X            X X X O
            {                                                                   // X O            X X X X
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
        }
...
//==============================================================================
//AI(W6.2c-INARM-batch5) 20260626: golden ainarm9045S_1x4_4.cpp:198 (CheckXYPitch_9045_1x4_4_13)
void CheckXYPitch_9045_1x4_4_13(int *iX, int *iY, int iSht, bool bPlace, int iMovePitchX, int iMovePitchY)              //QQQ 待處理
{
    int iMode       =GetNowSiteKitMode_9045_1x4_4_13(iSht, bPlace);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
...
//==============================================================================
//AI(W6.2c-INARM-batch5) 20260626: golden ainarm9045S_1x4_4.cpp:286 (MoveInArm2XYToShuttle_9045_1x4_4_13)
bool MoveInArm2XYToShuttle_9045_1x4_4_13(int iSht, bool IncludeZ, bool bPlace)
{
    ...
    CheckXYPitch_9045_1x4_4_13(&iXPos, &iYPos, iSht, bPlace, iMovePitchX, iMovePitchY);
...
//==============================================================================
//AI(W6.2c-INARM-batch5) 20260626: golden ainarm9045S_1x4_4.cpp:368 (DoPlaceToHotPlate_9045_1x4_4_13)
bool DoPlaceToHotPlate_9045_1x4_4_13()
{
...
//==============================================================================
//AI(W6.2c-INARM-batch5) 20260626: golden ainarm9045S_1x4_4.cpp:633 (DoInArmPlaceToShuttle_9045_1x4_4_13; label IN_ARM_SHUTTLE_1x4_13)
bool DoInArmPlaceToShuttle_9045_1x4_4_13()
{
    IN_ARM_SHUTTLE_1x4_13:
...
//==============================================================================
//AI(W6.2c-INARM-batch5) 20260626: golden ainarm9045S_1x4_4.cpp:1497 (DoInArm_9045S_1x4_4_SuckerMap; engine SuckerMap dispatch :4732; hardcoded picker map, no GetShuttleCol)
void DoInArm_9045S_1x4_4_SuckerMap()
{
    Prod.fInArmSuck4x8[0][0][0]=LastSet.bUseTestSocket[0][0][0];
    Prod.fInArmSuck4x8[0][0][2]=LastSet.bUseTestSocket[0][0][1];
    Prod.fInArmSuck4x8[0][1][0]=LastSet.bUseTestSocket[0][0][2];
    Prod.fInArmSuck4x8[0][1][2]=LastSet.bUseTestSocket[0][0][3];

    Prod.fInArmSuck4x8[1][0][0]=LastSet.bUseTestSocket[1][0][0];
    Prod.fInArmSuck4x8[1][0][2]=LastSet.bUseTestSocket[1][0][1];
    Prod.fInArmSuck4x8[1][1][0]=LastSet.bUseTestSocket[1][0][2];
    Prod.fInArmSuck4x8[1][1][2]=LastSet.bUseTestSocket[1][0][3];

    Prod.bInSuckUse[0][0][0]=true;
    Prod.bInSuckUse[0][0][2]=true;
    Prod.bInSuckUse[0][1][0]=true;
    Prod.bInSuckUse[0][1][2]=true;
    Prod.bInSuckUse[1][0][0]=true;
    Prod.bInSuckUse[1][0][2]=true;
    Prod.bInSuckUse[1][1][0]=true;
    Prod.bInSuckUse[1][1][2]=true;
...
//==============================================================================
//AI(W6.2c-INARM-batch5) 20260626: golden ainarm9045S_1x4_4.cpp:1528 (DoInArm_9045S_1x4_4; engine DoInArm dispatch :4567; IRREGULAR -- S in func, enum e9045_1x4_4_13; cursor int &Task=iArmTask)
void DoInArm_9045S_1x4_4()
{
    int &Task=iArmTask;
...
//            break;
        ...
        case 15000:                                                             //ChungHung 20120206 Hotplate check
            ...
            if(DoInArmTryPickFromHotPlate_9045_1x4_4_13(bOneTimeHotPlateCheckAll, bTryPickFromHotPlateShowError))
            {
                for(int i=0; i<2; i++)
                {
                    for(int j=0; j<4; j++)
...
//AI(W6.2c-INARM-batch5) 20260626: golden ainarm9045S_1x4_4.cpp:2437 (DoInArmTryPickFromHotPlate_9045_1x4_4_13; label IN_ARM_TRY_PICK_HOTPLATE_1x4_13; SOFT_SIMULTE block kept verbatim, #else real path active)
bool DoInArmTryPickFromHotPlate_9045_1x4_4_13(bool bOneTimeCheckAll, bool bShowErrorMessage)
{
    IN_ARM_TRY_PICK_HOTPLATE_1x4_13:
        ...
        case 350:
            for(int i=0; i<2; i++)
            {
                for(int j=0; j<2; j++)
                {
                    ...
                    {
                        ip=iPlacePlate[0];
                        //Ifor 20200520 Fix:2x2 Hotplate Check Hangup
                        //==>
                        if(bPitchOver12000 && HotPlateForm.XDivision==4)                                                //Steven 20160912 : For 1x2 XPitch>40mm
                        {
                            ix=iPlacePlateX[0]+j;
                        }
                        else if(bPitchOver12000 && HotPlateForm.XDivision==8)
                        {
                        ...
                        else if(HotPlateForm.XDivision==12)
                        {
                            if(iPlacePlateX[0]==8 || iPlacePlateX[0]==9)
                                ix=iPlacePlateX[0]+j2;
                            else
                                ix=iPlacePlateX[0]+j2*2;
                        }
                        else if(bPitchOver12000 && HotPlateForm.XDivision==6)                                           //JerryYang 20171207 (Steven) Quad site 2x2支援hot plate 6x11(X-Pitch:35mm)
                        {
                            if(iPlacePlateX[0]!=4)
                                ix=iPlacePlateX[0]+j2;
                            else
                        ...
                        else if(bPitchOver12000 && HotPlateForm.XDivision==4)                                           //JerryYang 20180501 (jou) : Quad site 2x2 支援hot plate 4x8 (x-pitch:45mm)
                        {
                            ix=iPlacePlateX[0]+j;
                        }
                        else
            ...
            Task=400;
            goto IN_ARM_TRY_PICK_HOTPLATE_1x4_13;
        case 400:
            for(int i=0; i<2; i++)
            {
                for(int j=0; j<4; j++)
