...
static inline bool fOffSet_UseOutArmSetupTeach_2x8_32(int /*iArea*/) { return false; } //golden cOffSet.h:490 -- offline: false
#endif
        ...
        case 200:
            if(MoveOutArmToShuttleIncludeZ_9045_2x8_32(0, iOutArmWhichKit, true))                                                                               //kevin 20210717 change function
            {
                if((AccessLevel>=1 || CosFunction.bOPCanPressStepAndTStart) &&                                                                                  //JerryYang 20170417 (wei) OP權限也能按Step跟T.Start
                   (Sen[SnRKManualStep].IsOn() || fOffSet->UseAutoOffsetFunction("Output Shuttle1") || fOffSet_UseOutArmSetupTeach_2x8_32(OutOfsOutSh1)))             //JerryYang 20180921 Setup Teach功能 //jou 2013-08-29 Use Auto Offset Funtion
                {
                    bEnterOffset=false;
...
bool DoPickFromShuttle2_9045_2x8_32(bool IncludeSHT)
{
    ...
    switch(Task)
    {
        ...
        case 200:
            ...
            {
                if((AccessLevel>=1 || CosFunction.bOPCanPressStepAndTStart) &&                                                                                  //JerryYang 20170417 (wei) OP權限也能按Step跟T.Start
                   (Sen[SnRKManualStep].IsOn() || fOffSet->UseAutoOffsetFunction("Output Shuttle2") || fOffSet_UseOutArmSetupTeach_2x8_32(OutOfsOutSh2)))             //JerryYang 20180921 Setup Teach功能 //jou 2013-08-29 Use Auto Offset Funtion
                {
                    bEnterOffset=false;
                    fMain->Pause(__FUNC__);                                       // translated: golden no-arg Pause() -> FormsFacade Pause(AnsiString) sibling convention
                    Task=500;
...
//            break;
        case 100:
            ...
            if((AccessLevel>=1 || CosFunction.bOPCanPressStepAndTStart) &&                                                                                      //JerryYang 20170417 (wei) OP權限也能按Step跟T.Start
               (Sen[SnRKManualStep].IsOn() || fOffSet->UseAutoOffsetFunction(asAutoOffset) || fOffSet_UseOutArmSetupTeach_2x8_32(OutOfsAuto1+iWhichAuto)))            //JerryYang 20180921 Setup Teach功能 //jou 2013-08-29 Use Auto Offset Funtion
            {
                bEnterOffset=false;
                fMain->Pause(__FUNC__);                                       // translated: golden no-arg Pause() -> FormsFacade Pause(AnsiString) sibling convention
                Task=200;
            ...
            if(iWhichAuto<=iAutoRight)
            {
                if(MOT[iMMAuto[iWhichAuto]].fHasTray==true)
                {
                    ...
                    if((flag1 || flag2 || flag3) && LastSet.iRealDummy!=DUMMY)
                    {
                        ...
                        if(ret==K_SKIP)
                        {
                            #if 0 // TODO(W7): fProductionInfo form (golden ProductionInfo.h TfProductionInfo : public TForm) -- no FormsFacade stand-in; gated (offline no IC-count update)
