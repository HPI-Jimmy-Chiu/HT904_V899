// =============================================================================
//  acatchtray.h  --  TrayArm (CatchTray) ENGINE public interface (W6.3 translation)
//
//  Faithful translation of golden  acatchtray.h  (BCB6, Big5/cp950).
//  Translator: AI(W6.3-CATCHTRAY) 20260626
//
//  Names / signatures / default args preserved VERBATIM so the team can
//  reverse-look-up the BCB6 version.  No __fastcall / __property / PACKAGE in
//  this header.  vclcompat AnsiString (1-based) replaces the VCL AnsiString.
//
//  SCOPE (W6.3) -- this is the DIRECT mirror of the in/out-arm engine waves.
//  acatchtray.cpp is the TrayArm ENGINE: it moves Loader / Empty / Color / Auto
//  trays via the CatchTraySuck KitSuck and pumps the central switch(CatchTrayTask)
//  SM (DoCatchTray) which dispatches into 5 sub-SMs (DoCatchFromLoader /
//  CatchNewTrayFromBuffer / DoPlaceTrayToAuto / DoPlaceToBuffer / DoSlapTray).
//  The .cpp keeps the 6 task-SM skeletons + cursor values + switch(Task)
//  structure + formulas + fall-throughs + #ifdef SOFT_SIMULTE blocks VERBATIM
//  and ACTIVE; the not-yet-translated cross-module surface it calls
//  (fTrayMapping / Magazine / OCR / HSys / MyMessageBox / LoadCCD / AMR / SECS)
//  is satisfied by transparently-named offline shims so the engine LINKS into
//  lib ht9045_sm and the SMs pump over the Sim HAL.  See the .cpp file head.
//
//  Big5: every Chinese comment decoded via cp950 and preserved as UTF-8.
//  Final gate: ZERO U+FFFD.
// =============================================================================
//---------------------------------------------------------------------------
#ifndef acatchtrayH
#define acatchtrayH
//---------------------------------------------------------------------------
void DoCatchTray();
void InitialCatchTrayTask();
bool IsTrayArmMoveAvoidOutArmCrash();
int WhichAutoNeedTray();
bool IsTrayArmCatchTrayFail();                                                  //JerryYang 20200926 包成函式,偵測tray arm夾tray是否異常
extern bool IsTrayArmAtEmptyOrColor();
void CatchFromLoaderSetItemData();                                             //Sam 20220712 : 整合 CatchFromLoader SetItemData
void PlaceToBufferSetItemData();                                               //Sam 20221027 : 整合 CatchTray SetItemData
void CatchTraySetItemData();                                                   //Sam 20221027 : 整合 CatchTray SetItemData
bool DoLoadCarRotArmReadRFID(bool bAlarm=false);
void InitialLDRotArmReadRFIDTask();
//ChungHung 20140624 add AutoRetest catch Tray use 2 Output
bool C_CatchTray_Fix_Puch(bool bInitial=false);
bool C_CatchTray_Fix_Pop(bool bInitial=false);
bool bCheckNoCatchEmptyTrayToAuto();    //Frank 20170221 add

bool DoSlapTray(bool bInit=false); //ChungHung 20140701 add AutoRetest
bool MTrayXCanSafeMove();          //kevin 20170619 (wei) add
extern int CatchTrayTask;
extern int iCatchFromLoaderTask;
extern int iPlaceTrayToAutoTask;
extern int iPlaceToBufferTask;
extern int iSlapTrayTask;
extern bool bAMRFullICBin;                                                      //Eastsun 20260515 F011 整合 (a-side acatchtray.h L27 wiring)
extern bool DoReceiveAutoTray(int pos);                                         //Eastsun 20260515 F020
extern void InitialTrayIDTask();                    //wei 20161219 (Steven) Tray Mapping
extern void InitialTrayID2Task();                   //wei 20180808 MR Tray ID 2
extern void InitialTrayMapTask(int iTrayMap);       //Sam 20200323 : Modify Tray Function //wei 20161219 (Steven) Tray Mapping
extern void InitialDeviceRemainTask();              //wei 20170317 (steven) Device Remain 殘料檢測
extern bool IsOutArmSafe();
extern void AddFixTrayCount();
extern void AddTrayCount(int iAuto);
extern void ClearAllTrayCount();
extern void SwapTrayID();
extern void InitialCoverTrayTask();                                             //Eastsun 20260515 F011 整合:KYEC AMR cover-tray externs
extern bool DoCoverTray(int iAuto);                                             //Eastsun 20260515 F011 整合:KYEC AMR cover-tray externs
extern void InitialCoverTrayIDTask(int iPos);                                   //Eastsun 20260515 F016 整合:KYEC AMR cover-tray ID task
#endif
