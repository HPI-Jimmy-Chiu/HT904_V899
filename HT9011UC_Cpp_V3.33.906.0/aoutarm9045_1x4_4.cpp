// =============================================================================
//  aoutarm9045_1x4_4.cpp  --  Out-Arm per-site VARIANT (1x4_4, single-row 4-picker 4-pick)
//
//  Faithful translation of golden aoutarm9045_1x4_4.cpp (64007 bytes, largest of
//  the out-arm variants; BCB6, Big5/cp950).
...
//  1x4_4 layout.  This file owns (golden names VERBATIM):
//    GetNowShuttleMode_1x4_4, OutArmZNeedDown_1x4_4, CheckOutArmXYPitch_1x4_4,
//    MoveOutArmToShuttleIncludeZ_9045_1x4_4, DoPickFromShuttle_9045_1x4_4,
//    DoOutArm_9045_1x4_4.
//
//  ENGINE DISPATCH (LIVE variant): the out-arm engine aoutarm9045.cpp routes
//  iInArmType==e9045_1x4_4 -> DoOutArm_9045_1x4_4() and
//  DoPickFromShuttle_9045_1x4_4(int).  The DoPickFromShuttle ladder ALSO routes
//  iInArmType==e9045_1x4_4_Back here (golden engine line 648-651), while in the
//  DoOutArm ladder e9045_1x4_4_Back is an EMPTY //-body arm (golden engine line
//  512-515) and e9045_1x4_4 calls DoOutArm_9045_1x4_4 (516-519).  The file layout
//  suffix == the callee suffix (ENUM->CALLEE map: 1x4_4 -> 1x4_4).
//
...
//  CP950 source.  The DoOutArm_9045_1x4_4 fall-throughs are preserved EXACTLY:
//    case-1 -> case-5, case-3301 -> case-3500, case-3100 -> case-3300,
//    plus the "if(Task!=N) break;" guard idiom after the falling-through cases
//    (5/10/50/100/1140/1150/2040/2050/3300).
//
...
//
//  REPORTED substrate ADD (serial Integrate, not touched here):
//    CheckDuplicateBarCode() -- golden home aoutarm.h:130; real call at golden
//    line 1337, guarded by CosFunction.bBarcodeDuplicateFileByOutArm (false
//    offline).  Forward-declared below behind AOUT_CHECKDUPLICATEBARCODE_FWD so
//    this TU compiles standalone; offline-safe behavior: return true (barcode OK /
//    not duplicate -> the guarded `==false` branch never forces Task=3000).
//
//  INCLUDE REWRITE (mirror of the golden header set, translated subset):
//    golden: MachineDefine.h ; aoutarm9045_1x4_4.h ; aArmHeader.h ; MyKitSuck.h ;
...
#include "MachineType.h"            // e9045_1x4_4 / e9045_1x4_4_Back / ep16Picker / iXYPitch16Bd_Be / iXYPitchVariable / eBulkBox / CC_KYEC_LEE global enums
//---------------------------------------------------------------------------
#include "aoutarm9045_1x4_4.h"
#include "aArmHeader.h"             // __FUNC__ shim, RecordProcess, OutArm, etc.
#include "aoutarm9045.h"            // engine: GetOutArmPitch*_9045 / DoOutArmSuckPreOn / IsCheckOutArmDestroyActiveFinish / GetOutShuttleStatus_9045 / InitialOutArmNeedSuck / OutArmNeedCheckOffset / iOUTARM_SUCK / SearchTrayToPlace_9045 / VerifyFixTrayLink / CheekNeedToDoOutArmAdditionalFunction / MoveOutArmXY_To_ESDSafePos / OutArmContinuousMove_9045
...
#include "cmydef.h"                 // global scalar universe + IC consts + iInArmType + enums + bEnterOffset / bCheckShuttle1/2Flag / iCleanOut / IniConfig / USE_* / iXpitch* / Mxxx motor ids / iAutoIndex / iMMAuto / AseIcRecord / ...
#include "cprod.h"                  // Prod / TestIF / TestIF_File / ArmSpeed / OutArmOffSet / iXTrayColor
#include "cpublic.h"                // CosFunction (bBarcodeDuplicateFileByOutArm)
#include "common.h"
...
#include "FormsFacade.h"            // fMain offline stand-in (chkInPickLoadError gated out by SOFT_SIMULTE) / fFixAICCD gated out by USE_Fix_AI_CCD #if 0
...
//==============================================================================
#ifndef OUTARM_1x4_4_FWD_GUARD
#define OUTARM_1x4_4_FWD_GUARD
...
// shuttle column map (golden ainarm9045.h:GetShuttleCol(int,int,bool=false))
int  GetShuttleCol(int iRow, int iCol, bool bPlace=false);                       // golden ainarm9045.h -- offline: column index passthrough
...
// out-arm pick/place per-shuttle flags + bin-tray finish flag (golden file-globals)
extern bool bPickSH1Flag;                                                      // golden aoutarm9045.cpp -- offline: false
extern bool bPickSH2Flag;                                                      // golden aoutarm9045.cpp -- offline: false
extern bool bSortingAllBinTrayFinish;                                          // golden aoutarm9045.cpp / JerryYang 20150910 -- offline: false
// magazine place-target cursors (golden Magazine.h / file-globals)
extern int  iOutArmWhichAuto;                                                  // golden aoutarm9045.cpp -- offline: place-target cursor
extern int  iAuto3MagazineIndex;                                               // golden Magazine.h -- offline: magazine index
extern int  iMagAtAuto;                                                        // golden Magazine.h -- offline: magazine Auto slot
extern bool bMagCatchTrayfalg;                                                 // golden Magazine.h / JerryYang 20221121 -- offline: false
extern bool bChaneMagTrayflag;                                                 // golden Magazine.h -- offline: false
extern bool bMagNeedChangeTray;                                                // golden Magazine.h -- offline: false
#endif // OUTARM_1x4_4_FWD_GUARD

//==============================================================================
//==  REPORTED substrate ADD: CheckDuplicateBarCode() (golden aoutarm.h:130).
//==  Owned by the serial Integrate phase (aoutarm_shims.cpp).  Forward-declared
//==  behind a guard so 1x4_4 compiles standalone; offline-safe behavior: true
//==  (barcode OK / not duplicate).  Used ONLY at case-3000 under the
//==  CosFunction.bBarcodeDuplicateFileByOutArm guard (false offline).
//==============================================================================
#ifndef AOUT_CHECKDUPLICATEBARCODE_FWD
#define AOUT_CHECKDUPLICATEBARCODE_FWD
bool CheckDuplicateBarCode();                                                  // golden aoutarm.h:130 -- offline: true
#endif

//==============================================================================
//  GetNowShuttleMode_1x4_4  -- golden aoutarm9045_1x4_4.cpp:42
//==============================================================================
...
int GetNowShuttleMode_1x4_4(int iSht)
{
...
//==============================================================================
//  OutArmZNeedDown_1x4_4  -- golden aoutarm9045_1x4_4.cpp:88
//==============================================================================
bool OutArmZNeedDown_1x4_4(int iSht)
{
    if(InitialOutArmNeedSuck(iSht)==false)
        return false;

    ptrOutSHT   =(iSht==0)?&FRCarryKit:&BRCarryKit;
    int iMode   =GetNowShuttleMode_1x4_4(iSht);
...
//------------------------------------------------------------------------------
//  CheckOutArmXYPitch_1x4_4  -- golden aoutarm9045_1x4_4.cpp:144
//------------------------------------------------------------------------------
bool CheckOutArmXYPitch_1x4_4(int *iX, int *iY, int iSht, int iMovePitchX, int iMovePitchY)
{
    int iMode       =GetNowShuttleMode_1x4_4(iSht);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
    ...
    if(USE_OUT_Y_IS_AUTO_PITCH==true)                                           //Row A  //JerryYang 20251218 : IN/OUT ARM支援不同模組
        *iY=*iY-iMovePitchY;
    else
        *iY=*iY;
    ...
    *iX=*iX+GetOutArmXToShuttleOffset_9045(iSht, iOffsetPos);
    ...
    if(OutArmZNeedDown_1x4_4(iSht)==false)
       return false;
    return true;
}
//==============================================================================
//  MoveOutArmToShuttleIncludeZ_9045_1x4_4  -- golden aoutarm9045_1x4_4.cpp:382
//==============================================================================
bool MoveOutArmToShuttleIncludeZ_9045_1x4_4(int iSht, bool bZDown)
{
    ...
    if(CheckOutArmXYPitch_1x4_4(&iXPos, &iYPos, iSht, iMovePitchX, iMovePitchY)==false && bZDown==true)
        return false;
...
//  Ver : 2003_07
//  DoPickFromShuttle_9045_1x4_4  -- golden aoutarm9045_1x4_4.cpp:462
//==============================================================================
    ...
    switch(Task)
    {
        case 1:
            if(ArmSpeed[OutArm].dWaitOnSH!=0)                                   //kevin 20210525 out arm 起動在shuttle 上等待 時間
            {
                ...
                {
                    if(MoveOutArmToShuttleIncludeZ_9045_1x4_4(iSht, false))     //移到 Shuttle 上等待
                    {
        ...
        case 10:
            ...
            Task=200;
        case 200:
            if(MoveOutArmToShuttleIncludeZ_9045_1x4_4(iSht, true))              //kevin 20210717 change function
            {
        ...
        case iOUTARM_SUCK:
            ...
            {
                #if 0 // TODO(W7): fFixAICCD UI form (TfFixAICCD) not provided by FormsFacade offline
...
//==============================================================================
//  DoOutArm_9045_1x4_4  -- golden aoutarm9045_1x4_4.cpp:733
//==============================================================================
...
//                        #endif
        ...
        case 1150:
            ...
            if(MoveOutArmToShuttleIncludeZ_9045_1x4_4(iWhichShuttle))
            {
                IsCatchTrayReadySupplyNewTray();

                if(OutArmSuck.iXStep!=1)
                {
                    iBackMode1=GetNowShuttleMode_1x4_4(0);
                }
...
//#endif
                ...
                if(OutArmSuck.iXStep!=1)
                {
                    iMode1=GetNowShuttleMode_1x4_4(0);
...
//                if(WhichAutoNeedTray()==0)                                      //JerryYang 20170731 (wei) Out arm要讓位才不會hang up
        ...
        case 1200:
            ...
            if(DoPickFromShuttle_9045_1x4_4(0))
            {
...
//                        #endif
        ...
        case 2050:
            ...
            if(MoveOutArmToShuttleIncludeZ_9045_1x4_4(iWhichShuttle))
            {
                ...
                if(OutArmSuck.iXStep!=1)
                {
                    iBackMode2=GetNowShuttleMode_1x4_4(1);
                }
                Task=2100;
            }
...
//#endif
                ...
                if(OutArmSuck.iXStep!=1)
                {
                    iMode2=GetNowShuttleMode_1x4_4(1);
        ...
        case 2200:
            ...
            if(DoPickFromShuttle_9045_1x4_4(1))
            {
                PreSetOutAdditionalFlag();                                                                              //Steven 20220614 : Fixed for out arm additional function
...
//#else
                ...
                MOT[MInShuttle2].fCanMoveR=true;
//#endif
                if(CosFunction.bBarcodeDuplicateFileByOutArm==true &&
                   CheckDuplicateBarCode()==false)
                {
                    Task=3000;
                    break;
                }
