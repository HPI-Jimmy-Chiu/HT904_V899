// =============================================================================
//  ainarm9045_2x2_4_12.cpp  --  In-Arm per-site VARIANT (2x2_4_12, 2x2 dual-row)
//
//  Faithful translation of golden ainarm9045_2x2_4_12.cpp (2321 lines, BCB6, Big5).
//  Translator: AI(W6.2c-2x2_4_12) 20260626
//  Translation wave: W6.2c (in-arm per-site-config variant: 2x2_4_12; two physical
//  rows of 4, 8 sucker columns, XPHSuckToSht_2x2_12 identity map).  Sibling of the
//  already-translated leaves ainarm_SearchPickPlate / ainarm_SearchPlacePlate and
//  the engine ainarm9045.cpp.
//
//  ROLE: the per-site in-arm pick/place state machines for the 2x2_4_12 layout.
//  The engine's dispatch ladder DoInArm_9045() / DoInArm_9045_SuckerMap() routes
//  iInArmType==e9045_2x2_4_12 here (un-gated at the integrate stage).  This file
//  owns (golden file:line cited per fn below):
//    XPHSuckToSht_2x2_12[8] (:32), GetNowSiteKitMode_2x2_4_12 (:35),
//    InArmZNeedDown_2x2_4_12 (:136), CheckXYPitch_2x2_4_12 (:173),
//    MoveInArm2XYToShuttle_9045_2x2_4_12 (:275), DoPlaceToHotPlate_9045_2x2_4_12
//    (:359), DoInArmPlaceToShuttle_9045_2x2_4_12 (:621),
//    DoInArm_9045_2x2_4_12_SuckerMap (:1499), DoInArm_9045_2x2_4_12 (:1524).
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula,
//  fall-through and the goto/label (IN_ARM_SHUTTLE_2x2_4_12) is transcribed
...
//  (every called symbol verified present in an existing project header at
//  translate time).  No body is #if 0-gated.
//
...
//  aArmHeader shim.  SOFT_SIMULTE is NOT defined and this file has NO #ifdef
//  SOFT_SIMULTE block.  Numeric semantics (int vs double, integer division,
//  double pitch math) kept EXACT.
//
//  SUBSTRATE NEEDS: none.  Every called external (ptrInSHTBackup /
//  DoCheckAutoSiteMappingPosition / Zteach / PickFromHPList / the engine
//  Do*/Init*/Move*/Check* family / Prod.fInArmSuck4x8 / etc.) already has a home
//  in an existing header included below; the only file-scope datum this TU owns
//  (XPHSuckToSht_2x2_12) is DEFINED here, matching golden.
//
...
#include "ainarm9045_2x2_4_12.h"
#include "aArmHeader.h"             // __FUNC__ shim, RecordProcess, CheckHeaterOK, Zteach, etc.
#include "ainarm9045.h"             // engine: GetInArmToShuttleOffset_9045 / GetShuttleCol / MoveInArmXYToShuttle_9045 / DoInArmPickFromLoadStage_9045 / DoInArmPlaceToShuttle_9045 / InArmNeedCheckOffset / DoInArmCheckShuttleFloating / DoInArmIonFanGiveWay / bCheckYPitchHome / ...
#include "ainarm_SearchPickPlate.h" // shared HP-pick externs (DoInArmPickFromHotPlate_9045 / HasHotReadyIC_9045 / GetHeaterWaitTime / bInArmTryPickFromHotPlateFinish / ...)
#include "ainarm_SearchPlacePlate.h"// shared HP-place externs (DoPlaceToHotPlate_9045 / SearchPlateToPlace / MoveInArmXYToHotPlatePlace / DoPlaceToHPSwapData / GetPlaceToHotPlateSuckCol / Row2CanPutHP / GetHotPlateYHalfPos / GetVariableXInHotPlateData / iPlaceHP / HPPlaceLog / CheckHasSpaceToPlace_9045)
#include "csystem.h"                // CheckInArmSuckICFallDownToHasNullIC / DoInArmAutoSiteMapping / CheckInArmDestroyICFail / IsInArmOneCycleFinish / IsInArmCleanOutFinish / DoSiteMappingCHK / AutoSiteMappingCheckShuttle predicates
#include "Motor/mymotor.h"          // MOT[], InArmContinuousMove_9045
#include "cmydef.h"                 // global scalar universe + IC consts + enums (eInArmType / picker enum / Tempture_* / rsm* / CC_* / iXpitchMaxX3 / iXpitchMaxX7)
#include "common.h"
#include "mycylin.h"
#include "myswitch.h"               // SW[] (SwShuttleVibration1/2 .On())
#include "cprod.h"                  // Prod / TestIF / TestIF_File / ArmSpeed / ArmSpeed_File / LastSet / bInSuckUse
#include "cpublic.h"                // CosFunction
#include "atester.h"                // dwStartShuttle1Soak / dwStartShuttle2Soak / bOneTimeWait
#include "aHotPlateSubstrate.h"     // InArmSuck/FLCarryKit/BLCarryKit/OutArmSuck/FTestSuck/BTestSuck + ptrInSHTBackup + cursors/timers + engine shims + PickFromHPList + DoCheckAutoSiteMappingPosition + Init*/Adjust* family
#include "FormsFacade.h"            // HotPlateForm / TrayForm / Zteach / fContact / fObserver / fYieldMonitoring offline stand-ins / DUMMY
#include "atester_shims.h"          // fObserver (RecordInArmTime/AddTimeData) / fContact (IsRun2DCheck) / MoveInArm2XYToWait
#include "acatchtray_shims.h"        // InitProcessSingleMotorTask / InShtInLF / InSHT1InLF / InSHT2InLF
#include "canary_support.h"         // ShowErrorMessage / ShowMyMessage / RecordProcess / LastSet / __FUNC__ / K_RETRY / Tempture_Hot
//==============================================================================
// PROVENANCE: every function below is a verbatim translation from golden
// ainarm9045_2x2_4_12.cpp at the line cited above its definition.
//==============================================================================
