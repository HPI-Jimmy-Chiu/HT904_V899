// =============================================================================
//  aoutarm9045_1x4_4_Back.cpp  --  Out-Arm per-site VARIANT (1x4_4_Back)
//
//  Faithful translation of golden aoutarm9045_1x4_4_Back.cpp (BCB6, Big5/cp950).
...
//  1x4_4_Back layout.  This file owns (golden names VERBATIM):
//    GetNowShuttleMode_1x4_4_Back, OutArmZNeedDown_1x4_4_Back,
//    GetOutShuttleStatus_1x4_4_Back, CheckOutArmXYPitch_1x4_4_Back,
//    MoveOutArmToShuttleIncludeZ_9045_1x4_4_Back,
//    DoPickFromShuttle1_9045_1x4_4_Back, DoPickFromShuttle2_9045_1x4_4_Back,
//    DoMoveOutArmXYToPlace_9045_1x4_4_Back, SearchUnLoadTrayUpDown_9045_1x4_4_Back,
//    SetOutArm_9045_1x4_4_Back, DoPlaceToAuto_9045_1x4_4_Back,
//    DoOutArm_9045_1x4_4_Back.
//  Every symbol carries the full _1x4_4_Back suffix -> UNIQUE never-called weight.
//
//  DEAD VARIANT (MODE=dead): enum e9045_1x4_4_Back is PRESENT in MachineType.h
//  but the engine aoutarm9045.cpp DoOutArm ladder arm for e9045_1x4_4_Back is an
//  EMPTY //-body and the DoPickFromShuttle ladder routes e9045_1x4_4_Back to the
//  LIVE DoPickFromShuttle_9045_1x4_4(iSht).  Hence DoOutArm_9045_1x4_4_Back and
//  the _1x4_4_Back pick helpers are NEVER dispatched.  This TU links as dead
//  weight (no dispatch arm, no engine extern un-gate, no static-stub removal).
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula, branch,
//  fall-through, goto and commented-out code is transcribed VERBATIM from the
//  golden CP950 source.  The case fall-throughs (case-1->case-5 in DoOutArm;
//  case-10->case-200, case-1->case-200 in DoPickFromShuttle1/2;
//  case-1->case-10 in DoPlaceToAuto; case-100->case-110 fall, case-3100->3300,
//  case-3300->3301, case-3301->3500, case-500 fall) are preserved EXACTLY.
//
...
//  the #ifdef SOFT_SIMULTE blocks (chkInPickLoadError tests in
//  DoPickFromShuttle1/2 case iOUTARM_SUCK) are compiled OUT, kept VERBATIM.
//
//  Big5: every Chinese comment decoded via cp950 and preserved as UTF-8.
//  Final gate: ZERO U+FFFD.
//
//  INCLUDE REWRITE (mirror of the golden header set, translated subset):
//    golden: MachineDefine.h ; #pragma hdrstop ; aoutarm9045_1x4_4_Back.h ;
...
#include "cmydef.h"                 // global scalar universe + IC consts + iInArmType + enums + bEnterOffset / bCheckShuttle1/2Flag / iCleanOut / IniConfig / USE_* / iXpitch* / Mxxx motor ids / iWhichAuto / iWhichBuff / iWhichMag / iAuto3MagazineIndex / iMagAtAuto / Sn* / C_* / sJAM* / sWAR* / ...
#include "cprod.h"                  // Prod / TestIF / TestIF_File / ArmSpeed / OutArmOffSet / iXTrayColor / TRAY_TYPE_PARA / BinSelect / TrayForm
...
#include "aHotPlateSubstrate.h"     // OutArmSuck / FRCarryKit / BRCarryKit / FTestSuck / BTestSuck / tRotate + cursors + SetMotorSpeed + DoOutArmPlaceToAuto / DoOutArmAdditionalFunction / DoOutArmAfterPlaceToAuto / InitPlaceToAutoTask / InitOutArmTask / iWhichBuff / iWhichMag
#include "acarry_shims.h"           // MoveOutArmToAutoSafe / MoveOutArmXY_ToShuttleAlarmArea / LastSet / NULL_IC
#include "acatchtray_shims.h"       // InitProcessSingleMotorTask / MoveOutArmXY_ToFix_Tray_Full
...
#include "FormsFacade.h"            // fMain / fOffSet offline stand-ins
...
//==  NOTE: several of these are file-local `static`/commented in the engine
//==  (NOT link-visible to this separate TU) -> they need a shared offline home.
//==============================================================================
#ifndef OUTARM_1x4_4_Back_FWD_GUARD
#define OUTARM_1x4_4_Back_FWD_GUARD
...
// place-to-auto task cursor (golden file-global)
...
//-- 1x4_4_Back is the FIRST out-arm variant to drive the FULL place-to-auto
//-- surface (DoMoveOutArmXYToPlace/SetOutArm/SearchUnLoadTrayUpDown/DoPlaceToAuto).
//-- The following golden engine globals/helpers live in aoutarm.h / aoutarm.cpp
//-- (engine file-locals or commented-out in the translated tree) -> forward-declare
//-- behind this guard with the golden CALL-SITE signatures so the dead TU compiles;
//-- REPORTED for the serial Integrate agent to add ONCE in a shared shim.
...
void SearchBinBoxUpDown(bool &bCanPick2ICAtOnceTime, int &iStep, int &iRow, int &iCol); // golden aoutarm.h:83 (variant call-site arity 4) / Steven 20230601 -- offline: no-op
int  Find_OutArm_PickerMaxUseCountOnTime(int iUseSuck, int &iStep, int &iRow, int &iCol, bool bFlag); // golden aoutarm.h:84 (variant call-site arity 5) -- offline: 0
void Find_OutArm_Single(int iUseSuck, int &iRow, int &iCol);                   // golden aoutarm.h:86 -- offline: no-op (ix/iy unchanged)
void AdjustOutArmClosePitchCondition(int iw, bool &bCanPick2ICAtOnceTime, int &iStep); // golden aoutarm.h:89 (variant call-site arity 3) -- offline: no-op
void IfUseOnebyOne(int iAuto);                                                 // golden aoutarm.h:101 / Steven 20111101 -- offline: no-op
...
int  AutoCalculateOutArmYClosePitch(int iWhichAuto);                           // golden aoutarm.h:111 / ChungHung 20131231 -- offline: TestIF.iARM_Y_PITCH (sane default)
bool CheckUseFixBinBoxFunction();                                              // golden aoutarm.h:120 / pig 2013.04.08 -- offline: false
int  iCalculateOutArmXPitch(int iXPos, int iWhich=-1);                         // golden aoutarm.h / Steven 20151112 (variant call-site arity 1 & 2) -- offline: iXpitchMaxX3
// out-arm continuous-move (8-arg XY-變距 overload; COMMENTED OUT in golden mymotor.cpp:3458) -> dead-link stub
...
// bin-box / magazine position indices (golden cmydef.h -- not in translated cmydef.h)
...
// X-pitch non-stander suck-column remap (golden cmydef.h base XPHNotStander -- commented out in translated cmydef.h)
extern const int XPHNotStander[4];                                             // golden cmydef.h -- offline: {0,2,1,3} suck-col remap (1x4_4_Back is FIRST out-arm user)
// one-by-one place mode flag (golden aoutarm.h:99 -- not in translated tree)
extern bool bUseOnebyOne;                                                      // golden aoutarm.h:99 / 2010-12-07 -- offline: false
// unload-motor index table (golden cmydef.h:4079 iUnloadMotorIndex[eTrayCount] -- commented out in translated cmydef.h)
extern int iUnloadMotorIndex[eTrayCount];                                      // golden cmydef.h:4079 -- offline: per-tray motor index
// out-arm suck IC fall-down check (golden aoutarm.h:90, non-static engine aoutarm9045.cpp:146 -- distinct from substrate CheckOutArmSuckICFallDown)
bool CheckOutSuckICFallDown(bool AutoClean=false);                             // golden aoutarm.h:90 -- offline: false (no fall-down)
// place-to-auto destroy-pause delay timer (golden aoutarm.h:28)
extern TQPF_Timer DoPlaceToAutoDelay;                                          // golden aoutarm.h:28 -- offline: software timer
// shuttle-pick error flag (golden aoutarm.h:30)
extern bool bPickShuttleError;                                                 // golden aoutarm.h:30 -- offline: false
// out-arm place-to-auto (non-suffixed engine helper, golden aoutarm9045.cpp:2525 / Steven 20110216; distinct from DoOutArmPlaceToAuto_9045())
bool DoOutArmPlaceToAuto(int iToWhichAuto);                                     // golden aoutarm9045.cpp:2525 -- offline: true (placed)
#endif // OUTARM_1x4_4_Back_FWD_GUARD

//==============================================================================
//==  GetNowShuttleMode_1x4_4_Back  -- golden aoutarm9045_1x4_4_Back.cpp:35
//==============================================================================
...
//  Ver : 2003_07
//==============================================================================
int GetNowShuttleMode_1x4_4_Back(int iShuttle)
{
    if(iShuttle==0)
...
//==============================================================================
//==  OutArmZNeedDown_1x4_4_Back  -- golden aoutarm9045_1x4_4_Back.cpp:67
//==============================================================================
bool OutArmZNeedDown_1x4_4_Back(int iShuttle)                                   //kevin 20120612 ourarm double pick
{
    ...
    //jou 980916 start : offset shuttle right hang up
    if(iShuttle==0)
    {
    ...
    int Mode=GetNowShuttleMode_1x4_4_Back(iShuttle);
    ...
    for(j=0; j<MAX_ARM_Col; j++)
    {
        OutArmSuck.Suck[1][j].SetNeedSuck(false);                               //inital

        if(Mode==0)
        {
            if(ptrOutSHT->Item[0][j])
                OutArmSuck.Suck[1][j].SetNeedSuck(true);
        }

        if(Mode==1 && (j==0|| j==2))
        {
            if(ptrOutSHT->Item[0][XPHNotStander[j]])
                OutArmSuck.Suck[1][j].SetNeedSuck(true);
        }
...
//        {
//            if(ptrOutSHT->Item[i][XPHNotStander[j]])
//                OutArmSuck.Suck[i][j].SetNeedSuck(true);
//        }
...
//==============================================================================
//==  GetOutShuttleStatus_1x4_4_Back  -- golden aoutarm9045_1x4_4_Back.cpp:169
//==============================================================================
void GetOutShuttleStatus_1x4_4_Back(bool *flag)
{
    int j;

    for(j=0; j<MAX_ARM_Col; j++)
    {
        if(OutArmSuck.Suck[1][j].GetNeedSuckStatus())
            flag[1+j*2]=true;
        else
            flag[1+j*2]=false;
    }
...
//------------------------------------------------------------------------------
//==  CheckOutArmXYPitch_1x4_4_Back  -- golden aoutarm9045_1x4_4_Back.cpp:182
//------------------------------------------------------------------------------
bool CheckOutArmXYPitch_1x4_4_Back(int *iX,int *iY,int iShuttle)                //kevin 20120612 ourarm double pick fix
{
    int iMode=GetNowShuttleMode_1x4_4_Back(iShuttle);
    ...
    *iY+=TestIF.iARM_Y_PITCH;
    ...
    if(iMode==0)
    {
        ...
        {
            *iX-=TestIF.dSiteXPitch/2;                                          //Steven 20141128 : OK
            *iY-=iMovePitchY;
        }
        else
        {
            *iX+=TestIF.dSiteXPitch/2;                                          //Isaac 20171204 (Steven) : Xpitch40->50mm, 2000->TestIF.dSiteXPitch/2
            //jou 980313
//            *iX-=(4000-TestIF.dSiteXPitch)/2;//Eliot 2010_0824    //Isaac Mark
        }
    }
    else if(iMode==1)
    {
        if(USE_IN_OUT_ARM_Y_PITCH==iXYPitchVariable)                            //ChungHung 20140304 add for AutoYPitch
        {
            *iX-=TestIF.dSiteXPitch;                                            //Steven 20141128 : OK
            *iY-=iMovePitchY;
        }
        else
        {
            *iX-=TestIF.dSiteXPitch/2;
        //jou 2010-07-17 mark code : 偏心多加了3cm
        }
    }
    else if(iMode==2)
    {
...
//    }
    ...
    if(OutArmZNeedDown_1x4_4_Back(iShuttle)==false)                             //kevin 20120612 ourarm double pick  start
       return false;
    return true;                                                                //kevin 20120612 end
}
//==============================================================================
//==  MoveOutArmToShuttleIncludeZ_9045_1x4_4_Back -- golden aoutarm9045_1x4_4_Back.cpp:377
//==============================================================================
bool MoveOutArmToShuttleIncludeZ_9045_1x4_4_Back(int iShuttle, bool bZDown)     //kevin 20210612 Out arm add Z bZDown= true 往下
{
    ...
    if(CheckOutArmXYPitch_1x4_4_Back(&iXPos, &iYPos, iShuttle)==false && bZDown==true)                                  //kevin 20120612 ourarm double pick
         return false;
    ...
    GetOutShuttleStatus_1x4_4_Back(&ZDownFlag[0]);                              //一定要在CheckXYPitch()function後面
    for(i=0; i<MAX_ARM_Row; i++)
    {
        for(j=0; j<MAX_ARM_Col; j++)
        {
...
//==============================================================================
//==  DoPickFromShuttle1_9045_1x4_4_Back -- golden aoutarm9045_1x4_4_Back.cpp:477
// 輸出臂從 Shuttle 1 吸取 IC 流程動作
//  Ver : 2003_07
//==============================================================================
bool DoPickFromShuttle1_9045_1x4_4_Back(bool IncludeSHT)
{
    ...
    switch(Task)
    {
        case 1:
            if(ArmSpeed[OutArm].dWaitOnSH!=0)                                   //kevin 20210525 out arm 起動在shuttle 上等待 時間
            {
                ...
                {
                    if(MoveOutArmToShuttleIncludeZ_9045_1x4_4_Back(1, false))   //移到 Shuttle 上等待
                    {
        ...
        case 10:
            ...
            Task=200;
        case 200:
            if(MoveOutArmToShuttleIncludeZ_9045_1x4_4_Back(0, true))                                                                                            //kevin 20210717 change function
            {
                if((AccessLevel>=1 || CosFunction.bOPCanPressStepAndTStart) &&                                                                                  //JerryYang 20170417 (wei) OP權限也能按Step跟T.Start
                   (Sen[SnRKManualStep].IsOn() || fOffSet->UseAutoOffsetFunction("Output Shuttle1") || (false /*AI(W6.2c-OUT-1x4_4_Back) 20260627: golden fOffSet->UseOutArmSetupTeach(OutOfsOutSh1); TfOffSet stub lacks UseOutArmSetupTeach -> #if-0-equivalent offline false (Setup-Teach disabled offline), OR-semantics preserved. REPORTED: add TfOffSet::UseOutArmSetupTeach(int)->false*/)))             //JerryYang 20180921 Setup Teach功能 //jou 2013-08-29 Use Auto Offset Funtion
                {
                    bEnterOffset=false;
                    fMain->Pause("");                                           //AI(W6.2c-OUT-1x4_4_Back) 20260627: golden fMain->Pause() 0-arg; FormsFacade TfMain::Pause requires AnsiString -> pass offline-inert "" (REPORTED: add default Func="")
                    Task=500;
                    break;
                }
        ...
        case iOUTARM_SUCK:
            ...
            for(j=0; j<MAX_ARM_Col; j++)
            {
                SHTHasIC[0][j]=FRCarryKit.Item[0][j];
            }
            ...
            for(i=0; i<1; i++)
            {
                for(j=0; j<MAX_ARM_Col; j++)
                {
                    ...
                    iSuckY  =j;
                    iShtX   =i;
                    if(XPitchIsStand())                                                                                 //kevin 20150121
                        iShtY=j;
                    else
                        iShtY=XPHNotStander[j];
            ...
            for(j=0; j<MAX_ARM_Col; j++)
                if(OutArmSuck.Item[1][j]==NULL_IC && OutArmSuck.Suck[1][j].GetNeedSuckStatus())
                    return false;
        ...
        case 2000:
            if(MoveOutArmToAutoSafe())
            {
                ...
                for(j=0; j<MAX_ARM_Col; j++)
                {
                    if(bSuckDuplicateErr[1][j]) bHasDuplicateErr=true;
                    if(OutArmSuck.Suck[1][j].Error)
                    {
                        ErrPart+=OutArmSuck.Suck[1][j].sName;
                        bHasErr=true;
                    }
                    else
                    {
                        bSuckDuplicateErr[1][j]=false;
                    }
                }
                    ...
                    if(ret==K_SKIP)
                    {
                        for(i=0; i<1; i++)
                        {
                            for(j=0; j<MAX_ARM_Col; j++)
                            {
                                if(OutArmSuck.Suck[1][j].Error)
                                {
                                    if(XPitchIsStand())                                                                 //jou 981121 start : X pitch isn't stander hang up
                                        iShtY=j;                                                                        //kevin 20141103
                                    else
                                        iShtY=XPHNotStander[j];                                                         //kevin 20141103
...
//------------------------------------------------------------------------------
//==  DoPickFromShuttle2_9045_1x4_4_Back -- golden aoutarm9045_1x4_4_Back.cpp:739
//------------------------------------------------------------------------------
bool DoPickFromShuttle2_9045_1x4_4_Back(bool IncludeSHT)
{
    ...
    switch(Task)
    {
        case 1:
            ...
            Task=200;
        case 200:
            if(MoveOutArmToShuttleIncludeZ_9045_1x4_4_Back(1, true))                                                                                            //kevin 20210717 change function
            {
                if((AccessLevel>=1 || CosFunction.bOPCanPressStepAndTStart) &&                                                                                  //JerryYang 20170417 (wei) OP權限也能按Step跟T.Start
                   (Sen[SnRKManualStep].IsOn() || fOffSet->UseAutoOffsetFunction("Output Shuttle2") || (false /*AI(W6.2c-OUT-1x4_4_Back) 20260627: golden fOffSet->UseOutArmSetupTeach(OutOfsOutSh2) -> offline false, REPORTED*/)))             //JerryYang 20180921 Setup Teach功能 //jou 2013-08-29 Use Auto Offset Funtion
                {
                    bEnterOffset=false;
                    fMain->Pause("");                                           //AI(W6.2c-OUT-1x4_4_Back) 20260627: golden fMain->Pause() 0-arg -> offline-inert "" (REPORTED)
                    Task=500;
                    break;
                }
                OutArmSuck.ResetAll();                                                                                                                          //Steven 20160323 : 避免未開啟真空
...
//==============================================================================
//==  DoMoveOutArmXYToPlace_9045_1x4_4_Back -- golden aoutarm9045_1x4_4_Back.cpp:979
//==============================================================================
bool DoMoveOutArmXYToPlace_9045_1x4_4_Back(bool &bCanPick2ICAtOnceTime, int &ix, int &iy, int &iStep, bool ReadMove)
{
    ...
    else if(bCanPick2ICAtOnceTime==true)
    {
        ...
        if(iTempXpos<(MOT[MOutArmX].Motor->PSoftLimitN+10) ||
           iTempXpos>(MOT[MOutArmX].Motor->PSoftLimitP-50))                     //Steven 20141217 : For Auto 1使用G & H吸嘴   //jou 2014-11-20 100->50 fix out arm device > 25mm place to Fix3 hang up
        {
    ...
    else
    {
        for(int i=0; i<MAX_ARM_Row; i++)
        {
            for(int j=0; j<MAX_ARM_Col; j++)
            {
                if(bOutArmSuckActive[1][j])
                {
                    iYPos=Prod.YStart[iOutPutTray][1][j]-iy*OutputForm[iOutPutTray]->YPitch;
...
//==============================================================================
//==  SearchUnLoadTrayUpDown_9045_1x4_4_Back -- golden aoutarm9045_1x4_4_Back.cpp:1225
//==============================================================================
bool SearchUnLoadTrayUpDown_9045_1x4_4_Back(bool &bCanPick2ICAtOnceTime, int &iStep, int &ix, int &iy)
{
    if(AUTO3_IS_MAGAZINE==1             &&                                      //JerryYang 20221215 : Magazine把fix區當buffer區功能
       TestIF_File.iMagFixTrayType==1   &&
       iWhichAuto>=iMagPos)
    ...
    {
        int ct=Find_OutArm_PickerMaxUseCountOnTime(4, iStep, ix, iy, false);
        if(ct<=1)
        {
            bCanPick2ICAtOnceTime=false;
        }
        ...
        {
            if(DoMoveOutArmXYToPlace_9045_1x4_4_Back(bCanPick2ICAtOnceTime, ix, iy, iStep, false)==true)
                return true;
            bCanPick2ICAtOnceTime=false;
        }
        else
...
//==============================================================================
//==  SetOutArm_9045_1x4_4_Back -- golden aoutarm9045_1x4_4_Back.cpp:1264
//==============================================================================
bool SetOutArm_9045_1x4_4_Back()
{
    ...
    if(SearchUnLoadTrayUpDown_9045_1x4_4_Back(bCanPick2ICAtOnceTime, iStep, ix, iy)==false)
        return false;
    ...
    return DoMoveOutArmXYToPlace_9045_1x4_4_Back(bCanPick2ICAtOnceTime, ix, iy, iStep, true);
}
//------------------------------------------------------------------------------
//==  DoPlaceToAuto_9045_1x4_4_Back -- golden aoutarm9045_1x4_4_Back.cpp:1296
//------------------------------------------------------------------------------
bool DoPlaceToAuto_9045_1x4_4_Back()
{
    int ct;
    bool flag;
    static int iRetry=0;
    ...
    switch(Task)
    {
        ...
        case 10:
            ...
            flag=SetOutArm_9045_1x4_4_Back();
            if(flag)
            {
                Task=50;
            }
            else                                                                //jou 2012-09-04 修正極限無法到達時,回填Null_IC會造成找不到地方放,會放在 x=-1,y=-1的位置上
            {
                if(bOverTray==true)
                {
                    MOT[iUnloadMotorIndex[ct]].SetTray(HAS_IC, __FUNC__);      //AI(W6.2c-OUT-1x4_4_Back) 20260627: translated TTrayMotor::SetTray requires AnsiString Func -> pass __FUNC__ (golden 0-arg default)
                    return true;
                }
            }
            break;
...
//            break;
        case 100:
            ...
            if((AccessLevel>=1 || CosFunction.bOPCanPressStepAndTStart) &&                                                                                      //JerryYang 20170417 (wei) OP權限也能按Step跟T.Start
               (Sen[SnRKManualStep].IsOn() || fOffSet->UseAutoOffsetFunction(asAutoOffset) || (false /*AI(W6.2c-OUT-1x4_4_Back) 20260627: golden fOffSet->UseOutArmSetupTeach(OutOfsAuto1+iWhichAuto) -> offline false, REPORTED*/)))            //JerryYang 20180921 Setup Teach功能 //jou 2013-08-29 Use Auto Offset Funtion
            {
                bEnterOffset=false;
                fMain->Pause("");                                                                                                                              //AI(W6.2c-OUT-1x4_4_Back) 20260627: golden fMain->Pause() 0-arg -> offline-inert "" (REPORTED)
                Task=200;
...
//==============================================================================
//==  DoOutArm_9045_1x4_4_Back -- golden aoutarm9045_1x4_4_Back.cpp:1490
//==============================================================================
...
//                        #endif
        ...
        case 1150:
            ...
            if(MoveOutArmToShuttleIncludeZ_9045_1x4_4_Back(iWhichShuttle, false))
            {
                IsCatchTrayReadySupplyNewTray();

                if(XPitchIsStand()==false)                                                                                                                      //Jou 20110328
                {
                    iBackMode1=GetNowShuttleMode_1x4_4_Back(0);
                }
                //KaiChen 20171225 (Steven)：Add Speed Display
                //==>
        ...
        case 1100:
            ...
            {
                bOutArmNeedMoveToSht=true;                                                                                                                      //JerryYang 20170828 修正Out arm讓位後位置錯誤問題
                //==> Eastsun 20260521 整合
            ...
            {
                if(bOutArmNeedMoveToSht==true)                                                                                                                  //JerryYang 20170828 修正Out arm讓位後位置錯誤問題
                {
                    bOutArmNeedMoveToSht=false;
                    Task=1150;
                    break;
                }
//#ifdef Carry4
...
//#endif
                ...
                if(XPitchIsStand()==false)                                                                                                                      //Jou 20110328
                {
                    iMode1=GetNowShuttleMode_1x4_4_Back(0);
...
//                if(WhichAutoNeedTray()==0)                                      //JerryYang 20170731 (wei) Out arm要讓位才不會hang up
        ...
        case 1200:
            ...
            if(DoPickFromShuttle1_9045_1x4_4_Back(true))
            {
...
//                    iOutArmPickPlaceCnt[OutOfsOutSh1]++;  //JerryYang 20230523 : 沒在用, Mark掉  //JerryYang 20180921 Setup Teach功能
        ...
        case 1230:
            ...
            if(DoPickFromShuttle1_9045_1x4_4_Back(false))
            {
                PreSetOutAdditionalFlag();                                                                                                                      //Steven 20220614 : Fixed for out arm additional function
                if(FRCarryKit.HasIC())
                {
...
//                        #endif
        ...
        case 2050:
            ...
            if(MoveOutArmToShuttleIncludeZ_9045_1x4_4_Back(iWhichShuttle, false))
            {
                ...
                if(XPitchIsStand()==false)                                                                                                                      //Jou 20110328
                {
                    iBackMode2=GetNowShuttleMode_1x4_4_Back(1);
                }
                Task=2100;
            }
            break;
...
//#endif
                ...
                if(XPitchIsStand()==false)                                                                                                                      //Jou 20110328
                {
                    iMode2=GetNowShuttleMode_1x4_4_Back(1);
...
//            }
        ...
        case 2200:
            ...
            if(DoPickFromShuttle2_9045_1x4_4_Back(true))
            {
                PreSetOutAdditionalFlag();                                                                                                                      //Steven 20220614 : Fixed for out arm additional function
                if(BRCarryKit.HasIC())
                {
...
//                    iOutArmPickPlaceCnt[OutOfsOutSh2]++;  //JerryYang 20230523 : 沒在用, Mark掉  //JerryYang 20180921 Setup Teach功能
        ...
        case 2230:
            ...
            if(DoPickFromShuttle2_9045_1x4_4_Back(false))
            {
                PreSetOutAdditionalFlag();                                                                                                                      //Steven 20220614 : Fixed for out arm additional function
                if(BRCarryKit.HasIC())
                {
            ...
            else
            {
                    ...
                    CheckUseFixBinBoxFunction()==false))                                                                                                        //kevin 20160822
                {
                    if(iWhichAuto>=iAutoCnt)
                    {
                        ...
                        {
                            if(iWhichAuto==iFixPosMin && (false /*AI(W6.2c-OUT-1x4_4_Back) 20260627: golden BinSelect[iTestRunMode].bFixLink[1]; translated SYSTEM_BIN_SELECT lacks bFixLink member -> offline false (fix-link disabled), AND-semantics preserved. REPORTED: add SYSTEM_BIN_SELECT::bFixLink[eTrayCount]*/))
                            {
                                for(j=0; j<MAX_ARM_Col; j++)
                                {
                ...
                if(MOT[iMMAuto[iWhichAuto]].fHasTray==true)
                {
                    ...
                    if((flag1 || flag2 || flag3) && LastSet.iRealDummy!=DUMMY)
                    {
                        ...
                        if(ret==K_SKIP)
                        {
                            ...
                            MOT[iMMAuto[iWhichAuto]].ClearTray(__FUNC__);       //AI(W6.2c-OUT-1x4_4_Back) 20260627: translated TTrayMotor::ClearTray requires AnsiString Func -> pass __FUNC__ (golden 0-arg default)
...
//jou 2010-01-25 end
        ...
        case 3300:
            ...
            if(Task!=3301)                                                                                                                                      //Steven 20180813 : add Out arm speed
                break;
        case 3301:
            if(DoPlaceToAuto_9045_1x4_4_Back())                                                                                                                 // 吸嘴可能還在下方
            {
