// =============================================================================
//  ainarm9045_2x3_6.cpp  --  In-Arm per-site VARIANT (2x3_6, 6-site 2-physical-row)
//
//  Faithful translation of golden ainarm9045_2x3_6.cpp (3507 lines, BCB6, Big5).
//  Translator: AI(W6.2c-2x3_6) 20260626
//  Translation wave: W6.2c (in-arm per-site-config variant: 2x3_6; two physical
//  rows (A/B), up to 4 picker columns, _6Site2X3N test mode).  Sibling of the
//  already-translated leaves ainarm_SearchPickPlate / ainarm_SearchPlacePlate and
//  the engine ainarm9045.cpp; mirrors the committed ainarm9045_1x3_4.cpp include
//  rewrite.
//
//  ROLE: the per-site in-arm pick/place state machines for the 2x3_6 layout.
//  The engine's dispatch ladder DoInArm_9045() / DoInArm_9045_SuckerMap() routes
//  iInArmType==e9045_2x3_6 here (un-gated at the Integrate stage).  This file owns:
//    SetInOutArmParameter_2x3_6, GetNowSiteKitMode_2x3_6, InArmZNeedDown_2x3_6,
//    CheckXYPitch_2x3_6, MoveInArm2XYToShuttle_9045_2x3_6,
//    DoPlaceToHotPlate_9045_2x3_6, DoInArmPlaceToShuttle_9045_2x3_6,
//    DoInArm_9045_2x3_6_SuckerMap, DoInArm_9045_2x3_6,
//    DoInArmTryPickFromHotPlate_9045_2x3_6, GetShuttleState_2x3_6;
//    plus the file-scope tables XPHSuckToSht_2x3_6 / XPHSuckToSht_2x3_6_OutArm.
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula,
//  fall-through and goto/label (IN_ARM_SHUTTLE_2x3_6 / IN_ARM_TRY_PICK_HOTPLATE_2x3_6)
...
//  ainarm9045 engine + ainarm_SearchPick/PlacePlate.  No body is #if 0-gated.
//
...
//  DoInArmTryPickFromHotPlate_9045_2x3_6 keep their golden #else (real) path
...
const int XPHSuckToSht_2x3_6[3][8]={{0, 1, 2, 3, 4, 5, 6, 7},
                                    {3, 1, 0, 2, 4, 5, 6, 7},                   //Sam 20250218 : 大 Pitch 要用右邊6吸嘴不然行程可能會不夠
                                    {0, 1, 2, 3, 4, 5, 6, 7}};

const int XPHSuckToSht_2x3_6_OutArm[3][8]={{0, 1, 2, 3, 4, 5, 6, 7},
                                           {0, 2, 1, 3, 4, 5, 6, 7},
                                           {0, 1, 2, 3, 4, 5, 6, 7}};

//==============================================================================
bool DoInArmTryPickFromHotPlate_9045_2x3_6(bool bOneTimeCheckAll, bool bShowErrorMessage);
//==============================================================================
...
//        CopyInitSuck(&InArmSuckBackup , &InArmSuck , 1, 7, 1, 7);
    ...
    InArmSuck .SetPickerCount(2, 4, 2, 3, 1, 0, 0);
    OutArmSuck.SetPickerCount(2, 4, 2, 3, 1, 0, 0);
}
//==============================================================================
...
int GetNowSiteKitMode_2x3_6(int iSht, bool bPlace)
{
    ...
    if(bPlace==true)
    {
        ...
        else
        {
            ...
            else if(InArmSuck.iXStep==2)
            {
                if(InArmSuck.iYStep==1 && bRunAutoClean==false)
                {                                                               //吸嘴       ==>  蝦頭
                    if(InArmSuck.Item[0][2]>=HAS_IC ||                          //Q X O X       O X X Q //Sam 20250218 : 大 Pitch 要用右邊6吸嘴不然行程可能會不夠
                       InArmSuck.Item[1][2]>=HAS_IC)                            //Q X 0 X       O X X Q
                        return 10002;                                           //左邊四顆
                    else                                                        //if(InArmSuck.Item[0][1]>=HAS_IC || InArmSuck.Item[0][3]>=HAS_IC ||   //吸嘴       ==>  蝦頭
                         //   InArmSuck.Item[1][1]>=HAS_IC || InArmSuck.Item[1][3]>=HAS_IC)     //X O X O       X O O Q
                        return 10102;                                           //右邊四顆                                        //X O X O       X O O Q
                }
                else
                {
    ...
    else
    {
        ...
        {
            if(ptrInSHTBackup.Item[0][0]==iX || ptrInSHTBackup.Item[0][3]==iX)
                return 10000;
            else if(ptrInSHTBackup.Item[0][1]==iX || ptrInSHTBackup.Item[0][2]==iX)
                return 10100;
            else if(ptrInSHTBackup.Item[1][0]==iX || ptrInSHTBackup.Item[1][3]==iX)
                return 10001;
            else                                                                //if(ptrInSHTBackup.Item[1][1]==iX || ptrInSHTBackup.Item[1][2]==iX)
                return 10101;
        }
        else                                                                    //if(InArmSuck.iXStep==4)
        {
...
//==============================================================================
void InArmZNeedDown_2x3_6(int iSht, bool bPlace)
{
    if(InitialInArmNeedSuck(iSht, bPlace)==false)
        return;

    int iSuckRow, iSuckCol, iShtCol;
    int iMode   =GetNowSiteKitMode_2x3_6(iSht, bPlace);
        ...
        for(int j=0; j<4; j++)
        {
               ...
               (iModeRow==2 || iModeRow==i))
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
                    ...
                    (iModeRow==2 || iModeRow==i))                               // O X O X        X O O X
            {                                                                   // O X O X        X O O X
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
            else if(iModeCol==101 && (j==1 || j==3) &&                          //吸嘴       ==>  蝦頭
                    (iModeRow==2 || iModeRow==i))                               // X O X Q        O X X Q
            {                                                                   // X O X Q        O X X Q
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
            ...
            else if(iModeCol==201 && j==0 &&                                    //吸嘴       ==>  蝦頭
                    (iModeRow==2 || iModeRow==i))                               // X O X X        X O X X
            {                                                                   // X O X X        X O X X
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
            else if(iModeCol==202 && j==1 &&                                    //吸嘴       ==>  蝦頭
                    (iModeRow==2 || iModeRow==i))                               // X X O X        X X O X
            {                                                                   // X X O X        X X O X
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
            else if(iModeCol==203 && j==2 &&                                    //吸嘴       ==>  蝦頭
                    (iModeRow==2 || iModeRow==i))                               // X X X Q        X X Q X
            {                                                                   // X X X Q        X X Q X
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
        }
...
//==============================================================================
void CheckXYPitch_2x3_6(int *iX, int *iY, int iSht, bool bPlace, int iMovePitchX, int iMovePitchY)
{
    int iMode       =GetNowSiteKitMode_2x3_6(iSht, bPlace);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
    ...
    else                                                                        //基準為第三隻吸嘴
    {
        ...
        {
            *iX=*iX+(dMovePitchX*0.0-TestIF.dSiteXPitch*1.0);                   //Sam 20250218 : 大 Pitch 要用右邊6吸嘴不然行程可能會不夠
        }
        else if(iModeCol==101)                                                  //Ab --> Aa
        {
...
//==============================================================================
bool MoveInArm2XYToShuttle_9045_2x3_6(int iSht, bool IncludeZ, bool bPlace)
{
    ...
    CheckXYPitch_2x3_6(&iXPos, &iYPos, iSht, bPlace, iMovePitchX, iMovePitchY);
...
//==============================================================================
bool DoPlaceToHotPlate_9045_2x3_6()
{
...
//==============================================================================
bool DoInArmPlaceToShuttle_9045_2x3_6()
{
    IN_ARM_SHUTTLE_2x3_6:
...
//            break;
        ...
        case 15000:                                                             //ChungHung 20120206 Hotplate check
            ...
            if(DoInArmTryPickFromHotPlate_9045_2x3_6(bOneTimeHotPlateCheckAll, bTryPickFromHotPlateShowError))
            {
                for(int i=0; i<2; i++)
                {
                    for(int j=0; j<4; j++)
...
bool DoInArmTryPickFromHotPlate_9045_2x3_6(bool bOneTimeCheckAll, bool bShowErrorMessage)
{
    IN_ARM_TRY_PICK_HOTPLATE_2X3_6:
    ...
    switch(Task)
    {
        ...
        case 350:
            if(HotPlateForm.XDivision==6)
            {
                ...
                Task=400;
                goto IN_ARM_TRY_PICK_HOTPLATE_2X3_6;
            }
            else
            {
                iXItem10Step=0;
