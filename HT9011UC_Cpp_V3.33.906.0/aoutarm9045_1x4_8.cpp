// =============================================================================
//  aoutarm9045_1x4_8.cpp  --  Out-Arm per-site VARIANT (1x4_8, single-row 4-picker 8-pick)
//
//  Faithful translation of golden aoutarm9045_1x4_8.cpp (BCB6, Big5/cp950).
...
//  1x4_8 layout.  This file owns (golden names VERBATIM):
//    GetNowShuttleMode_1x4_8, OutArmZNeedDown_1x4_8, CheckOutArmXYPitch_1x4_8,
//    MoveOutArmToShuttleIncludeZ_9045_1x4_8, DoPickFromShuttle_9045_1x4_8,
//    DoOutArm_9045_1x4_8.
//
//  ENGINE DISPATCH (LIVE variant): the out-arm engine aoutarm9045.cpp routes
//  iInArmType==e9045_1x4_8_Hot -> DoOutArm_9045_1x4_8() and
//  DoPickFromShuttle_9045_1x4_8(int).  The file layout suffix == the callee
//  suffix (ENUM->CALLEE map: 1x4_8_Hot -> 1x4_8).
//
...
//  CP950 source.  The DoOutArm_9045_1x4_8 fall-throughs are preserved EXACTLY:
//    case 1->5, case 3301->3500, case 3100->3300, plus the "if(Task!=N) break;"
//    pseudo-fall-throughs (case 5/10/50/100, 1140->1150, 2040->2050, 3300->3301).
//
...
//
//  NOTE (signature): 1x4_8's MoveOutArmToShuttleIncludeZ_9045_1x4_8 is the 2-param
//  form (int iSht, bool bZDown) -- it calls DoOutArmSuckPreOn(iSht, 0, ...) with a
//  literal kit 0, mirrored VERBATIM.  (1x4_2 carried an extra iKit param; 1x4_8
//  does not.)
//
...
//    golden: MachineDefine.h ; aoutarm9045_1x4_8.h ; aArmHeader.h ; MyKitSuck.h ;
...
#include "MachineType.h"            // e9045_1x4_8_Hot / ep16Picker / eBulkBox / CC_KYEC_LEE / iXYPitch16Bd_Be / iXYPitchVariable global enums
//---------------------------------------------------------------------------
...
#include "FormsFacade.h"            // fMain offline stand-in (chkInPickLoadError gated out by SOFT_SIMULTE); fFixAICCD UI gated #if 0
...
//==============================================================================
#ifndef OUTARM_1x4_8_FWD_GUARD
#define OUTARM_1x4_8_FWD_GUARD
...
// shuttle column mapper (golden ainarm9045.h; that header clashes with the substrate, so declared here)
int  GetShuttleCol(int iRow, int iCol, bool bExtra=false);                     // golden ainarm9045.h -- offline: passthrough column
#endif // OUTARM_1x4_8_FWD_GUARD

//==============================================================================
//  輸出臂 x y 軸移到 shuttle 1
//  Ver : 2003_07
//  GetNowShuttleMode_1x4_8  -- golden aoutarm9045_1x4_8.cpp:37
//==============================================================================
int GetNowShuttleMode_1x4_8(int iSht)
{
    ...
    else
    {
        if(OutArmSuck.iXStep==2)
        {
            if(ptrOutSHT->Item[0][0]>=HAS_IC || ptrOutSHT->Item[0][1]>=HAS_IC)
                ...
                return (iSht==0)?11001:11000;
        }
        else                                                                    //if(OutArmSuck.iXStep==4)
        {
            if(ptrOutSHT->Item[0][0]>=HAS_IC)
                return (iSht==0)?20001:10000;
            else if(ptrOutSHT->Item[0][1]>=HAS_IC)
                return (iSht==0)?20101:20100;
            else if(ptrOutSHT->Item[0][2]>=HAS_IC)
                return (iSht==0)?21001:21000;
            else                                                                //if(ptrOutSHT->Item[0][3]>=HAS_IC)
                return (iSht==0)?21101:21100;
        }
    }
}
//==============================================================================
//  OutArmZNeedDown_1x4_8  -- golden aoutarm9045_1x4_8.cpp:83
//==============================================================================
bool OutArmZNeedDown_1x4_8(int iSht)
{
    if(InitialOutArmNeedSuck(iSht)==false)
        return false;

    ptrOutSHT   =(iSht==0)?&FRCarryKit:&BRCarryKit;
    int iMode   =GetNowShuttleMode_1x4_8(iSht);
    ...
    int i=(iSht==0)?1:0;
    {
        for(int j=0; j<OutArmSuck.iPickCol; j++)
        {
            ...
            {
                SetOutArmNeedDestory(i, j, 0, iShtJ);
            }
...
//------------------------------------------------------------------------------
//  CheckOutArmXYPitch_1x4_8  -- golden aoutarm9045_1x4_8.cpp:139
//------------------------------------------------------------------------------
bool CheckOutArmXYPitch_1x4_8(int *iX, int *iY, int iSht, int iMovePitchX, int iMovePitchY)
{
    int iMode       =GetNowShuttleMode_1x4_8(iSht);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
    ...
    *iX=*iX+GetOutArmXToShuttleOffset_9045(iSht, iOffsetPos);
    ...
    if(OutArmZNeedDown_1x4_8(iSht)==false)
        return false;
    return true;
}
//==============================================================================
//  MoveOutArmToShuttleIncludeZ_9045_1x4_8  -- golden aoutarm9045_1x4_8.cpp:394
//==============================================================================
bool MoveOutArmToShuttleIncludeZ_9045_1x4_8(int iSht, bool bZDown)
{
    ...
    if(CheckOutArmXYPitch_1x4_8(&iXPos, &iYPos, iSht, iMovePitchX, iMovePitchY)==false && bZDown==true)
        return false;
...
//  Ver : 2003_07
//  DoPickFromShuttle_9045_1x4_8  -- golden aoutarm9045_1x4_8.cpp:459
//==============================================================================
    ...
    switch(Task)
    {
        case 1:
            if(ArmSpeed[OutArm].dWaitOnSH!=0)                                   //kevin 20210525 out arm 起動在shuttle 上等待 時間
            {
                ...
                {
                    if(MoveOutArmToShuttleIncludeZ_9045_1x4_8(iSht, false))     //移到 Shuttle 上等待
                    {
        ...
        case 10:
            ...
            Task=200;
        case 200:
            if(MoveOutArmToShuttleIncludeZ_9045_1x4_8(iSht, true))              //kevin 20210717 change function
            {
        ...
        case iOUTARM_SUCK:
            ...
            if(USE_Fix_AI_CCD==true && TestIF_File.bEnableFix2BGAAICCD==true)                                                                                   //Sam 20231107 : 補 Fix AOI 模式
            {
                #if 0 // TODO(W7): Fix-AI-CCD form fFixAICCD->OutArmCycleCounterUpdate() (no FormsFacade TfFixAICCD offline) -- same gating as 1x1_1 exemplar
                    ...
                    if(ret==K_SKIP)
                    {
                        for(int i=0; i<OutArmSuck.iPickRow; i++)
                        {
                            for(int j=0; j<OutArmSuck.iPickCol; j++)
                            {
                                ...
                                {
                                    iShtCol=GetShuttleCol(i, j);
                                    PorcessJAM0201OutArmPickUpErrorSkip(i, 0, iShtCol, iSuckRow, j);                    //Steven 20161214 : 加上OutArm吸取異常Skip的ErrorLog
                                }
                        ...
                        if(USE_Fix_AI_CCD==true &&
                           TestIF_File.bEnableFix2BGAAICCD==true)                                                       //Sam 20231107 : 補 Fix AOI 模式
                        {
                            #if 0 // TODO(W7): Fix-AI-CCD form fFixAICCD->OutArmCycleCounterUpdate() (no FormsFacade TfFixAICCD offline)
                            fFixAICCD->OutArmCycleCounterUpdate();
                            #endif
                        }
                        iOutShtRetryCount=0;
...
//==============================================================================
//  DoOutArm_9045_1x4_8  -- golden aoutarm9045_1x4_8.cpp:729
//==============================================================================
...
//                        #endif
        ...
        case 1150:
            ...
            if(MoveOutArmToShuttleIncludeZ_9045_1x4_8(iWhichShuttle, false))
            {
                IsCatchTrayReadySupplyNewTray();

                if(OutArmSuck.iXStep!=1)
                {
                    iBackMode1=GetNowShuttleMode_1x4_8(0);
                }
...
//#endif
                ...
                if(OutArmSuck.iXStep!=1)
                {
                    iMode1=GetNowShuttleMode_1x4_8(0);
...
//                if(WhichAutoNeedTray()==0)                                      //JerryYang 20170731 (wei) Out arm要讓位才不會hang up
        ...
        case 1200:
            ...
            if(DoPickFromShuttle_9045_1x4_8(0))
            {
...
//                        #endif
        ...
        case 2050:
            ...
            if(MoveOutArmToShuttleIncludeZ_9045_1x4_8(iWhichShuttle, false))
            {
                ...
                if(OutArmSuck.iXStep!=1)
                {
                    iBackMode2=GetNowShuttleMode_1x4_8(1);
                }
                Task=2100;
            }
...
//#endif
                ...
                if(OutArmSuck.iXStep!=1)
                {
                    iMode2=GetNowShuttleMode_1x4_8(1);
        ...
        case 2200:
            ...
            if(DoPickFromShuttle_9045_1x4_8(1))
            {
                PreSetOutAdditionalFlag();                                                                              //Steven 20220614 : Fixed for out arm additional function
...
//#endif
        ...
        case 5100:                                                                                                      //判斷另一個Shuttle是否有IC
            if(iPickFinish==0x01)
            {
                if(BRCarryKit.UseSiteHasIC() &&
                   OutSHT2InRT() &&
                   bCheckShuttle2Flag==false)                                                                           // change another shuttle
                {
                    iWhichShuttle=1;
                    Task=2000;
                }
                else if(BRCarryKit.UseSiteHasIC() ||
                        BTestSuck.UseSiteHasIC())                                                                       //Steven 20151023 : 強制等待另外一個蝦頭
                {
                    Task=5200;
                }
                else
            ...
            else if(iPickFinish==0x02)
            {
                ...
                else if(FRCarryKit.UseSiteHasIC() ||
                        FTestSuck.UseSiteHasIC())                                                                       //Steven 20151023 : 強制等待另外一個蝦頭
                {
                    Task=5200;
                }
                else
        ...
        case 5300:
            if(iPickFinish==0x01)
            {
                flag3=MoveOutArmToShuttleIncludeZ_9045_1x4_8(1, false);
            }
            else
            {
                flag3=MoveOutArmToShuttleIncludeZ_9045_1x4_8(0, false);
            }
