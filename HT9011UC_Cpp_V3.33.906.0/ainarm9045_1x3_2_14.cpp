// =============================================================================
//  ainarm9045_1x3_2_14.cpp  --  In-Arm per-site VARIANT (1x3_2_14, 1x3 close-site)
//
//  Faithful translation of golden ainarm9045_1x3_2_14.cpp (2993 lines, BCB6, Big5).
//  Translator: AI(W6.2c-INARM-1x3_2_14) 20260626
//  Translation wave: W6.2c (in-arm per-site-config variant: 1x3_2_14; 1x3 with
//  Ab/Ac/Ad picker columns + close-site 3x7 HP support).  Sibling of the already-
//  translated leaf ainarm9045_1x3_4 and the engine ainarm9045.cpp.
//
//  ROLE: the per-site in-arm pick/place state machines for the 1x3_2_14 layout.
//  The engine's dispatch ladder DoInArm_9045() / DoInArm_9045_SuckerMap() routes
//  iInArmType==e9045_1x3_2_14 here (un-gated at the Integrate stage).  This file
//  owns (golden file:line in the per-function provenance comments below):
//    SetInOutArmParameter_1x3_2_14, GetNowSiteKitMode_1x3_2_14,
//    InArmZNeedDown_1x3_2_14, CheckXYPitch_1x3_2_14,
//    MoveInArm2XYToShuttle_9045_1x3_2_14, DoPlaceToHotPlate_9045_1x3_2_14,
//    DoInArmPlaceToShuttle_9045_1x3_2_14, DoInArm_9045_1x3_2_14_SuckerMap,
//    DoInArm_9045_1x3_2_14, DoInArmTryPickFromHotPlate_9045_1x3_2_14,
//    GetShuttleState_1x3_2_14, CheckCloseSiteMode_1x3_14, CheckCloseSiteMode_2x3N_14.
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula,
//  fall-through and goto/label (IN_ARM_SHUTTLE_1x3_2_14 /
//  IN_ARM_TRY_PICK_HOTPLATE_1x3_2_14) is transcribed VERBATIM from the golden
...
//
//  Note: GetNowSiteKitMode_1x3_2_14 / InArmZNeedDown_1x3_2_14 / CheckXYPitch_1x3_2_14
//  / MoveInArm2XYToShuttle_9045_1x3_2_14 take an EXTRA iKit arg (3-deep grid);
//  signatures preserved verbatim from golden.
//
...
//  DoInArmTryPickFromHotPlate_9045_1x3_2_14 (golden :2653) keeps its golden #else
//  (real) path active, reproduced verbatim.  Numeric semantics (int vs double,
//  integer division, double pitch math) kept EXACT.
//
...
//==============================================================================
//  golden ainarm9045_1x3_2_14.cpp :32  (forward declaration)
bool DoInArmTryPickFromHotPlate_9045_1x3_2_14(bool bOneTimeCheckAll, bool bShowErrorMessage);
//==============================================================================
//  golden ainarm9045_1x3_2_14.cpp :34
...
//        CopyInitSuck(&InArmSuckBackup , &InArmSuck , 0, 1, 0, 3);
    ...
    InArmSuck.SetPickerCount(1, 2, 1, 3, 3, 2, 2);
    OutArmSuck.SetPickerCount(1, 2, 1, 3, 3, 2, 2);
}
//==============================================================================
//  golden ainarm9045_1x3_2_14.cpp :56
int GetNowSiteKitMode_1x3_2_14(int iSht, int iKit, bool bPlace)
{
    ...
    if(bPlace==true)
    {
        if(TestIF.iAutoClean_Function && bUse8Picker==false && bRunAutoClean)
        {
            ...
            else if(InArmSuck.iXStep==2)
            {
                ...
                {
                    //if(InArmSuck.Item[1][3]>=HAS_IC)
                    return 21000;
                }
            }
            else                                                                //if(InArmSuck.iXStep==3)                                      //Steven 20251113 : for 1x3_14 close site Ab
            {
                if(InArmSuck.Item[1][0]>=HAS_IC)
                    return 30000;
                else
    ...
    else
    {
        ...
        else                                                                    //if(InArmSuck.iXStep==3)                                          //Steven 20251113 : for 1x3_14 close site Ab
        {
            if(ptrInSHTBackup.Item[0][0]==iX)
                return 30000;
            else                                                                //if(ptrInSHTBackup.Item[0][1]==iX)                            //Eastsun 20251218 : 從Test Clean 出來後都在同一位置吸 註解掉解決此問題 ptrInSHTBackup.Item[0][2]==iX 才是有效的
                return 30100;
        }
    }
}
//==============================================================================
//  golden ainarm9045_1x3_2_14.cpp :171
void InArmZNeedDown_1x3_2_14(int iSht, int iKit, bool bPlace)
{
    if(InitialInArmNeedSuck(iSht, bPlace)==false)
        return;

    int iSuckRow, iSuckCol, iShtCol;
    int iMode   =GetNowSiteKitMode_1x3_2_14(iSht, iKit, bPlace);
        ...
        for(int j=0; j<4; j++)
        {
               ...
               (iModeRow==2 || iModeRow==i))                                    // O X X O        O O X X
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
                    ...
                    (iModeRow==2 || iModeRow==i))                               // Q X X O        X X O Q
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
                    ...
                    (iModeRow==2 || iModeRow==i))                               // X X X O        X O X X
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
                    ...
                    (iModeRow==2 || iModeRow==i))                               // Q X X X        X X X Q
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
            else if(iModeCol==211 && j==3 &&                                    //吸嘴       ==>  蝦頭
                    (iModeRow==2 || iModeRow==i))                               // X X X O        X X O X
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
            else if(iModeCol==300 && j==0 &&                                    //吸嘴       ==>  蝦頭
                    (iModeRow==2 || iModeRow==i))                               // O X X O        O X X X
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);    //Steven 20251113 : for 1x3_14 close site Ab
            }
            else if(iModeCol==301 && j==3 &&                                    //吸嘴       ==>  蝦頭
                    (iModeRow==2 || iModeRow==i))                               // X X X O        X X O X
            {
                SetInArmNeedDestory(bPlace, i, iShtCol, iSuckRow, iSuckCol);
            }
...
//==============================================================================
//  golden ainarm9045_1x3_2_14.cpp :234
void CheckXYPitch_1x3_2_14(int *iX, int *iY, int iSht, int iKit, bool bPlace, int iMovePitchX, int iMovePitchY)
{
    int iMode       =GetNowSiteKitMode_1x3_2_14(iSht, iKit, bPlace);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
    ...
    else
    {
        ...
        {
            *iX=*iX+(dMovePitchX*2.0-TestIF.dSiteXPitch*1.0);                   //Steven 20251113 : for 1x3_14 close site Ab
        }
        else if(iModeCol==301)                                                  //Ad --> Ac
        {
            *iX=*iX+(-dMovePitchX*1.0+TestIF.dSiteXPitch*1.0);
...
//==============================================================================
//  golden ainarm9045_1x3_2_14.cpp :365
bool MoveInArm2XYToShuttle_9045_1x3_2_14(int iSht, int iKit, bool IncludeZ, bool bPlace)
{
    ...
    CheckXYPitch_1x3_2_14(&iXPos, &iYPos, iSht, iKit, bPlace, iMovePitchX, iMovePitchY);
...
//==============================================================================
//  golden ainarm9045_1x3_2_14.cpp :447
bool DoPlaceToHotPlate_9045_1x3_2_14()
{
...
//==============================================================================
//  golden ainarm9045_1x3_2_14.cpp :725
bool DoInArmPlaceToShuttle_9045_1x3_2_14()
{
    IN_ARM_SHUTTLE_1x3_2_14:
    ...
    switch(Task)
    {
       ...
       case 1610:
        ...
        case 2200:
            ...
            iInArmPutIcToSH[iSht][iKit]=1;                                                                                                                      //Ifor 20171121 : Test 查看異常資料
            break;
        case 2250:
            if(MoveInArmXYToShuttle_9045(iSht, iKit, ZAxisDown, true))
            {
...
//==============================================================================
//  golden ainarm9045_1x3_2_14.cpp :1591
void DoInArm_9045_1x3_2_14_SuckerMap()
{
    for(int j=0; j<8; j++)
    {
        ...
        {
            if(TestIF.iTestMode==_6Site2X3N)
            {
...
//    Prod.bInSuckUse[0][0][4]=true;                                              //Steven 20240508 : fixed for 1x3_14
//    Prod.bInSuckUse[1][0][4]=true;
    Prod.iSiteMap[0][0][4]  =-1;
    Prod.iSiteMap[1][0][4]  =-1;
}
//==============================================================================
//  golden ainarm9045_1x3_2_14.cpp :1631
void DoInArm_9045_1x3_2_14()
{
    int &Task=iArmTask;
...
//            break;
        ...
        case 15000:                                                                                                     //ChungHung 20120206 Hotplate check
            ...
            if(DoInArmTryPickFromHotPlate_9045_1x3_2_14(bOneTimeHotPlateCheckAll, bTryPickFromHotPlateShowError))       //Ifor 20151013 需新增1x3_2_14 Function
            {
                for(int i=0; i<2; i++)
                {
                    for(int j=0; j<4; j++)
...
//==============================================================================
//Ifor  20151013 1x3_2_14 Hotplate check start
//  golden ainarm9045_1x3_2_14.cpp :2571
bool DoInArmTryPickFromHotPlate_9045_1x3_2_14(bool bOneTimeCheckAll, bool bShowErrorMessage)
{
    IN_ARM_TRY_PICK_HOTPLATE_1x3_2_14:
...
//    static bool bSuckDuplicateErr[2][4]={{false, false, false, false}, {false, false, false, false}};
    ...
    switch(Task)
    {
        ...
        case 350:
            ...
            Task=400;
            goto IN_ARM_TRY_PICK_HOTPLATE_1x3_2_14;
//            break;
        case 400:
            for(i=0; i<2; i++)
            {
                for(j=0; j<2; j++)
                {
                    if(InArmSuck.Item[i][j*InArmSuck.iPickStep]==HAS_TRY_SUCK_IC)
                    {
                        Task=1;
                        return false;
                    }
...
//            break;
        ...
        case 500:
            for(i=0; i<2; i++)
            {
                for(j=0; j<2; j++)
                {
                    if(InArmSuck.Item[i][j*3]==HAS_NULL_IC)
                    {
                        InArmSuck.SetItemData(i, j*3, NULL_IC);
                    }
                }
            }
            ...
            if(IsHotPlateCheckFinsih())                                         //Hotplate check
            {
                if(bOneTimeCheckAll==true)
                {
...
//==============================================================================
//  golden ainarm9045_1x3_2_14.cpp :2933
int GetShuttleState_1x3_2_14(int iSht, bool bPick)
{
    if(bPick==bAutoPick)                                                        //Pick
    {
        if(ptrInSHT->ArmUpSideAllTypeIC(NULL_IC, 0, 2)==false)                  //左上有料
            return 1;
        else if(ptrInSHT->ArmUpSideAllTypeIC(NULL_IC, 2, 1)==false)             //右上有料
            return 3;
        else
            return 0;
    }
        ...
        if(bUse8Picker)
        {
            if(ptrInSHT->ArmUpSideAllTypeIC(NULL_IC, 0, 2) &&                   //左上無料
               ((bAutoCleanPlaceToSht==false && InArmSuck.ArmUpSideNoIC()) ||
                ...
                return 1;
            else if(ptrInSHT->ArmUpSideAllTypeIC(NULL_IC, 2, 1) &&              //右上無料
                    ((bAutoCleanPlaceToSht==false && InArmSuck.ArmUpSideNoIC()) ||
                     (bAutoCleanPlaceToSht==true  && InArmSuck.ArmUpSideNoIC()==false)))
                return 3;
            else
...
//==============================================================================
//  golden ainarm9045_1x3_2_14.cpp :2975
void CheckCloseSiteMode_1x3_14()                                                //Steven 20251113 : for 1x3_14 run 3x7 HP
{
...
//==============================================================================
//  golden ainarm9045_1x3_2_14.cpp :2985
void CheckCloseSiteMode_2x3N_14()                                               //Steven 20251113 : for 1x3_14 run 3x7 HP
{
    if(TestIF_File.iSiteMap[0][1]==0 &&
