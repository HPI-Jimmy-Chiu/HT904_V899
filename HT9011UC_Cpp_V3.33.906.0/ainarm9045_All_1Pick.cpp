// =============================================================================
//  ainarm9045_All_1Pick.cpp  --  In-Arm per-site VARIANT (All_1Pick / ep1Picker)
//
//  Faithful translation of golden ainarm9045_All_1Pick.cpp (2761 lines, BCB6,
//  Big5).  Translator: AI(W6.2c-All_1Pick) 20260626
//  Translation wave: W6.2c (in-arm per-site-config variant: All_1Pick -- the
//  single-picker path, selected by USE_PICKER_COUNT==ep1Picker, NOT by an
//  iInArmType test).  Sibling of the already-translated leaves
//  ainarm_SearchPickPlate / ainarm_SearchPlacePlate and the engine ainarm9045.cpp.
//
//  ROLE: the per-site in-arm pick/place state machines for the single-picker
//  All_1Pick layout.  The engine's dispatch ladder DoInArm_9045() routes the
//  FIRST arm (USE_PICKER_COUNT==ep1Picker) here (un-gated at integrate stage).
//  The golden engine DECLARES DoInArm_9045_All_1Pick_SuckerMap() but NEVER
//  dispatches it (verified by full-file grep: extern line only, zero call sites).
//  This file owns:
//    SetInOutArmParameter_All_1Pick, GetNowSiteKitMode_All_1Pick,
//    GetNowInShuttleRowCol_All_1Picker, InArmZNeedDown_All_1Pick,
//    CheckXYPitch_All_1Pick, MoveInArm2XYToShuttle_9045_All_1Pick,
//    DoPlaceToHotPlate_9045_All_1Pick, DoInArmPlaceToShuttle_9045_All_1Pick,
//    DoInArm_9045_All_1Pick_SuckerMap, DoInArmTryPickFromHotPlate_9045_All_1Pick,
//    DoInArm_9045_All_1Pick, GetShuttleState_All_1Pick.
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula,
//  fall-through and goto/label (IN_ARM_SHUTTLE_2x4_8 /
//  IN_ARM_TRY_PICK_HOTPLATE_All_1) is transcribed VERBATIM from the golden CP950.
...
//  atester_shims + canary_support + ainarm9045 engine + ainarm_SearchPick/
//  PlacePlate.  No body is #if 0-gated.
//
//  VCL/Borland conversions: AnsiString (1-based, ::sprintf) via vclcompat;
//  __fastcall/__property NOT present in this file; __FUNC__ via canary_support
//  shim.  SOFT_SIMULTE is NOT defined -> the #ifdef SOFT_SIMULTE block in
//  DoInArmTryPickFromHotPlate_9045_All_1Pick case 360 keeps its golden #else
//  (real) path active, reproduced VERBATIM.  Numeric semantics (int vs double,
//  integer division %100 / /100, double pitch math) kept EXACT.
//
//  SUBSTRATE GAP (REPORTED, not added here): TMyKitSuck::RowHasDefineIC(int,int)
//  is dereffed by GetShuttleState_All_1Pick (InArmSuck.RowHasDefineIC(...)).  The
//  target's TMyKitSuck in aHotPlateSubstrate.h does NOT yet declare it (golden
//  MyKitSuck.h:298 / MyKitSuck.cpp:514).  Integrate adds it ONCE; left as an
//  expected unresolved substrate extern for standalone syntax-check.
//
...
#include "ainarm9045.h"             // engine: GetInArmToShuttleOffset_9045 / InArmNeedCheckOffset / MoveInArmXYToShuttle_9045 / GetShuttleCol / DoInArmPickFromLoadStage_9045 / DoInArmPickFromHotPlate_9045 / ...
...
#include "ainarm_SearchPlacePlate.h"// shared HP-place externs (SearchPlateToPlace / MoveInArmXYToHotPlatePlace / DoPlaceToHPSwapData / GetPlaceToHotPlateSuckCol / Row2CanPutHP / GetHotPlateColStep / iPlaceHP / HPPlaceLog / PickFromHPList)
#include "csystem.h"                // CheckInArmSuckICFallDownToHasNullIC / DoInArmAutoSiteMapping / CheckInArmDestroyICFail / AutoSiteMappingCheckShuttle predicates
#include "Motor/mymotor.h"          // MOT[], InArmContinuousMove_9045
#include "cmydef.h"                 // global scalar universe + IC consts + enums (eInArmType / picker enum / Tempture_* / rsm* / CC_* / iHPWideHP / In_Shuttle_Auto_Latch)
#include "common.h"
#include "mycylin.h"
...
#include "aHotPlateSubstrate.h"     // InArmSuck/FLCarryKit/BLCarryKit/OutArmSuck/FTestSuck/BTestSuck + ainarm2 cursors/timers + engine shims + PickFromHPList + tRotate
#include "FormsFacade.h"            // HotPlateForm / TrayForm / fMain (DebugOneCycleHotPlate) offline stand-ins
#include "atester_shims.h"          // fObserver (RecordInArmTime/AddTimeData) / fContact (IsRun2DCheck) / fYieldMonitoring (DoAutoCloseSite)
...
//==============================================================================
// SetInOutArmParameter_All_1Pick  (golden ainarm9045_All_1Pick.cpp:36)
//==============================================================================
...
//    CopyInitSuck(&OutArmSuckBackup, &OutArmSuck, 1, 0, 1, 2);
    ...
    OutArmSuck.SetPickerCount(1, 1, 2, 2, 1, 0, 0);
}
//==============================================================================
...
//                    0                     =標準
//         100        |         101         =左右分兩次
// GetNowSiteKitMode_All_1Pick  (golden ainarm9045_All_1Pick.cpp:67)
//==============================================================================
int GetNowSiteKitMode_All_1Pick(int iSht, bool bPlace)
{
    if(bPlace==bAutoPlace)
    {
        for(int irow=0; irow<FLCarryKit.iShtRow; irow++)                        //Steven 20241206 : InArmSuck.iShtRow -->　FLCarryKit.iMaxRow
        {
            for(int icol=0; icol<FLCarryKit.iShtCol; icol++)                    //Steven 20241206 : InArmSuck.iShtCol -->　FLCarryKit.iMaxCol
            {
                if(iSht==0)
                {
                    if(FLCarryKit.Item[irow][icol]==NULL_IC && Prod.fInArmSuck4x8[iSht][irow][icol])
                    {
                ...
                {
                    if(BLCarryKit.Item[irow][icol]==NULL_IC && Prod.fInArmSuck4x8[iSht][irow][icol])
                    {
                        return icol*100+irow;
                    }
                }
...
//---------------------------------------------------------------------------
// GetNowInShuttleRowCol_All_1Picker  (golden ainarm9045_All_1Pick.cpp:120)
//---------------------------------------------------------------------------
bool GetNowInShuttleRowCol_All_1Picker(int iSht, int *iUseRow, int *iUseCol, bool bPick)
{
    if(bPick==bAutoPlace)
    {
        for(int irow=0; irow<FLCarryKit.iShtRow; irow++)                        //Steven 20241206 : InArmSuck.iShtRow -->　FLCarryKit.iMaxRow
...
//==============================================================================
// InArmZNeedDown_All_1Pick  (golden ainarm9045_All_1Pick.cpp:180)
//==============================================================================
void InArmZNeedDown_All_1Pick(int iSht, bool bPlace)
{
    if(InitialInArmNeedSuck(iSht, bPlace)==false)
        return;
    int iMode   =GetNowSiteKitMode_All_1Pick(iSht, bPlace);
    int iModeRow=iMode%100;
    int iModeCol=iMode/100;
    SetInArmNeedDestory(bPlace, iModeRow, iModeCol, 0, 0);
}
//==============================================================================
// CheckXYPitch_All_1Pick  (golden ainarm9045_All_1Pick.cpp:190)
//==============================================================================
void CheckXYPitch_All_1Pick(int *iX, int *iY, int iSht, bool bPlace, int iMovePitchX, int iMovePitchY)
{
    int iMode       =GetNowSiteKitMode_All_1Pick(iSht, bPlace);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
    int iOffsetPos  =GetInArmToShuttleOffset_9045(iSht, 0, 0, false);
    int iTarget=0;
    if(iSht==0)
    {
        iTarget=MInShuttle1;
    ...
    GetShtRowColPos(iTarget,iModeCol,iModeRow,*iX,*iY);
...
// MoveInArm2XYToShuttle_9045_All_1Pick  (golden ainarm9045_All_1Pick.cpp:214)
//==============================================================================
bool MoveInArm2XYToShuttle_9045_All_1Pick(int iSht, bool IncludeZ, bool bPlace)
{
    ...
    CheckXYPitch_All_1Pick(&iXPos, &iYPos, iSht, bPlace, iMovePitchX, iMovePitchY);
...
//==============================================================================
// DoPlaceToHotPlate_9045_All_1Pick  (golden ainarm9045_All_1Pick.cpp:267)
//==============================================================================
bool DoPlaceToHotPlate_9045_All_1Pick()
{
...
//==============================================================================
// DoInArmPlaceToShuttle_9045_All_1Pick  (golden ainarm9045_All_1Pick.cpp:568)
//==============================================================================
bool DoInArmPlaceToShuttle_9045_All_1Pick()
{
    IN_ARM_SHUTTLE_2x4_8:
...
//                for(int j=0; j<4; j++)
                {
                    int iMode=GetNowSiteKitMode_All_1Pick(0,true);
                    iSuckRow=0;
                    iSuckCol=0;
                    iShtRow =iMode%100;
                    iShtCol =iMode/100;
...
//            }
        ...
        case 1500:
            ...
            if(iInArmType!=e9045_1x1_1             &&                           //KenHsieh 20251106 : single site (1x1) mode 不進行 In Sht Latch 判斷
...
//            }
            ...
            for(int irow=0; irow<FLCarryKit.iShtRow; irow++)                    //JerryYang 20250901
            {
                for(int icol=0; icol<FLCarryKit.iShtCol; icol++)
                {
                    if(FLCarryKit.Item[irow][icol]==NULL_IC &&
                      (Prod.fInArmSuck4x8[0][irow][icol]==false ||              //Ifor 20260317 : fix disabled site causing IsFLCarrKitAllHasIC always false
                      (LastSet.bUseTestSocket[0][irow][icol]==false &&
                       LastSet.bUseTestSocket[1][irow][icol]==false)))
                    {
                        FLCarryKit.SetItemData(irow, icol, HAS_NULL_IC);
                    }
                }
            }
            ...
            SetShuttlefCanMoveL(iSht, true, __FUNC__, Task);
...
//                for(int j=0; j<4; j++)
                {
                    int iMode=GetNowSiteKitMode_All_1Pick(iSht,true);
                    iSuckRow=0;
                    iSuckCol=0;
                    iShtRow =iMode%100;
                    iShtCol =iMode/100;
...
//            }
        ...
        case 2500:
            ...
            GetNowSiteKitMode_All_1Pick(iSht,true);

            for(int irow=0; irow<BLCarryKit.iShtRow; irow++)
            {
                for(int icol=0; icol<BLCarryKit.iShtCol; icol++)
                {
                    if(BLCarryKit.Item[irow][icol]==NULL_IC &&
                       (Prod.fInArmSuck4x8[1][irow][icol]==false ||             //Ifor 20260317 : fix disabled site causing IsBLCarrKitAllHasIC always false
                       (LastSet.bUseTestSocket[0][irow][icol]==false &&
                        LastSet.bUseTestSocket[1][irow][icol]==false)))
                    {
                        BLCarryKit.SetItemData(irow, icol, HAS_NULL_IC);
                    }
                }
            }
...
//==============================================================================
// DoInArm_9045_All_1Pick_SuckerMap  (golden ainarm9045_All_1Pick.cpp:1492)
//  NOTE: DEFINED here but the golden engine DoInArm_9045_SuckerMap NEVER
//        dispatches it (faithful).  Declared in the engine externs only so the
//        symbol resolves at link.
//==============================================================================
void DoInArm_9045_All_1Pick_SuckerMap()
{
//    Prod.fInArmSuck4x8[0][0][0]=true;
//    Prod.fInArmSuck4x8[1][0][0]=true;
                   ...
                   tRotate.ActiveRotate)
                {
                       ...
                       iRotate_Type==e1MotRotate1Dut)
                    {
                        ...
                        else
                        {
                            ...
                            Prod.RotateDutDate[1][i][j]=tRotate.RotateDutDate[1][i][iShtCol];
                        }
                    }
                }
            }
...
// DoInArmTryPickFromHotPlate_9045_All_1Pick  (golden ainarm9045_All_1Pick.cpp:1568)
//==============================================================================
bool DoInArmTryPickFromHotPlate_9045_All_1Pick(bool bOneTimeCheckAll, bool bShowErrorMessage)
{
    IN_ARM_TRY_PICK_HOTPLATE_All_1:
...
//    static bool bSuckDuplicateErr[2][4]={{false, false, false, false}, {false, false, false, false}};
    ...
    switch(Task)
    {
        ...
        case 100:
            ...
            break;
        case 350:
            if(InArmSuck.Item[0][0]==HAS_TRY_SUCK_IC || InArmSuck.Item[0][0]==HAS_NULL_IC)                              //JerryYang 20251201 : Fix單吸嘴模組Hot plate initial check error
            {
                ix=iPlacePlateX[0];
                ip=iPlacePlate[0];
                iy=iPlacePlateY[0];
                ...
                if(InArmSuck.Item[0][0]==HAS_NULL_IC || InArmSuck.Suck[0][0].Suck() || InArmSuck.Suck[0][0].Error)
                {
                    ...
                    InArmReleaseDelayToHot.SetSecAndOn(ArmSpeed[InArm].dHPVacuumTI);
                    Task=360;
                }
            }
            ...
            break;
        case 360:
            if(InArmReleaseDelayToHot.Off())
            {
                if(InArmSuck.Item[0][0]==HAS_TRY_SUCK_IC || InArmSuck.Item[0][0]==HAS_NULL_IC)
                {
                    ix=iPlacePlateX[0];
                    ip=iPlacePlate[0];
                    iy=iPlacePlateY[0];
                    if(InArmSuck.Suck[0][0].Suck() || InArmSuck.Suck[0][0].Error)
                    {
                        MOT[MMPlate1+ip].SetTraySingleData(ix, iy, HAS_TRY_SUCK_IC);
                        ...
                        #else
                            if(InArmSuck.Item[0][0]==HAS_TRY_SUCK_IC && InArmSuck.Suck[0][0].Error==false && LastSet.iRealDummy==REALLY)
                            {
                                if(bHPCleanout)                                 //wei 20160624 Hotplate clean out
                                {
                                    if(iHPShuttle==0)
                                ...
                                InArmSuck.SetItemData(0, 0, HAS_SUCK_IC);
                            }
                            else
                            {
                                InArmSuck.SetItemData(0, 0, NULL_IC);
            ...
            Task=400;
            goto IN_ARM_TRY_PICK_HOTPLATE_All_1;
        case 400:
            if(InArmSuck.Item[0][0]==HAS_TRY_SUCK_IC)
            {
                Task=1;
            ...
            if(MoveInArmZToPlateSafe(Task))
            {
                ...
                if(bShowErrorMessage)
                {
                    ...
                    bSuckFinish[0][0]=false;
                    if(InArmSuck.Item[0][0]==HAS_SUCK_IC)
                    {
                        ErrPart+=InArmSuck.Suck[0][0].sName;
                        bHasError=true;
                    }

                    if(bTryPickHPDuplicateErr[0][0])
                    {
                    ...
                    if(bHasError)
                    {
                        ...
                        bTryPickHPDuplicateErr[0][0]=true;
                        Task=450;
                    }
                    else
                    {
                        bTryPickHPDuplicateErr[0][0]=false;
                        Task=500;
                    }
                }
                else
                {
                    if(bOneTimeCheckAll)
                    {
                        if(InArmSuck.Item[0][0]==HAS_SUCK_IC)                   //JerryYang 20251201 : Fix單吸嘴模組Hot plate initial check error
                        {
                            InArmSuck.SetItemData(0, 0, HAS_HOT_IC);
                        }
                        else
                        {
                            InArmSuck.SetItemData(0, 0, HAS_NULL_IC);
                        }
                    }
                    Task=500;
                }
            ...
            break;
        case 450:
            for(int i=0; i<1; i++)                                              //JerryYang 20251201 : Fix單吸嘴模組Hot plate initial check error
            {
                for(int j=0; j<1; j++)
                {
...
//==============================================================================
// DoInArm_9045_All_1Pick  (golden ainarm9045_All_1Pick.cpp:1821)
//  The ep1Picker FIRST dispatch arm of the engine ladder.
//==============================================================================
void DoInArm_9045_All_1Pick()
{
    ...
    switch(Task)
    {
        ...
        case 200:                                                               //kevin 20120115 判斷關site是否吸ic
            ...
            if(Prod.fInArmSuck4x8[iSht][0][0+iKit32]==false &&
...
//            if(Prod.fInArmSuck4x8[iShtHP][0][0+iKitHP32]==false &&
//               InArmSuck.Suck[0][0].GetStatus())
//            {
//                bCheckCloseSiteHaveIC=true;
//                ErrPart+=InArmSuck.Suck[0][0].sName;
...
//            }
        ...
        case 15000:                                                             //ChungHung 20120206 Hotplate check
            ...
            if(DoInArmTryPickFromHotPlate_9045_All_1Pick(bOneTimeHotPlateCheckAll, bTryPickFromHotPlateShowError))
            {
                if(InArmSuck.Item[0][0]==HAS_TRY_SUCK_IC)
                    InArmSuck.SetItemData(0, 0, NULL_IC);
...
//==============================================================================
// GetShuttleState_All_1Pick  (golden ainarm9045_All_1Pick.cpp:2721)
//  NOTE: derefs InArmSuck.RowHasDefineIC(int,int) -- a TMyKitSuck method NOT yet
//        present in aHotPlateSubstrate.h (golden MyKitSuck.h:298).  REPORTED as a
//        substrate gap for the serial Integrate agent to add ONCE.
//==============================================================================
int GetShuttleState_All_1Pick(int iSht, bool bPick)
{
