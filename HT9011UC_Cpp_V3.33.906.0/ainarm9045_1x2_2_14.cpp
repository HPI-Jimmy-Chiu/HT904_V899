// =============================================================================
//  ainarm9045_1x2_2_14.cpp  --  In-Arm per-site VARIANT (1x2_2_14)
//
//  Faithful translation of golden ainarm9045_1x2_2_14.cpp (2965 lines, BCB6, Big5).
//  Translator: AI(W6.2c-INARM-1x2_2_14) 20260626
//  Translation wave: W6.2c batch-1 (in-arm per-site-config variant: 1x2_2_14;
//  enum e9045_1x2_2_14).  Sibling of the already-translated leaves
//  ainarm_SearchPickPlate / ainarm_SearchPlacePlate and the engine ainarm9045.cpp,
//  and of the committed variants 1x1_1 / 2x4_16 / 2x8_32 / 1x3_4.
//
//  ROLE: the per-site in-arm pick/place state machines for the 1x2_2_14 layout.
//  The engine's dispatch ladder DoInArm_9045() / DoInArm_9045_SuckerMap() routes
//  iInArmType==e9045_1x2_2_14 here (un-gated at the serial Integrate stage).
//  This file owns (golden order):
//    GetNowSiteKitMode_1x2_2_14, InArmZNeedDown_1x2_2_14, CheckXYPitch_1x2_2_14,
//    MoveInArm2XYToShuttle_9045_1x2_2_14, DoPlaceToHotPlate_9045_1x2_2_14,
//    DoInArmPlaceToShuttle_9045_1x2_2_14, DoInArm_9045_1x2_2_14_SuckerMap,
//    DoInArm_9045_1x2_2_14, DoInArmTryPickFromHotPlate_9045_1x2_2_14,
//    GetShuttleState_1x2_2_14, SetInArmHasDropToShuttle_1x2_2_14.
//  (No SetInOutArmParameter_1x2_2_14 / MoveInArm2XYToShuttle is split: this
//   variant has MoveInArm2XYToShuttle_9045_1x2_2_14 + DoInArmPlaceToShuttle but
//   NO SetInOutArmParameter of its own -- mirrors golden.)
//
//  FAITHFUL: every fn/var/cursor value, switch(Task) structure, formula,
//  fall-through and goto/label (IN_ARM_SHUTTLE_1x2_2_14 /
//  IN_ARM_TRY_PICK_HOTPLATE_1X2_2) is transcribed VERBATIM from the golden CP950.
...
//  SOFT_SIMULTE is NOT defined -> the two #ifdef SOFT_SIMULTE blocks in
//  DoInArmTryPickFromHotPlate_9045_1x2_2_14 (golden :2588 + :2690) keep their
...
#include "ainarm9045.h"             // engine: GetInArmToShuttleOffset_9045 / GetInArmXToShuttleOffset_9045 / GetInArmYToShuttleOffset_9045 / GetInArmPitchX_9045 / GetInArmPitchY_9045 / GetVariableYInShuttleData / GetVariableXInHotPlateData / GetShuttleCol / InArmContinuousMove? (via Motor) / TransferInShuttleRatio / InArmZNeedDown_9045 / ...
#include "ainarm_SearchPickPlate.h" // shared HP-pick externs (bInArmTryPickFromHotPlateFinish / HasHotReadyIC_9045 / iTryPickHP / BackupPlacePos / RestorePlacePos / IsHotPlateCheckFinsih / ...)
#include "ainarm_SearchPlacePlate.h"// shared HP-place externs (SearchPlateToPlace / MoveInArmXYToHotPlatePlace / DoPlaceToHPSwapData / DoPlaceToHPBackupData / GetPlaceToHotPlateSuckCol / GetPlaceToHotPlateCol / Row2CanPutHP / iPlaceHP / HPPlaceLog / ...)
...
#include "cmydef.h"                 // global scalar universe + IC consts + enums (eInArmType / Tempture_* / rsm* / CC_* / i1x2_4UseACEGPicker / bDropAtSht*NeedCheckVac / ...)
#include "common.h"
#include "mycylin.h"
...
#include "aHotPlateSubstrate.h"     // InArmSuck/FLCarryKit/BLCarryKit/OutArmSuck/FTestSuck/BTestSuck + ptrInSHT/ptrInSHTBackup + ainarm2 cursors/timers + engine shims + PickFromHPList
...
//==============================================================================
// SUBSTRATE EXTERN (REPORTED for Integrate): golden ainarm2.h:9 declares
//   extern bool flag1_1x2_2[2];  (def ainarm2.cpp:41).  Used by case 100/360 of
//   DoInArmTryPickFromHotPlate_9045_1x2_2_14 (HotPlateForm.XDivision==3 path).
//   NOT yet present in aHotPlateSubstrate.h.  Forward-declared here so this TU
//   is self-consistent; the Integrate agent should add it ONCE to the substrate
//   header (aHotPlateSubstrate.h) and remove this local decl if it collides.
#ifndef HT9045_FLAG1_1X2_2_DECLARED
#define HT9045_FLAG1_1X2_2_DECLARED
extern bool flag1_1x2_2[2];                                                     // golden ainarm2.h:9 (def ainarm2.cpp:41)
#endif
//==============================================================================
bool DoInArmTryPickFromHotPlate_9045_1x2_2_14(bool bOneTimeCheckAll, bool bShowErrorMessage);
//==============================================================================
// golden ainarm9045_1x2_2_14.cpp:34
int GetNowSiteKitMode_1x2_2_14(int iSht, bool bPlace)
{
...
//==============================================================================
// golden ainarm9045_1x2_2_14.cpp:108
void InArmZNeedDown_1x2_2_14(int iSht, bool bPlace)
{
    if(InitialInArmNeedSuck(iSht, bPlace)==false)
        return;

    int iSuckRow, iSuckCol, iShtCol;
    int iMode   =GetNowSiteKitMode_1x2_2_14(iSht, bPlace);
...
//==============================================================================
// golden ainarm9045_1x2_2_14.cpp:145
void CheckXYPitch_1x2_2_14(int *iX, int *iY, int iSht, bool bPlace, int iMovePitchX, int iMovePitchY)
{
    int iMode       =GetNowSiteKitMode_1x2_2_14(iSht, bPlace);
    int iModeRow    =iMode%100;
    int iModeCol    =iMode/100;
...
//==============================================================================
// golden ainarm9045_1x2_2_14.cpp:238
bool MoveInArm2XYToShuttle_9045_1x2_2_14(int iSht, bool IncludeZ, bool bPlace)
{
    ...
    CheckXYPitch_1x2_2_14(&iXPos, &iYPos, iSht, bPlace, iMovePitchX, iMovePitchY);
...
//==============================================================================
// golden ainarm9045_1x2_2_14.cpp:320
bool DoPlaceToHotPlate_9045_1x2_2_14()
{
...
//==============================================================================
// golden ainarm9045_1x2_2_14.cpp:604
bool DoInArmPlaceToShuttle_9045_1x2_2_14()
{
    IN_ARM_SHUTTLE_1x2_2_14:
...
//==============================================================================
// golden ainarm9045_1x2_2_14.cpp:1490
...
//==============================================================================
// golden ainarm9045_1x2_2_14.cpp:1577
...
//            break;
        ...
        case 15000:                                                             //ChungHung 20120206 Hotplate check
            ...
            if(DoInArmTryPickFromHotPlate_9045_1x2_2_14(bOneTimeHotPlateCheckAll, bTryPickFromHotPlateShowError))
            {
                for(int i=0; i<2; i++)
                {
                    for(int j=0; j<4; j++)
...
// golden ainarm9045_1x2_2_14.cpp:2483
bool DoInArmTryPickFromHotPlate_9045_1x2_2_14(bool bOneTimeCheckAll, bool bShowErrorMessage)
{
    ...
    switch(Task)
    {
        case 1:
            ...
            if(HotPlateForm.XDivision==2 && iPlacePlateX[0]==0)
            {
        ...
        case 360:                                                                                                                                               //20111015  Dell    HotPlateForm.XDivision == 3
            ...
            if(flag1_1x2_2[0] && InArmSuck.Item[0][0]==HAS_TRY_SUCK_IC)
                return false;
            if(flag1_1x2_2[1] && InArmSuck.Item[0][3]==HAS_TRY_SUCK_IC)                                                                                         //jou 20231018 : 修正1x2_14 try pick from hotplate異常
                return false;
            Task=400;
            goto IN_ARM_TRY_PICK_HOTPLATE_1X2_2;
        case 400:
        ...
        case 450:
            ...
            if(bSuckFinish[0][0] && bSuckFinish[0][3])                          //jou 20231018 : 修正1x2_14 try pick from hotplate異常
            {
                if(InArmSuck.HasType(HAS_SUCK_IC))
                {
                    Task=400;
...
//==============================================================================
// golden ainarm9045_1x2_2_14.cpp:2916
int GetShuttleState_1x2_2_14(int iSht, bool bPick)                              //Steven 20221027 : 修正Auto Clean
{
    int j2=GetJStep();
...
//==============================================================================
// golden ainarm9045_1x2_2_14.cpp:2949
