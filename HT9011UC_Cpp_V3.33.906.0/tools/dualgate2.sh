#!/bin/bash
# ############################################################################
# AI(W906-FW-GATEVERDICT) 20260827: **已停用，請改用 tools/dualgate.sh。**
#
# 下面那段「CPU 夠，重疊時峰值約 10/16，不會互相餓死」的推理**是錯的**，
# 而且錯在一個我當時沒量的地方：吃緊的不是 CPU，是**單一測試的 wall-clock 預算**。
#
# 實測（20260827，w35）：
#   dfm2rc_fidelity 單獨跑 → Release 355.37s 通過 / Debug 565.94s 通過
#   同一顆測試疊在 Release build 上 → **兩側都撞破 600s 逾時**
# 也就是說本腳本把一個會過的 gate 變成假紅，而它買到的加速只有
# **2m25s / 37m43（6%）**。用正確性換 6% 是虧的。
#
# 附帶記一個我自己犯的錯：追這件事的時候，我一邊在同一台機器上跑
# CPU 取樣／列舉行程／spawn benchmark／背景 grep 全樹，一邊量 w35r 的 fidelity，
# 然後拿那個被自己污染的數字去「推翻」重疊假設。**量測期間不要碰機器。**
#
# 保留本檔是為了留下這段記錄，不是為了再用它。
# ############################################################################
