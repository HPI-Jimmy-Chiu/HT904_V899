// =============================================================================
//  Automation/uRENESAS_Server.cpp  --  RENESAS FT-CT TCP/IP host-protocol server
//
//  Golden source: HT9011UC_Code_V3.33.906.0_20260618/Automation/uRENESAS_Server.cpp
//                 (3187 lines, cp950)
//  Translation wave: PT-W3 "renesas"
//  Translator: AI(W906-PT-W3) 20260807
//
//  See uRENESAS_Server.h for the file-pair banner (ROLE / WAVE SCOPE / GATE
//  REGISTER [G1]-[G3]).  This file's own additions to that register:
//
//  [G4] `ServerSocket->OnListen=OnProcSocketListen;` / `ServerSocket->
//       OnClientWrite=OnProcSocketWrite;` (golden ctor :29,31).
//       vclcompat/ServerSocket.h's TServerSocket exposes OnClientConnect/
//       OnClientDisconnect/OnClientError/OnClientRead ONLY (grepped
//       20260807; no OnListen/OnClientWrite field -- confirmed against that
//       file's own "API SURFACE BUILT -- nothing beyond what the cited
//       consumer needs" list, and this wave may not edit that pre-existing
//       file to add one).  GATED, and ZERO BEHAVIOUR DELTA: both golden
//       handler BODIES are empty (OnProcSocketListen golden :79-81,
//       OnProcSocketWrite golden :107-109 -- literally `{}`), so even a
//       real BCB6 build wiring these does nothing observable; the method
//       bodies are still translated faithfully below (as empty functions)
//       in case a future ServerSocket.h revision adds the slots.
//  [G5] `ShowLotEndMessage(str, str2, false)` / `ShowLotEndMessage(msg, "",
//       false)` (golden DoNeedSupplyOrNot :2875,2879) -- declared in golden
//       mymessbox.h, a modal Yes/No popup form.  mymessbox.h/.cpp are not
//       translated anywhere in the port tree (grepped 20260807; same "no
//       TForm anywhere" situation as GATE [G1]).  GATED: `ret` keeps its
//       golden-declared initial value of -1 through both branches (neither
//       assigns it), so `if(ret==1)` is false and DoNeedSupplyOrNot returns
//       false on every path through this gate.  FAITHFUL BECAUSE: with no
//       operator present to click Yes/No in a headless build, "do not
//       auto-supply untested material" is the conservative reading of an
//       un-answerable prompt -- it never silently consumes/creates product
//       the operator was never asked about.  BEHAVIOUR DELTA ON A REAL
//       MACHINE: an operator would see a dialog and could choose to supply;
//       this build always declines.
//
//  NOT gates (same-result substitutions using already-real vclcompat calls,
//  zero behaviour delta -- listed so a reviewer does not mistake "no #if 0"
//  for "skipped"):
//    * golden `fnsplit(LastSet.LastOpenFilename,cDrive,cDir,cName,cExt);
//      asHdParameter=cName;` (GetEndMessge golden :1980-1981) -- fnsplit is a
//      Borland-specific libc extension with no vclcompat equivalent, and
//      golden only ever uses the `cName` (filename sans extension, sans
//      directory) output (cDrive/cDir/cExt are computed and discarded).
//      Reproduced with vclcompat's own real ExtractFileName()+ExtractFileExt()
//      (SysUtils.h) -- identical observable result via a different call.
//    * golden `ZeroMemory(iAutoTrayCount, sizeof(iAutoTrayCount));` (:774) --
//      ZeroMemory is a WinAPI macro for `memset(p,0,n)`; written as memset
//      directly.
//    * golden `CreateDirectory(asRenesasTaskPath.c_str(),NULL);` (SaveTaskLog
//      :2711,2718) -- raw WinAPI single-level mkdir; vclcompat::CreateDir()
//      (SysUtils.h) is the exact same single-level-mkdir operation, already
//      real, already used tree-wide -- substituted directly.
//    * golden `Strings[i].AnsiCompareIC(RecvData.ProdInfo.HdParameter)==0`
//      (:484) -- AnsiCompareIC (case-insensitive compare) is not in
//      vclcompat/AnsiString.h; reproduced with `.UpperCase()==.UpperCase()`
//      (both already-real AnsiString methods), the same case-fold-then-
//      compare result.
//
//  FACADE ADDITIONS NEEDED (methods/fields on ALREADY-PORTED, OTHER-WAVE-
//  OWNED classes this file calls exactly as golden does, matching the
//  established Automation/HANA_ART.cpp precedent for "write the call as
//  golden makes it; a future integrate step adds the facade member" --
//  see that file's own "FACADE ADDITIONS NEEDED" section for the identical
//  situation).  None of these are invented here; every one is a genuine gap
//  measured against the CURRENT content of the named pre-existing header
//  (grepped in full 20260807), which this wave does not edit:
//    forms/fSCKART.h TfSCKART (golden Automation/SCK_ART.h TfSCKART):
//      + int    iLotCount            (golden SCK_ART.h -- ALREADY exists on
//                                      Automation/SCK_ART.h's SckArtState /
//                                      SCK_ART_Remainder.h's SckArtRemainderState,
//                                      NOT on the real global `fSCKART` --
//                                      see forms/fSCKART.h's own
//                                      "RECONCILIATION DEBT" banner, which
//                                      explicitly forbids a 6th declaration
//                                      site; this file's call sites are the
//                                      measurement of what that reconciliation
//                                      still owes the real object)
//      + int    iManualStart         (ditto -- SckArtRemainderState :707)
//      + AnsiString sLotStartTime    (golden SCK_ART.h; read/written by
//                                      SaveModeEndFile/case 20000)
//      + int    iCurrentFlexARTStep  (golden SCK_ART.h:264 -- ALREADY on
//                                      SckArtRemainderState, not on fSCKART)
//      + int    iWaitGPIBLotR        (golden SCK_ART.h:259)
//      + int    iOutputJamCnt        (golden SCK_ART.h; paired with the
//                                      already-real iInputJamCnt)
//      + bool   bLdCntExdInputCnt    (golden SCK_ART.h:350 -- ALREADY on
//                                      SckArtState (Automation/SCK_ART.h),
//                                      not on fSCKART; read by
//                                      DoNeedSupplyOrNot)
//      + void   ClearLotInfo()       (golden SCK_ART.h; body SCK_ART.cpp,
//                                      not yet ported to fSCKART)
//      + void   CheckNeedRT()        (golden SCK_ART.h; ditto)
//    forms/fMain.h TfMain (golden main.h):
//      + void   EnabledSetupFile(bool)
//      + int    iHasChangeFile
//      + void   SetLotState(int)
//      (cbSetupFileName/cbSetupFileNameChange/Clarn_Data/Start/Pause/
//       palMainStatus are ALREADY real on TfMain and used as-is below.)
//    forms/fLotInfo.h TfLotInfo (golden uLotInfo.h):
//      + TButton* btClearBarcodeList   (only ->Click() is touched)
//    forms/fSortCT.h TfSortCT (golden cSortCT.h):
//      + void   ShowLoadingIC()
//      + void   ShowSortIC()
//  (cSocket.h's real `LotSummary` global / `TLotSummary::ClearRTData()` --
//  checked 20260807, ALREADY real; no facade addition needed there.)
//
//  Big5 Chinese comments decoded via cp950 (python open(...,encoding=
//  'cp950')), reproduced as real UTF-8.  ZERO U+FFFD (checked before
//  hand-off).
// =============================================================================
#include "MachineDefine.h"
#include "uRENESAS_Server.h"

#include "csystem.h"           // HasICUnderMachine/HasAnyICInMachine/DoSystemMessage(real)
#include "cmydef.h"            // global flags: bAlarmBuzzer/bRenesasFTCTAlarm/bReplyFTCTAlarm/
                               // bContinueMessageByReply71/SystemStart/iCleanOut/OFF_LINE/
                               // iAutoTrayCount/bQAModeFlag/SystemHour.../SystemYear...
#include "common.h"            // asRenesasCommPath/asRenesasTaskPath/MySecondsBetween/MySleep/
                               // WriteDataToFile
#include "cprod.h"             // TestIF_File (SYSTEM_TEST_IF)/BinSelect[]/FT/OffT
#include "MachineType.h"       // rsmInitial_ART
#include "aHotPlateSubstrate.h"// SetRunStartMode(int)
#include "forms/fMain.h"       // fMain (TfMain)
#include "forms/fSCKART.h"     // fSCKART (TfSCKART)
#include "forms/fLotInfo.h"    // fLotInfo (TfLotInfo)
#include "forms/fSortCT.h"     // fSortCT (TfSortCT)
#include "cSocket.h"           // LotSummary (TLotSummary, real)
#include "canary_support.h"    // LastSet (LAST_GENERAL_SET shim), RecordProcess (real)
#include "ckernel.h"           // DoSystemMessage() (real)
#include "vclcompat/SysUtils.h"// ExtractFileName/ExtractFileExt/DirectoryExists/ForceDirectories/
                               // CreateDir/TSearchRec/FindFirst/FindNext/FindClose/faAnyFile/
                               // faDirectory/IntToHex
...
TRENESAS_Server::TRENESAS_Server()
{
    ...
    // [G4]: golden also does `ServerSocket->OnListen=OnProcSocketListen;` and
    // `ServerSocket->OnClientWrite=OnProcSocketWrite;` here -- see file
    // banner GATE [G4] (no such event slot on vclcompat::TServerSocket; both
    // golden handler bodies are empty, so this is a zero-behaviour-delta
    // gate).
#if 0
    ServerSocket->OnListen=OnProcSocketListen;
#endif
    ServerSocket->OnClientRead=[this](TObject* Sender, TCustomWinSocket* Socket)
        { OnProcSocketRead(Sender, Socket); };
#if 0
...
//---------------------------------------------------------------------------
void TRENESAS_Server::OnProcSocketListen(TObject* Sender, TCustomWinSocket* Socket)
{
}
//---------------------------------------------------------------------------
void TRENESAS_Server::OnProcSocketRead(TObject* Sender, TCustomWinSocket* Socket)
{
    AnsiString asMsg;
...
//---------------------------------------------------------------------------
void TRENESAS_Server::OnProcSocketWrite(TObject* Sender, TCustomWinSocket* Socket)
{
}
//---------------------------------------------------------------------------
void TRENESAS_Server::OnProcSocketError(TObject* Sender, TCustomWinSocket* Socket, TErrorEvent ErrorEvent, int &ErrorCode)
{
    ErrorCode=0;                                                                //Kirin 20320120
    SaveTaskLog("OnProcSocketError", -1);                                       //Kirin 20220127 Add Log
}
//---------------------------------------------------------------------------
void TRENESAS_Server::OnProcSocketConnect(TObject* Sender, TCustomWinSocket* Socket)
{
    SaveTaskLog("OnProcSocketConnect", -1);                                     //Kirin 20220127 Add Log
}
//---------------------------------------------------------------------------
//void __fastcall TRENESAS_Server::OnProcSocketConnecting(TObject* Sender, TCustomWinSocket* Socket)
//{
//
//}
//---------------------------------------------------------------------------
void TRENESAS_Server::OnProcSocketDisconnect(TObject* Sender, TCustomWinSocket* Socket)
{
...
void TRENESAS_Server::AppendToMemo(AnsiString asMsg)
{
    //if(FLogMemo!=NULL)
    {
        AnsiString Path, s="";
        GetTimeInfo();
        s.sprintf("%s %02d:%02d:%02d.%03d %s",FormatDateTime("yyyy/mm/dd", Now()).c_str(), SystemHour, SystemMin, SystemSec, SystemMSec,asMsg.c_str());   //Sam 20230420 : 時間紀錄到小數點
        //FLogMemo->Lines->Add(s);

        Path.sprintf("%s\\%04d_%02d_%02d", asRenesasCommPath.c_str(), SystemYear, SystemMonth, SystemDate);
        if(!DirectoryExists(Path))
            ForceDirectories(Path);
        Path.sprintf("%s\\%04d_%02d_%02d\\%04d_%02d_%02d_%02d.txt", asRenesasCommPath.c_str(), SystemYear, SystemMonth, SystemDate, SystemYear, SystemMonth, SystemDate , SystemHour);
...
void TRENESAS_Server::DoProcRecvData(int iLen,char* cbuf)
{
    ...
    else if(asTempCMD==RENESAS_CMD.ProductInfoCMD())                              //10 //INPUTQTY //HTSET,702,LotCnt,LotID
    {
        ...
        asLogMsg.sprintf(   "CMD:%s\n"        ,RecvData.ProdInfo.CMD.c_str()           );
        asLogMsg.cat_printf("LotNo:%s\n"      ,RecvData.ProdInfo.LotNo.c_str()         );
        asLogMsg.cat_printf("LotID:%s\n"      ,RecvData.ProdInfo.LotID.c_str()         );
        asLogMsg.cat_printf("HdParameter:%s\n",RecvData.ProdInfo.HdParameter.c_str()   );
        asLogMsg.cat_printf("Product:%s\n"    ,RecvData.ProdInfo.Product.c_str()       );
        asLogMsg.cat_printf("StepCode:%s\n"   ,RecvData.ProdInfo.StepCode.c_str()      );
        asLogMsg.cat_printf("Qty:%s\n"        ,RecvData.ProdInfo.Qty.c_str()           );
        asLogMsg.cat_printf("Date:%s\n"       ,RecvData.ProdInfo.Date.c_str()          );
        asLogMsg.cat_printf("=======================================================\n");
        AppendToMemo(asLogMsg);
    }
    else if(asTempCMD==RENESAS_CMD.StartCMD())                                    //20((00|10|20))  //LOTCLEAR?+LOTORDER0/LOTORDER2/LOTORDER2  //HTSET,700+HTSET,703/HTSET,704/HTSET,704
    {
        ...
        asLogMsg.sprintf(   "CMD:%s\n"    ,RecvData.ModeStart.CMD.c_str()          );
        asLogMsg.cat_printf("Status:%s\n" ,RecvData.ModeStart.Status.c_str()       );
        asLogMsg.cat_printf("Date:%s\n"   ,RecvData.ModeStart.Date.c_str()         );
        asLogMsg.cat_printf("=======================================================\n");
        AppendToMemo(asLogMsg);
    }
    else if(asTempCMD==RENESAS_CMD.HandlerStartCMD())                             //50
    {
        ...
        asLogMsg.sprintf(   "CMD:%s\n"    ,RecvData.HandlerStart.CMD.c_str()   );
        asLogMsg.cat_printf("Date:%s\n"   ,RecvData.HandlerStart.Date.c_str()  );
        asLogMsg.cat_printf("=======================================================\n");
        AppendToMemo(asLogMsg);
    }
    else if(asTempCMD==RENESAS_CMD.EndCMD())                                      //30
    {
        ...
        asLogMsg.sprintf(   "CMD:%s\n"    ,RecvData.ModeEnd.CMD.c_str()    );
        asLogMsg.cat_printf("Status:%s\n" ,RecvData.ModeEnd.Status.c_str() );
        asLogMsg.cat_printf("Date:%s\n"   ,RecvData.ModeEnd.Date.c_str()   );
        asLogMsg.cat_printf("=======================================================\n");
        AppendToMemo(asLogMsg);
    }
    else if(asTempCMD==RENESAS_CMD.LotEndCMD())                                   //40
    {
        ...
        asLogMsg.sprintf(   "CMD:%s\n"    ,RecvData.LotEnd.CMD.c_str()     );
        asLogMsg.cat_printf("LECD:%s\n"   ,RecvData.LotEnd.LECD.c_str()    );
        asLogMsg.cat_printf("Date:%s\n"   ,RecvData.LotEnd.Date.c_str()    );
        asLogMsg.cat_printf("=======================================================\n");
        AppendToMemo(asLogMsg);
    }
    else if(asTempCMD==RENESAS_CMD.TestEndCMD())                                  //70
    {
        ...
        asLogMsg.sprintf(   "CMD:%s\n"    ,RecvData.TestEnd.CMD.c_str()     );
        asLogMsg.cat_printf("Date:%s\n"   ,RecvData.TestEnd.Date.c_str()    );
        asLogMsg.cat_printf("=======================================================\n");
        AppendToMemo(asLogMsg);
    }
    ...
    else if(asTempCMD==RENESAS_CMD.SendCleanOutCMD())
    {
        ...
        asLogMsg.sprintf(   "CMD:%s\n"        ,RecvData.SendCleanOut.CMD.c_str()           );
        asLogMsg.cat_printf("=======================================================\n");
        AppendToMemo(asLogMsg);
    }
}
            ...
            if(TestIF_File.bRENESAS_EnableFTCT==true)
            {
                ...
                if(HasICUnderMachine()==false && HasAnyICInMachine()==false)    //也要檢查 Auto tray
                {
                    ...
                    if(fMain->cbSetupFileName->Text!=RecvData.ProdInfo.HdParameter) //工作檔不一樣才切換
                    {
                        ...
                        {
                            // AI(W906-PT-W3) 20260807: golden `.AnsiCompareIC(...)==0` (case-
                            // insensitive compare, not in vclcompat/AnsiString.h) -- same-result
                            // substitution via already-real UpperCase(), see file banner "NOT
                            // gates" list.
                            if (AnsiString(fMain->cbSetupFileName->Items->Strings[i]).UpperCase()==RecvData.ProdInfo.HdParameter.UpperCase())  // 不分大小寫比較
                            {
                                flag=true;
                                break;
                            }
...
//            else
//            {
            fSCKART->sLotID.sprintf("%s", RecvData.ProdInfo.LotID.c_str());
            fSCKART->sProcessCode.sprintf("%s", RecvData.ProdInfo.StepCode.c_str());
            bRenFTCTManualStartTmp=false;
            fSCKART->iManualStart=false;
            fSCKART->AccessFile(false, 1);
//            }
//            LastSet.REN_iUseControlLoaderInputCount=fSCKART->iLotCount;
            ...
            sData3.sprintf("Lot start: %s, %d", fSCKART->sLotID.c_str(), fSCKART->iLotCount);
        ...
        case 30000:
        {
            ...
            if(TestIF_File.bRENESAS_EnableFTCT==true)
            {
                ...
                if(RecvData.ModeStart.Status=="00")
                {
                    ...
                    memset(iAutoTrayCount, 0, sizeof(iAutoTrayCount));          // AI(W906-PT-W3) 20260807: golden ZeroMemory(...) -- same-result substitution, see file banner "NOT gates" list
...
{
    AnsiString asMsg;
    asMsg.sprintf("%s%c%c",RENESAS_CMD.OnLineReplyCMD().c_str(),cLineEndData[0],cLineEndData[1]);
//    if(asOnLineErr==Err_Code.OK)
    {
        asMsg.cat_printf("Result:%s%c%c",asSendErrCode.c_str(),cLineEndData[0],cLineEndData[1]);
        asMsg.cat_printf("Comment:%s%c%c",asSendErrComm.c_str(),cLineEndData[0],cLineEndData[1]);
    }
//    else
//    {
...
AnsiString TRENESAS_Server::GetProductInfoMessge()
{
    ...
    asMsg.sprintf("%s%c%c",RENESAS_CMD.ProductInfoReplyCMD().c_str(),cLineEndData[0],cLineEndData[1]);
...
//    }
    asMsg.cat_printf("HdParameter:%s%c%c"   ,RecvData.ProdInfo.HdParameter.c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("Product:%s%c%c"       ,RecvData.ProdInfo.Product.c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("%c",0x0);
    if(asSendErrCode=="N0001")
...
AnsiString TRENESAS_Server::GetStartMessge()
{
    AnsiString asMsg;
    GetErrCode(iErrInt,asSendErrCode,asSendErrComm);
    asMsg.sprintf("%s%c%c",RENESAS_CMD.StartReplyCMD().c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("Status:%s%c%c",RecvData.ModeStart.Status.c_str(),cLineEndData[0],cLineEndData[1]); //Kirin 20191218 FTCT
    asMsg.cat_printf("Result:%s%c%c",asSendErrCode.c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("Comment:%s%c%c",asSendErrComm.c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("%c",0x0);
    if(asSendErrCode=="N0001")
...
AnsiString TRENESAS_Server::GetHandlerStartMessge()
{
    AnsiString asMsg;
    GetErrCode(iErrInt,asSendErrCode,asSendErrComm);
    asMsg.sprintf("%s%c%c",RENESAS_CMD.HandlerStartReplyCMD().c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("Result:%s%c%c",asSendErrCode.c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("Comment:%s%c%c",asSendErrComm.c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("%c",0x0);
    if(asSendErrCode=="N0001")
...
AnsiString TRENESAS_Server::GetEndMessge()
{
    ...
    // AI(W906-PT-W3) 20260807: golden uses Borland-specific `fnsplit()` (no
    // vclcompat equivalent) purely to strip directory+extension off
    // LastSet.LastOpenFilename into `cName`; every other fnsplit output
    // (cDrive/cDir/cExt) is computed and discarded (golden :1961-1982).
    // Same-result substitution via already-real ExtractFileName/
    // ExtractFileExt -- see file banner "NOT gates" list.
    {
        AnsiString asFileNameOnly=ExtractFileName(AnsiString(LastSet.LastOpenFilename));
        AnsiString asExt=ExtractFileExt(asFileNameOnly);
        asHdParameter=asFileNameOnly.SubString(1, asFileNameOnly.Length()-asExt.Length());
    }
    ...
    //<==
    //Kirin 20201023 (Marvin) 改用fnsplit分離檔名串資料，舊的註解掉
    asMsg.sprintf("%s%c%c",RENESAS_CMD.EndReplyCMD().c_str(),cLineEndData[0],cLineEndData[1]);
    ...
    asMsg.cat_printf("Result:%s%c%c",asSendErrCode.c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("Comment:%s%c%c",asSendErrComm.c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("TotalQty:%s%c%c",RecvData.ProdInfo.Qty.c_str(),cLineEndData[0],cLineEndData[1]);      //Kirin 20191218 (Marvin) 數量上報修改
...
//    asMsg.cat_printf("HdParameter:%s%c%c",asHdParameter.c_str(),cLineEndData[0],cLineEndData[1]);   //Kirin 20191218 (Marvin) FTCT上報修改
    asMsg.cat_printf("HdParameter:%s%c%c",fMain->cbSetupFileName->Text.c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("%c",0x0);
    if(asSendErrCode=="N0001")
...
{
    AnsiString asMsg;
    asMsg.sprintf("%s%c%c",RENESAS_CMD.LotEndReplyCMD().c_str(),cLineEndData[0],cLineEndData[1]);
...
AnsiString TRENESAS_Server::GetTestEnd()
{
    AnsiString asMsg;
    GetErrCode(iErrInt,asSendErrCode,asSendErrComm);
    asMsg.sprintf("%s%c%c",RENESAS_CMD.TestEndReplyCMD().c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("Result:%s%c%c"    ,asSendErrCode.c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("Comment:%s%c%c"   ,asSendErrComm.c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("%c",0x0);
    if(asSendErrCode=="N0001")
...
//    iErrInt=0; //Kirin 20220225
    GetErrCode(iErrInt,asSendErrCode,asSendErrComm);
    asMsg.sprintf("%s%c%c",RENESAS_CMD.SendAlarmReplyCMD().c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("Result:%s%c%c"    ,asSendErrCode.c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("Comment:%s%c%c"   ,asSendErrComm.c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("%c",0x0);
...
AnsiString TRENESAS_Server::GetSendCleanOut()
{
    AnsiString asMsg;
    GetErrCode(iErrInt,asSendErrCode,asSendErrComm);
    asMsg.sprintf("%s%c%c",RENESAS_CMD.SendCleanOutReplyCMD().c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("Result:%s%c%c"    ,asSendErrCode.c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("Comment:%s%c%c"   ,asSendErrComm.c_str(),cLineEndData[0],cLineEndData[1]);
    asMsg.cat_printf("%c",0x0);
    if(asSendErrCode=="N0001")
...
//---------------------------------------------------------------------------
void TRENESAS_Server::OnProcTimer(TObject *Sender)
{
    DoProcOnShow();
    DoProcess();
...
//    }
    ...
    asFileName.sprintf("%s_%s_", fSCKART->sLotID.c_str(), asTempSendModeStart.c_str()); //Kirin 20200110 改成用asTempSendModeStart來儲存
    asFileName.cat_printf("%s.csv", fSCKART->sLotStartTime.c_str());
    ...
    asFileName.sprintf("%s_%s_", fSCKART->sLotID.c_str(), asTempSendModeStart.c_str()); //Kirin 20191219 FTCT
    asFileName.cat_printf("%s.csv", fSCKART->sLotStartTime.c_str());
    asFileName=asFTCTModeEndFilePath+asFileName;
    SaveFilePtr=fopen(asFileName.c_str(), "wb+");
    fwrite(asData.c_str(),asData.Length(),1,SaveFilePtr);
...
//===========================================================================
void TRENESAS_Server::SaveTaskLog(const char *Task_Name,int iTask)
{
    ...
    asStr.printf("%s: %s , Task:%d",FormatDateTime("yyyy/mm/dd hh:nn:ss", tTime).c_str(),Task_Name,iTask);

    if (!DirectoryExists(asRenesasTaskPath.c_str()))
    {
        CreateDir(asRenesasTaskPath); // AI(W906-PT-W3) 20260807: golden CreateDirectory(...,NULL) -- same-result substitution, see file banner "NOT gates" list
    }

    asDirectory.sprintf("%s\\Task",asRenesasTaskPath.c_str());

    if (!DirectoryExists(asDirectory.c_str()))
    {
        CreateDir(asDirectory);
    }
...
bool TRENESAS_Server::DoNeedSupplyOrNot(bool bNotEnough)
{
            ...
            (bOverPick==true || bNotEnough==true))  //還有IC，或是不足
    {
        ...
        if(bOverPick==true)
        {
            ...
            // [G5]: golden `ret=ShowLotEndMessage(str, str2, false);` -- see
            // file banner GATE [G5] (mymessbox.h/.cpp not translated
            // anywhere in the port tree; `ret` faithfully keeps its -1
            // default through this gate).
#if 0
        ...
        if(bNotEnough==true)
        {
            // [G5]: golden `ret=ShowLotEndMessage("The number of receipts and
            // the number of Lots(QTY) do not match.\r\n Want to supply
            // untested products?", "", false);` -- see file banner GATE [G5].
#if 0
...
//---------------------------------------------------------------------------
// [G1]: golden `__fastcall TErrorForm::TErrorForm(TComponent* Owner):TForm(Owner, 0)`
// -- the `:TForm(Owner,0)` base-init and every property assignment below
// (Caption/BorderStyle/Position/Width/Height/Color/FormStyle/BorderIcons on
// `this`, and Parent/AutoSize/Alignment/Layout/SetBounds/WordWrap/Font-
// >Size/Font->Color/Transparent on the child TLabel, and Parent/Caption/
// SetBounds/OnClick on the child TButton) are pure VCL layout/paint --
// see file banner GATE [G1] for why this whole block is gated as one unit
// (no window is ever painted in this headless build) and why lblCaption
// (SetMessage's target) and the OnClick wiring are NOT gated (both remain
// real, observable behaviour).
TErrorForm::TErrorForm(TObject* Owner) : Visible(false)
{
#if 0
...
//---------------------------------------------------------------------------
void TErrorForm::BtnCloseClick(TObject* Sender)
{
    this->Hide(); // 隱藏而非釋放
...
{
    lblCaption="Return Error TO FTCT : "+str1+"\r\nCode : "+str2;
}
//---------------------------------------------------------------------------
TErrorForm* TRENESAS_Server::frm = NULL;
void TRENESAS_Server::ShowErrorMessageByReplyError(AnsiString sSendErrCode, AnsiString sSendErrComm)
{
    if(!frm)
        frm=new TErrorForm(NULL); // 第一次呼叫才建立
...
//---------------------------------------------------------------------------
// [G1]: same gated-layout-block idiom as TErrorForm::TErrorForm above --
// see that ctor's comment and file banner GATE [G1].
TContinueForm::TContinueForm(TObject* Owner)      //RogerYang 20251107 : Add Continue Form
    : lblFontColor(0), Visible(false)
{
#if 0
...
//---------------------------------------------------------------------------
void TContinueForm::BtnCloseClick(TObject* Sender)                   //RogerYang 20251107 : Add Continue Form
{
    this->Hide(); // 隱藏而非釋放
...
void TRENESAS_Server::ShowContinueMessageByReply71(bool bAlarm)
{
    if(!contfrm)
        contfrm=new TContinueForm(NULL); // 第一次呼叫才建立

    AnsiString Str="";
    if(bAlarm==true)
    {
        contfrm->lblFontColor=1; // clRed -- see [G1]: TColor constants (clRed/clBlue) are
                                 // pure paint state with no vclcompat equivalent; the two
                                 // distinct values are preserved as a 0/1 discriminator
                                 // (this->lblFontColor) since nothing reads the literal
                                 // TColor value, only the two-way distinction golden makes.
    ...
    {
        contfrm->lblFontColor=2; // clBlue
        Str="Waiting After Reply 71 Command";
    }
    contfrm->lblCaption=Str;
