// =============================================================================
//  Automation/auto9045.h  --  OLP/GPIB host-command config accessor layer
//  (Get*/Set* pairs consumed by the OLP/RS232/GPIB command dispatcher) + a
//  handful of small ancillary functions (ATK UDP site/temp encoders, ASE-KR
//  bin-result encoders, OneCycle/HomeAndStart triggers, system-state gates).
//
//  Translation wave: W5-Final-Auto9045
//  Translator: AI(W5-Final-Auto9045) 20260711
//  Golden source: HT9011UC_Code_V3.33.906.0_20260618/Automation/auto9045.h
//                 (190 lines, cp950).
//
//  Declarations are VERBATIM copies of the golden signatures (names, types,
//  default args, order) -- confirmed via a prior recon that auto9045.h/.cpp is
//  a plain free-function module (NOT a VCL TForm), so every declaration below
//  translates as a plain free function with zero class-membership change.
//  See auto9045.cpp's file-head comment for the full scope note, the TU-local
//  gated-dependency list, and the FormsFacade/atester_shims extension
//  candidates this unit's body needed but could not add itself (hard-boundary
//  files this wave may not edit).
//
...
//AI(ht9045-v899) 20260515: return Greatek OEE start result through LotInfo_REPLY without adding a new command
