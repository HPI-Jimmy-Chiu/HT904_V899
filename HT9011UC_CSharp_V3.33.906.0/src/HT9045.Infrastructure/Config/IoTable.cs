...
{
    /// <summary>
    /// Loads IO_Table.csv into a list of <see cref="IoTableRow"/>.
    ///
    /// Rules mirroring database.cpp LoadIoData behaviour:
    ...
    ///  - File is Big5-encoded (cp950); read via Encoding.GetEncoding(950).
    ///  - Split on comma; trailing empty fields are tolerated (short row is OK).
    ///  - Column order is positional and MUST match IoTableRow.FromFields index.
    ///  - ISABase (col 8) is parsed to int and maps to eIOType enum values.
    /// </summary>
    public static class IoTable
    {
        ...
        /// </summary>
        public static List<IoTableRow> Load(string path)
        {
            if (path == null) throw new ArgumentNullException("path");

            var result = new List<IoTableRow>();
            ...
            foreach (string rawLine in text.Split('\n'))
            {
                ...
                result.Add(IoTableRow.FromNamedRow(header, fields));
            }
