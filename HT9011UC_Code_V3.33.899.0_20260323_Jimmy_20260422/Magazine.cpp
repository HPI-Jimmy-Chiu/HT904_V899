...
//}
//------------------------------------------------------------------------------
void AddLoadingCount_Buffer(int iPosRow, int iPosCol, int iTrayRow, int iTrayCol, int WhichAuto)
{
                       ...
                       OutArmSuck.Item[i][j]==NULL_IC)
                    {
                           ...
                           OutArmSuck.Suck[i][j].Suck())                        //wei 20150318 suck 判斷是否動作
                        {
                            ...
                            if(fAGV->IsATK_AMR() &&
                               LastSet.iUnloadFixTray==eAtkTfMoveFixIC)         //Steven 20260202 : for ATK AMR
                            {
                                AddLoadingCount_Buffer(i, j, iTrayRow, iOutArmXPosition[i][j], iPickWhichBuff);
                                OutArmSuck.iWhichAuto[i][j]=iFixToWhichAuto-eFix1;
                            }
                            else
                            {
                                AddLoadingCount_Buffer(i, j, iTrayRow, iTrayCol[j], iPickWhichBuff);
                            }
...
//    bool C_Auto3_EdgePush=false;//kevin 20140603
    ...
    switch(Task)
    {
        case 1:
            ...
            if(LastSet.iRealDummy!=DUMMY && Sen[SnAuto3EdgePush].IsOn()==true)
            {
