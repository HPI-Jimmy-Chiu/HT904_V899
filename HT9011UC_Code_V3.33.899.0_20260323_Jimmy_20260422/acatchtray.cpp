...
bool C_CatchTray_Fix_Puch(bool bInitial)                                        //ChungHung 20140624 add AutoRetest catch Tray use 2 Output
{
    ...
    switch(iTask)
    {
        ...
        case 100:
            Cylinder[C_CatchTray_FixOff].Off();
            //AI(ht9045-v899) 20260901: 還原20260703改用On()立即回報成功的寫法,不等到位sensor會使夾緊後只剩0.5秒就進case2350判定,且夾不緊時不再跳JAM31047 (CASE-20260901-001)
            if(bflag==false)
    ...
    if(LastSet.iRealDummy!=DUMMY && IniConfig.bC03UseCatchTray)
    {
        switch(USE_CATCH_TRAY_MODEL)
        {
            ...
            case 4:                                                             //2 Sensor(ART & No Cover)  //Steven 20241211 : add
                //AI(ht9045-v899) 20260901: 還原20260703 Greatek極性反轉,新舊判定互斥(舊:FixOn到位=成功;新:兩顆到位皆OFF才算成功),HHT604G為原廠正常極性故夾盤成功反被判fail,JAM0604 100%誤報 (CASE-20260901-001)
