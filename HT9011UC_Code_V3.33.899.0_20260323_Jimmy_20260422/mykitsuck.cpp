...
//=====================================
_fastcall TMyKitSuck::TMyKitSuck()
{
...
//----------------------------------------------------------------------------
//bool TMyKitSuck::HasDefineICFor12Site(int IC_TYPE)                              //JerryYang 20160616 : 12site只判斷所使用的site
//{
//    bool bHasDefineIC=false;
//    for(int i=0; i<iMaxRow; i++)
//    {
...
//}
//----------------------------------------------------------------------------
//bool TMyKitSuck::HasDefineICFor10Site(int IC_TYPE)                            //JerryYang 20160616 : 12site只判斷所使用的site
//{
//    bool bHasDefineIC=false;
//    for(int i=0; i<iMaxRow; i++)
//    {
...
//----------------------------------------------------------------------------
//int TMyKitSuck::CountReal_Has_NUll_IC()                                         //kevin 20140903 判斷是否在第二次放IC因關SITE 造成無法清除  IN ARM HAS_NULL_IC
//{
//    int iNullICCount=0;
//    int iOtherCount=0;
//    for(int i=0; i<iMaxRow; i++)
...
//----------------------------------------------------------------------------
/*bool TMyKitSuck::HasNoScanIC()
{
    for(int i=0; i<iMaxRow; i++)
    {
        for(int j=0; j<iMaxCol; j++)
        {
            if(Item[i][j]!=NULL_IC && bScan[i][j]==false)
                return true;
        }
    }
    return false;
...
//------------------------------------------------------------------------------
//bool TMyKitSuck::ArmLeftSideNoIC(int MiddleValue)
//{
//    for(int i=0; i<iMaxRow; i++)
//    {
//        for(int j=0; j<MiddleValue; j++)
//        {
//            //jou 980406 place to shuttle hang
//            if(Item[i][j*2]!=NULL_IC)
//                return false;
//        }
//    }
//    return true;
...
//------------------------------------------------------------------------------
//bool TMyKitSuck::ArmRightSideNoIC(int MiddleValue)
//{
//    for(int i=0; i<iMaxRow; i++)
//    {
//        for(int j=0; j<MiddleValue; j++)
...
//------------------------------------------------------------------------------
//bool TMyKitSuck::ArmUpSideNullIC(int iMax, int iCol)                            //kevin 20201006 add 12 site
//{
//    for(int j=0; j<iMax; j++)
//    {
//        if(Item[0][j+iCol]!=NULL_IC)
//            return false;
//    }
//    return true;
//}
//------------------------------------------------------------------------------
//bool TMyKitSuck::ArmDownSideNullIC(int iMax, int iCol)                          //kevin 20201006 add 12 site
//{
//    for(int j=0; j<iMax; j++)
//    {
//        if(Item[1][j+iCol]!=NULL_IC)
//            return false;
//    }
//    return true;
//}
...
//------------------------------------------------------------------------------
//void TMyKitSuck::ClearSingle(int i, int j)
//{
//    Item[i][j]=NULL_IC;
//    SetItemData(i, j, NULL_IC);
//    iWhichSite[i][j]    =-1;
...
//                          22
void TMyKitSuck::MergeSuck(class TMyKitSuck &Source1, class TMyKitSuck &Source2, bool InsertCol=false)
{
...
//------------------------------------------------------------------------------
//2013-01-15    Dell    Add nn Mode
void TMyKitSuck::SplitSuck(class TMyKitSuck &Source1, class TMyKitSuck &Source2, bool InsertCol=false)
{
    TMyKitSuck *tmSource[]={&Source1, &Source2};
    int MinRow, MinCol;
...
void TMySucker::OnSuck()                                                        // no delay,no alarm
{
    if(LastSet.iRealDummy==DUMMY)
        return;
    if(LastSet.iRealDummy==HAS_TRAY && SuckerName.AnsiCompare("CatchSuck")!=0)
        return;
...
void TMySucker::On()                                                            // no delay,no alarm
{
    ...
    if(bLoadInarmAutoHigh==false && LastSet.iRealDummy==HAS_TRAY && SuckerName.AnsiCompare("CatchSuck")!=0)
...
//    iNozzleEvent=1;                                                           //Klutter 20210817 修改屬性方便修正吸嘴回home異常事件
    ...
    if(LastSet.iRealDummy==DUMMY ||
      ...
      (LastSet.iRealDummy==HAS_TRAY && SuckerName.AnsiCompare("CatchSuck")!=0))
    {
        switch(OnTask)
        {
            case 1:
...
//------------------------------------------------------------------------------
/*void TMySucker::ReCheck()
{
    if(Enable==false)
        return;
    bool Flag=false;
    if(ISABase==eMotionNet)                                                     //Nickliu 20230310 add Suck ISABase Type
    {
        Flag=MyLaneIO.IOInputBit(SenRing, SenIP, SenPort, SenBit, SensorName);
        if(Enable)
            Status=Flag;
        else
            Status=MyLaneIO.IOOutBitStatus(OnRing, OnIP, OnPort, OnBit, OnPortName);
    }
    else
    {
        Flag=IOInputBit(SenPort, SenBit);                                       //Nickliu 20230310 add Suck ISABase Type
        if(Enable)
            Status=Flag;
        else
            Status=IOOutBitStatus(OnPort, OnBit);
    }                                                                           //Nickliu 20230310 add Suck ISABase Type
}*/
//------------------------------------------------------------------------------
