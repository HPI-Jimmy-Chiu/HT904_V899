...
#include "BarCode.h"
#include "Magazine.h"
//#include "OCR.h"                                                                //KenHsieh 20230406 : 新增OCR Data + Bin Log功能
#include "TesterTCP.h"                                                          //RogerYang 20260210 : 田揚志需求，有設定的才可以一次放全部，OS報表要照Tray盤放料順序顯示
#include "AGV.h"
//---------------------------------------------------------------------------
...
//    if(MachineTypeChoice==Type_HT9046_LS &&                                     //Steven 20211005 : 針對46LS且大IC於Fix換盤讓位要換位置
//       CUSTOMER_CODE==CC_AMKOR_Korea)                                         //Steven 20250926 : Mark
        iXPos=MOT[MOutArmX].Motor->PSoftLimitN+2000;
//    else
//        iXPos=Prod.XOutArm_Shuttle1_Pick[iOutArmYBase][iOutArmXBase];

    if(CUSTOMER_CODE==CC_ASE_SG && bMoveY==true)                             //Ifor 20250926 add:Fix Tray 取放時移開Out Arm
    {
        iYPos=Prod.iOutArmSafeY-15000;
        iXPos=Tech.iOutArmAuto2X;
    }
    else
    {
        if(CUSTOMER_CODE==CC_ASE_SG)                                         //Ifor 20260115 fix 換Tray Suck Aa IC 撞到OutShuttle Z Sensor
            iXPos=Tech.iOutArmAuto2X;
...
bool IsCatchTrayReadySupplyNewTray()
{
       ...
       bAtuoTrayICDetectErr==false)                                             //Isaac 20180109 (Steven) : auto123可前進後退
    {
        ...
        if(MOT[MOutArmY].CompareCommandPos(Prod.iOutArmSafeY, 2)==1 &&          //Steven 20240110 : 避免比對異常
           iPosX==-35000)
        {
            iCatchTrayControlManual=1;
            return true;
...
void SetFixTrayMiddleDtata()
{
    ...
    if((USE_Scanner_AOI_Inspection==true && ScannerAOIIF.iEnableScannerMode!=0) ||      //Ifor 20200108 add: Use Scan AOI 需自動跳過破孔位置放IC
...
//------------------------------------------------------------------------------
void SetOutArmNeedDestory(int iSuckRow, int iSuckCol, int iShtRow, int iShtCol)
{
