      ...
      int ARow, TRect &Rect, TGridDrawState State)
{
    ...
    else if(ACol==0)
    {
        ...
        str.sprintf("%c%c", 'A'+(int)(ChangeToFloatNonPcnt((double)((ARow-1)), (double)(TestSocket.iShtCol))), 'a'+((ARow-1)%TestSocket.iShtCol));   //AI(ht9045-v899) 20260515: 修正南茂回報Yield Chart左欄site標籤(Aa/Ab/Ba...)空白; ChangeToFloatNonPcnt回傳float, 'A'+float 走%c變數列引數會被當成double傳入導致印出亂碼/空白, 補強制轉int還原V897行為
        MyDrawText(pCanvas, Rect, str.c_str(), clBtnFace);
    }
    else if(ACol==1)
    {
    ...
    else if(ACol==2)
    {
        ...
        if(IsNNMode()==NN_2Row)
        {
            str.sprintf("%c%c", 'C'+(int)(ChangeToFloatNonPcnt((double)((ARow-1)), (double)(TestSocket.iShtCol))), 'a'+((ARow-1)%TestSocket.iShtCol));   //AI(ht9045-v899) 20260515: 同上修正, NN_2Row site標籤強制int
            MyDrawText(pCanvas, Rect, str.c_str(), clBtnFace);
        }
        else if(IsNNMode()==NN_1Row)
        {
            str.sprintf("%c%c", 'B'+(int)(ChangeToFloatNonPcnt((double)((ARow-1)), (double)(TestSocket.iShtCol))), 'a'+((ARow-1)%TestSocket.iShtCol));   //AI(ht9045-v899) 20260515: 同上修正, NN_1Row site標籤強制int
            MyDrawText(pCanvas, Rect, str.c_str(), clBtnFace);
        }
        else
        {
