...
//    SummaryHead->Add(str.sprintf("%-34s ",    AnsiString("LoadBoard ID:")));
    ...
    SummaryHead->Add(str.sprintf("%-34s %d",  AnsiString("Reject:"),     TastCategory.iRejectCount));
    ...
    if(bViewOnly==false)
    {
//        if(IniConfig.bB05_OSReport)                                           //RogerYang 20260210 : YZ said the summary no need to upload
...
void __fastcall TfTesterTCP::ProcessOSTrayData(bool bViewOnly)
{
    ...
    for(int iAuto=0; iAuto<eTrayCount; iAuto++)
    {
        ...
        if(Prod.iTrayType[iAuto]!=tNotUse)
        {
            ...
            if(CheckFileExist(SourceFile))
            {
                ...
                if(RichEdit[iAuto]->Lines->Count>0)
                {
                    for(int i=0; i<SummaryHead->Count; i++)
                    ...
                    if(bViewOnly==false)
                    {
                        if(IniConfig.bB05_OSReport && iAuto!=0)                 //RogerYang 20260210 : YZ said the Auto1 data no need to upload
                        {
    ...
    if(bViewOnly==false)
    {
//        if(IniConfig.bB05_OSReport)                                           //RogerYang 20260210 : YZ said the All Tray Data no need to upload
