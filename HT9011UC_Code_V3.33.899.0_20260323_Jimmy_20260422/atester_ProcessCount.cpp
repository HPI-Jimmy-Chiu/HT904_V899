        ...
        else
        {
               ...
               (TestIF_File.iAutoClean_Mode&M_SOCKET_ALARM))                    //ChungHung 20131225 add
            {
                ...
                if(TestIF.iAutoClean_Function &&
                   ...
                   (TestIF_File.iAutoClean_Mode & M_INTERVAL) &&
                   iCleanOut==0 &&
                   iAutoClean_IndexContactCount>=(IntervalCount-1))
                {
                    if(CosFunction.bSmartAutoClean &&
                       TestIF.bACSmart &&
...
void ProcessCount(int Index)
{
    ...
    if(TestIF_File.iShuttleMode==0 ||
      (TestIF_File.iShuttleMode==1 && TestIF_File.iShuttle_Sel==Index))         //Ifor 20220419 add:Auto Clean 關Arm不累加Interval Count
    {
