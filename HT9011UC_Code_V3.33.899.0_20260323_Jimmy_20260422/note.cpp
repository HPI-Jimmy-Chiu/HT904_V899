...
int ShowErrorMessage(AnsiString Code, int KCode, int Pos, bool bDuplicateErr, AnsiString errPart)   //Steven 20091231 顯示整合的錯誤訊息
{
    ...
    if(Code.Length()==8)
    {
        ...
        {
            if(CUSTOMER_CODE==CC_KYEC_LEE &&
               CosFunction.bHiSiliconFunction==true)                            //Ifor 20181219 : add KYEC 要求溫度相關Alarm 測試中需先Index 上升才Alarm
            {
...
//    fNote->TempCode=Code;                                                       //JerryYang 20250311 : 移到下面,避免同時多個ALARM的時候TempCode被刷掉  //ChungHung 20111230 add CleanOut no music
    ...
    //AI(ht9045-v899) 20260407: only Pause when Note closed via BtnPauseClick (SoftStop==true), skip when closed via Start to prevent false MES2111
    if(fNote->ReturnCode==K_SKIP && SoftStop==true)
...
//    if(XResolution>1300)
    ...
    if((edErrorCode->Text=="MES0923" && TrayForm.iManualRemoveLoader==2) ||     //Steven 20130819 : 加入開門檢查的功能
       (edErrorCode->Text=="MES0101" && TrayForm.iManualRemoveLoader==2 && CosFunction.bPickupErrorAtLoaderNeedOpenDoor) ||  //JerryYang 20160220 add for Amkor-Philippine,In ARM至Loader吸取異常要開門確認後才能繼續
       (edErrorCode->Text=="MES0923" && CosFunction.bOpenDoorCheckLoaderAfterTrayEnd))                                       //Jimmychiu 20250826 : Open Door Check Loader After TrayEnd
    {
        ...
        lblCheckLoadEN->Caption="Please check if there has any device on loader tray";
        lblCheckLoadCH->Caption="請確認入料盤上是否有產品";
...
//            edtCheckBin->Text="";
            ...
            if(IniConfig.bA65_BundleIDList &&
               IniConfig.bP53_ForcedScanBinCodeOfUnloader)                      //JerryYang 20240821 : 自動讀取BIN LABEL
            {
...
void __fastcall TfNote::FormClose(TObject *Sender, TCloseAction &Action)
{
    ...
    {
        bool bIsJam=CheckRecordJamType(edErrorCode->Text, true);
        HGem->ReportAlarm(edErrorCode->Text, bIsJam, iDuplicateError, ShowMessageEdit1->Text, true);//kevin 20170905 (Steven) 換成Edit 可以看後面字串
    }
...
void __fastcall TfNote::Timer1Timer(TObject *Sender)
{
    ...
    if(Enable_PLCSafety_IO && bPLCFlag)                                         //KenHsieh 20250307 : fix PLC safedoor 通訊延遲問題
    {
