...
#include "MyMotor.h"
#include "database.h"
#include "ProductionInfo.h"                                                     //AI(BVL-3766) 20260420: 開關 site 時要即時重寫 GTK info.txt
#include "cMyDB.h"
#include "Password.h"
...
void __fastcall TFormHS::RecordEPLog_HS(AnsiString aFileName)
{
    ...
    if(INSTALL_DOUBLE_EP==DOUBLE_EP_NORMAL || INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)                            //AI(ht9045-v899) 20260504: was ==true; restored V874 ==1||==3 to enable Dual EP read for Multi EP.
    {
    ...
    if(bEnable_KLT_Function==true)                                              //Steven 20220610 : KLT羅昱良要求改用By SITE FORCE
    {
        ...
        if(INSTALL_DOUBLE_EP==DOUBLE_EP_NORMAL || INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)                        //AI(ht9045-v899) 20260504: was ==true; restored V874 ==1||==3.
        {
...
//       CUSTOMER_CODE==CC_KYEC_LEE)                                            //Ifor 20190905 :add 海思 V02.30 版  上傳Server資料增加Lot ID欄位
    ...
    if(CUSTOMER_CODE==CC_KYEC_LEE)                                              //AI(ht9045-v899) 20260504: port from V896 (Ifor 20250417). KYEC-only per-site EP log append.
       ...
       fContact->fShow==false)                                                  //JerryYang 20190115 (Steven) 避免EP check誤發alarm
    {
        if(LastSet.iRealDummy==REALLY)                                          //JerryYang 20181129 (Steven) : REAL模式才發alarm
        {
            ...
            {
                if(INSTALL_DOUBLE_EP==DOUBLE_EP_NORMAL || INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)                //AI(ht9045-v899) 20260504: was ==true; restored V874 ==1||==3 for Multi EP DualAlarm.
                {
                    if(ADAM_DualAlarm(iType))
                    {
...
bool __fastcall TFormHS::ReadMultiEP()
{
    ...
    if(IsMultiEPPressureRouteActive()==true)                                    //AI(ht9045-v899) 20260526: do not read/log Multi EP data while common EP route is selected.
    {
//      ADAMTCP_ReadReg(ip, 5, 1, 24, buffer);                                  //V896 left commented; enable after hardware verify.
        {
            for(int i=0; i<16; i++)
            {
                ibuffer[i]=buffer[i];
                ibuffer[i]=ibuffer[i]/16;
                iKpaTransferKG[i]=MultiTransferKG(ibuffer[i], iMapping[i])/4;
            }
