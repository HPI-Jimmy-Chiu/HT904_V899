...
bool Open_ADAM_6024()                                                           //Jimmychiu 20230804 : 整合全部連線檢查
{
    ...
    if(INSTALL_DOUBLE_EP==DOUBLE_EP_INDIVIAL || INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)                            //AI(ht9045-v899) 20260430: Multi EP board (Address[2]=172.16.8.112, physical 2nd board) for reg=1/33.
...
int ADAM_ReadPA(double *dValue, int iCH)                                        //Ifor 20190104 : add   //wei 20220309 Add EP Return Voltage
{
    ...
    *dValue=v;                                                                  //wei 20220309 Add EP Return Voltage
    ...
    int PA=EP_MINMPA*1000.0+(EP_MAXKPA-EP_MINMPA*1000.0)*((v-EP_MinAFB)/(EP_MAXAFB-EP_MinAFB));
    if(iCH==2)                                                                  //kevin 20200325 add dual force data
        PA=EPDual_MINMPA*1000.0+(EPDual_MAXKPA-EPDual_MINMPA*1000.0)*((v-EPDual_MinAFB)/(EPDual_MAXAFB-EPDual_MinAFB));
    return PA;
}
//---------------------------------------------------------------------------
...
double KpaTransferKG(int fInputMPA, bool bDualForce)                            //Ifor 20150826 :新增 Kpa 轉 公斤 Function  //JerryYang 20171023 (wei) int -> double
{
    ...
    if(bDualForce==true && (INSTALL_DOUBLE_EP==DOUBLE_EP_INDIVIAL || INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI))      //Ifor 20191003 : add Die Force 可以自定義Kit直徑 //AI(ht9045-v899) 20260430: extend to Multi EP half.
    {
        if(fDiameter==6.0)
        {
    ...
    else
    {
        ...
        else
        {
            ...
            {
                if(INSTALL_DOUBLE_EP==DOUBLE_EP_NONE)                                        //JerryYang 20210119 : 增加dual EP防呆
                {
...
double MultiTransferKG(int fInputMPA, bool bDualForce)
{
            ...
            if(bDualForce==false)
            {
                switch(TestIF.iTestMode)
                {
                    ...
                    case _16Site2X8: //2x8
                        ...
                        fInputKG=fInputKG/(fContact->dDutCount*fComplianceUnit);//JerryYang 20221216 : fix 16 site只開中間8site的時候氣量只剩下一半
                        break;
...
//        }
        ...
        //AI(ht9045-v899) 20260511: 把舊 Independent EP loop 限制在 DOUBLE_EP_INDIVIAL 才跑。
        //   原因：DOUBLE_EP_MULTI 時 slSLKTypeInd->Count 可能 >=2，但 SLKIndClass 實際只有 16 筆，
        //         i==1 時 SLKIndClass[16+j] 越界 → 隨機記憶體偶然 match diameter → iAPAXEPValue[]
        //         與下游 iAPAXDualEPValue[] 被 overwrite。
        //   改為兩段互斥：INDIVIAL 走這段；MULTI 走下方新 block。並加 size bound 防呆。
        if(INSTALL_DOUBLE_EP==DOUBLE_EP_INDIVIAL)
        {
            int k=0;
            int iSLKIndSize=(int)fContactForce->SLKIndClass.size();             //AI(ht9045-v899) 20260511: bounds guard
            for(int i=0; i<fContactForce->slSLKTypeInd->Count; i++)             //JerryYang 20210413 : 讀取獨立EP offset
            {
                for(int j=0; j<16; j++)
                {
                    int idx=i*16+j;
                    if(idx>=iSLKIndSize)                                        //AI(ht9045-v899) 20260511: 防止 SLKIndClass 與 slSLKTypeInd 不同步時越界
                        break;
                    if(fContactForce->SLKIndClass[idx]->dDiameter==fDiameter*10)
                    {
                        fInputKGInd=fInputKG;
                        fLoadRateInd=fContactForce->SLKIndClass[idx]->dLoadRate;
                        if((fInputKGInd-fContactForce->SLKIndClass[idx]->dContactOffset)>0)
                        {
                            fInputKGIndRel=fInputKGInd-fContactForce->SLKIndClass[idx]->dContactOffset;
                        }
                        ...
                        iResultTemp=((fMaxUnit-fMinUnit)/(fMaxMPA-fMinMPA))*(fInputMPAInd-fMinMPA)+fMinUnit;    //Steven 20110312
        ...
        //AI(ht9045-v899) 20260526: in Multi EP production, normal air force fills outer/site EP values.
        if(INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI && bDualForce==false)
        {
            int kMep=0;
            ...
            for(int iMep=0; iMep<fContactForce->slSLKTypeInd->Count; iMep++)
            {
                for(int jMep=0; jMep<8; jMep++)
                {
                    ...
                    if(dDiameterMep==fDiameter*10)
                    {
                        ...
                        int iResultTempMep = ((fMaxUnit-fMinUnit)/(fMaxMPA-fMinMPA))*(fInputMPAIndMep-fMinMPA)+fMinUnit;
        ...
        //AI(ht9045-v899) 20260526: in Multi EP production, DoubleForce fills inner/dual EP values.
        if(INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI && bDualForce==true)
        {
            double fDieDiameterMep = DeviceForm_File.dDieForceKitDiameter;
            int kMep=0;
            int iDieForceSizeMep=(int)fContactForce->DieForceOneByOneSLKClass.size();
            for(int iMep=0; iMep<fContactForce->slDieForceOneByOneSLKType->Count; iMep++)
            {
                for(int jMep=0; jMep<8; jMep++)
                {
                    int idxMep=iMep*8+jMep;
                    if(idxMep>=iDieForceSizeMep)
                        break;
                    if(fContactForce->DieForceOneByOneSLKClass[idxMep]->dDiameter==fDieDiameterMep*10)
                    {
                        double fInputKGIndMep = fInputKG;
                        double fLoadRateIndMep = fContactForce->DieForceOneByOneSLKClass[idxMep]->dLoadRate;
                        double fOffsetMep = fContactForce->DieForceOneByOneSLKClass[idxMep]->dContactOffset;
                        double fInputKGIndRelMep = fInputKGIndMep;
                        if((fInputKGIndMep - fOffsetMep) > 0)
                            fInputKGIndRelMep = fInputKGIndMep - fOffsetMep;
                        double dTempIndMep = (fDieDiameterMep*fDieDiameterMep*M_PI/4.0*fLoadRateIndMep);
                        double fInputFIndMep = (dTempIndMep!=0) ? (fInputKGIndRelMep/dTempIndMep) : fMinMPA;
                        double fInputMPAIndMep = fInputFIndMep/10.197;
                        int iResultTempMep = ((fMaxUnit-fMinUnit)/(fMaxMPA-fMinMPA))*(fInputMPAIndMep-fMinMPA)+fMinUnit;
                        iResultTempMep = CheckRange(iResultTempMep, 0, int(fMaxUnit));
                        iResultTempMep = iResultTempMep*16;
                        if(kMep<8) iAPAXDualEPValue[kMep] = iResultTempMep;
                        kMep++;
                    }
                }
            }
        ...
        iResult=((fMaxUnit-fMinUnit)/(fMaxMPA-fMinMPA))*(fInputMPA-fMinMPA)+fMinUnit;    //Steven 20110312
    }
...
void APAX_WriteData(bool bDir, WORD wdata, int iArm)
{
    ...
    if(bDir==true)
    {
        if(TestIF_File.iTestMode==DualSite)
        {
            ...
            //AI(ht9045-v899) 20260430: removed iIndEPCnt==8 branch (rolled back).
        }
        else
        {
            if(iIndEPCnt==4)                                                    //JerryYang 20210413 : Add 4組獨立EP版本
    ...
    if(iArm==0 || iArm==2 || iIndEPCnt==4)                                      //JerryYang 20210413 : Add 4組獨立EP版本
    {
...
//                bConnectStatus=false;
    ...
    if(iIndEPCnt==16)                                                           //AI(ht9045-v899) 20260430: rolled back iIndEPCnt==8 (use INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI instead).
    {
        for(i=0; i<16; i++)
        {
            wData[i]=0;
