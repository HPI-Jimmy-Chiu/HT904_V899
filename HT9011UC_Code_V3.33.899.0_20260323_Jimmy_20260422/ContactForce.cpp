...
{
    //AI(ht9045-v899) 20260504: full port from V874.3 Ifor (撤回 20260430 砍半版); iCount=(Tag%8)+1, 1..4=Arm1, 5..8=Arm2. SPEC-V899-MultiEP-FullPort, ADR-0004.
    AnsiString Str1, Str2;
    sDiameter   =Dia;
    iTag        =Tag;
    bShow       =bDefault;
    ...
    if(INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)
    {
        //AI(ht9045-v899) 20260504: full port V874 (Tag%8): 1..4=Arm1, 5..8=Arm2.
        if(iCount<5)
    ...
    if(INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)
    {
        if(iCount<3)
            ...
            Str2.sprintf("%s mm contact offset 2_%d:", Dia, iCount-2);
    ...
    : TForm(Owner)
{
    ...
    if(CosFunction.bUseDynamicKitDiameter)                                      //Steven 20170605 : 可以自定義Kit直徑
    {
        ...
        //AI(ht9045-v899) 20260526: align EP one-by-one site trim diameters with V874/general SLK types instead of legacy 20,30 only.
        edtCurrentTypeInd->Text=edtCurrentType->Text;
        edtVisibleInd->Text    =edtVisible->Text;
        ...
        //AI(ht9045-v899) 20260504: full port V874.3 INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI SLKIndClass: i*8+j (8 sites: Arm1_1..4, Arm2_1..4). SPEC-V899-MultiEP-FullPort, ADR-0004.
        if(INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)
        {
        ...
        for(int i=0; i<slSLKTypeInd->Count; i++)
        {
            for(int j=0; j<16; j++)
            {
                if(slSLKTypeInd->Strings[i]!="" && atof(slSLKTypeInd->Strings[i].c_str())>15.0)
                {
                    if(iIndEPCnt==4 && (j==2 || j==3 || j>=6))                  //JerryYang 20210413 : 4組獨立EP版本使用0,1,4,5
                    {
                        SLKIndClass.push_back(new THTSLKIndClass(slSLKTypeInd->Strings[i], i*16+j, false));
                    }
                    else
                    {
                        SLKIndClass.push_back(new THTSLKIndClass(slSLKTypeInd->Strings[i], i*16+j, (bool)atoi(slSLKTypeIndVisible->Strings[i].c_str())));
                    }
                    //AI(ht9045-v899) 20260430: removed iIndEPCnt==8 branch (was 4 site outer-only); replaced by INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI half-version (2 site x 2 cyl x 2 arm).
                }
            }
        }
        }
...
//        }
        ...
        if(INSTALL_DOUBLE_EP>0)                                                 //Ifor 20191003 : add Die Force 可以自定義Kit直徑
        {
            ...
            //AI(ht9045-v899) 20260504: full port V874.3 INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI OneByOne container.
            //   Per Kit: 8 sites = Arm1_1..4 + Arm2_1..4. iTag = i*8 + j (j=0..7). SPEC-V899-MultiEP-FullPort.
            ...
            if(INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)
            {
                for(int i=0; i<slDieForceOneByOneSLKType->Count; i++)
                {
                    for(int j=0; j<8; j++)
                    {
                        if(slDieForceOneByOneSLKType->Strings[i]!="" && atof(slDieForceOneByOneSLKType->Strings[i].c_str())>15.0)
                        {
                            DieForceOneByOneSLKClass.push_back(new THTDieForceOneByOneSLKClass(slDieForceOneByOneSLKType->Strings[i], i*8+j, (bool)atoi(slDieForceOneByOneSLKTypeVisible->Strings[i].c_str())));
                        }
                    }
                }
            }
...
void __fastcall TfContactForce::FormShow(TObject *Sender)
{
    ...
    {
        //AI(ht9045-v899) 20260526: Show Multi EP site trim page for outer EP per-site calibration.
    ...
    //AI(ht9045-v899) 20260430: 16 EP independent + Multi EP half (INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)
    //   use 2 boards (Arm1 + Arm2). Show Arm2 EP calibration UI to set Arm2 board
    //   Max KPA / Min MPA / Max VDC / Min VDC.
...
//    gbEPSetting->Visible=(AccessLevel>=iDefHonPrecLevel);
    if(INSTALL_DOUBLE_EP==DOUBLE_EP_NORMAL)                                                    //Ifor 20191003 : add Die Force 可以自定義Kit直徑
    {
    ...
    else if(INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)                                               //AI(ht9045-v899) 20260430: Multi EP half - show OneByOne tab.
    {
        tsDieForceDynamicKit->TabVisible=false;
        tsDieForceOneByOneKit->TabVisible=true;
        for(unsigned int i=0; i<DieForceOneByOneSLKClass.size(); i++)
        {
            DieForceOneByOneSLKClass[i]->trckbrDieForceOneByOneDiameter->Min=80;
            DieForceOneByOneSLKClass[i]->trckbrDieForceOneByOneDiameter->Max=150;
        }
    }
    else
    {
...
void TfContactForce::ReadFile()
{
    ...
    if(INSTALL_DOUBLE_EP>0)                                                     //Ifor 20191003 : add Die Force 可以自定義Kit直徑
    {
        ...
        //AI(ht9045-v899) 20260504: full port V874.3 OneByOne ReadFile. Per Kit 8 entries: j<4=Arm1_(j+1), j>=4=Arm2_(j-3). SPEC-V899-MultiEP-FullPort.
        if(INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)
        {
            for(int i=0; i<slDieForceOneByOneSLKType->Count; i++)
            {
    ...
    //AI(ht9045-v899) 20260504: full port V874.3 SLKIndClass ==3 ReadFile (Arm1_1..4 / Arm2_1..4). SPEC-V899-MultiEP-FullPort.
    if(INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)
    {
        for(int i=0; i<slSLKTypeInd->Count; i++)
        {
...
void TfContactForce::WriteFile()
{
    ...
    if(CosFunction.bUseDynamicKitDiameter==true)                                //Steven 20170605 : 可以自定義Kit直徑
    {
        ...
        //AI(ht9045-v899) 20260504: full port V874.3 SLKIndClass ==3 WriteFile (Arm1_1..4 / Arm2_1..4). SPEC-V899-MultiEP-FullPort.
        if(INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)
        {
            for(int i=0; i<slSLKTypeInd->Count; i++)
            {
    ...
    //AI(ht9045-v899) 20260504: full port V874.3 OneByOne WriteFile. Per Kit 8 entries: j<4=Arm1_(j+1), j>=4=Arm2_(j-3). SPEC-V899-MultiEP-FullPort.
    if(INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)
    {
        for(int i=0; i<slDieForceOneByOneSLKType->Count; i++)
        {
            for(int j=0; j<8; j++)
            {
                if(j<4)
                    ...
                    sGroup.sprintf("DieForceDiameter_%0.3fmm_Arm2_%d", DieForceOneByOneSLKClass[i*8+j]->dDiameter, j-3);
                dTemp=double(DieForceOneByOneSLKClass[i*8+j]->trckbrDieForceOneByOneDiameter->Position)/100.0;
                WriteIniData(FileName, sGroup, "LoadRate",      dTemp);
                WriteIniData(FileName, sGroup, "ContactOffset", DieForceOneByOneSLKClass[i*8+j]->edtDieForceOneByOneContactOffset->Text);
            }
        }
    }
...
//---------------------------------------------------------------------------
//AI(ht9045-v899) 20260430: ported from V874.3 Ifor.
