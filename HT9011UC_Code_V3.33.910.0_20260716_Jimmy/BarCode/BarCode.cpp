    ...
    : TForm(Owner)
{
    ...
    for(int i=0; i<BAR_CODE_COUNT; i++)
    {
        ...
        bHasPin1Error           [i]     =false;                                 //==> Eastsun 20260526 #026-4.Pin1 init :KYEC
    ...
    //Ifor 20260512 add: Barcode CSV Compare cache
    slCSVPassCache = new TStringList;
    slCSVPassCache->Clear();
    bCSVCacheLoaded = false;
    //Ifor 20260610 add: Barcode CSV Compare - per-IC record
    asCSVActiveLotID = "";
    asCSVLogFileName = "";
    iCSVRecordCT = 0;
...
void __fastcall TfBarCode::FormShow(TObject *Sender)
{
    ...
    //==> Eastsun 20260526 #026-4.PinN.PU4 Pin1 Visible :KYEC
    cbUsePinInspection->Visible     =CosFunction.b2DUsePinInspection;           //Ifor 20230207 add:In House 2D Use Pin1 Inspection Function
    //<== Eastsun 20260526 #026-4.PinN.PU4
    ...
    //==> Eastsun 20260527 整合#028.AAL.P05 cbAutoAdjustLight Visible :KYEC
    if(BAR_CODE_INSTALL==ebctUseCCDMode)
    {
        cbAutoAdjustLight->Visible          =(CosFunction.bUseBarcodeAutoAdjustLight);
        edAutoAdjustLightTimeOut->Visible   =(CosFunction.bUseBarcodeAutoAdjustLight);
    }
    else
    {
        cbAutoAdjustLight->Visible          =false;
        edAutoAdjustLightTimeOut->Visible   =false;
    }
    //<== Eastsun 20260527 #028.AAL.P05
    SetVisible();
    ...
    cbBarcodeCSVCompare->Visible=true;                                          //Ifor 20260511 add: Barcode CSV Compare
...
//        }
    ...
    //<==
    //Ifor 20210407 add: 自製OCR
    TestIF_File.bEnableBarcodeCSVCompare=ReadIniData(szDir, sGroup, "bEnableBarcodeCSVCompare", false); //Ifor 20260511 add: Barcode CSV Compare
    bCSVCacheLoaded = false;  // Reset cache, reload on next compare
...
//    cbExposureTimeOut->Checked      =TestIF_File.bExposureTimeOut;
    ...
    //==> Eastsun 20260526 #026-4.PinN.PU5 Pin1 Checked :KYEC
    cbUsePinInspection->Checked     =TestIF_File.b2DUsePinInspection;           //Ifor 20230207 add:In House 2D Use Pin1 Inspection Function
    //<== Eastsun 20260526 #026-4.PinN.PU5
    ...
    else
    {
        ...
        else if(CUSTOMER_CODE==CC_ASE_CL)    //JerryYang 20250120 : ASECL SONG要求強制開啟
        {
            if(LastSet.iTester==ON_LINE)
            {
                cbCheckCodeByLot->Enabled=false;
            }
            else
            {
                cbCheckCodeByLot->Enabled=true;
            }
        }
        else
        {
    ...
    //==> Eastsun 20260527 整合#028.AAL.P07 UI bind AutoAdjustLight :KYEC
    cbAutoAdjustLight->Checked      =TestIF_File.bUseBarcodeAutoAdjustLight;
    edAutoAdjustLightTimeOut->Text  =AnsiString(TestIF_File.iAutoAdjustLightTimeOut);
    //<== Eastsun 20260527 #028.AAL.P07
    ...
    cbBarCodeMultiRecipe->Checked   =TestIF_File.bBarCodeMultiRecipe;           //Eastsun 20260527 整合//Ifor 20241031 add:使用BarCode Multi Recipe
    ...
    cbCheckEnable2DIDFunction->Checked       =TestIF_File.bCheckEnable2DIDFunction;         //KaiHuang 20201028 : ASE-CL 生產時檢查是否開啟 2D
    cbCheckEnableBottom2DIDFunction->Checked =TestIF_File.bCheckEnableBottom2DIDFunction;   //KaiHuang 20201028 : ASE-CL 生產時檢查是否開啟 Bottom 2D

    if(CosFunction.bRead2DIDFromServer==true)                                   //Jimmychiu 20230925 : read 2did in json file
    {
        cbCheckCodeByServer2DID->Checked=TestIF_File.bCheckCodeByServer2DID;
        le2DID_ServerURL->Text=TestIF_File.asMes2DID_URL;
        //==> Eastsun 20260527 整合#BCIR.P06 UI sync :KYEC
        cbBarCodeInspReport->Checked=TestIF_File.bBarCodeInspReport;                                       //Sam 20240426 : Add BarCoder Inspection Report
        //<== Eastsun 20260527 #BCIR.P06
        lbfinalpathShow->Caption=GetBarcodeByServerData();
    }
    ...
    cbBarcodeCSVCompare->Checked=TestIF_File.bEnableBarcodeCSVCompare;          //Ifor 20260511 add: Barcode CSV Compare
    if(CosFunction.bReadClipCodeFromUnloader==true)
    {
        ReadUnloaderClipIni();
    }
...
void __fastcall TfBarCode::spbSaveClick(TObject *Sender)
{
    ...
    WriteIniData(szDir, sGroup, "Check Enable 2DID Function",       (cbCheckEnable2DIDFunction->Checked)?1:0);      //KaiHuang 20201028 : ASE-CL 生產時檢查是否開啟 2D
    WriteIniData(szDir, sGroup, "Check 2DID Allow List Function",   (cb2DIDAllowList->Checked)?1:0);               //JerryYang 20241104 : 支援2DID白名單功能
    WriteIniData(szDir, sGroup, "Check Enable Bottom 2DID Function",(cbCheckEnableBottom2DIDFunction->Checked)?1:0);//KaiHuang 20201028 : ASE-CL 生產時檢查是否開啟 2D
    ...
    WriteIniData(szDir, sGroup, "bBarCodeMultiRecipe",(cbBarCodeMultiRecipe->Checked)?1:0);                         //Eastsun 20260527 整合//Ifor 20241031 add:使用BarCode Multi Recipe

    WriteIniData(szDir, sGroup, "b2DUsePinInspection",              (cbUsePinInspection->Checked)?1:0);              //Eastsun 20260526 整合//Ifor 20230207 add:In House 2D Use Pin1 Inspection Function  //PU6 20260526 fix target
    ...
    WriteIniData(szDir, sGroup, "Bar Code Use Auto Adjust Light",      (cbAutoAdjustLight->Checked)?1:0);          //Eastsun 20260527 整合
    WriteIniData(szDir, sGroup, "Bar Code Auto Adjust Ligh Time Out",   edAutoAdjustLightTimeOut->Text);           //Eastsun 20260527 整合
    ...
    //==> Eastsun 20260527 整合#BCIR.P05 WriteIniData :KYEC
    WriteIniData(szDir, sGroup, "bBarCodeInspReport", (cbBarCodeInspReport->Checked)?1:0);                 //Sam 20240426 : Add BarCoder Inspection Report
    //<== Eastsun 20260527 #BCIR.P05

    if(CosFunction.bMakeWhite2DIDList==true)                                    //RogerYang 20251202 : JCET 2D FT1白名單/FT2比對功能
    {
        WriteIniData(szDir, sGroup, "Make White 2DID List",     (chkMakeWhite2DIDList->Checked)?1:0);
    }
    WriteIniData(szDir, sGroup, "bEnableBarcodeCSVCompare", (cbBarcodeCSVCompare->Checked)?1:0);            //Ifor 20260511 add: Barcode CSV Compare
...
bool TfBarCode::Barcode_StartScan_In(int BarCodeIndex, int iXpos, AnsiString &cDeviceInf, AnsiString cLastDeviceInf)
{
    ...
    //==> Eastsun 20260526 #026-4.P3.T3 Pin1 trig CMD @SE+ :KYEC
    if(CosFunction.b2DUsePinInspection && TestIF_File.b2DUsePinInspection)
    {
        strTrigCMD="@SE+";
    }
    else
    //<== Eastsun 20260526 #026-4.P3.T3
           ...
           BAR_CODE_INSTALL==ebcUseOCR)                                         //Ifor 20210407 add: 自製OCR
        {
            switch(BarCodeIndex)
            {
                case 0:
                  ...
                  else                                                          //Steven 20260420 : log warning and try reconnect when socket not active
                  {
                      AddCCDCommunicationLog(BarCodeIndex, "Shuttle1_A socket not active, try reconnect");
                      try { ClientSocket_Shuttle1_A->Open(); } catch(...) { AddCCDCommunicationLog(BarCodeIndex, "Shuttle1_A reconnect failed"); }
                  }
                  break;
                case 1:
                  ...
                  {
                      AddCCDCommunicationLog(BarCodeIndex, "Shuttle1_B socket not active, try reconnect");
                      try { ClientSocket_Shuttle1_B->Open(); } catch(...) { AddCCDCommunicationLog(BarCodeIndex, "Shuttle1_B reconnect failed"); }
                  }
                  break;
                case 2:
                  ...
                  {
                      AddCCDCommunicationLog(BarCodeIndex, "Shuttle2_A socket not active, try reconnect");
                      try { ClientSocket_Shuttle2_A->Open(); } catch(...) { AddCCDCommunicationLog(BarCodeIndex, "Shuttle2_A reconnect failed"); }
                  }
                  break;
                case 3:
                  ...
                  {
                      AddCCDCommunicationLog(BarCodeIndex, "Shuttle2_B socket not active, try reconnect");
                      try { ClientSocket_Shuttle2_B->Open(); } catch(...) { AddCCDCommunicationLog(BarCodeIndex, "Shuttle2_B reconnect failed"); }
                  }
                  break;
            }
        }
    ...
    if(bBarcodeDataSaveReady[BarCodeIndex]==false)
    {
        ...
        //==> Eastsun 20260526 #026-4.P3.T6 Pin1 mode no simulate :KYEC
        else if(CosFunction.b2DUsePinInspection && TestIF_File.b2DUsePinInspection && cbBarCodeSimulate->Checked==false)
        {
            bSimulateBarCode=false;
        }
        //<== Eastsun 20260526 #026-4.P3.T6
...
//            if(Re.Length()<iSize)                                             //wei 20151120
        ...
        else
        {
            ...
            //==> Eastsun 20260526 #026-4.P3.T7 Pin1 mode skip duplicate check :KYEC
            if(CosFunction.b2DUsePinInspection && TestIF_File.b2DUsePinInspection)
            {
            }
            else
      ...
      TCustomWinSocket *Socket)
{
    ...
    //==> Eastsun 20260526 #026-4.P3.T4 Pin1 mode skip OnConnect :KYEC
    if(CosFunction.b2DUsePinInspection && TestIF_File.b2DUsePinInspection)
        return;
    //<== Eastsun 20260526 #026-4.P3.T4
      ...
      TCustomWinSocket *Socket)
{
    ...
    if(iBufferLenght>0)                                                         //13.09.30.01   klutter
    {
        if(BAR_CODE_INSTALL==ebctEtherNetCCD ||                                 //Ifor 20190129 : add Cognex EtherNet
           ...
           //==> Eastsun 20260526 #026-4.P3.T8 Pin1 mode + version :KYEC
           (CosFunction.b2DUsePinInspection && TestIF_File.b2DUsePinInspection && RunInfo.BarcodeSoftwareVersion!=""))
           //<== Eastsun 20260526 #026-4.P3.T8
        {
            ...
            else
            {
                        ...
                        len<=TestIF_File.iBarCodeMaxLength)                     //wei 20151127 字元數比對
                {
                    ...
                    else
                    {
                        ...
                        {
                            bBarcodeDataSaveReady[iSH_BarCode]=false;
                            bHasPin1Error[iSH_BarCode]=true;             //==> Eastsun 20260526 #026-4.B3 :KYEC
                        }
                        //==> Eastsun 20260526 #026-4.B3 Pin1 NG handle :KYEC
                        else if(CosFunction.b2DUsePinInspection &&
                                TestIF_File.b2DUsePinInspection &&
                                cStr.Pos("@SENG+")!=0           )                 //Ifor 20240626 add:Pin1 NG immediate alarm
                        {
                            bBarcodeDataSaveReady[iSH_BarCode]=false;
                            bHasPin1Error[iSH_BarCode]=true;
                        }
                        else if(CosFunction.b2DUsePinInspection &&
                                TestIF_File.b2DUsePinInspection &&
                                cStr=="PNOK"                  )                 //Ifor 20240626 add:Pin1 NG immediate alarm
                        {
                            bBarcodeDataSaveReady[iSH_BarCode]=false;
                            bHasPin1Error[iSH_BarCode]=true;
                        }
                        //<== Eastsun 20260526 #026-4.B3
                        //==> Eastsun 20260526 #026-4.P2 @SENG+/@SETO+ Pin1 Error :KYEC
                        else if(cStr=="@SENG+" || cStr=="@SETO+")
                        {
                            bBarcodeDataSaveReady[iSH_BarCode]=false;
                            bHasPin1Error[iSH_BarCode]=true;  //Ifor 20250707 add:Pin1 Error
                        }
                        //<== Eastsun 20260526 #026-4.P2
                        else if(cStr=="ERROR_Read")
                        {
                            bBarcodeDataSaveReady[iSH_BarCode]=false;
                            bHasPin1Error[iSH_BarCode]=true;  //==> Eastsun 20260526 #026-4.P2 :KYEC
                            cStr="ERROR";
                        }
                        else
                        {
        ...
        if(Socket2DReceiveList[Tag]->Count>0)
        {
            ...
            else if(dVisionVer>=2)                                              //新版Command     //wei 20170119 (Steven) 版本號大於2
            {
                ...
                else //if(iFunction==0)
                {
                    ...
                    //==> Eastsun 20260527 整合#BCIR.P15 GG branch :KYEC
                    else if(sPrefix=="GG")                                              //Sam 20240426 : Add BarCoder Inspection Report
                    {
                        if(sList->Count>=4)
                        {
                            if(bCCDBarcodeGetResultOK[Tag]==false)
                            {
                                CCDBarcode_Decoding(Tag);
                                bCCDBarcodeGetResultOK[Tag]=true;
                                sReply.sprintf("2DID GGOK : %s", sDownCCDRecevieData[Tag]);
                            }
                            else
                            {
                                sReply.sprintf("2DID GGTO : %s", sDownCCDRecevieData[Tag]);
                            }
                        }
                        else
                        {
                            sReply.sprintf("2DID GGNG   : %s", sDownCCDRecevieData[Tag]);
                        }
                    }
                    //<== Eastsun 20260527 #BCIR.P15
                    else
                    {
            ...
            else //if(dVisionVer==1)
            {
                ...
                else
                {
                    ...
                    {
                        iGetSE2[Tag]=iResult;
                        //==> Eastsun 20260527 整合#028.AAL.P09 reset auto-adjust after SE2 :KYEC
                        bCCDBarcodeAutoAdjustLight[Tag]=false;
                        bBarcodeNeedAutoAdjust[Tag]=false;
                        //<== Eastsun 20260527 #028.AAL.P09
                    }
...
void TfBarCode::CCDBarcode_Decoding(int Channel)
{
    ...
    //==> Eastsun 20260527 整合#BCIR.P07a locals :KYEC
    AnsiString dataInsp;
    int iInspListCount=0;
    AnsiString sSYC="", sANU="", sUEC="", sDCD="", sGNU="", sCM="", sFPD="", sOVR="";   //Sam 20240426 : Add BarCoder Inspection Report
    //<== Eastsun 20260527 #BCIR.P07a
    ...
    TStringList *sList  =new TStringList();
    TStringList *sInspList  =new TStringList();                                 //Eastsun 20260527 整合   //Sam 20240426 : Add BarCoder Inspection Report
        ...
        for(int i=0; i<iCol; i++)
        {
            if(sList->Count-1>=i)
            {
                if(dVisionVer>=2)                                               //新版Command      //wei 20170119 (Steven) 版本號大於2
                    ...
                    iPos=i+3;
                //==> Eastsun 20260527 整合#BCIR.P07c SaveInspReport :KYEC
                if(TestIF_File.bBarCodeInspReport && dVisionVer>=2)              //Sam 20240426 : Add BarCoder Inspection Report
                {
                    dataInsp=sList->Strings[iPos];
                    dataInsp=StringReplace(dataInsp, " ", "", TReplaceFlags()<<rfReplaceAll);
                    sInspList->CommaText=dataInsp;
                    iInspListCount=sInspList->Count;
                    if(iInspListCount>=15)
                    {
                        for(int x=0; x<iInspListCount-14; x++)
                        {
                            if(x==0)
                                dataSplit=sInspList->Strings[x];
                            else if(x>0)
                                dataSplit+=","+sInspList->Strings[x];
                        }

                        sOVR=sInspList->Strings[iInspListCount-13];
                        sSYC=sInspList->Strings[iInspListCount-11];
                        sCM =sInspList->Strings[iInspListCount-9];
                        sDCD=sInspList->Strings[iInspListCount-7];
                        sANU=sInspList->Strings[iInspListCount-5];
                        sGNU=sInspList->Strings[iInspListCount-3];
                        sUEC=sInspList->Strings[iInspListCount-1];

                        if(dataSplit!="")
                        {
                            fBarCode->SaveInspReport(asATCEvenLotID,
                                                     GetLastOpenFN(),
                                                     dataSplit,
                                                     sSYC,
                                                     sANU,
                ...
                //==> Eastsun 20260526 #026-4.P3.T10 Pin1 / format6 skip regex :KYEC
                else if((CosFunction.b2DUsePinInspection && TestIF_File.b2DUsePinInspection) || TestIF_File.b2DIDStringFormat==6)
                {
                    len=dataSplit.Length();
                }
                //<== Eastsun 20260526 #026-4.P3.T10
                else
                {
    ...
    sList->Clear();                                                             //Ifor 20170603 (wei) TStringList 刪除前先 Clean
    delete regex;
    //==> Eastsun 20260527 整合#BCIR.P07d delete sInspList :KYEC
    sInspList->Clear();
    delete sInspList;                                                                     //Sam 20240426 : Add BarCoder Inspection Report
    //<== Eastsun 20260527 #BCIR.P07d
    delete sList;
}
//---------------------------------------------------------------------------
    ...
    else
    {
        ...
        else if(TestIF_File.iMulti2DType==e2x2In2CCD)
        {
            ...
            for(int j=0; j<iCol; j++)
            {
                ...
                if(ptrMulti2DIDSHT->Item[0][j]!=HAS_NULL_IC && ptrMulti2DIDSHT->Item[0][j]!=NULL_IC)
                {
                    if((TestIF_File.iMulti2DMap[0][0]>0 && (cMilti2DBuffer[i2DRow+0][i2DCol+0]=="" || cMilti2DBuffer[i2DRow+0][i2DCol+0]=="0")) ||   //Wei 20260519 : 需要修改 否則沒掃到不會異常
...
{
    static bool bTimerRun=false;
    //==> Eastsun 20260527 整合#027-2.MR.F1 bBarCodeMultiRecipe declare :KYEC
    AnsiString strBarcodeSetupFile="";
    if(TestIF_File.bEnableBarCode==true &&
       BAR_CODE_INSTALL==ebctUseCCDMode &&
       TestIF_File.bBarCodeMultiRecipe==true)
    {
        strBarcodeSetupFile=fMain->cbSetupFileName->Text+fLotInfo->edtBarcodeRecipe->Text;
    }
    else
    {
        strBarcodeSetupFile=fMain->cbSetupFileName->Text;
    }
    //<== Eastsun 20260527 #027-2.MR.F1
    if(InitialOK==false || bTimerRun==true)                                     //Steven 20160512 : 2D Timer加上保護
    {
        return;
    }
    ...
    //==> Eastsun 20260526 #026-4.P1.3 :KYEC
    static bool bBarcodeMode=false;  //Ifor 20230531 add:2D/Pin1 mode switch
    static bool bOldMode=false;      //Ifor 20230531 add:2D/Pin1 mode switch
    //<== Eastsun 20260526 #026-4.P1.3
       ...
       ClientSocket_Shuttle2_A->Active && ClientSocket_Shuttle2_B->Active )     //ifor 20230116 change 避免重新連線斷線
    {
        switch(Task)
        {
            ...
            case 100:
                if(bSFCGetVR==true)
                {
                    //==> Eastsun 20260526 #026-4.P3.T11 Pin1 mode Task=4000 :KYEC
                    if(CosFunction.b2DUsePinInspection && TestIF_File.b2DUsePinInspection)
                        Task=4000;
                    else
                    //<== Eastsun 20260526 #026-4.P3.T11
                    if(TestIF_File.bEnableBarCode)
                        Task=1000;
...
//                    if(strBarcodeSetupFile.UpperCase().AnsiCompare(sCurrentGF)==0)
//                    {
//                        Task=2000;
//                        bGetLF=true;
//                    }
//                    else
            ...
            case 1200:
                ...
                else
                {
                    ...
                    {
                        if(dVisionVer>=2)                                       //wei 20170119 (Steven) 版本號大於2
                            Str.sprintf("LF,0,%s,1", strBarcodeSetupFile);      //==> Eastsun 20260527 整合#027-2.MR.F1.r01 bBarCodeMultiRecipe :KYEC
                        else if(TestIF_File.bEnableMulti2D)                     //Steven 20200810 : 一個IC使用多個2DID
                            Str.sprintf("LF%s,1,%d", strBarcodeSetupFile, TestIF_File.iMulti2DCount);   //==> Eastsun 20260527 #027-2.MR.F1.r02 bBarCodeMultiRecipe :KYEC
                        else
                            Str.sprintf("LF%s,1", strBarcodeSetupFile);         //==> Eastsun 20260527 整合#027-2.MR.F1.r03 bBarCodeMultiRecipe :KYEC
                    }
                    else
                    {
                        if(dVisionVer>=2)                                       //wei 20170119 (Steven) 版本號大於2
                            Str.sprintf("LF,0,%s,0", strBarcodeSetupFile);      //==> Eastsun 20260527 整合#027-2.MR.F1.r04 bBarCodeMultiRecipe :KYEC
                        else if(TestIF_File.bEnableMulti2D)                     //Steven 20200810 : 一個IC使用多個2DID
                            Str.sprintf("LF%s,0,%d", strBarcodeSetupFile, TestIF_File.iMulti2DCount);   //==> Eastsun 20260527 整合#027-2.MR.F1.r05 bBarCodeMultiRecipe :KYEC
                        else
                            Str.sprintf("LF%s,0", strBarcodeSetupFile);         //==> Eastsun 20260527 整合#027-2.MR.F1.r06 bBarCodeMultiRecipe :KYEC
                    }
                }
            ...
            case 2250:
                if(bSFCGetGF)
                {
                    ...
                    {
                        sBuffer=strBarcodeSetupFile;                            //==> Eastsun 20260527 整合#027-2.MR.F1.r07 bBarCodeMultiRecipe :KYEC
                    }
            ...
            case 2300:
                ...
                {
                    Str.sprintf("LF,1,%s", strBarcodeSetupFile);                //==> Eastsun 20260527 整合#027-2.MR.F1.r08 bBarCodeMultiRecipe :KYEC
                }
    ...
    //==> Eastsun 20260526 #026-4.P1.3 2D/Pin1 mode switch :KYEC
    if(CosFunction.b2DUsePinInspection)
    {
        if(ClientSocket_Shuttle1_A->Active)
        {
            if(bBarcodeMode==false || bOldMode!=TestIF_File.b2DUsePinInspection)
            {
                if(TestIF_File.b2DUsePinInspection==true)
                {
                    Str="@FNPN+";
                    SendCCDCommand(iBarCode1_1, "Switch To Pin1.", Str);
                }
                else
                {
                    Str="@FN2D+";
                    SendCCDCommand(iBarCode1_1, "Switch To 2D.", Str);
                }
                bBarcodeMode=true;
                bOldMode=TestIF_File.b2DUsePinInspection;
            }
        }
    }
    //<== Eastsun 20260526 #026-4.P1.3
...
bool TfBarCode::DoBarcodeCheck(int iBarCode, AnsiString &Str, int iXItem)       //wei 20160505 Barcode 比對Lot
{
    ...
    //==> Eastsun 20260526 #026-4.P3.T5 Pin1 mode skip BarcodeCheck :KYEC
    if(CosFunction.b2DUsePinInspection && TestIF_File.b2DUsePinInspection)
        return true;
    //<== Eastsun 20260526 #026-4.P3.T5
    if(TestIF_File.bCheckCodeByLot)
    {
...
bool TfBarCode::DoBarcodeChangeFile()                                           //wei 20160728 Barcode File切換
{
    ...
    //==> Eastsun 20260527 整合#027-2.MR.F2 bBarCodeMultiRecipe declare :KYEC
    AnsiString strBarcodeSetupFile="";
    if(TestIF_File.bEnableBarCode==true &&
       BAR_CODE_INSTALL==ebctUseCCDMode &&
       TestIF_File.bBarCodeMultiRecipe==true)
    ...
    //<== Eastsun 20260527 整合#027-2.MR.F2
    ...
    switch(Task)
    {
        ...
        case 100:
            if(BAR_CODE_INSTALL==ebctEtherNetCCD)
            {
                if(ClientSocket_Shuttle1_A->Active)
                {
                    str1.printf("%s%s\\%s.cfg", sBarcodeFile, HSys.asCCDBarCodeIP[0], strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r01 bBarCodeMultiRecipe :KYEC
                }
                else
                {
                    Task=200;
            ...
            {
                str1.sprintf("LF%s,0", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r02 bBarCodeMultiRecipe :KYEC
                memoCCDCommLog->Lines ->Add(str1.c_str());
            ...
            else if(BAR_CODE_INSTALL==ebcUseOCR)                                //Ifor 20210407 add: 自製OCR
            {
                if(ClientSocket_Shuttle1_A->Active)
                {
                    str1.printf("LF%s", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r03 bBarCodeMultiRecipe :KYEC
            ...
            {
                str.sprintf("%s, %s%s\\%s.cfg\r\n",HSys.asBarCodeComPort[0], sBarcodeFile, HSys.asBarCodeComPort[0], strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r04 bBarCodeMultiRecipe :KYEC
                str1.printf("%s%s\\%s.cfg", sBarcodeFile, HSys.asBarCodeComPort[0], strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r05 bBarCodeMultiRecipe :KYEC
                if(FileExists(str1))
                {
                    ClientSocket_BarcodeChangeFile->Socket->SendBuf(str.c_str(),str.Length());
                    moBarcodeChangeFile->Lines ->Add(str.c_str());
        ...
        case 200:
            if(BAR_CODE_INSTALL==ebctEtherNetCCD)
            {
                if(ClientSocket_Shuttle1_B->Active)
                {
                    str1.printf("%s%s\\%s.cfg", sBarcodeFile, HSys.asCCDBarCodeIP[1], strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r06 bBarCodeMultiRecipe :KYEC
                }
                else
                {
                    Task=300;
            ...
            {
                str1.sprintf("LF%s,0", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r07 bBarCodeMultiRecipe :KYEC
                memoCCDCommLog->Lines ->Add(str1.c_str());
            ...
            else if(BAR_CODE_INSTALL==ebcUseOCR)                                //Ifor 20210407 add: 自製OCR
            {
                if(ClientSocket_Shuttle1_B->Active)
                {
                    str1.printf("LF%s", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r08 bBarCodeMultiRecipe :KYEC
            ...
            {
                str.sprintf("%s, %s%s\\%s.cfg\r\n",HSys.asBarCodeComPort[1], sBarcodeFile, HSys.asBarCodeComPort[1], strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r09 bBarCodeMultiRecipe :KYEC
                str1.printf("%s%s\\%s.cfg", sBarcodeFile, HSys.asBarCodeComPort[1], strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r10 bBarCodeMultiRecipe :KYEC
                if(FileExists(str1))
                {
                    ClientSocket_BarcodeChangeFile->Socket->SendBuf(str.c_str(),str.Length());
                    moBarcodeChangeFile->Lines ->Add(str.c_str());
        ...
        case 300:
            if(BAR_CODE_INSTALL==ebctEtherNetCCD)
            {
                if(ClientSocket_Shuttle2_A->Active)
                {
                    str1.printf("%s%s\\%s.cfg", sBarcodeFile, HSys.asCCDBarCodeIP[2], strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r11 bBarCodeMultiRecipe :KYEC
                }
                else
                {
                    Task=400;
            ...
            {
                str1.sprintf("LF%s,0", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r12 bBarCodeMultiRecipe :KYEC
                memoCCDCommLog->Lines ->Add(str1.c_str());
            ...
            else if(BAR_CODE_INSTALL==ebcUseOCR)                                //Ifor 20210407 add: 自製OCR
            {
                if(ClientSocket_Shuttle2_A->Active)
                {
                    str1.printf("LF%s", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r13 bBarCodeMultiRecipe :KYEC
            ...
            {
                str.sprintf("%s, %s%s\\%s.cfg\r\n",HSys.asBarCodeComPort[2], sBarcodeFile, HSys.asBarCodeComPort[2], strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r14 bBarCodeMultiRecipe :KYEC
                str1.printf("%s%s\\%s.cfg", sBarcodeFile, HSys.asBarCodeComPort[2], strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r15 bBarCodeMultiRecipe :KYEC
                if(FileExists(str1))
                {
                    ClientSocket_BarcodeChangeFile->Socket->SendBuf(str.c_str(),str.Length());
                    moBarcodeChangeFile->Lines ->Add(str.c_str());
        ...
        case 400:
            if(BAR_CODE_INSTALL==ebctEtherNetCCD)
            {
                if(ClientSocket_Shuttle2_B->Active)
                {
                    str1.printf("%s%s\\%s.cfg", sBarcodeFile, HSys.asCCDBarCodeIP[3], strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r16 bBarCodeMultiRecipe :KYEC
                }
                else
                {
                    Task=500;
            ...
            {
                str1.sprintf("LF%s,0", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r17 bBarCodeMultiRecipe :KYEC
                memoCCDCommLog->Lines ->Add(str1.c_str());
            ...
            else if(BAR_CODE_INSTALL==ebcUseOCR)                                //Ifor 20210407 add: 自製OCR
            {
                if(ClientSocket_Shuttle2_B->Active)
                {
                    str1.printf("LF%s", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r18 bBarCodeMultiRecipe :KYEC
            ...
            {
                str.sprintf("%s, %s%s\\%s.cfg\r\n",HSys.asBarCodeComPort[3], sBarcodeFile, HSys.asBarCodeComPort[3], strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r19 bBarCodeMultiRecipe :KYEC
                str1.printf("%s%s\\%s.cfg", sBarcodeFile, HSys.asBarCodeComPort[3], strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F2.r20 bBarCodeMultiRecipe :KYEC
                if(FileExists(str1))
                {
                    ClientSocket_BarcodeChangeFile->Socket->SendBuf(str.c_str(),str.Length());
                    moBarcodeChangeFile->Lines ->Add(str.c_str());
...
bool TfBarCode::MoveInArmXY_ToBottom2DID()
{
    ...
    for(int i=0; i<InArmSuck.iMaxRow; i++)                                      //KaiChen 20200428 ：新增Bottom2DID Z軸 OffSet 移動
    {
        for(int j=0; j<InArmSuck.iMaxCol; j++)
        {
            if(IniConfig.bE33InOutArmZOffsetSameOne)                            //JerryYang 20260409 : fix Bottom 2DID offset
            {
                iZPos[i][j]=10+InArmOffSet[InOfsLoader]->GetPlace()+InArmOffSet[InOfsLoader]->GetPlace(i, j);
            }
            else
            {
...
void __fastcall TfBarCode::TimerBottom8CCDInitialTimer(TObject *Sender)         //KaiChen 20200513 : Bottom 2D 8CCD
{
    ...
    AnsiString Str;
    //==> Eastsun 20260527 整合#027-2.MR.F3 bBarCodeMultiRecipe declare :KYEC
    AnsiString strBarcodeSetupFile="";
    if(TestIF_File.bEnableBarCode==true &&
       BAR_CODE_INSTALL==ebctUseCCDMode &&
       TestIF_File.bBarCodeMultiRecipe==true)
    ...
    //<== Eastsun 20260527 整合#027-2.MR.F3
       ...
       ClientSocket_Bottom_4->Port==5014)
    {
        switch(Task)
        {
            ...
            case 1200:
                ...
                {
                    if(dVisionVer>=2)                                           //wei 20170119 (Steven) 版本號大於2
                        Str.sprintf("LF,0,%s,1", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F3.r01 bBarCodeMultiRecipe :KYEC
                    else
                        Str.sprintf("LF%s,1", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F3.r02 bBarCodeMultiRecipe :KYEC
                }
                else
                {
                    if(dVisionVer>=2)                                           //wei 20170119 (Steven) 版本號大於2
                        Str.sprintf("LF,0,%s,0", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F3.r03 bBarCodeMultiRecipe :KYEC
                    else
                        Str.sprintf("LF%s,0", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F3.r04 bBarCodeMultiRecipe :KYEC
                }
            ...
            case 2250:
                if(bSFCGetGF)
                {
                    if(strBarcodeSetupFile.UpperCase().AnsiCompare(sSFCCurrentGF)==0)   //==> Eastsun 20260527 整合#027-2.MR.F3.r05 bBarCodeMultiRecipe :KYEC
                    {
                        Task=3000;
                        bSFCGetLF=true;
                    }
            ...
            case 2300:
                bSFCGetLF=false;
                SendCCDCommand(iBottom_1, "SFC Change File", "LF,1,"+strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F3.r06 bBarCodeMultiRecipe :KYEC
                tChangeSetupFile.SetSecAndOn(10);
                Task=2350;
            case 2350:
                if(bSFCGetLF)
       ...
       ClientSocket_Bottom_8->Port==5018)
    {
        switch(Task2)
        {
            ...
            case 1200:
                ...
                {
                    if(dVisionVer>=2)                                           //wei 20170119 (Steven) 版本號大於2
                        Str.sprintf("LF,0,%s,1", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F3.r07 bBarCodeMultiRecipe :KYEC
                    else
                        Str.sprintf("LF%s,1", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F3.r08 bBarCodeMultiRecipe :KYEC
                }
                else
                {
                    if(dVisionVer>=2)                                           //wei 20170119 (Steven) 版本號大於2
                        Str.sprintf("LF,0,%s,0", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F3.r09 bBarCodeMultiRecipe :KYEC
                    else
                        Str.sprintf("LF%s,0", strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F3.r10 bBarCodeMultiRecipe :KYEC
                }
            ...
            case 2250:
                if(bSFCGetGF)
                {
                    if(strBarcodeSetupFile.UpperCase().AnsiCompare(sSFCCurrentGF)==0)   //==> Eastsun 20260527 整合#027-2.MR.F3.r11 bBarCodeMultiRecipe :KYEC
                    {
                        Task2=3000;
                        bSFCGetLF=true;
                    }
            ...
            case 2300:
                bSFCGetLF=false;
                SendCCDCommand(iBottom_5, "SFC Change File", "LF,1,"+strBarcodeSetupFile);   //==> Eastsun 20260527 整合#027-2.MR.F3.r12 bBarCodeMultiRecipe :KYEC
                tChangeSetupFile.SetSecAndOn(10);
                Task2=2350;
            case 2350:
                if(bSFCGetLF)
...
bool TfBarCode::UpdateWhite2DIDList(AnsiString sLotID, AnsiString sRunMode)     //RogerYang 20251210 : JCET 2D FT1白名單/FT2比對功能
{
    if(fLotInfo->cbRunMode->Text.Pos("FT")>0 ||   //改成都要上傳
        fLotInfo->cbRunMode->Text.Pos("QA")>0)                                  //RogerYang 20260615 : 新增QA
    {
...
//---------------------------------------------------------------------------
//==> Eastsun 20260526 #026-4.PinN.PU7 Pin1 Click handler :KYEC
void __fastcall TfBarCode::cbUsePinInspectionClick(TObject *Sender)
{
    AnsiString Str="";

    if(CosFunction.b2DUsePinInspection) //Ifor 20230531 add:2D/Pin1 模式切換
    {
        if(cbUsePinInspection->Checked==true)
        {
            Str="@FNPN+";
            SendCCDCommand(iBarCode1_1, "Switch To Pin1.", Str);
        }
...
//---------------------------------------------------------------------------
//<== Eastsun 20260526 #026-4.PinN.PU7

//==> Eastsun 20260527 整合#BCIR.P08 SaveInspReport methods :KYEC
FILE *WriteBarcodeLOG;
void TfBarCode::SaveInspReport(AnsiString sLot, AnsiString sRecipe, AnsiString sCode,
                               AnsiString sSYC, AnsiString sANU, AnsiString sUEC, AnsiString sDCD,
                               AnsiString sGNU, AnsiString sCM,  AnsiString sFPD, AnsiString sOVR)   //Sam 20240426 : Add BarCoder Inspection Report
{
    if(TestIF_File.bBarCodeInspReport==false)
        return;
    AnsiString asPatch="", asFilePatch="", asStr="";
    bool bHasFile=false;
    TStringList *SList;

    asPatch.sprintf("%s\\%04d_%02d_%02d", asBarCodeInspReportPath,
                                    RunInfo.LotStartYear,
                                    RunInfo.LotStartMonth,
                                    RunInfo.LotStartDate);
    if(DirectoryExists(asPatch)==false)
        ForceDirectories(asPatch);

    asFilePatch.sprintf("%s\\%04d%02d%02d%02d%02d_%s.txt", asPatch,
                                            RunInfo.LotStartYear,
                                            RunInfo.LotStartMonth,
                                            RunInfo.LotStartDate,
                                            RunInfo.LotStartHour,
                                            RunInfo.LotStartMin,
                                            sLot);
    bHasFile=FileExists(asFilePatch);
    if(bHasFile==false)
    {
        SList=new TStringList();
        asStr="                             HONPRIC 2D BARCODE INSPECTION REPORT";
        SList->Add(asStr);
        asStr="--------------------------------------------------------------------------------------------------";
        SList->Add(asStr);
        asStr="LOT      : "+sLot;
        SList->Add(asStr);
        asStr="RECIPE   : "+sRecipe;
        SList->Add(asStr);
        asStr="--------------------------------------------------------------------------------------------------";
        SList->Add(asStr);
        asStr="";
        SList->Add(asStr);
        SList->Add(asStr);
        asStr="Data Matrix Code| SYC  G| ANU  G| UEC  G| DCD G| GNU G| CM  G| FPD G| OVR G|";
        SList->Add(asStr);
        asStr="==================================================================================================";
        SList->Add(asStr);
        SList->SaveToFile(asFilePatch);
        SList->Clear();
        delete SList;
    }
    asStr.sprintf("%s  |      %s|      %s|      %s|     %s|     %s|     %s|     %s|     %s|",
                   sCode, sSYC, sANU, sUEC, sDCD, sGNU, sCM, sFPD, sOVR);

    WriteBarcodeLOG=fopen(asFilePatch.c_str(), "a+");
    if(WriteBarcodeLOG!=NULL)
    {
        try
        {
            fprintf(WriteBarcodeLOG, "%s \n", asStr);
        }
        catch(...)
        {
            MyDBIProcess("Exception", "TfBarCode::SaveInspReport");
        }
    }
    fclose(WriteBarcodeLOG);
}
//---------------------------------------------------------------------------
void TfBarCode::SaveInspReportEnd(AnsiString sLot, AnsiString sRecipe)   //Sam 20240426 : Add BarCoder Inspection Report
{
    if(TestIF_File.bBarCodeInspReport==false)
        return;
    AnsiString asPatch="", asFilePatch="", asStr="";
    bool bHasFile=false;
    TStringList *SListCheck;
    bool bHasEnd=false;
    ...
    {
        bHasEnd=false;
        SListCheck=new TStringList;
        SListCheck->Clear();
        SListCheck->LoadFromFile(asFilePatch);
        if(SListCheck->Count>20)
        {
            for(int i=SListCheck->Count-10; i<SListCheck->Count; i++)
            {
                if(SListCheck->Strings[i].Pos("Code Reference")>0)
                    bHasEnd=true;
            }
        }
        ...
        if(bHasEnd==false)
        {
            ...
            if(WriteBarcodeLOG!=NULL)
            {
                ...
                {
                    asStr="\n";
                    asStr+="Code Reference: \n";
                    asStr+="---------Set------ \n";
                    asStr+="SYC : Symbol Contrast \n";
                    asStr+="ANU : Axial Non-Uniformity \n";
                    asStr+="UEC : Unused Error Correction \n";
                    asStr+="DCD : Decodability \n";
                    asStr+="GNU : Grid Non-Uniformity \n";
                    asStr+="CM  : Cell Modulation \n";
                    asStr+="FPD : Fixed Pattern Damage \n";
                    asStr+="OVR : Overall \n";
                    fprintf(WriteBarcodeLOG, "%s", asStr);
                }
                catch(...)
                {
                    MyDBIProcess("Exception", "TfBarCode::SaveInspReportEnd");
                }
            }
            fclose(WriteBarcodeLOG);
        }
...
//---------------------------------------------------------------------------
//<== Eastsun 20260527 #BCIR.P08
//---------------------------------------------------------------------------
//Ifor 20260512 add: Barcode CSV Compare - Load cache from D:\BarcodeFile folder
// Scan files containing "{FileID}" in filename, parse Pass Bin IDs into slCSVPassCache
// Pass Bin is determined by Prod.iIsPassT6[bin]==1 (dynamic, not hardcoded)
//---------------------------------------------------------------------------
void TfBarCode::LoadBarcodeCSVCache()
{
    TSearchRec sr;
    AnsiString asFileID;
    AnsiString asFolder;
    ...
    slCSVPassCache->Clear();
    bCSVCacheLoaded = true;

    //Ifor 20260610: primary key = current LotID; fallback = manual override field
    asFileID = asATCEvenLotID.Trim();
    if(asFileID == "")
    {
        WriteBarCodeLog(0, "CSV Compare: LotID and override both empty, skip load");
        return;
    }

    asFolder = "D:\\BarcodeFile\\";
    if(!DirectoryExists(asFolder))
    {
        WriteBarCodeLog(0, "CSV Compare: Folder not found - " + asFolder + " (create)");
        if(!ForceDirectories(asFolder))
        {
            WriteBarCodeLog(0, "CSV Compare: Folder create failed - " + asFolder);
            return;
        }
    }

    // Search files containing {FileID} under D:\BarcodeFile folder
    if(FindFirst(asFolder + "*.csv", faAnyFile, sr) == 0)
    {
        do
        {
            // Match: filename contains {FileID}
            if(sr.Name.Pos(asFileID) > 0)
            {
                AnsiString asFilePath = asFolder + sr.Name;
                TStringList *slFile = new TStringList;
                try
                {
                    slFile->LoadFromFile(asFilePath);
                    WriteBarCodeLog(0, "CSV Compare: Parsing file=" + sr.Name + " Lines=" + IntToStr(slFile->Count - 1));
                    //CSV format: No Unit, Date, Time, Barcode ID, Result
                    for(int i = 1; i < slFile->Count; i++)
                    {
                        AnsiString sLine, S, aNumber, aDate, aTime, aID, aResult;
                        int iBinResult;
                        AnsiString aTrimID;
                        ...
                        if(S.Pos(",") <= 0) continue;
                        aNumber = S.SubString(1, S.Pos(",") - 1);
                        S = S.Delete(1, S.Pos(","));                            //Ifor 20260525 fix: must assign back

                        if(S.Pos(",") <= 0) continue;
                        aDate = S.SubString(1, S.Pos(",") - 1);
                        S = S.Delete(1, S.Pos(","));

                        if(S.Pos(",") <= 0) continue;
                        aTime = S.SubString(1, S.Pos(",") - 1);
                        S = S.Delete(1, S.Pos(","));

                        if(S.Pos(",") <= 0) continue;
                        aID = S.SubString(1, S.Pos(",") - 1);
                        S = S.Delete(1, S.Pos(","));

                        aResult = S.Trim();

                        // If Result is a Pass Bin (Prod.iIsPassT6[bin]==1), add to cache
                        iBinResult = atoi(aResult.c_str());
                        if(iBinResult > 0 && iBinResult < iTestBinCount &&
                           Prod.iIsPassT6[iBinResult] == 1)
                        {
                            aTrimID = aID.Trim();
                            if(slCSVPassCache->IndexOf(aTrimID) < 0)
                            {
                                slCSVPassCache->Add(aTrimID);
                                WriteBarCodeLog(0, "CSV Compare: Added to cache ID=" + aTrimID);
                            }
                        }
                    }
                }
                ...
                {
                    WriteBarCodeLog(0, "CSV Compare: Exception reading - " + asFilePath);
                }
                delete slFile;
            }
        } while(FindNext(sr) == 0);
    ...
    WriteBarCodeLog(0, "CSV Compare: Cache loaded, Pass count=" + IntToStr(slCSVPassCache->Count));
}
//---------------------------------------------------------------------------
//Ifor 20260512 add: Barcode CSV Compare
// return: 0=normal test, 1=Pass Bin found -> skip test (set ERROR)
//---------------------------------------------------------------------------
int TfBarCode::DoBarcodeCSVCompare(AnsiString aBarcodeID)
{
    if(TestIF_File.bEnableBarcodeCSVCompare == false)
        return 0;

    //Ifor 20260610: detect LotID change -> reset cache + log file
    EnsureCSVForCurrentLot();

    // Require either current LotID or manual override before loading
    if(asATCEvenLotID.Trim() == "")
        return 0;

    // Load cache on first call (after EnsureCSVForCurrentLot, bCSVCacheLoaded may have been reset)
    if(!bCSVCacheLoaded)
        LoadBarcodeCSVCache();

    // Check if this Barcode ID is in the Pass cache
    if(slCSVPassCache->IndexOf(aBarcodeID.Trim()) >= 0)
    {
        WriteBarCodeLog(0, "CSV Compare: BarcodeID=" + aBarcodeID + " is Pass, alarm/unlock required");
        return 1;
    }
...
//---------------------------------------------------------------------------
//Ifor 20260610 add: Barcode CSV Compare
// Detect Lot change. If asATCEvenLotID differs from the tracked Lot,
// reset cache + log file path so a fresh CSV is created for the new Lot.
//---------------------------------------------------------------------------
void TfBarCode::EnsureCSVForCurrentLot()
{
    AnsiString asNowLot = asATCEvenLotID.Trim();
    if(asNowLot != asCSVActiveLotID)
    {
        asCSVActiveLotID = asNowLot;
        asCSVLogFileName = "";
        iCSVRecordCT     = 0;
        slCSVPassCache->Clear();
        bCSVCacheLoaded  = false;
        WriteBarCodeLog(0, "CSV Compare: Lot change detected, reset cache (Lot=" + asNowLot + ")");
    }
}
//---------------------------------------------------------------------------
//Ifor 20260610 add: Barcode CSV Compare
// Append one row per IC: "No,Date,Time,BarcodeID,Result" to
//   D:\BarcodeFile\{LotID}_2DLogfile_{YYYYMMDD}_{HHMMSS}.csv
// File name is fixed on first write per Lot (timestamp = first record).
//---------------------------------------------------------------------------
void TfBarCode::AppendBarcodeCSVRecord(AnsiString aBarcodeID, AnsiString aScanTime, AnsiString aResult)
{
    if(TestIF_File.bEnableBarcodeCSVCompare == false)
        return;
    ...
    AnsiString asLot = asATCEvenLotID.Trim();
    if(asLot == "")
    {
        WriteBarCodeLog(0, "CSV Compare: Append skipped (LotID empty) BarcodeID=" + aBarcodeID);
        return;
    }
    if(aBarcodeID.Trim() == "")
    {
        WriteBarCodeLog(0, "CSV Compare: Append skipped (BarcodeID empty) Lot=" + asLot);
        return;
    }

    AnsiString asFolder = "D:\\BarcodeFile\\";
    if(!DirectoryExists(asFolder))
    {
        if(!ForceDirectories(asFolder))
        {
            WriteBarCodeLog(0, "CSV Compare: Append failed, folder create error - " + asFolder);
            return;
        }
    }

    // Parse aScanTime as TDateTime; fall back to Now() if invalid
    TDateTime ScanTime;
//    try
//    {
//        ScanTime = StrToDateTime(aScanTime);                                    //Ifor 20260610 異常
//    }
//    catch(...)
//    {
//        ScanTime = Now();
//    }
    ...
    int y, m, d, h, min, s;
    // 解析字串是否符合預期格式
    if(sscanf(aScanTime.c_str(), "%d/%d/%d %d:%d:%d", &y, &m, &d, &h, &min, &s) == 6)
    {
        try
        {
            // TDateTime 的整數部分是日期，小數部分是時間，所以可以直接相加
            ScanTime = TDateTime((unsigned short)y, (unsigned short)m, (unsigned short)d) +
                       TDateTime((unsigned short)h, (unsigned short)min, (unsigned short)s, (unsigned short)0);
        }
        catch(...)
        {
            // 捕捉防呆：例如日期解析出來是 2月30日 這種不合理時間
            ScanTime = Now();
        }
    }
    else
    ...
    // First write of this Lot -> fix the file name with current timestamp
    if(asCSVLogFileName == "")
    {
        TDateTime tNow = Now();
        AnsiString asDate = tNow.FormatString("yyyymmdd");
        AnsiString asTime = tNow.FormatString("hhnnss");
        asCSVLogFileName.sprintf("%s%s_2DLogfile_%s_%s.csv",
                                 asFolder.c_str(), asLot.c_str(), asDate.c_str(), asTime.c_str());
        iCSVRecordCT = 0;

        // Write header if file does not exist yet
        if(!FileExists(asCSVLogFileName))
        {
            FILE *pHdr = fopen(asCSVLogFileName.c_str(), "a+");
            if(pHdr != NULL)
            {
                fputs("No Unit,Date,Time,Barcode ID,Result\n", pHdr);
                fclose(pHdr);
            }
            else
            {
                WriteBarCodeLog(0, "CSV Compare: Header write failed - " + asCSVLogFileName);
                asCSVLogFileName = "";
                return;
            }
        }
        WriteBarCodeLog(0, "CSV Compare: New log file=" + asCSVLogFileName);
    }

    iCSVRecordCT++;

    // Normalize Result: "E" stays "E"; numeric bin keeps numeric (Error bin -> "E")
    AnsiString asWriteResult;
    if(aResult == "E" || aResult == "")
    {
        asWriteResult = "E";
    }
    else
    {
        int iBin = atoi(aResult.c_str());
        if(iBin >= iTestBinCount)
            asWriteResult = "E";
        else
            asWriteResult = AnsiString(iBin);
    }

    AnsiString asRow;
    asRow.sprintf("%d,%s,%s,%s,%s",
                  iCSVRecordCT,
                  ScanTime.FormatString("yyyymmdd").c_str(),
                  ScanTime.FormatString("hh:nn:ss").c_str(),
                  aBarcodeID.Trim().c_str(),
                  asWriteResult.c_str());

    FILE *pFile = fopen(asCSVLogFileName.c_str(), "a+");
    if(pFile != NULL)
    {
        fputs(asRow.c_str(), pFile);
        fputs("\n", pFile);
        fclose(pFile);
    }
    else
    {
        WriteBarCodeLog(0, "CSV Compare: Row append failed - " + asCSVLogFileName);
    }
}
//---------------------------------------------------------------------------
//Ifor 20260610 add: Barcode CSV Compare
// Password unlock for the alarm raised when a Barcode is found as Pass in CSV.
// Returns true if operator authenticated -> caller may continue test.
// Returns false if cancelled / wrong -> caller should treat as skip (Error Bin).
// Password source: General.ini  [VENDER] BarcodeCSV_Unlock  (default = "Hontech")
//---------------------------------------------------------------------------
bool TfBarCode::DoUnlockBarcodeCSVAlarm()
{
    AnsiString asUnlockPwd = CheckAndReadIniDataGeneral("VENDER", "BarcodeCSV_Unlock", AnsiString("Hontech"));

    fPassword->edPassword->Clear();
    fQwertyKey->ShowQwertyKey(fPassword->edPassword, N_NO_SYMBOL|N_NO_SPACE|N_PASSWORD);

    if(fPassword->edPassword->Text.Trim() == "")
    {
        WriteBarCodeLog(0, "CSV Compare: Unlock cancelled (no password entered)");
        fPassword->edPassword->Clear();
        return false;
    }
    if(fPassword->edPassword->Text != asUnlockPwd)
    {
        ShowMyMessage("Barcode CSV Compare: Password incorrect, skip test");
        WriteBarCodeLog(0, "CSV Compare: Unlock failed (wrong password)");
        fPassword->edPassword->Clear();
        return false;
    }
    fPassword->edPassword->Clear();
    WriteBarCodeLog(0, "CSV Compare: Unlocked by password, allow test");
    return true;
}
//---------------------------------------------------------------------------
//Ifor 20260625 add: Barcode CSV Compare
// 每次 LOT START 都強制換新 LOG 檔並重載比對快取。
// 解決重跑同一 LotID 時 EnsureCSVForCurrentLot 偵測不到 Lot 變更，
// 導致 LOG 不分檔、且舊批 Pass ID 一直沒被載入快取而不報警的問題。
//---------------------------------------------------------------------------
void TfBarCode::ResetBarcodeCSVForLotStart()
{
    if(TestIF_File.bEnableBarcodeCSVCompare == false)
    {
        return;
    }
    asCSVActiveLotID = asATCEvenLotID.Trim();   //對齊目前 Lot, 避免 EnsureCSVForCurrentLot 重複處理
    asCSVLogFileName = "";                       //強制本批產生新 LOG 檔(分檔)
    iCSVRecordCT     = 0;
    slCSVPassCache->Clear();
    bCSVCacheLoaded  = false;                    //下次比對時重載(含前幾批同 Lot 檔)
    WriteBarCodeLog(0, "CSV Compare: LotStart reset (Lot=" + asCSVActiveLotID + ")");
}
