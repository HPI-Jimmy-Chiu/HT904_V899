...
__published:    // IDE-managed Components
    ...
    TCheckBox *cbCheckEnable2DIDFunction;
    TCheckBox *cbCheckEnableBottom2DIDFunction;
    TCheckBox *cbBarcodeCSVCompare;                                             //Ifor 20260511 add: Barcode CSV Compare
        TCheckBox *cbUsePinInspection;
        TCheckBox *cbBarCodeInspReport;
        TCheckBox *cbBarCodeMultiRecipe;
        TCheckBox *cbAutoAdjustLight;
        TEdit *edAutoAdjustLightTimeOut;
    ...
    void __fastcall btnInitialGetBarcodeClick(TObject *Sender);
        void __fastcall cbUsePinInspectionClick(TObject *Sender);
private:    // User declarations
    bool bStartTest[4];
...
public:     // User declarations
    ...
    bool    bHasPin1Error          [BAR_CODE_COUNT];     //==> Eastsun 20260526 #026-4.Pin1 :KYEC
    ...
    bool DoBarcodeScanInShuttle_1(bool bErrorSkip=false);
    bool DoBarcodeScanInShuttle_2(bool bErrorSkip=false);
...
//    TStringList *list4;                                                         //wei 20160505 Barcode 比對Lot
    ...
    //==> Eastsun 20260527 整合#028.AAL.P02 BarcodeAutoAdjustLightTimeOut decl :KYEC
    TQPF_Timer BarcodeAutoAdjustLightTimeOut[2];
    //<== Eastsun 20260527 #028.AAL.P02
    ...
    //==> Eastsun 20260527 整合#028.AAL.P03 bCCDBarcodeAutoAdjustLight decl :KYEC
    bool bCCDBarcodeAutoAdjustLight[BAR_CODE_COUNT];
    //<== Eastsun 20260527 #028.AAL.P03
    ...
    //==> Eastsun 20260527 #BCIR.P10 method decls :KYEC
    void SaveInspReport(AnsiString sLot, AnsiString sRecipe, AnsiString sCode,
    ...
    void SaveInspReportEnd(AnsiString sLot, AnsiString sRecipe);                                //Sam 20240426 : Add BarCoder Inspection Report
    //<== Eastsun 20260527 #BCIR.P10
    //Ifor 20260512 add: Barcode CSV Compare
    TStringList *slCSVPassCache;
    bool bCSVCacheLoaded;
    void LoadBarcodeCSVCache();
    int DoBarcodeCSVCompare(AnsiString aBarcodeID);
    //Ifor 20260610 add: Barcode CSV Compare - per-IC record + alarm unlock
    AnsiString asCSVActiveLotID;
    AnsiString asCSVLogFileName;
    int        iCSVRecordCT;
    void EnsureCSVForCurrentLot();
    void AppendBarcodeCSVRecord(AnsiString aBarcodeID, AnsiString aScanTime, AnsiString aResult);
    bool DoUnlockBarcodeCSVAlarm();
    void ResetBarcodeCSVForLotStart();                                         //Ifor 20260625 add: 每次 LOT START 強制換 LOG 檔並重載比對快取
};
//---------------------------------------------------------------------------
