...
//const AnsiString sTrayName[eTrayCount]={"Auto1", "Auto2", "Auto3", "Auto4", "Auto5", "Auto6",      //Ifor 20260715 : align order with e6TrayName in MachineType.h
...
void __fastcall TfBinSel::ReadFile(bool bDelOffline, bool bChangeNeme, AnsiString sFileName)//kevin 20160928 新增舊bin 檔名轉換
{
    ...
    if(bDelOffline==true)
    {
        if(bPurgeOutAllDevice)                                                  //JerryYang 20251020 : 渠梁半清機功能
        {
        }
        else
        {
...
//      if(bTest==false) //kevin 20151119 不使用256 bin show form
        ...
        //==> Eastsun 20260508 整合: Ifor 20220426 add KYEC 要求一定要有一個 ART Retest
        //Ifor 20220426 add: KYEC 要求一定要有一個ART Retest
        //==>
        if(CUSTOMER_CODE==CC_KYEC_LEE && (tag==FT_ART || tag==OffT))
        {
            ARTBinCheck(tag);
        }
        //<==
    ...
    // EastSun 20260508 : A59 override before reading Bin file (KYEC) - if user wants FT/RT bin separately, force flags here so the SavePath logic below picks the correct file
//    if(CUSTOMER_CODE==CC_KYEC_LEE)    //Eastsun 20260710整合
...
void __fastcall TfBinSel::FormShow(TObject *Sender)
{
    ...
    {
        spbAOIBin->Visible = true;
    }
    else
    {
        spbAOIBin->Visible = false;
    }
    //這一行請保持在最下面!!-----------------
...
void __fastcall TfBinSel::FormDestroy(TObject *Sender)
{
    try
    {
        ...
        for(int i=eBinTypeTotal-1; i>=0; i--)                                   //Steven 20140510 : Secs Gem
        {
            ...
            if(sAOIBinTraySetting[i]!=NULL)
                delete sAOIBinTraySetting[i]   ;
        }
    }
    catch(...)
    {
               ...
               (IniConfig.bA02BinModelPrime==false && (IniConfig.bFTBin2RTBin==true || CosFunction.bDisableRTBinSet)))
            {
                ...
                if(FormSysTools->CheckSectionExist(GroupName))
                {
                    ...
                    if(bHasNewSetupData)                                        //kevin 20161108
                    {
                        ...
                        sAOIBinTraySetting[tag]->CommaText=FormSysTools->CheckAndReadIniData(GroupName,"AOIBinTraySetting", AnsiString(""));//Eastsun 20260316 : AOI 要分bin
                    }
                }
            }
            else
        ...
        {                                                                       //Ifor 20170316 (wei) add KYEC MRT Mode
            ...
            if(FormSysTools->CheckSectionExist(GroupName))
            {
                ...
                if(bHasNewSetupData)                                            //kevin 20161108
                {
                    ...
                    sAOIBinTraySetting      [tag]->CommaText=FormSysTools->CheckAndReadIniData(GroupName, GroupName + "AOIBinTraySetting", AnsiString(""));//Eastsun 20260316 : AOI 要分bin
                }
            }
        }
...
//    QQ1=QQ2;
    ...
    else
    {
        for(int i=0; i<iTestBinCount; i++)
        {
            ...
            BinSelect[tag].iAOICategData[i]             =(i<sAOIBinTraySetting[tag]->Count      )?atoi(sAOIBinTraySetting       [tag]->Strings[i].c_str()):ePosNoUse;   //Eastsun 20260316 : AOI 分BIN
...
void TfBinSel::SaveFunctionData(int tag)
{
    ...
    if(tag==eBinRT)
    {
           ...
           (IniConfig.bA02BinModelPrime==false && (IniConfig.bFTBin2RTBin==true || CosFunction.bDisableRTBinSet)))
        {
            ...
            FormSysTools->WriteIniData(GroupName,"AOIBinTraySetting", sAOIBinTraySetting    [tag]->CommaText);  //Eastsun 20260316 : AOI 要分bin
        }
        else
        {
            GroupName="Bin Func RT";
...
//---------------------------------------------------------------------------
//==> Eastsun 20260508 整合: Ifor 20220426 add KYEC 要求一定要有一個 ART Retest
void __fastcall TfBinSel::ARTBinCheck(int tag)
{
    ...
    {
        for(int i=0; i<3; i++)                                              //Ifor 20220426 add Pass Bin 不可以 Retest
        {
            if(MyBinPanel[tag]->iT6IsFail[i]==0)                            // rename: iIsFail→iT6IsFail
            {
                MyBinPanel[tag]->bT6ART[i]=0;                               // rename: bAutoRetest→bT6ART
            }
        }
...
//---------------------------------------------------------------------------
void __fastcall TfBinSel::spbAOIBinClick(TObject *Sender)
{
    if(fBinAOISel == NULL)
    {
        return;
    }
    fBinAOISel->Position = poScreenCenter;                                      //Ifor 20260714 : center modal so it is not shown off-screen; VCL resets Position to poDesigned after each show
    try
    {
        fBinAOISel->ShowModal();
        fBinAOISel->Save(iTestRunMode);                                         //Ifor 20260715 : save AOI bin tray setting on dialog exit
    }
    catch(Exception &e)
    {
        RecordProcess("spbAOIBinClick ShowModal fail : " + e.Message);
    }
}
//---------------------------------------------------------------------------
std::vector<int> TfBinSel::AOI_SplitToInt(int tag)
{
    std::vector<int> v;

    TStringList *list = new TStringList();
    list->Delimiter = ',';
    list->DelimitedText = MyBinPanel[tag]->edBinSetT3Pos->Text;

    for(int i=0;i<list->Count;i++)
        v.push_back(StrToIntDef(list->Strings[i],0));
