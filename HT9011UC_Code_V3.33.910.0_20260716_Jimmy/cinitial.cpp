...
#include "cSiteUseManager.h"  //AI(ht9045-v899) 20260405: anti-fragmentation HP site manager
#include "myEthercatmotor.h"                                                    //RogerYang 20250326 整合EtherCAT Mot
...
#include "MyLaneIo.h"
#include "note.h"
#include "ObserveMagazine.h"                                                    //Eastsun 20260515 F021: Magazine monitor
...
//                continue;
            ...
            S=Sen[i].Name;
            if(IsSafePLCIOInstall() &&                                          //Austin 20190531.01 增加PLC_IO模組及安全相關IO.
    ...
    while(1)
    {
        ...
        else                                                                    //Ztex 2024.12.19 Add Open Door Take Out Suck IC
        {
            if(Tri_Temp_Machine==1)
            {
                for(int i=0; i<7; i++)
                {
                    if(Tri_Temp_Machine==1 && IsSafePLCIOInstall()==true)//所有PLC都要的機制
...
void SetTechDataToProd_TrayArm()
{
    ...
    for(int i=0; i<2; i++)                                                      //JerryYang 20260615 : 
    {
...
//==> Eastsun 20260525 整合 : 從 a-side 取 GetShuttleSize (LaserSensorShuttle 需要)
