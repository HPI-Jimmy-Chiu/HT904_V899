...
__fastcall TMyTempPanel::TMyTempPanel(AnsiString Alias, int iTag)
{
    ...
    edPreOffset     =new TEdit(palTemp);       //箇干纕
    edPreOfsTime    =new TEdit(palTemp);       //干纕计
    edAfterOfs      =new TEdit(palTemp);       //干纕计挡干纕
    //<== Eastsun 20260526 #026-4.A7

    edKit_Low       =new TEdit(palTemp);                                        //JerryYang 20221003 Analog璶―穝糤Kit翴offset
    ...
    edKit_Low->Parent=palTemp;                                                  //JerryYang 20221003 Analog璶―穝糤Kit翴offset
    ...
    edKit_Mid->Parent=palTemp;                                                  //JerryYang 20221003 Analog璶―穝糤Kit翴offset
    ...
    edKit_Lowbase->Parent=palTemp;                                              //JerryYang 20221003 Analog璶―穝糤Kit翴offset
    ...
    edKit_Base->Parent=palTemp;                                                 //JerryYang 20221003 Analog璶―穝糤Kit翴offset
    ...
    edKit_HighBase->Parent=palTemp;                                             //JerryYang 20221003 Analog璶―穝糤Kit翴offset
    ...
    edOffset->OnMouseDown=edBySetupOffSetMouseDown; //Ifor 20231219 add Use By Setup Offset Range
    ...
    edInitTempOffset->OnMouseDown=edinitialMouseDown;                           //kevin 20210421 edBaseMouseDown; ->edinitialMouseDown 縒ミ 絛瞅
    ...
    edKit_Low->Tag       =iTag;                                                 //JerryYang 20221003 Analog璶―穝糤Kit翴offset
    edKit_Mid->Tag       =iTag;
...
__fastcall TMyTempPanel::~TMyTempPanel()
{
    try
    {
        ...
        delete edKit_Low;                                                       //JerryYang 20221003 Analog璶―穝糤Kit翴offset
        delete edKit_Mid;
...
//---------------------------------------------------------------------------
void __fastcall TMyTempPanel::SetParent(TTabSheet *tsTemp)                      //ヘ玡砞﹚
{
...
void __fastcall TMyTempPanel::SetEnable(bool bflag)
{
    ...
    edKit_Low->Enabled=bflag;                                                   //JerryYang 20221003 Analog璶―穝糤Kit翴offset
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    ...
    dTempMax=fTemp_Set->MaxTempSetting();                                       //Steven 20170427 : 肚诀ノ程放
...
//---------------------------------------------------------------------------
void __fastcall TMyTempPanel::edinitialMouseDown(TObject *Sender,               //kevin 20210421 縒ミoffset range
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    TEdit *Buffer;
    Buffer=(TEdit *)Sender;
...
//---------------------------------------------------------------------------
void __fastcall TMyTempPanel::edBySetupOffSetMouseDown(TObject *Sender,         //Ifor 20231219 add Use By Setup Offset Range
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    if(IniConfig.bL49UseBySetupOffsetRange==true)
        fQwertyKey->ShowQwertyKey((TEdit *)Sender, N_DOUBLE, 2, true, (double)IniConfig.iL49UseBySetupOffsetRange, (double)-IniConfig.iL49UseBySetupOffsetRange);
    else
        fQwertyKey->ShowQwertyKey((TEdit *)Sender, N_DOUBLE, 2, true, (double)InputLimit.iTempHigh, (double)InputLimit.iTempLow);
}
//---------------------------------------------------------------------------
