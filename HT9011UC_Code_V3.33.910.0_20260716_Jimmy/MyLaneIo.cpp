...
bool TLaneIO::IOInputBit(int Ring, int IP, int Port, int Bit, int iISABase, AnsiString Alias)                           //Sam 20230724 : 新增 PCI1203 IO 模組
{
    ...
    {                                                                           //Austin 20190218.01 增加PLC IO for CE
        ...
        if(IsSafePLCIOInstall()==true && Port>=wInputStartRegNum && Port<=wInputEndRegNum)
        {
            if(Ring<0 || Ring>=ePLCGrp_Count)
                return false;
            
            //HT9064LS we set two PLCIO modules, and setting the colum of "Lane" of 0 and 1 for module 1 and module second in Sensor_9046LS.DB
            int iSlave = Ring;
            if(iPLCSafetyVer==202204)
            {
                return bPLCInData[iSlave][Port-wInputStartRegNum+2][Bit];                                   //joe 20220406.01 debug safe plc
            }
            else
            {
                return bPLCInData[iSlave][Port-wInputStartRegNum+1][Bit];                                   //第1個Byte為系統狀態 預留
            }
        }
        else
        {
