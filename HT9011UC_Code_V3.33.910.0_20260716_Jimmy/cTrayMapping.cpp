...
static bool ShouldParseAutoBinLabelList()
{
    return IniConfig.bA69_ScanBinLabelToBinCodeEdit ||
    ...
    : TForm(Owner)
{
    ...
    listTrayIDByLot=new TStringList;                                            //JerryYang 20250120 : add
...
void __fastcall TfTrayMapping::FormShow(TObject *Sender)
{
    ...
    edLoaderCWSpeed->Enabled=(CUSTOMER_CODE==CC_ASE_CL);                        //JerryYang 20250120 : add
    if(USE_TRAY_MAPPING==etmDeviceRemain)
    {
        tsTrayCCD->TabVisible=false;
        //AI(ht9045-v899) 20260505: 原本只設 Visible=true，未設 false 導致 .dfm 預設如果是 true 會一直顯示。
        //                               改為明確跟隨 CosFunction。bTrayMapFromLoader=false 時同時強制關閉功能勾選
        cbEnableDeviceRemainLaser->Visible=CosFunction.bTrayDeviceCheckFromLoader;
        if(!CosFunction.bTrayDeviceCheckFromLoader)
            cbEnableDeviceRemainLaser->Checked=false;
        cbEnableTrayMapLaser->Visible=CosFunction.bTrayMapFromLoader;
        if(!CosFunction.bTrayMapFromLoader)
            cbEnableTrayMapLaser->Checked=false;
    }
    ...
    if(CUSTOMER_CODE==CC_ASE_CL)                                                //KaiChen 20200115 ：中壢日月光 TrayMapping
    {
        ...
        palTrayMapFunGroup->Visible=true;                                       //KaiChen 20200515 ：中壢日月光 TrayMapping
        palTrayMapGroup->Visible=true;                                          //KaiChen 20200515 ：中壢日月光 TrayMapping

        cbEnableTrayID              ->Visible=false;
        cbEnableTrayDeviceCnt       ->Visible=false;
        cbEnableDeviceRemain        ->Visible=false;
        if(USE_KEYENCE_EMPTY==3)                                                //JerryYang 20250120 : add
        {
        }
        else
        {
...
void __fastcall TfTrayMapping::FormClose(TObject *Sender, TCloseAction &Action)
{
    ...
    //AI(ht9045-v899) 20260505: 關閉表單時停止模擬並重置 LaserScan 狀態，避免下次開啟該表單時?留舊狀態繼續跑
    SetSimulationState(false);
    asErrorLaserScan="";
    iLaserScanTask=1;
    iAutoRetryCount=0;
...
void __fastcall TfTrayMapping::FormDestroy(TObject *Sender)
{
    ...
    delete listTrayIDByLot;                                                     //JerryYang 20250120 : add
...
void __fastcall TfTrayMapping::DoIniDataToForm()
{
    ...
    cbCheckTrayIDByLot->Checked     =TestIF_File.bCheckTrayIDBylot;             //JerryYang 20250120 : add
...
void __fastcall TfTrayMapping::spbSaveClick(TObject *Sender)
{
    ...
    WriteIniData(szDir, "Configuration", "bCheckTrayIDBylot",                   (cbCheckTrayIDByLot->Checked)?1:0);
...
void __fastcall TfTrayMapping::TimerDownCCDTrayConnectTimer(TObject *Sender)
{
       ...
       fAGV->IsSPIL_AMR())                                                      //JerryYang 20250521 : For AMR
    {
        ...
        {
            if(i==iKeyenceCoverTrayID_AUTO3 && TrayForm.bEnableAMR==false && CUSTOMER_CODE==CC_KYEC_LEE)
                continue;                                                       //Eastsun 20260710 Merge
            if(clntsckt_Tray_Keyence[i]->Active                           &&    //相同的IP跟Port就不要重新連線
               clntsckt_Tray_Keyence[i]->Address==edAddress_Keyence[i]->Text &&
               clntsckt_Tray_Keyence[i]->Port   ==edPort_Keyence[i]->Text)
            {
...
void __fastcall TfTrayMapping::TimerProcessTrayDataTimer(TObject *Sender)
{
       ...
       SystemStart==false)
    {
        ...
        {
            //AI(ht9045-v899) 20260505: Loop>1 時按鈕 Caption 顯示「Running n/N」進度
            if(iLoopCount>1)
            {
                AnsiString asProg;
                asProg.sprintf("Running %d/%d", iLoopNow+1, iLoopCount);
                if(btnSimulateRemainIC->Caption!=asProg)
                    btnSimulateRemainIC->Caption=asProg;
            }
            ...
            if(cbLaserScanMode->Text==AnsiString("Check Device"))
            {
                if(DoLaserScanFromLoader(eAOIType_TrayDeviceCheckLaser,false,true))
                {
                    ...
                    if(fTrayMapping->asErrorLaserScan!="")
                    {
                        ...
                        //AI(ht9045-v899) 20260505: Scan 異常後立即停止後續 Loop，避免使用者解除異常後自動續跑
                        RecordMsgLaser("SimulateRemainIC Stop by Error: "+fTrayMapping->asErrorLaserScan);
                        WriteLaserScanFlowLog("SimulateRemainIC Stop by Error: "+fTrayMapping->asErrorLaserScan);
                        DoLaserScanFromLoader(eAOIType_TrayDeviceCheckLaser,true,true);
                        cbAutoLearning->Checked=false;
                        SetSimulationState(false);
                        btnSimulateRemainIC->Caption="Stopped (Error)";
                        bTimerRun=false;
                        return;
                    }
            ...
            else    //Loader Tray Map
            {
                ...
                {
                    iLoopNow++;
                    //AI(ht9045-v899) 20260505: Tray Map 模式下 Scan 異常也立即停止 Loop
                    if(fTrayMapping->asErrorLaserScan!="")
                    {
                        RecordMsgLaser("SimulateRemainIC Stop by Error: "+fTrayMapping->asErrorLaserScan);
                        WriteLaserScanFlowLog("SimulateRemainIC Stop by Error: "+fTrayMapping->asErrorLaserScan);
    ...
    if(iBufferLenght>0)                                                         //13.09.30.01   klutter
    {
        ...
        if(EthernetBuffer!="")
        {
            ...
            else if(Tag==iKeyenceCoverTrayID_AUTO1)
            {
                ...
                if(ShouldParseAutoBinLabelList())  //JerryYang 20240821 : 自動讀取BIN LABEL
                {
                    ...
                    {
                        if(i==0 && IniConfig.bA69_ScanBinLabelToBinCodeEdit==false)
                        {
                        ...
                        else
                        {
                            if(fNote->fShow)
                            {
                                ...
                                if(((IniConfig.bA69_ScanBinLabelToBinCodeEdit ? sCoverTrayList->Count : (sCoverTrayList->Count-1))>iCnt) && iCnt>0)    //JerryYang 20250429 : fix Auto In/Out
                                {
...
{
    //AI(ht9045-v899) 20260505: Memo 上限放大到 100 行；Sim/AutoLearning 模式下同步寫入 D:\HT9045_Log\LaserScanSim\YYYY_MM\YYYY-MM-DD.log
    if(mLoaderDCCommLog->Lines->Count>100)
        mLoaderDCCommLog->Lines->Clear();
    mLoaderDCCommLog->Lines->Add(asStr);

    if(bSimulate || cbAutoLearning->Checked)
    {
        AnsiString asDir, asPath;
        asDir.sprintf("D:\\HT9045_Log\\LaserScanSim\\%04d_%02d", SystemYear, SystemMonth);
        MyForceDirectories(asDir, "TfTrayMapping::ShowMSDCCommLog");
        asPath.sprintf("%s\\%04d-%02d-%02d.log", asDir, SystemYear, SystemMonth, SystemDate);
        WriteDataToFile(asPath.c_str(), asStr.c_str());
    }
}
//---------------------------------------------------------------------------
void TfTrayMapping::RecordSimTaskMsg(AnsiString asMsg)                          //AI(ht9045-v899) 20260505: 只在 Sim/AutoLearning 模式下記錄流程細節，避免量產時 log 灌爆
{
    if(!(bSimulate || cbAutoLearning->Checked))
        return;
    asMsg=FormatDateTime("yyyy-mm-dd HH:MM:ss ", Now())+asMsg;
...
//---------------------------------------------------------------------------
void TfTrayMapping::WriteLaserScanFlowLog(AnsiString asMsg)                     //AI(ht9045-v899) 20260505: 永遠寫檔的流程 log，供客戶端量產回傳分析
{
    AnsiString asDir, asPath, asLine;
    asDir.sprintf("D:\\HT9045_Log\\LaserScanFlow\\%04d_%02d", SystemYear, SystemMonth);
    MyForceDirectories(asDir, "TfTrayMapping::WriteLaserScanFlowLog");
    asPath.sprintf("%s\\%04d-%02d-%02d.log", asDir, SystemYear, SystemMonth, SystemDate);
    asLine.sprintf("%04d-%02d-%02d %02d:%02d:%02d.%03d %s",
                   SystemYear, SystemMonth, SystemDate,
                   SystemHour, SystemMin, SystemSec, SystemMSec, asMsg);
    WriteDataToFile(asPath.c_str(), asLine.c_str());
    //AI(ht9045-v899) 20260505: Sim/AutoLearning 模式下同步顯示在 Memo 上方便現場觀察
    if(bSimulate || cbAutoLearning->Checked)
        ShowMSDCCommLog(asLine);
}
//---------------------------------------------------------------------------
AnsiString TfTrayMapping::DumpLaserScanState()                                  //AI(ht9045-v899) 20260505: 一次抓 MLoaderY/Cylinder/Sensor 狀態，方便客戶 log 還原當下情境
{
    AnsiString s;
...
bool TfTrayMapping::DoLaserScanFromLoader(int iAOIType,bool bIsFirst,bool bIsRunMoveOut)
{
    ...
    else
    {
        ...
        if(asErrorLaserScan!="")
        {
            //AI(ht9045-v899) 20260505: 錯誤中斷時也要還原 raw speed；EdgePush/TrayY_Fixer 隨台車移動不干涉但若已於 case 2500 ON 需關閉避免殘留夾盤
            if(INSTALL_OCR_YMot==eocrYMotInstal && iLaserScanTask>1)
            {
                GetAOI(iAOIType)->RestoreLoaderYSpeedRaw(iLdYSpdBackup);
                if(iLaserScanTask>=2500 && iLaserScanTask<8000)
                {
                    Cylinder[C_LoaderEdgePush].Off();
                    Cylinder[C_TrayY_Fixer].Off();
                }
                WriteLaserScanFlowLog(AnsiString("[ERROR_PATH] task=")+IntToStr(iLaserScanTask)+" err="+asErrorLaserScan+" "+DumpLaserScanState());
            }
            else
            {
                WriteLaserScanFlowLog(AnsiString("[ERROR_PATH] task=")+IntToStr(iLaserScanTask)+" err="+asErrorLaserScan);
            }
            return true;
        }
        switch(iLaserScanTask)
        {
            case 1://Initial
                ...
                {
                    //AI(ht9045-v899) 20260505: 防呆 — 客戶反應未做全機 Home 就跑 Check Device，這裡先檢查 MLoaderY HomeFlag，未 Home 則自動單軸 Home
                    //  HomeFlag==1 = 成功; ==2 = 失敗; ==0 = 進行中/未開始
                    if(MOT[MLoaderY].HomeFlag!=1)
                    {
                        WriteLaserScanFlowLog(AnsiString("[Task 1] MLoaderY not homed (HomeFlag=")+IntToStr(MOT[MLoaderY].HomeFlag)+"), auto-home start");
                        iLaserScanTask=10;                                      //AI(ht9045-v899) 20260505: 跳到自動 Home 子流程
                        return false;
                    }
                    //AI(ht9045-v899) 20260505: 只記 log 不做還原 (RestoreLoaderYSpeedRaw 已改為固定回 100%)
                    iLdYSpdBackup=GetAOI(iAOIType)->SetAndBackUpLoaderYSpeed(iLoaderYCWSpeedPerc);
                    WriteLaserScanFlowLog(AnsiString("[Task 1] Snapshot inst.spd=")+IntToStr(iLdYSpdBackup)+" set perc="+IntToStr(iLoaderYCWSpeedPerc)+"% bIsRunMoveOut="+IntToStr(bIsRunMoveOut)+" "+DumpLaserScanState());
                    if(bIsRunMoveOut)
                    {
                        iLaserScanTask=500;
                    }
                ...
                return false;
            //AI(ht9045-v899) 20260505: 自動 Home MLoaderY 子流程 (case 10/15/20)，避免客戶忘了全機 Home 就直接跑模擬
            //  HomeFlag 語意：==1 成功 / ==2 失敗 / ==0 進行中。實際驅動需呼叫 MotorHome(false) (Home() 只是底層觸發不會更新 HomeFlag)
            case 10:
                if(MOT[MLoaderY].HomeFlag==1)
                {
                    WriteLaserScanFlowLog("[Task 10] MLoaderY already homed, back to Task 1");
                    iLaserScanTask=1;
                    return false;
                }
                //AI(ht9045-v899) 20260505: Home 前只需關 C_LoaderUpPress
                //  C_LoaderUpPress = 吸料區壓料桿，固定鎖在機台 (不隨台車移動)，台車要移動必須先退開避免撞機
                //  C_LoaderEdgePush / C_TrayY_Fixer = 固定在台車上、隨台車移動，Home 不會干涉，故不動作
                Cylinder[C_LoaderUpPress].Off();
                iAutoHomeTimeout.SetSecAndOn(2);                                //先等 2 秒讓壓桿退回到位
                WriteLaserScanFlowLog(AnsiString("[Task 10] C_LoaderUpPress OFF, wait 2s ")+DumpLaserScanState());
                iLaserScanTask=15;
                return false;
            case 15:
                if(!iAutoHomeTimeout.Off())                                     //AI(ht9045-v899) 20260505: 等 2 秒到 timeout (.Off() 從 true→false 表示時間到)
                    return false;
                //再次確認壓桿真的離開 (若 OnSensor 還亮且 OffSensor 沒亮，代表氣缸沒退、可能氣壓不足或感測器異常 → 中止避免撞機)
                if(Cylinder[C_LoaderUpPress].OnSensor() &&
                   !Cylinder[C_LoaderUpPress].OffSensor())
                {
                    asErrorLaserScan="C_LoaderUpPress sensor still ON after 2s. 台車不可移動，請檢查氣壓或壓桿氣缸後再試。";
                    WriteLaserScanFlowLog("[Task 15] ERROR C_LoaderUpPress sensor still ON, abort home (avoid hit)");
                    return false;
                }
                MOT[MLoaderY].MotorInitial();                                   //AI(ht9045-v899) 20260505: 重置 Home Task=1 並設 HomeFlag=0
                iAutoHomeTimeout.SetSecAndOn(30);
                WriteLaserScanFlowLog(AnsiString("[Task 15] C_LoaderUpPress confirmed retracted, MotorInitial called ")+DumpLaserScanState());
                iLaserScanTask=20;
                return false;
            case 20:
                //AI(ht9045-v899) 20260505: 必須呼叫 MotorHome(false) 才會跑狀態機更新 HomeFlag；之前用 Home() 會每 loop 重置 HomeReset 導致馬達抖動
                MOT[MLoaderY].MotorHome(false);
                if(MOT[MLoaderY].HomeFlag==1)                                   //成功
                {
                    WriteLaserScanFlowLog(AnsiString("[Task 20] MLoaderY home OK, Pos=")+IntToStr(MOT[MLoaderY].ReadPos()));
                    iLaserScanTask=1;
                }
                else if(MOT[MLoaderY].HomeFlag==2)                              //AI(ht9045-v899) 20260505: 馬達自己 retry 用完判定失敗 → 立刻報錯不用等外部 timeout
                {
                    asErrorLaserScan="MLoaderY auto-home failed (motor reported HomeFlag=2). Please run Home All manually and check home sensor / servo alarm.";
                    WriteLaserScanFlowLog(AnsiString("[Task 20] ERROR motor reported HomeFlag=2 ")+DumpLaserScanState());
                }
                else if(iAutoHomeTimeout.Off())
                {
                    asErrorLaserScan="MLoaderY auto-home timeout (30s). Please run Home All manually.";
                    WriteLaserScanFlowLog(AnsiString("[Task 20] ERROR auto-home external timeout HomeFlag=")+IntToStr(MOT[MLoaderY].HomeFlag));
                }
                return false;
            case 500:
                //AI(ht9045-v899) 20260505: 台車要移動前一律放開 C_LoaderUpPress (固定鎖在機台的壓桶)，避免壓著台車?機。不 Alarm。
            ...
            case 2500:                                                          //RogerYang 20250830 : Add殘料檢(台車)
                if(INSTALL_OCR_YMot==eocrYMotInstal)
                {
                    //AI(ht9045-v899) 20260505: 台車要移動前一律放開 C_LoaderUpPress (固定鎖在機台的壓桶)，不 Alarm
                    Cylinder[C_LoaderUpPress].Off();
                    //AI(ht9045-v899) 20260505: 在台車進堆疊區前先夾盤 (LoaderEdgePush + TrayY_Fixer)，避免進站途中盤面位置漂移
                    Cylinder[C_LoaderEdgePush].On();
                    Cylinder[C_TrayY_Fixer].On();
                    //AI(ht9045-v899) 20260505: 防呆：再次鎖定 MLoaderY Speed 為 iLoaderYCWSpeedPerc，防止外部干擾
                    MOT[MLoaderY].SetSpeed(iLoaderYCWSpeedPerc);
                    WriteLaserScanFlowLog(AnsiString("[Task 2500] Cyl ON + lock spd perc=")+IntToStr(iLoaderYCWSpeedPerc)+"% "+DumpLaserScanState());
                    iLaserScanTask=2600;
                }
                else
                    iLaserScanTask=3000;
            ...
            case 2600:                                                          //RogerYang 20250830 : Add殘料檢(台車)
                //AI(ht9045-v899) 20260505: 目標位置改為 SurePos (吸料等待位)；夾盤已於 case 2500 提早 ON，此處不再重複；速度於 case 2500 已鎖定
                if(MOT[MLoaderY].MotorMove(Prod.iMLoaderYSurePos))
                {
                    WriteLaserScanFlowLog(AnsiString("[Task 2600] Arrived SurePos tgt=")+IntToStr(Prod.iMLoaderYSurePos)+" actual="+IntToStr(MOT[MLoaderY].ReadPos()));
                    iLaserScanTask=3000;
                }
                break;
            case 3000:
            ...
            case 7000:
                if(INSTALL_OCR_YMot==eocrYMotInstal)
                {
                    //AI(ht9045-v899) 20260505: 防呆：DoMoveOut 內部會再驅動 MLoaderY，先再次鎖速避免被外部覆蓋
                    MOT[MLoaderY].SetSpeed(iLoaderYCWSpeedPerc);
                    WriteLaserScanFlowLog(AnsiString("[Task 7000] Re-lock spd perc=")+IntToStr(iLoaderYCWSpeedPerc)+"% before DoMoveOut "+DumpLaserScanState());
                }
            ...
            case 8000:
                if(INSTALL_OCR_YMot==eocrYMotInstal)                            //RogerYang 20251028 : Add
                {
                    Cylinder[C_LoaderEdgePush].Off();
                    Cylinder[C_TrayY_Fixer].Off();
                    WriteLaserScanFlowLog(AnsiString("[Task 8000] Cyl OFF ")+DumpLaserScanState());      //AI(ht9045-v899) 20260505: log LaserScan cylinder off state before finish
                }
                iLaserScanTask=9999;
                return false;
            case 9999:// finish
                if(INSTALL_OCR_YMot==eocrYMotInstal)                            //RogerYang 20250906 : Add for MLoaderY
                {
                    //AI(ht9045-v899) 20260505: 用 raw speed 還原 (對應 case 1 的真備份)；舊寫法把 raw 當百分比再餵會導致速度變極小
                    GetAOI(iAOIType)->RestoreLoaderYSpeedRaw(iLdYSpdBackup);
                    WriteLaserScanFlowLog(AnsiString("[Task 9999] Done. Restore raw spd=")+IntToStr(iLdYSpdBackup)+" "+DumpLaserScanState());
                }
                return true;
            default:
                //AI(ht9045-v899) 20260505: 異常 default 也要還原 raw speed；夬盤氣缸?在 case 2500~7999 期間才需關閉 (case 2500 才 ON)
                if(INSTALL_OCR_YMot==eocrYMotInstal)
                {
                    GetAOI(iAOIType)->RestoreLoaderYSpeedRaw(iLdYSpdBackup);
                    if(iLaserScanTask>=2500 && iLaserScanTask<8000)
                ...
                WriteLaserScanFlowLog(AnsiString("[DEFAULT] Wrong Step=")+IntToStr(iLaserScanTask)+" "+DumpLaserScanState());
...
{
    static bool _b=true;
    //AI(ht9045-v899) 20260505: 與 btnMoveInClick(SurePos) 對稱，MoveOut 應退到回車位 CarPos，原來用 OCRPos 在這畫面上沒意義
    TrayMoveOut(_b, 0, Prod.iMLoaderYCarPos);
    _b=!_b;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void TfTrayMapping::ClearTrayIDByLot()                                          //JerryYang 20250120 : add
{
    fTrayMapping->listTrayIDByLot->Clear();
    RecordProcess("Clear Tray ID list.");
    fTrayMapping->listTrayIDByLot->SaveToFile(asTrayIDByLot);
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
bool TfTrayMapping::DoTrayIDCheck(AnsiString Str)                               //JerryYang 20250120 : add
{
    int iIndex=0,iPos=0;
