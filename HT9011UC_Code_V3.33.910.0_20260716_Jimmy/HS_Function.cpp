...
#include "ProductionInfo.h"                                                     //AI(BVL-3766) 20260420: rewrite GTK info.txt RunSite when site on/off
#include "cObserver.h"
#include "aoutarm.h"
...
void __fastcall TFormHS::TimerAutoBackupTimer(TObject *Sender)
{
    ...
    static bool bUpdateSwVer=true;                                              //KevinCheng 20251120 : N17 SW Version
    bool bHasAlarm=false;
    bool bResult=false;                                                         //KevinCheng 20251120 : N17 SW Version
    int iSaveTimePerHour=10000;                                                 //KevinCheng 20251120 : N17 SW Version
    int iNowTime;
    AnsiString str, sFileName1;                                                 //KevinCheng 20251120 : N17 SW Version
    AnsiString strFileName="", strPath;                                         //KevinCheng 20251120 : N17 SW Version
    double dValue=0.0;
...
//    static bool bIdelTrigger_0000AM=false;
    ...
    if(IniConfig.bN17UploadSwVersion)
    {
        if(bUpdateSwVer)
        {
            if(iNowTime%iSaveTimePerHour>3)
            {
                bUpdateSwVer=false;

                sFileName1.sprintf("%s--%s.txt", IniConfig.SocketHandlerID, fMain->StatusBar1->Panels->Items[3]->Text);
                strFileName=asSWVersionPath+sFileName1;                             //Steven 20230215 : 存檔路徑加上年月
                if(FileExists(strFileName))
                {
                    strPath.sprintf("%s\\%s", IniConfig.asN17SWVersionPath, sFileName1);
                    bResult=CopyFile(strFileName.c_str(), strPath.c_str(), false);
                    if(bResult==false)
                    {
                        ShowMyMessage("Uploaded SW version file error\r\nPlease check the path of N-17.","上傳SW version file失敗\r\n請檢查N-17路徑是否存在");
                    }
                    else
                    {
                        RecordProcess("Uploaded SW version file successfully.");   //Steven 20190722 : add TSV log
                    }
                }
            }
        }
...
void TFormHS::CalculateUploadToHostIntervalTimeFunction()                       //Ifor 20160302 Add FTP Upload Log To Host start
{
    ...
    if(ctEnd-ctStart>=iIntervalMSec)
    {
        ...
        {
            //==> Eastsun 20260511 整合: Ifor 2024xxxx KYEC RecordLog 條件記錄
//            RecordLog_HS();
            if(CUSTOMER_CODE==CC_KYEC_LEE)
            {
                if(HasICUnderMachine()  ||
                   HasAnyICInMachine()  )
                    RecordLog_HS(false, true);
            }
            else
            {
                RecordLog_HS();
...
//---------------------------------------------------------------------------
void __fastcall TFormHS::RecordLog_HS(bool bAnyWaySend, bool bRealTime) //Eastsun 20260511 整合: Ifor 20250123 add: KYEC 要求一直紀錄 //ChungHung 20151117 modify for KYEC_HS start
{
    TDateTime dtPresent=Now();
    static TDateTime dtBefore=dtPresent;
    if(bSysLotStart==true || bRealTime==true)                                               //Eastsun 20260511 整合: Ifor 20250123 add: KYEC 要求一直紀錄
    {
//        if(bEPLogStart_KYEC)                                                    //Ifor 20160329 精簡 HISI FTP Log 流程
//        {
//            if(CUSTOMER_CODE!=CC_KYEC_LEE)  // Eastsun 20260511 整合: Ifor 20250123 KYEC 要求 Contact 才紀錄
//            {
...
//        }
        ...
        if(bTempLogStart_KYEC || bRealTime==true)                                           //Eastsun 20260511 整合: Ifor 20250123 add: KYEC 要求一直紀錄
        {
            sTemperatureLogFile=GetLastFileLogName_HS(1);
            RecordTemperatureLog_HS(sTemperatureLogFile);
        }

        if(bESDLogStart_KYEC || bRealTime==true)                                            //Eastsun 20260511 整合: Ifor 20250123 add: KYEC 要求一直紀錄
        {
            sESDLogFile=GetLastFileLogName_HS(2);
            RecordESDLog_HS(sESDLogFile);
        }

        //==> Eastsun 20260511 整合: Ifor 20250123 KYEC FTP UP Load GroundMan Log File
        if(USE_GROUND_MAN>0 && bRealTime==true)
        {
            sGroundManLogFile=GetLastFileLogName_HS(7);
            RecordGroundManLog_HS(sGroundManLogFile);
        }
        //<== Eastsun 20260511 整合
    ...
    else if((SystemStart==false && bSysLotStart==false) || bAnyWaySend)
    {
        if(IniConfig.bN10Enable_FTPUpLoadLog==true)                             //上傳
        {
            ...
            {
                //==> Eastsun 20260511 整合: Ifor 2024xxxx KYEC 不在這裡上傳 temperature
                if(CUSTOMER_CODE!=CC_KYEC_LEE)
...
AnsiString __fastcall TFormHS::GetLastFileLogName_HS(int iType)                 //iType 0:EP 1:Temperature 2:ESD 3:ATCEvent 5:ArmTestInfoEvent_Log 6:ChangeEvent_Log
{
    ...
    try
    {
        ...
        {
            case 0:
                //==> Eastsun 20260511 整合: Ifor 20250117 add:KYEC 要求修改檔案名稱 (case 0 EP_Log)
                if(CUSTOMER_CODE==CC_KYEC_LEE)
                {
                    aFileName.sprintf("%s_%s_EP_%s", IniConfig.sMachineType, IniConfig.SocketHandlerID, GetDateInfoByString());
                }
                else if(CheckIniData(sRecord_FilePath, "LastFileName", "EP_Log"))
                //<== Eastsun 20260511 整合
                {
                ...
                break;
            case 1:
                //==> Eastsun 20260511 整合: Ifor 20250117 add:KYEC 要求修改 Temperature_Log 檔案名稱
                if(CUSTOMER_CODE==CC_KYEC_LEE)
                {
                    aFileName.sprintf("%s_%s_Temperature_%s", IniConfig.sMachineType, IniConfig.SocketHandlerID, GetDateInfoByString());
                }
                else if(CheckIniData(sRecord_FilePath, "LastFileName", "Temperature_Log"))
                //<== Eastsun 20260511 整合
                {
                ...
                break;
            case 2:
                //==> Eastsun 20260511 整合: Ifor 20250117 add:KYEC 要求修改 ESD_Log 檔案名稱
                if(CUSTOMER_CODE==CC_KYEC_LEE)
                {
                    aFileName.sprintf("%s_%s_ESD_%s", IniConfig.sMachineType, IniConfig.SocketHandlerID, GetDateInfoByString());
                }
                else if(CheckIniData(sRecord_FilePath, "LastFileName", "ESD_Log"))
                //<== Eastsun 20260511 整合
                {
                ...
                break;
            case 3:
                //==> Eastsun 20260511 整合: Ifor 20250117 add:KYEC 要求修改 ATCEven_Log 檔案名稱
                if(CUSTOMER_CODE==CC_KYEC_LEE)
                {
                    aFileName.sprintf("%s_%s_ATCEven_%s", IniConfig.sMachineType, IniConfig.SocketHandlerID, GetDateInfoByString());
                }
                else if(CheckIniData(sRecord_FilePath, "LastFileName", "ATCEven_Log"))
                //<== Eastsun 20260511 整合
                {
                ...
                break;
            case 5:                                                             //Ifor 20190912 :add 海思 V02.30 版 Record Torque
                //==> Eastsun 20260511 整合: Ifor 20250117 add:KYEC ArmTestInfoEvent 命名
                if(CUSTOMER_CODE==CC_KYEC_LEE)
                {
                    aFileName.sprintf("%s_%s_ArmTestInfoEvent_%s", IniConfig.sMachineType, IniConfig.SocketHandlerID, GetDateInfoByString());
                }
                else if(CheckIniData(sRecord_FilePath, "LastFileName", "ArmTestInfoEvent_Log"))
                //<== Eastsun 20260511 整合
                {
                ...
                break;
            case 6:                                                             //Ifor 20191002 : add Change Log By Lot
                //==> Eastsun 20260511 整合: Ifor 20250117 add:KYEC ChangeEvent 命名
                if(CUSTOMER_CODE==CC_KYEC_LEE)
                {
                    aFileName.sprintf("%s_%s_ChangeEvent_%s", IniConfig.sMachineType, IniConfig.SocketHandlerID, GetDateInfoByString());
                }
                else if(CheckIniData(sRecord_FilePath, "LastFileName", "ChangeEvent_Log"))
                //<== Eastsun 20260511 整合
                {
...
void __fastcall TFormHS::RecordEPLog_HS(AnsiString aFileName)
{
    ...
    //==> Eastsun 20260511 整合: Ifor 20250117 KYEC EP_Log 月份子資料夾路徑覆寫 + ReadMultiEP
    if(CUSTOMER_CODE==CC_KYEC_LEE)  //Eastsun 20260525 INSTALL_DOUBLE_EP_3 整合
    {
        AnsiString sDirPath;
        sDirPath.sprintf("D:\\HandlerLog\\EP_Log\\%04d\\%02d\\", SystemYear, SystemMonth);
        Check_RecordFolder(sDirPath, true);
        FormHS->ReadMultiEP();
        sFullFileName = sDirPath + aFileName;
    }
    //<== Eastsun 20260511 整合
    ...
    //AI(ht9045-v899) 20260504: port from V896 (Ifor 20250417). Trigger Multi EP 16ch read
    //   so anMultiEPData is populated for KYEC log append below. ReadMultiEP() is no-op
    //   when the actual SwMultiEp pressure route is not active.
    ReadMultiEP();
    ...
    if(INSTALL_DOUBLE_EP==DOUBLE_EP_NORMAL || INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)        //AI(ht9045-v899) 20260504: was ==true(==1); add Multi EP Dual EP read/log.
    {
    ...
    if(INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI && TestIF_File.bIndEPSLK==true)
    {
        if(IndexStatus==Z1Down_Z2Up)
            sRecord=sTime+",ARM1,NA,NA,NA,NA"+strTester_STATUS+",NA,NA,NA,NA";
        else if(IndexStatus==Z1Up_Z2Down)
            sRecord=sTime+",ARM2,NA,NA,NA,NA"+strTester_STATUS+",NA,NA,NA,NA";
        else
            sRecord=sTime+",None,NA,NA,NA,NA"+strTester_STATUS+",NA,NA,NA,NA";
    }
    else
...
//       CUSTOMER_CODE==CC_KYEC_LEE)                                            //Ifor 20190905 :add 海思 V02.30 版  上傳Server資料增加Lot ID欄位
    ...
    //==> Eastsun 20260511 整合: Ifor 2024xxxx KYEC EP record 加 anMultiEPData
    if(CUSTOMER_CODE==CC_KYEC_LEE)
        sRecord=sRecord+anMultiEPData;
    WriteDataToFile(sFullFileName.c_str(), sRecord.c_str());
...
//---------------------------------------------------------------------------
//AI(ht9045-v899) 20260504: port from V896 (Ifor 20250417). Read Multi EP 16ch from APAX-5070.
//   iMapping[16] selects per-ch dual/site (true=dual, false=site).
//   Layout (matches APAX_WriteData wData2 mapping):
//     ch[0,1]   Arm1 Dual1/2     -> strData[34,35]
//     ch[2..5]  Arm1 Site1..4    -> strData[1..4]
//     ch[6,7]   Arm1 Dual3/4     -> strData[36,37]
//     ch[8,9]   Arm2 Dual1/2     -> strData[38,39]
//     ch[10..13] Arm2 Site1..4   -> strData[5..8]
//     ch[14,15] Arm2 Dual3/4     -> strData[40,41]
//   ADAMTCP_ReadReg call left commented (matches V896 source state); enable when wiring confirmed.
bool __fastcall TFormHS::ReadMultiEP()                                          //Eastsun 20260710整合 之前版本有問題
{
    char* ip = "172.16.8.112";
    WORD buffer[36];
    bool iMapping[]={true,true,false,false,false,false,true,true,
                     true,true,false,false,false,false,true,true};
    //int ibuffer[36];
    double ibuffer[36];
    double ibuffer1[36];
    //int iKpaTransferKG[36];
    double iKpaTransferKG[36];
    AnsiString strData[66];
    int iRet=-1;
    ...
    {
        iRet=ADAMTCP_ReadReg(ip, 4, 33, 24, buffer);                            //此處需要對應記憶體位置 要到研華軟體 上面看被設到哪邊
        if(iRet==0 || iRet==817)
        {
            ZeroMemory(strData, sizeof(strData));
            for(int i=0; i<16; i++)
            {
                //(((40073-36922)/5343)*0.374)+0.056
                ibuffer1[i]=(double)buffer[i];

                ibuffer[i]=((((ibuffer1[i]-36922)/5343)*0.374)+0.056)*1000;     //Frank 20260710 ADD//Eastsun 20260710整合 此處真實數值內差得來的相近值
                iKpaTransferKG[i]=MultiTransferKG(ibuffer[i],iMapping[i]);
            }
            strData[0]=fContact->edSetKg->Text;
            strData[33]=fContact->edDoubleForce->Text;
            if(IndexStatus==Z1Down_Z2Up) //ARM1                                 //Eastsun 20260625  京元要求 使用ARM1時 ARM2的值他們不想看到也不想看到零
            {
                strData[1]=FormatFloat("0.00", iKpaTransferKG[2]);
                strData[2]=FormatFloat("0.00", iKpaTransferKG[3]);
                strData[3]=FormatFloat("0.00", iKpaTransferKG[4]);
                strData[4]=FormatFloat("0.00", iKpaTransferKG[5]);

                strData[34]=FormatFloat("0.00", iKpaTransferKG[0]);
                strData[35]=FormatFloat("0.00", iKpaTransferKG[1]);
                strData[36]=FormatFloat("0.00", iKpaTransferKG[6]);
                strData[37]=FormatFloat("0.00", iKpaTransferKG[7]);
            ...
            else if(IndexStatus==Z1Up_Z2Down) //ARM2
            {
                strData[1]="";
                strData[2]="";
                strData[3]="";
                ...
                strData[5]=FormatFloat("0.00", iKpaTransferKG[10]);
                strData[6]=FormatFloat("0.00", iKpaTransferKG[11]);
                strData[7]=FormatFloat("0.00", iKpaTransferKG[12]);
                strData[8]=FormatFloat("0.00", iKpaTransferKG[13]);

                strData[38]=FormatFloat("0.00", iKpaTransferKG[8]);
                strData[39]=FormatFloat("0.00", iKpaTransferKG[9]);
                strData[40]=FormatFloat("0.00", iKpaTransferKG[14]);
                strData[41]=FormatFloat("0.00", iKpaTransferKG[15]);
            }
            else
            {
                strData[1]=FormatFloat("0.00", iKpaTransferKG[2]);
            ...
            for(int j=0; j<66; j++)
            {
                anMultiEPData=anMultiEPData+strData[j]+",";
            }
            for(int i=0; i<16; i++)
            {
                anMultiEPData += FormatFloat("0.00", ibuffer[i]) + ",";
            }
        }
        else
        {
            ShowMessage("讀取 APAX-5017 Slot " + IntToStr(5) + " 失敗");
        }
    }
    else
    {
...
void __fastcall TFormHS::RecordTemperatureLog_HS(AnsiString aFileName)
{
    ...
    //==> Eastsun 20260511 整合: Ifor 20250117 KYEC Temperature_Log 月份子資料夾路徑覆寫
    if(CUSTOMER_CODE==CC_KYEC_LEE)
    {
        AnsiString sDirPath;
        sDirPath.sprintf("D:\\HandlerLog\\Temperature_Log\\%04d\\%02d\\", SystemYear, SystemMonth);
        Check_RecordFolder(sDirPath, true);
        sFullFileName = sDirPath + aFileName;
    }
    //<== Eastsun 20260511 整合
    ...
    //==> Eastsun 20260511 整合: Ifor 2024xxxx KYEC 有 IC 才寫 Temperature_Log
    if(CUSTOMER_CODE==CC_KYEC_LEE)
    {
...
void __fastcall TFormHS::RecordESDLog_HS(AnsiString aFileName)
{
    ...
    //==> Eastsun 20260511 整合: Ifor 2024xxxx KYEC ESD_Log 月份子資料夾路徑覆寫
    if(CUSTOMER_CODE==CC_KYEC_LEE)
        sDirPath.sprintf("D:\\HandlerLog\\ESD_Log\\%04d\\%02d\\", SystemYear, SystemMonth);
    //<== Eastsun 20260511 整合
...
//           CUSTOMER_CODE==CC_ASE_KaohSiung)                                   //kevin 20191212 add
    {
            //==> Eastsun 20260511 整合: Ifor 2024xxxx KYEC + KLT==false 加 OSAT/Antenna 1~8
            if(CUSTOMER_CODE==CC_KYEC_LEE && bEnable_KLT_Function==false)
                sRecord=sRecord+","+"Lot ID,OSAT Lot number,Antenna1,Antenna2,Antenna3,Antenna4,Antenna5,Antenna6,Antenna7,Antenna8";
...
{
    //==> Eastsun 20260511 整合: Ifor 20250117 add: ATCEven_Log 月份子資料夾路徑 + KYEC 覆寫
    //確認資料夾是否存在
    AnsiString sDirPath=AnsiString("D:\\HandlerLog");
    Check_RecordFolder(sDirPath);
    sDirPath=sDirPath+"\\ATCEven_Log\\";
    Check_RecordFolder(sDirPath,true);
    if(CUSTOMER_CODE==CC_KYEC_LEE)                                              //Ifor 20250117 add:KYEC ATCEven_Log 月份子資料夾
    {
        sDirPath.sprintf("%s%04d%s%02d%s", sDirPath, SystemYear, "\\", SystemMonth, "\\");
    }
    //<== Eastsun 20260511 整合
    if(bEnableAutoMotive==true)                                                 //Ifor 20221024 add: KLT Auto Motive Function
    {
        //==> Eastsun 20260511 整合: Ifor 20250117 modify: 改傳 sDirPath（絕對路徑）取代字面 "\\ATCEven_Log"
        RecordAutoMotive(sDirPath, aFileName, "", eReocrd_ATCEven);
        //<== Eastsun 20260511 整合
    }
}
//------------------------------------------------------------------------------
...
{
    AnsiString sRecord="";
    //==> Eastsun 20260511 整合: Ifor 20250117 add: 路徑檢查 + KYEC ArmTestInfo 月份子資料夾路徑覆寫
    //確認資料夾是否存在
    AnsiString sDirPath=AnsiString("D:\\HandlerLog");
    Check_RecordFolder(sDirPath);
    sDirPath=sDirPath+"\\ArmTestInfo\\";
    Check_RecordFolder(sDirPath, true);
    if(CUSTOMER_CODE==CC_KYEC_LEE)
    {
        sDirPath.sprintf("D:\\HandlerLog\\ArmTestInfo\\%04d\\%02d\\", SystemYear, SystemMonth);
        Check_RecordFolder(sDirPath, true);
    }
    AnsiString sFullFileName=sDirPath+aFileName;
    //<== Eastsun 20260511 整合
    ...
    if(fFTP.Connect(asUserID, asPassword, asHost, 30000, NMOS_AUTO, IniConfig.bN10FtpPassive, IniConfig.iN10FtpPort))
    {
        ...
        if(FileExists(FileInfo().PathCombin(asDirPath, sFileName)))             //Sam 20180802 (wei) : 增加保護
        {
            ...
            else
            {
                ...
                {
                    AnsiString Cur;
                    Cur.sprintf("FTP Upload Fail LocalFile: %s, RemoteFile: %s%s", FileInfo().PathCombin(asDirPath, sFileName), asFtpUplaodPath, sFileName);
                    RecordProcess(Cur);
                }
                //<== Eastsun 20260511 整合
            }
...
//            ShowMyMessage("Bin setting error, please reconfigure the Bin data.", "Bin設定錯誤，請重新設定Bin資料。");
    ...
    //==> Eastsun 20260527 整合#027-2.MR.H1 BarcodeRecipe required :KYEC
    if(TestIF_File.bEnableBarCode==true &&
       BAR_CODE_INSTALL==ebctUseCCDMode &&
       TestIF_File.bBarCodeMultiRecipe==true)
    {
        if(fLotInfo->edtBarcodeRecipe->Text=="")
        {
            fLotInfo->sbSECSLotEnd->Down=true;
            fLotInfo->edtBarcodeRecipe->Enabled=true;
            ShowMyMessage("Please Enter Barcode Recipe!!");
            return true;
        }
    }
    //<== Eastsun 20260527 整合#027-2.MR.H1

    //==> Eastsun 20260511 整合: Ifor 20241217 add: KYEC MTL File Check
    if(CUSTOMER_CODE==CC_KYEC_LEE)
    {
        #ifndef SOFT_SIMULTE
        if((bCustomerForMTKVersion==true && FormHS->CheckMTKFileName()==false) ||
           (bCustomerForMTKVersion==false && FormHS->CheckMTKFileName()==true))
        {
            ShowMyMessage("Please Reboot The Handler HMI for Version Change");
            return true;
        }
        #endif
    }
    ...
    {
        if(fLotInfo->edtSysLotID->Text=="" || fLotInfo->edtSysOperatorID->Text=="")
        {
            ShowMyMessage("Please Enter LotID and Operator ID!!");
            return true;
        }
        else if(RunInfo.bLotStart==false)
        {
    ...
    if(CosFunction.bPickerLifeAlmNeedOneCycle &&                                //AI(ht9045-config) 20260521 (RogerYang) : SCC吸嘴壽命報警OneCycle優化
       IniConfig.bO20InOutArmPickerLifeTimeCount)
    {
        bool bAnyOver=false;
        for(int i=0; i<MAX_ARM_Row && !bAnyOver; i++)
            for(int j=0; j<MAX_ARM_Col && !bAnyOver; j++)
        ...
        for(int i=0; i<MAX_ARM_Row && !bAnyOver; i++)
            for(int j=0; j<MAX_ARM_Col && !bAnyOver; j++)
                if(TestIF_File.OutArmPickerLifeCnt[i][j]>=TestIF_File.InOutArmLifeCntSet)
                    bAnyOver=true;
        for(int i=0; i<2 && !bAnyOver; i++)
            for(int j=0; j<8 && !bAnyOver; j++)
        ...
        if(bAnyOver)
        {
            ShowMyMessage("Picker suck count over limit", "Please clear count before START");
            MyDBIProcess("Process", "Picker life count over limit, START blocked");
            return true;
        }
    }
      ...
      TCustomWinSocket *Socket)
{
    ...
    //==> Eastsun 20260526 #026-4.C2 Ifor 20240409 add:避免直接顯示Power Saving
    bPowersavingStart=true;
    //<== Eastsun 20260526 #026-4.C2
}
//---------------------------------------------------------------------------
      ...
      TCustomWinSocket *Socket)
{
    ...
    if(iBufferLength>0)
    {
        ...
        if(StrBuff.Pos("OFF")!=0)                                               //=="STATUS:POWEROFF")
        {
            ...
            //==> Eastsun 20260526 #026-4.A10 Ifor 20240409 add:SECS/GEM Power Report :KYEC
            if(CUSTOMER_CODE==CC_KYEC_LEE)
                bSECSGEM_Report=true;
            //<== Eastsun 20260526 #026-4.A10
...
//            bPowersavingStart=true;                                           //Ifor 20240229 add:避免復歸時異常報警
            ...
            //==> Eastsun 20260526 #026-4.A10 Ifor 20240409 add:SECS/GEM Power Report :KYEC
            if(CUSTOMER_CODE==CC_KYEC_LEE && bSECSGEM_Report==true)
            {
                RunInfo.bSecsGemMotorEnergySavingState=false;
                RunInfo.bSecsGemHeaterEnergySavingState=false;
                RunInfo.bSecsGemATCEnergySavingState=false;
                GetTimeInfo();
                RunInfo.PowerSavingEndTime.sprintf("%04d/%02d/%02d %02d:%02d:%02d", SystemYear, SystemMonth, SystemDate, SystemHour, SystemMin, SystemSec);
                if(IniConfig.bEnable_SECS_GEM==true)
                    EventReport(SECS_EVENT.PowerSavingEnd);
                bSECSGEM_Report=false;
            }
            //<== Eastsun 20260526 #026-4.A10
...
//---------------------------------------------------------------------------
bool __fastcall TFormHS::Check_FileFolderByFTP()                                //Eastsun 20260511 整合: Ifor 20240619 add: KYEC FTP folder check
{
    AnsiString asFtpUplaodPath;
    AnsiString Cur="";
    AnsiString asCurFtpUplaodPath;
    ...
    if(CUSTOMER_CODE==CC_KYEC_LEE)
    {
        ...
        try
        {
            ...
            if(IniConfig.cN10FtpHost!="")
                NMFTP2->Host = IniConfig.cN10FtpHost;
            NMFTP2->UserID   = IniConfig.cN10FtpUserName;
            NMFTP2->Password = IniConfig.cN10FtpPassword;
            NMFTP2->Port=21;
            NMFTP2->Connect();
            ...
            //檔案路徑:  /年_月/日/機台型號_機台ID/
            Cur=NMFTP2->CurrentDir;
            asFtpUplaodPath.sprintf("%s",IniConfig.cN10FtpUplaodPath);
            if((asFtpUplaodPath=="/") || (asFtpUplaodPath=="\\"))
            {
            }
            else
            {
                asCurFtpUplaodPath=Cur+asFtpUplaodPath;
                if(Cur!=asCurFtpUplaodPath)
                {
                    try
                    {
                        NMFTP2->ChangeDir(asCurFtpUplaodPath);
                    }
                    catch(...)
                    {
                        MyDBIProcess("Exception", "TFormHS::Check_FileFolderByFTP");
                        MyDBIProcess("Process", "FTP Make New Directory", asFtpUplaodPath);
                        NMFTP2->MakeDirectory(asCurFtpUplaodPath);
                        MySleep(20);
                        NMFTP2->ChangeDir(asCurFtpUplaodPath);
                    }
                }
            ...
            if(IniConfig.iN10UploadProductMethod==0)    //JerryYang 20190131 上傳production log可選擇00:00 or 08:00
            {
                asFtpUplaodPath.sprintf("/%04d_%02d", SystemYear, SystemMonth);
            }
            else
            {
                asFtpUplaodPath.sprintf("%s%04d_%02d", IniConfig.cN10FtpUplaodPath, SystemYear, SystemMonth);
            }
            asCurFtpUplaodPath=Cur+"/"+asFtpUplaodPath;
            if(Cur!=asCurFtpUplaodPath)
            {
            ...
            if(IniConfig.iN10UploadProductMethod==0)    //JerryYang 20190131 上傳production log可選擇00:00 or 08:00
            {
                asFtpUplaodPath.sprintf("/%02d", SystemDate);
            }
            else
            {
                asFtpUplaodPath.sprintf("%s%04d_%02d/%02d", IniConfig.cN10FtpUplaodPath, SystemYear, SystemMonth, SystemDate);
            }
            ...
            if(IniConfig.iN10UploadProductMethod==0)    //JerryYang 20190131 上傳production log可選擇00:00 or 08:00
            {
                asFtpUplaodPath.sprintf("/%s_%s", IniConfig.sMachineType, IniConfig.SocketHandlerID);
            }
            else
            {
                asFtpUplaodPath.sprintf("%s%04d_%02d/%02d/%s_%s", IniConfig.cN10FtpUplaodPath, SystemYear, SystemMonth, SystemDate, IniConfig.sMachineType, IniConfig.SocketHandlerID);
            }
            asCurFtpUplaodPath=Cur+asFtpUplaodPath;
            if(Cur!=asCurFtpUplaodPath)
            {
...
//---------------------------------------------------------------------------
bool __fastcall TFormHS::CheckMTKFileName(bool bInitial)                        //Eastsun 20260511 整合: Ifor 20241217 add: MTL File Check
{
    bool bUSE_MTK=false;
    AnsiString sVer="";

    if(fMain->cbSetupFileName->Text.SubString(1,4)== "1080" ||
       fMain->cbSetupFileName->Text.SubString(1,4)== "6947" ||
       fMain->cbSetupFileName->Text.SubString(1,4)== "9672" ||
       fMain->cbSetupFileName->Text.SubString(1,4)== "9188" ||
       fMain->cbSetupFileName->Text.SubString(1,4)== "9098" )
    {
        bUSE_MTK=true;
    }
    else
    ...
    if(bInitial==true)
    {
        bCustomerForMTKVersion=bUSE_MTK;

        if(bCustomerForMTKVersion==true)
        {
            asHandlerVersion = asHandlerVersion+"_MTK";
        }
        fMain->StatusBar1->Panels->Items[3]->Text=asHandlerVersion;
        WriteIniDataGeneral("Version", "Ver", asHandlerVersion);
    }

    return bUSE_MTK;
}
//---------------------------------------------------------------------------
void __fastcall TFormHS::Check_RecordFolder(AnsiString asDirPath, bool bNeedTime) //Eastsun 20260511 整合: Ifor 20250117 add: Check Record Folder
{
    AnsiString strYearFold="", strMonthFold="";

    if(DirectoryExists(asDirPath)==false)
        ForceDirectories(asDirPath);

    if(CUSTOMER_CODE==CC_KYEC_LEE && bNeedTime==true)                           //Ifor 20250117 add:KYEC 要求修改檔案名稱
    {
        strYearFold.sprintf("%s%04d%s", asDirPath, SystemYear, "\\");
        if(!(DirectoryExists(strYearFold)))
            ForceDirectories(strYearFold);

        strMonthFold.sprintf("%s%02d%s", strYearFold, SystemMonth, "\\");
        if(!(DirectoryExists(strMonthFold)))
            ForceDirectories(strMonthFold);
    }
}
//---------------------------------------------------------------------------
void __fastcall TFormHS::RecordGroundManLog_HS(AnsiString aFileName)            //Eastsun 20260511 整合: Ifor 20250123 KYEC FTP UP Load GroundMan Log File
{
    //確認資料夾是否存在
    AnsiString sDirPath=AnsiString("D:\\HandlerLog\\GroundMan_Log\\");

    Check_RecordFolder(sDirPath, true);
    sDirPath.sprintf("D:\\HandlerLog\\GroundMan_Log\\%04d%s%02d%s", SystemYear, "\\", SystemMonth, "\\");
    AnsiString sFullFileName=sDirPath;
    ...
    {
        sRecord ="Ver:"+asHandlerVersion+"\n";               //kevin 20191212 add ver
        sRecord=sRecord+"Date,Time,InArm,OutArm,Arm1,Arm2,HP1,HP2,SH1,SH2,InArm_A,InArm_B,InArm_C,InArm_D,InArm_E,InArm_F,InArm_G,InArm_H,OutArm_A,OutArm_B,OutArm_C,OutArm_D,OutArm_E,OutArm_F,OutArm_G,OutArm_H,Loader,Empty,Color,Auto1,Auto2,Auto3,Lot ID,OSAT Lot number";
        WriteDataToFile(sFullFileName, sRecord);
    }

    if(anGroundManLog.Pos(",")>=1)
    {
        AnsiString sTime=GetDateInfoByString("/")+","+GetOnlyTimeInfoByString(":");
        sRecord=sTime           +","+
                anGroundManLog  +","+
