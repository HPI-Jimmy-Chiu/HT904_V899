    ...
    else
    {
        ...
        if(iMin>=TestIF_File.iAutoClean_AlarmCount)
        {
            ...
            if(TestIF_File.iAutoClean_AlarmCount>0 && iMin!=iLastAutoCleanAlarmCount)
            {
                ...
                RecordProcess(sLog);
                ShowErrorMessage("WAR16313", K_RETRY, MMSystem, false);
            }
        }
        else
        {
...
//       (TestIF_File.iTestMode==_6Site2X3N &&                                     //RogerYang 20260324 : Add
//       fCleaning->iDeviceCount==6))
//    {
        for(int Y=0; Y<MOT[MMAutoCleanKit].Tray.YItem; Y++)
        {
            for(int X=0; X<MOT[MMAutoCleanKit].Tray.XItem; X++)
...
//    {
//        for(int Y=0; Y<MOT[MMAutoCleanKit].Tray.XItem; X++)
//        {
//            for(int Y=0; Y<MOT[MMAutoCleanKit].Tray.YItem; Y++)
//            {
//                if(MOT[MMAutoCleanKit].Tray.Data[X][Y]==HAS_CLEAN_IC)
...
//    if(iRow==1 || iRow==2)                                                      //JerryYang 20160411 當X-Pitch > 40mm,要分成4次放到shuttle
//    {
//        iKitStep=0;
//        iSuckRow=iRow-1;
//    }
                ...
                TestIF_File.iAutoClean_XDivision>4)                             //Steven 20240305 : 修正Auto Clean Pitch判斷
        {
            if(iXpos+iYpos*MOT[MMAutoCleanKit].Tray.XItem<TestIF.iAutoClean_DeveicePices)
            {
                if(InArmSuck.iModeX==9 &&
                   ...
                   iCloseSiteStep_2x8==3)                                       //KevinCheng 20260126 : 針對矽品中科增加特殊關Site判斷
                {
                    iCleanCount=atoi(fMain->AutoCleanStringGrid->Cells[iXpos][iYpos+1].c_str());
                    if(iCleanCount<0)
                        iCleanCount=0;
...
//            }
        ...
        case 510:
            ...
            if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                             //KenHsieh 20250923 : Auto clean for InSht sensor Latch 判斷
            {
        ...
        case 2002:
            ...
            {
                bWaitSECS=false;
                bRunAutoClean=false;                                            //KevinCheng 20250922 : 收到訊息後可繼續執行主程序
                return;
            }
            else
            {
...
//          break;                                                              //W8066 unreachable (both branches return above)
...
void DoShuttle1AutoClean()
{
    ...
    if(bInSh1DoLtc==true &&
       In_Shuttle_Auto_Latch==eInSHAutoLtc)                                     //KenHsieh 20250923 : Auto clean for InSht sensor Latch 判斷
    {
        return;
    }
            ...
            if(DoIndexAutoCleanDelay.Off())
            {
                ...
                else
                {
                    if(TestIF_File.iAutoClean_Tray!=eCKPos_CleanAir)
                    {
                        ...
                        for(int i=0; i<MAX_Index_Row; i++)                      //JerryYang 20260312 : fix誤發Drop error
                        {
                            for(int j=0; j<NEW_MAX_Index_Col; j++)
                            {
...
//            Task=1500;
            ...
            if(flag1==true)
            {
//                RecDebug(FTestSuck, Task);                                    //JerryYang 20260506 : mark掉
                Task=3750;
            }
            break;
        case 3750:
