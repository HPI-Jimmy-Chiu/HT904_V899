    ...
    : TForm(Owner)
{
    ...
    TLabel *tempMyBinSelARTFT[]=                                                //JerryYang 20220331 : 緯平要求show ART BIN
    {
    ...
    for(int i=0; i<eTrayCount; i++)
    {
        ...
        MyBinSelARTFT[i]    =tempMyBinSelARTFT[i];                              //JerryYang 20220331 : 緯平要求show ART BIN
    ...
    if(SPIL_FOR_QLE==1)                                                         //JerryYang 20251020 : 渠梁半清機功能
    {
...
void __fastcall TfShowBinSelect::FormDestroy(TObject *Sender)
{
    try
    {
        ...
        tsUPH->Clear();                                                         //Ifor 20170603 (wei) TStringList 刪除前先 Clean
    ...
    LogSoftwareOffTime("TfShowBinSelect, FormDestroy");                         //Steven 20210526 : 紀錄軟體執行時間
}
//---------------------------------------------------------------------------
...
void __fastcall TfShowBinSelect::ChangeBinDispStatus()                          //Steven 20110411 : 顯示目前顯示器的狀態
{
    ...
    static bool bJumpToUnloadMap=false;                                         //Ifor 20260715 : Bin display error, jump to tsUnloadMap one time flag
    static int iNoErrorDebounce=0;                                              //Ifor 20260715 : no error stable counter, for jump flag re-arm
    ...
    for(int i=0; i<eBinDispTotal; i++)                                          //JerryYang 20220909 : 12->eBinDispTotal
    {
        ...
        else if(USE_Scanner_AOI_Inspection==(int)eBtnAOI_TopBottomInstall &&    //Ifor 20260715 : Top&Bottom AOI install, MyBinDisp disable Fix1~Fix6 display, skip error check
                i>=eBinDispFix1 && i<=eBinDispFix6)
        {
        }
        else if(bErrFlag[i] ||
                (i>=3 && HSys.BinDisCtrl->UnitHasInstall(i)==false))            //Steven 20211221 : Loader, empty, color可能沒裝
        {
            bHasError=true;
            sTempChi=sTempChi+" "+UnLoadPanel[i]->Name;                         //Jimmychiu 20231031 : add Bin display error record
            if(iAlarmFlag==0)
            {
                iAlarmFlag=1;                                                   //Steven 20211130 : JSCC要求Bin顯示器異常要alarm
            }
        }
    }
    ...
    if(bHasError==false)
    {
        if(iNoErrorDebounce<60)                                                 //Ifor 20260715 : comm restart clears error 10~20 sec then error come back, debounce it
        {
            iNoErrorDebounce++;
        }
        else
        {
            bJumpToUnloadMap=false;                                             //Ifor 20260715 : error stable clear 60 sec, re-arm jump one time flag
        }
    }

    if(bHasError)
    {
        iNoErrorDebounce=0;                                                     //Ifor 20260715 : error exist, reset no error stable counter
        bChangeColor=!bChangeColor;
        if(bJumpToUnloadMap==false)                                             //Ifor 20260715 : jump one time only, operator can switch to other page
        {
            bJumpToUnloadMap=true;
    ...
    {
        if(CUSTOMER_CODE==CC_KYEC_LEE || CUSTOMER_CODE==CC_AMD_M)               //Ifor 20221026 add: KYEC 要求一直偵測
        {
        }
        else
        {
...
//            HSys.BinDisCtrl->CommBin->StopComm();
            ...
            ShowMyMessage("Please check bin display. It have communication error!", "請確認Bin顯示器的狀態。");
            #endif
        }
    }
    else if(IniConfig.bG16BinDispNeedAlarm)                                          //Steven 20211130 : JSCC要求Bin顯示器異常要alarm
    {
        ...
        if(iAlarmFlag==2 && AlarmDelay.Off())
        {
            ...
            sTempChi=AnsiString().sprintf("%s 異常位置:%s", "請確認Bin顯示器的狀態!", sTempChi);
            HSys.BinDisCtrl->CommBin->StopComm();
            HSys.BinDisCtrl->bFirstInit=true;
            HSys.BinDisCtrl->ProcessStopStart(true);
...
void __fastcall TfShowBinSelect::ShowBinSel()
{
    ...
    for(int i=0; i<iTestBinCount; i++)
    {
        ...
        for(int j=0; j<eTrayCount; j++)
        {
            ...
            if(j==Data)
            {
                ...
                else
                {
                    if(IniConfig.bSPILFunction==true &&
                       ...
                       TestIF_File.bSortingBy2DIDList==true)                    //JerryYang 20230322 : SPIL中山IT要求
                    {
                        if(S[j]=="")
                        {
            ...
            else
            {
                ...
                else if(IniConfig.bSPILFunction==true)                          //JerryYang 20250320 : SPIL改用逗號分隔
                {
                }
                else
                {
       ...
       TestIF_File.iRs232Mode==eRs23232Bin)                                     //Steven 20121112 : RS232支援32Bin
    {
        if(Data>0)
        {
            ...
            iBinTray        [Data] =true;                                       //kevin 20170328 (Steven) add 判斷Fix TRAY 是否使用 BIN
    ...
    else
    {
        if(iHWFix_BinBox==1 || Data==-1)                                        //kevin 20160819 //Steven 20230929 : && --> ||
        {
            ...
            iBinTray        [eBulkBox] =true;                                   //kevin 20170328 (Steven) add 判斷Fix TRAY 是否使用 BIN
            BinAssign       [eBulkBox]+="E";                                    //kevin 20180705 add Auto 123 fix 123 bin data
            bUnloadHasBin   [eBulkBox] =true;                                   //kevin 20180705 有BIN TRAY set true
            iTrayLastBin    [eBulkBox] =999;                                    //kevin 20180705 每一個TRAY最後一個BIN  Error bin
        }
        else if(Data>0)
        {
            if(IniConfig.bSPILFunction==true &&
            ...
            bUnloadHasBin   [Data] =true;                                       //kevin 20180705 有BIN TRAY set true
            iTrayLastBin    [Data] =999;                                        //kevin 20180705 每一個TRAY最後一個BIN  Error bin
            bUnloadHasBin   [Data] =false;                                      //kevin 20220906 有BIN TRAY set true
        }
    }

//    if(CosFunction.bSortingBy2DList==true &&                                  //JerryYang 20250505 : Mark掉
//       LastSet.iTester==_2D_SORT &&
...
//    {
        if(iTestBinCount<=16)
        {
            ...
            {
                S[i]+=". . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . ";
                if(IniConfig.bSPILFunction==true)                               //JerryYang 20250320 : SPIL改用逗號分隔
                {
...
//                        S[10+i]=S[10+(i-j)];
    ...
    for(int i=0; i<eTrayCount; i++)                                             //JerryYang 20220909 : 10->eTrayCount  //kevin 20160819  //kevin 20110901使用FIX分2TRAY
    {
        ...
        MyBinSel[i]->Caption=S[i];                                              //JerryYang 20250320 : SPIL改用逗號分隔
        if(IniConfig.bSPILFunction==true)
        {
        ...
        {
            int iLen=BinAssign[i].Length();                                     //RogerYang 20260403 remove Commas ","
    ...
    if(TrayForm.iFixTrayMode==false)                                            //要把剩下的三個改成橘色
    {
    ...
    ShowBinSel_ARTNor();                                                        //JerryYang 20220331 : 緯平要求show ART BIN
    ShowBinSel_ARTRT();
...
void __fastcall TfShowBinSelect::DoShowBinDigital()
{
    if(bUpdateBinDigital==true &&
       (NUMBER_PANEL_TYPE==3 ||                                                 //Steven : 彩色顯示器
        NUMBER_PANEL_TYPE==4))                                                  //Sam 20240604 : 新增 BinDisplay TFT
    {
        int Data;//, iBinSelCT=0;
        int iBinSet[MAX_BIN_UNIT][TEST_MAX_BIN];                                //iBinSet[0~2][20] 是Loader, Empty, Color，所以不設定     //Steven 20140402 : Fixed 記憶體破壞
        int iBinCount[TEST_MAX_BIN]={-1};                                       //每個位置被分配到幾個Bin                                 //Steven 20140402 : Fixed 記憶體破壞
        int iBinColor[TEST_MAX_BIN]={-1};                                       //1:red, 2:green, 3:orange                                //Steven 20140402 : Fixed 記憶體破壞
...
//        }
        ...
        //初始化
        for(int i=0; i<MAX_BIN_UNIT; i++)                                       //Steven 20140402 : Fixed 記憶體破壞
        {
            iBinColor[i]=ColorOrange;
            iBinCount[i]=0;
            for(int j=0; j<TEST_MAX_BIN; j++)                                   //Steven 20140402 : Fixed 記憶體破壞
                iBinSet[i][j]=-1;
        }
        ...
        {
            AddBinDisp[eBinDispMag1      ]=1;                                   //BIN顯示器全部15個, addr 0不使用,從addr1開始
        ...
        for(int i=0; i<iTestBinCount; i++)                                      //Steven 20121112 : RS232支援32Bin 15 --> iTestBinCount
        {
            ...
            if(Data<=0 || Data>e3TrayCount) continue;                           //JerryYang 20220909 : 修正保護    //Steven 20121112 : RS232支援32Bin 15 --> iTestBinCount
            for(int j=1; j<=e3TrayCount; j++)                                   //RogerYang 20250825 : "<" -> "<=" 修正最後一個顯示器沒寫入 //Steven 20140404 Start : Fixed 256Bin
            {
        ...
        if(Data>=0 && Data<e3TrayCount)                                         //JerryYang 20220909 : 9->iBinSelCT
        {
            if(Data>=eBinDispMag1 && Data<=eBinDispMag14)                       //JerryYang 20220909 : add magazine
            {
                ...
                {
                   int iSort=Data-9;
                   int iLinkCount=0;    //Ifor 20240909 Fix:記憶體溢位問題
                    for(int i=iSort; i<14; i++)
                    {
                        if(BinSelect[iTestRunMode].bMagazineLink[i]==true)
                        {
                            iLinkCount++;
                            iBinSet[Data+iLinkCount+3][iBinCount[Data+iLinkCount+3]]=999;    //Magazine bin有支援到三位數, 999拿來當ERROR BIN
                        }
                        else
                        {
                            break;
                ...
                iBinSet[Data+3][iBinCount[Data+3]]=999;                         //Magazine bin有支援到三位數, 999拿來當ERROR BIN
            }
            else
            {
        ...
        for(int i=3; i<MAX_BIN_UNIT; i++)                                       //Steven 20140402 : Fixed 記憶體破壞
            if(iBinSet[i][0]==-1)
...
void TfShowBinSelect::SetAutoVisible()
{
    ...
    if(USE_Scanner_AOI_Inspection==(int)eBtnAOI_TopBottomInstall)               //Ifor 20260715 : Top&Bottom AOI occupies Fix2/Fix3 tray, hide Fix2/Fix3
    {
        SetLabelVisible(eFix2, false);
        SetLabelVisible(eFix3, false);
    }
...
void __fastcall TfShowBinSelect::PageControl1Change(TObject *Sender)
{
    ...
    else if(PageControl1->ActivePage==tsIndex)                                  //Sam 20210505 : PTI 要求的兩段 Low Yeild
    {
        fShowBinSelect->Width=279;
        fShowBinSelect->Height=450;                                             //Sam 20211221 : 調整高度
    }
    else if(PageControl1->ActivePage==tsARTNormalBin)                           //tsTestBin  //JerryYang 20220331 : 緯平要求show ART BIN
    {
    ...
    else if(PageControl1->ActivePage==Tab_UPH)                                  //RogerYang 20250224 偉測需求 新增三列信息,耗時,數量,site
    {
        if(IniConfig.bVTESTFunction==true)
        {
            fShowBinSelect->Width=455;                                          //新增三個欄位
        }
        else
        {
            fShowBinSelect->Width=279;
    ...
    {
        fShowBinSelect->Width=279;
        fShowBinSelect->Width=357;                                              //JerryYang 20230322 : UI修改
        fShowBinSelect->Height=380;
    }
}
//---------------------------------------------------------------------------
void __fastcall TfShowBinSelect::ShowInitialString()
{
    ...
    {
        for(int i=0; i<iTestBinCount; i++)                                      //JerryYang 20220425 Nvidia Ryan要求改為顯示Bin字樣
    ...
    if(CosFunction.bCategoryInfoByContactCT)                                    //Sam 20240131 : 新增一組 CategoryInfo 紀錄資料使用 ContactCT
    {
    ...
    if(USE_AUTO_RETEST==eartInstall || CosFunction.bUseARTSortCount)            //wei 20150923 add ART計數 //Ifor 20170316 (wei) add MRT Mode
    {
...
{
    int i=0, sum=0, Sum_ART=0, iTemp=0, Sum_ConCT=0;                            //Sam 20240131 : 新增一組 CategoryInfo 紀錄資料使用 ContactCT
    ...
    {
        if(CosFunction.bLowYieldUseContactCounts)                               //Sam 20230620 : 優化 Smart Auto Clean
        {
        ...
        if(LastSet.iTester==_2D_SORT)                                           //Frank 20221122 : 2DID sorting for ATK
        {
            for(i=0; i<iTestBinCount+1; i++)                                    //Steven 20121112 : RS232支援32Bin 15 --> iTestBinCount
            {
                ...
                if(iByBinCnt[i]>0 || i==iTestBinCount)
                {
                    ...
                    if((USE_AUTO_RETEST==eartInstall && IniConfig.bA10_AutoReTest) ||
                        CosFunction.bUseARTSortCount)                           //wei 20150923 add ART計數  //Ifor 20170316 (wei) add MRT Mode
                    {
                        if(Sum_ART>0)
                        {
//                            f1=ChangeToFloat((double)LastSet.iBinData32_ART[0][i], (double)Sum_ART);
        ...
        else
        {
            for(i=0; i<iTestBinCount+1; i++)                                    //Steven 20121112 : RS232支援32Bin 15 --> iTestBinCount
            {
                ...
                StrGrdCategoryART->Cells[1][1+i]=LastSet.iBinData32_ART[0][i];  //wei 20150923 add ART計數
                StrGrdCategoryContCT->Cells[1][1+i]=ArmData[0]->GetSelBin(i)+ArmData[1]->GetSelBin(i);   //Sam 20240131 : 新增一組 CategoryInfo 紀錄資料使用 ContactCT
                if(sum>0)
                {
...
//                        f1=ChangeToFloat((double)LastSet.iBinData32_ART[0][i], (double)Sum_ART);
            ...
            if(LastSet.iTester!=_2D_SORT)                                       //JerryYang 20230322 : 2DID sort不需做
            {
                for(i=0; i<iTestBinCount; i++)                                  //Steven 20121112 : RS232支援32Bin 15 --> iTestBinCount
                {
...
//                            f1=ChangeToFloat((double)LastSet.iBinData32_ART[0][i], (double)Sum_ART);
                        ...
                        if(SystemStart && iHome==0)
                        {
                            ...
                            {
                                if(IniConfig.bI30ContFailBin)                   //kevin 20160407  add Fail bin 超過數量發警告訊息
                                {
                                    if(Prod.dFailureLimit[i]!=0.0 && f>=Prod.dFailureLimit[i])
                                    {
                                        ...
                                        if(IniConfig.bEnableAutoCleanFunction &&//JerryYang 20161121 修正Yield alarm要先做完one cycle的功能影響auto clean
                                ...
                                {
                                    if(CosFunction.bYieldControlBinSelectUseContactCount==true) //KaiChen 20181115 : BinSelect裡面 Yield控制使用 Contact Count
                                    {
                                           ...
                                           f>=Prod.dFailureLimit[i])
                                        {
                                            ...
                                            S1.sprintf("Category %d count over limit %2.1f%% ", i, Prod.dFailureLimit[i]);
                                            DoLowYieldAlarm("WAR07357", S1);    //Steven 20180627 (wei) : 整合Low Yield Alarm
                                            break;
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        if(CosFunction.bLowYieldUseContactCounts && //Sam 20230620 : 優化 Smart Auto Clean
                                           CUSTOMER_CODE!=CC_PTI)                   //Sam 20250102 : 修正力成 AI Clean
                                        {
                                            if(Sum_ConCT>Prod.iPersentIgnore[i])    //Sam 20250102 : 修正力成 AI Clean
                                                bNeedCheck=true;
                                        }
                                        else
                                        {
                            ...
                            else if(Prod.bFailCountEnable[i]==true)
                            {
                                if(IniConfig.bI30ContFailBin)                   //kevin 20160407  add Fail bin 超過數量發警告訊息
                                {
                                    if(Prod.iFailCountLimit[i]!=0 && LastSet.iBinData32[0][i]>=Prod.iFailCountLimit[i])
                                    {
                                        ...
                                        DoLowYieldAlarm("WAR07358", S1);        //Steven 20180627 (wei) : 整合Low Yield Alarm
                                        break;
                                    }
                                }
                                else
                                {
                                    ...
                                    if(CosFunction.bLowYieldUseContactCounts && //Sam 20230620 : 優化 Smart Auto Clean
                                       CUSTOMER_CODE!=CC_PTI)                   //Sam 20250102 : 修正力成 AI Clean
                                    {
                                        if(Sum_ConCT>Prod.iFailCountIgnore[i])  //Sam 20250102 : 修正力成 AI Clean
                                            bNeedCheck=true;
                                    }
                                    else
                                    {
       ...
       PageControl1->ActivePageIndex==2)                                        //jou 2014-06-19 Security Have 5 Level 3->iDefHonPrecLevel
    {
        ...
        if(iRow>0 && iRow<11)                                                   //Row介於1~10間
        {
            if(Application->MessageBoxA("Do you want to delete this record?", "Confirm", MB_OKCANCEL)==IDOK)
            {
                ...
                for(int i=1; i<=10; i++)                                        //重新計算UPH的平均值
                {
                    try
                    {
...
void __fastcall TfShowBinSelect::btnAutoCleanClick(TObject *Sender)
{
    if(bRunAutoClean)
        return;
    if(bIsASMAutoOneCycle)
        return;

    if(IniConfig.bEnableAutoCleanFunction && TestIF.iAutoClean_Function==true)
    {
        if(TestIF.iAutoClean_Mode & M_MANUAL)                                   //pig 2012.09.17 AutoClean改 start
        {
            ...
            else
            {
                ...
                {
                    ShowMyMessage("Auto Clean Need Home","Auto Clean 需要歸零");
                    return;
                }
                ...
                if(CheckIndexIsNormal()==false)                                 //JerryYang 20260513 : add
                {
                    ShowMyMessage("Index arm Z axis is not in safe position.","");
                    return;
                }
                ...
                if(IniConfig.bD51UseOnecycleCleanOutFinishTestArmAtRear)
                {
                    ...
                    {
                        ShowMyMessage("Index arm axis is not in standy position.","");
                        return;
                    }
                }
                else
...
void __fastcall TfShowBinSelect::btnClearCountClick(TObject *Sender)
{
    ...
    int ret=ShowMyMessageBox_YES_NO("Sure To Clear Counter?", "確定是否要重新計數？");
    if(ret==2)
    {
        return;
    }
...
void TfShowBinSelect::CaculateUPH()                                             //Steven 20160727 : Show immediate UPH
{
    ...
    static DWORD NowTickCount=-1, LastTimeTick=1, PassSec, PassSec2=0;
    int sum=0;
    ...
    if((OutArmSuck.HasIC() || ShuttleHasIC() || IndexHasIC()) && sum>iOldSum)
    {
        ...
        else
        {
            ...
            if(PassSec>0)
            {
                ...
                if(CUSTOMER_CODE==CC_ASE_CL)
                {
                    fMain->StatusBar1->Panels->Items[7]->Text="Net UPH: "+AnsiString(UPH)+"/"+AnsiString(iGrossUPH);
                }
                else
                {
...
//---------------------------------------------------------------------------
void __fastcall TfShowBinSelect::ShowBinSel_ARTNor()                            //JerryYang 20220331 : 緯平要求show ART BIN
{
...
//------------------------------------------------------------------------------
void __fastcall TfShowBinSelect::ShowBinSel_ARTRT()                             //JerryYang 20220331 : 緯平要求show ART BIN
{
    if(IniConfig.bSPILFunction && bCanRunSCKART)
    {
...
void __fastcall TfShowBinSelect::btnICMisPlacementClick(TObject *Sender)
{
    ...
    ret=ShowMyMessageBox_YES_NO("Are you sure to trigger IC Misplacement alarm?", "確定要觸發IC置偏異常?");
    ...
    if(ret==1)
    {
        ...
        if(Ptr->Tag==0)
        {
            ShowErrorMessage("JAM1110", 0, iMMAuto[0], false, "Manually Trigger"); //wei 20160511 矽品Auto紀錄沒有Tray Auto偵測有Tray 只能Skip
        }
        else if(Ptr->Tag==1)
        {
            ShowErrorMessage("JAM1210", 0, iMMAuto[1], false, "Manually Trigger"); //wei 20160511 矽品Auto紀錄沒有Tray Auto偵測有Tray 只能Skip
        }
        else if(Ptr->Tag==2)
        {
            ShowErrorMessage("JAM1310", 0, iMMAuto[2], false, "Manually Trigger"); //wei 20160511 矽品Auto紀錄沒有Tray Auto偵測有Tray 只能Skip
        }
    }
    edtTriggerAlm->Text="";
}
...
//---------------------------------------------------------------------------
void __fastcall TfShowBinSelect::btnAutoDeviceEjectionRemoveClick(              //JerryYang 20251020 : 渠梁半清機功能
      TObject *Sender)
{
    int ret;
    ...
    ret=ShowMyMessageBox_YES_NO("Sure To Eject tray?", "確定要進行退盤?");
    if(ret==2)
    {
        return;
    }
    ...
    if(InArmSuck.HasIC()     ||
       ...
       ShuttleHasIC() ||
       IndexHasIC() ||
       MOT[MInRotateKit].HasIC() ||
       MOT[MOutRotateKit].HasIC() ||
       MOT[MTrayX].fHasTray
       )
    {
        ShowMyMessage("請先完成ONE CYCLE");
        return;
    }

    for(int iAutoTarget=eAuto1; iAutoTarget<=iAutoRight; iAutoTarget++)
    {
        if(MOT[MMAuto1+iAutoTarget].fHasTray)
        {
            MOT[MMAuto1+iAutoTarget].InitNewTray(HAS_IC, false, "Click_btnAutoDeviceEjectionRemove");
            bNeedEject[iAutoTarget]=true;
        }
    }

    bEject=true;

    btnAutoDeviceEjectionRemove->Enabled=false;
    RecordProcess("退盤流程開始");
}
//---------------------------------------------------------------------------

void __fastcall TfShowBinSelect::btnAutoDeviceEjectionClick(
      TObject *Sender)
{
    AnsiString GroupStr="", FileName2="", szDir="";
    int ret;
    ...
    ret=ShowMyMessageBox_YES_NO("Sure To Purge out all device to AUTO1?", "確定要進行清機流程? 機台將會切換至OFFLINE模式並將所有IC放到AUTO1!");
    if(ret==2)
    {
        return;
    }
    ...
    if(bEject)
    {
        ShowMyMessage("退盤流程尚未完成!");
        return;
    }

    if(btnAutoDeviceEjectionRemove->Enabled==true)
    {
        ShowMyMessage("退盤流程尚未完成!");
        return;
    }
    ...
    for(int i=eAuto1; i<=iAutoRight; i++)
    {
        if(MOT[MMAuto1+i].fHasTray || MOT[MMAuto1_Car+i].fHasTray)
        {
            ShowMyMessage("AUTO區有TRAY盤,無法進行清機流程!");
            return;
        }
    }

    if(bPurgeOutAllDevice==true)
    {
        return;
    }
    ...
    TStringList *tsBin;
    tsBin=new TStringList();

    TStringList *tsBinPos;
    tsBinPos=new TStringList();

    szDir=GetRecipePath();
    FileName2=FileInfo().PathCombin(szDir, "BinasgnOff-Line.Data");
    ...
    for(int i=0; i<iTestBinCount; i++)                                          //Steven 20160614 : 避免降版導致分Bin異常, 將資料回寫到舊的
    {
        ...
        tsBin->Add(e3PosAuto1);
        FormSysTools->WriteIniData(GroupStr, "Bin", tsBin->CommaText);
        FormSysTools->WriteIniData(GroupStr, "Tray", s3TrayName[e3Auto1]);       //Steven 20241205 : Bin to tray增加文字顯示
    }

    for(int i=0; i<TEST_MAX_BIN; i++)
    {
        tsBinPos->Add(s3TrayName[e3Auto1]);;
    }

    GroupStr="Bin Func OffLine";

    FormSysTools->WriteIniData(GroupStr, "3722 BinTraySetting", tsBin->CommaText);
    FormSysTools->WriteIniData(GroupStr, "3722 BinTraySetName", tsBinPos->CommaText);
    ...
    bPurgeOutAllDevice=true;

    btnAutoDeviceEjection->Enabled=false;
    ...
    ShowMyMessage("已成功切換至Tester OFFLINE模式!!!");

    NewRecordProcess("MES2155", "Change to Off_Line", "by click AutoDeviceEjection"); //ChungHung 20140722 add add record
}
//---------------------------------------------------------------------------