...
public:                                                                         // User declarations
    ...
    TStringList *listTrayIDByLot;                                               //Sam 20190405 : Tray Decive Count
   ...
   // bool DoTrayDeciveCntAutoTuneCCD(int Count ,int iAuto,bool Reset=false);   //Sam 20190405 : Tray Decive Count
    ...
    bool bUseTrayID;
    bool bUseDeviceRemainLaser;                                                 //JimmyChiu 20220219 : 殘料檢查 From Loader
    ...
    TQPF_Timer iAutoHomeTimeout;                                                //AI(ht9045-v899) 20260505: DoLaserScanFromLoader 自動 Home MLoaderY 用 timeout
    AnsiString asErrorLaserScan;
    void RecordMsgLaser(AnsiString asMsg);
    void RecordErrorMsgLaser(AnsiString asMsg);
    void RecordSimTaskMsg(AnsiString asMsg);                                    //AI(ht9045-v899) 20260505: Sim/AutoLearning 模式下記錄流程細節 (Memo + 檔案)
    void WriteLaserScanFlowLog(AnsiString asMsg);                               //AI(ht9045-v899) 20260505: 生產模式也永遠寫檔供客戶回傳分析
    AnsiString DumpLaserScanState();                                            //AI(ht9045-v899) 20260505: 一次性備份 MLoaderY/Cylinder/Sensor 現況
    ...
    int  iNFCTask;
    bool DoTrayIDCheck(AnsiString Str);
    void TfTrayMapping::ClearTrayIDByLot();
};
//---------------------------------------------------------------------------
