...
bool bSetData=false;                                                            //kevin 20220930 add Shuttle sensor set need save
//---------------------------------------------------------------------------
__fastcall TfCCLink::TfCCLink(TComponent* Owner)
    : TForm(Owner)
{
    iCanBusSrachTask=1;                                                         //Sam 20210518 : 新增 CanBus 軟體配置
    iInShtLtcPercent=80;                                                        //KenHsieh 20251128 : InSht Latch add percent for auto teach
...
//    iSensorNo[8]=1;       iSensorNo[9]=1;                                     //kevin 20130420 socket sensor
    ...
    TMyTrayPtr[11]=mtInSocketBase25_32;                                         //JerryYang 20260205 : add第三組Socket sensor(8+16+8)
    //輸入框---------------------------------------------
    ...
    EditPtr[54]=edIn5_1;       EditPtr[55]=edIn5_2;                             //JerryYang 20260205 : add第三組Socket sensor(8+16+8)
    EditPtr[56]=edIn5_3;       EditPtr[57]=edIn5_4;
    EditPtr[58]=edIn5_5;       EditPtr[59]=edIn5_6;
    EditPtr[60]=edIn5_7;       EditPtr[61]=edIn5_8;
    ...
    LamePtr[56]=SocketLED25;   LamePtr[57]=SocketLED26;                         //JerryYang 20260205 : add第三組Socket sensor(8+16+8)
    LamePtr[58]=SocketLED27;   LamePtr[59]=SocketLED28;
    LamePtr[60]=SocketLED29;   LamePtr[61]=SocketLED30;
    LamePtr[62]=SocketLED31;   LamePtr[63]=SocketLED32;
    ...
    if(UseCanBusOrEtherCAT())                                                   //Sam 20230707 : EtherCAT Shuttle sensor
    {
        ...
        if(SOCKET_AMP_QTY_3rd>0)                                                //JerryYang 20260205 : add第三組Socket sensor(8+16+8)
        {
            iTotalDevices=5;
        }
...
//        }
        ...
        iAmplifierList[4]=SOCKET_AMP_QTY_3rd;                                   //JerryYang 20260205 : add第三組Socket sensor(8+16+8)

        iDesMACIDList[0]=11;                                                    //InShuttle Address
        iDesMACIDList[1]=12;                                                    //OutShuttle Address
        iDesMACIDList[2]=13;                                                    //Socket/ROTATE/Color Address
        iDesMACIDList[3]=14;
        iDesMACIDList[4]=15;                                                    //JerryYang 20260205 : add第三組Socket sensor(8+16+8)
    ...
    for(int i=0; i<12; i++)                                                      //kevin 20130420 總共有多少 TMyTrayPtr  //wei 20180409 color sensor  //JerryYang 20260205 : add第三組Socket sensor(8+16+8)
    {
...
void __fastcall TfCCLink::sbExitClick(TObject *Sender)
{
       ...
       SHUTTLE_SENSOR_TYPE==eSensorEtherCAT3)                                   //Sam 20230707 : EtherCAT Shuttle sensor
    {
        if(In_Shuttle_Auto_Latch!=eInSHAutoLtc ||
          ...
          (fShuttleMove->fShow==false &&
           fContact->fShow==false)))                                            //KenHsieh 20251121 : 修正使用EtherCAT 無法讀取shuttle sensor 之問題
        {
            MyEtherCAT->Pause();
        }
    }
    ...
    if(CUSTOMER_CODE==CC_ASE_KaohSiung)                                         //kevin 20220912 record CC-link read data
    {
        ...
        if(bSetData)                                                            //kevin 20221017 add 20220930 add Shuttle sensor set need save
        {
            ret=ShowMyMessageBox_YES_NO("Save SetData??", "確定要儲存嗎??");    //kevin 20220930 add Shuttle data save
            if(ret==1)
                SaveSetupFile();
            bSetData=false;                                                     //kevin 20220930 add Shuttle sensor set need save
        }
    }
    Close();
}
...
void __fastcall TfCCLink::FormShow(TObject *Sender)
{
    ...
    tsSocketBase3->TabVisible=SOCKET_AMP_QTY_3rd>0;                             //JerryYang 20260205 : add第三組Socket sensor(8+16+8)
    ...
    if(In_Shuttle_Auto_Latch==eInSHAutoLtc)
    {
        gbInSHLtcPercent->Visible=true;                                         //KenHsieh 20251128 : InSht Latch add percent for auto teach
        if(CANBUS_METHOD==1 && NUDN1_MACID11_AMP_QTY==2)                        //KenHsieh 20251031 : In Shuttle sensor 只裝2個放大器
        {
            btSetIn1->Visible=false;
            btSetIn2->Visible=false;
        }
    }
...
//    fCCLink->SaveSetupFile();                                                 //wei 20180622  CCLink Value Save File
        ...
        SHUTTLE_SENSOR_TYPE==eSensorEtherCAT3)                                  //Sam 20230707 : EtherCAT Shuttle sensor
    {
        if(In_Shuttle_Auto_Latch!=eInSHAutoLtc ||
          (In_Shuttle_Auto_Latch==eInSHAutoLtc    &&
          (fShuttleMove->fShow==false &&
           fContact->fShow==false)))                                            //KenHsieh 20251121 : 修正使用EtherCAT 無法讀取shuttle sensor 之問題(按關閉後要停止timer)
        {
            MyEtherCAT->Pause();
        }
    }
...
void __fastcall TfCCLink::SaveSetupFile()                                       //wei 20180622 (Steven) : CCLink Value Save File
{
    ...
    for(int i=0; i<16; i++)                                                     //0~15 寫值
    {
        ...
        {
            if(In_Shuttle_Auto_Latch==eInSHAutoLtc &&
               CANBUS_METHOD==1 && NUDN1_MACID11_AMP_QTY==2)                    //KenHsieh 20251031 : In Shuttle sensor 只裝2個放大器
            {
                if(i==0 || i==8)
                    iValue=GetIOValue(0, i/8);
                else
                    iValue=10;
            }
            else
        ...
        else
        {
            ...
            {
                if(i==0 || i==8)
                    iValue=MyCCLink->CCLinkBoard->MyNCUL[0].iSetValue[i/8];
                else
                    iValue=10;
            }
            else
...
void __fastcall TfCCLink::ReadSetupFile()                                       //wei 20180622  CCLink Value Save File
{
    ...
    else
    {
        ...
        if(UseCanBusOrEtherCAT())                                               //Sam 20230707 : EtherCAT Shuttle sensor
        {
            for(int i=0; i<16; i++)
            {
                   ...
                   CANBUS_METHOD==1 && NUDN1_MACID11_AMP_QTY==2)                //KenHsieh 20251031 : In Shuttle sensor 只裝2個放大器
                {
                    if(i==0 || i==8)
                    {
                        if(i==fCCLink->btIn1_8->Tag)
                            SetIOValue(0, 1, iValue[i]);
                        else                                                    //btIn2_8->Tag
                            SetIOValue(0, 0, iValue[i]);
                    }
                }
                else
                {
        ...
        else
        {
            if(In_Shuttle_Auto_Latch==eInSHAutoLtc &&
               CANBUS_METHOD==1 &&
               NUDN1_MACID11_AMP_QTY==2)                                        //KenHsieh 20251031 : In Shuttle sensor 只裝2個放大器
            {
                MyCCLink->DoSetValue(0, 1, iValue[0]);                          //btIn1_8->Tag
                MyCCLink->DoSetValue(0, 0, iValue[8]);                          //btIn2_8->Tag
            }
            else
            {
...
void __fastcall TfCCLink::Timer1Timer(TObject *Sender)
{
    ...
    if(UseCanBusOrEtherCAT())                                                   //Sam 20230707 : EtherCAT Shuttle sensor
    {
        ...
        else
        {
            ...
            if(SHUTTLE_SENSOR_TYPE==eSensorCanBus ||
               SHUTTLE_SENSOR_TYPE==eSensorCanBus3)                             //KenHsieh 20251209 : 區分CanBus & EtherCat
            {
                lbErrMess->Caption=DNM100UD->ErrorMessage;
                DNM100UD->TaskClear();
            }
            else
            {
                lbErrMess->Caption=MyEtherCAT->ErrorMessage;
            }
        }
        ...
        if(MachineTypeChoice==Type_HT9045 && UseSensorMod3())                   //Sam 20230707 : EtherCAT Shuttle sensor //eSensorCanBus3 待驗證  //Steven 20131008 : for HT9046AH
        {
            for(int i=0; i<12; i++)
            {
                ...
                CANBUS_METHOD==1 && NUDN1_MACID11_AMP_QTY==2)                   //9045 裝不下這個模組  //KenHsieh 20251031 : In Shuttle sensor 只裝2個放大器
        {
            for(int i=0; i<16; i++)
            {
                if(i==7 || i==15)                                               //只有SH1,2第8顆才需要狀態，其餘給OK
                {
                ...
                {
                    TMyTrayPtr[i/8]->SetCellNumber(i%8, 1, cSensorStatus[0]);
                    TMyTrayPtr[i/8]->SetCellColorIndex(i%8, 1, 0);
                }
            }

            for(int i=0; i<NUDN1_MACID11_AMP_QTY; i++)
            {
                iTemp=GetValue(0, i);                                           //Sam 20230707 : EtherCAT Shuttle sensor
                TMyTrayPtr[1-i]->SetCellNumber(7, 2, iTemp);

                if(i==1)                                                        //kevin 20220912 shuttle 2 data log
                    iSensor[0][0]=iTemp;                                        //kevin 20220912 log Shuttle 1 display
                else
                    iSensor[1][0]=iTemp;                                        //kevin 20220912 log Shuttle 2 display

                iTemp=GetIOValue(0, i);                                         //Sam 20230707 : EtherCAT Shuttle sensor
                TMyTrayPtr[1-i]->SetCellNumber(7, 3, iTemp);

                if(i==1)                                                        //kevin 20220912 shuttle 2 data log
                    iSensor[2][0]=iTemp;                                        //kevin 20220912 log Shuttle 1 display
                else
                    iSensor[3][0]=iTemp;                                        //kevin 20220912 log Shuttle 2 display

                LamePtr[i*8]->Value=!GetIO(0, i);                               //i*8:SH1 start from 8 //Sam 20230707 : EtherCAT Shuttle sensor
            }
        }
        ...
        if(USE_SOCKET_SENSOR && SOCKET_AMP_QTY_3rd>0 && SOCKET_AMP_QTY_3rd<=8)      //JerryYang 20260205 : add第三組Socket sensor(8+16+8)
        {
            for(int i=0; i<SOCKET_AMP_QTY_3rd; i++)                             //socket sensor
            {
                if(i>=0 && i<=7)
                {
                    TMyTrayPtr[11]->SetCellNumber(i, 1, cSensorStatus[iStatus]);
                    TMyTrayPtr[11]->SetCellColorIndex(i, 1, iStatus);

                    iTemp=GetValue(3, i);                                           //Sam 20230707 : EtherCAT Shuttle sensor
                    TMyTrayPtr[11]->SetCellNumber(i, 2, iTemp);

                    iTemp=GetIOValue(3, i);                                         //Sam 20230707 : EtherCAT Shuttle sensor
                    TMyTrayPtr[11]->SetCellNumber(i, 3, iTemp);

                    LamePtr[i+56]->Value=GetIO(3, i);                               //反向 D-on //Sam 20230707 : EtherCAT Shuttle sensor
                }
            }
        }
    }
    else
    {
        ...
        if(USE_COLOR_TRAY_SENSOR==1 && COLOR_AMP_QTY>0)                         //JerryYang 20200327 整理第三個通訊模組socket/ rotate/ color
        {
            ...
            {
                for(int i=0; i<SOCKET_AMP_QTY; i++)                             //socket sensor //KenHsieh 20250107 : 數量修改為變數
                {
                    TMyTrayPtr[6]->SetCellNumber(i, 1, cSensorStatus[iStatus]);
                    TMyTrayPtr[6]->SetCellColorIndex(i, 1, iStatus);
                ...
                for(int i=0; i<ROTATE_AMP_QTY; i++)                             //rotate    //KenHsieh 20250107 : 數量修改為變數
                {
                    TMyTrayPtr[7]->SetCellNumber(i, 1, cSensorStatus[iStatus]);
                    TMyTrayPtr[7]->SetCellColorIndex(i, 1, iStatus);

                    iTemp=MyCCLink->CCLinkBoard->MyNCUL[2].iCurrValue[i+iRotateStart];  //KenHsieh 20250107 : 需增加起始數量，避免顯示錯誤數值
                    TMyTrayPtr[7]->SetCellNumber(i, 2, iTemp);

                    iTemp=MyCCLink->CCLinkBoard->MyNCUL[2].iSetValue[i+iRotateStart];   //KenHsieh 20250107 : 需增加起始數量，避免顯示錯誤數值
                    TMyTrayPtr[7]->SetCellNumber(i, 3, iTemp);
                    if(CUSTOMER_CODE==CC_TERAPOWER) //Sam 20191112 : 英傑要求 Roate Sensor 要反向
                    {
                        LamePtr[i+28]->Value=!MyCCLink->CCLinkBoard->MyNCUL[2].bOnOff[i+iRotateStart];  //KenHsieh 20250107 : 需增加起始數量，避免顯示錯誤數值
                    }
                    else
                    {
                        LamePtr[i+28]->Value=MyCCLink->CCLinkBoard->MyNCUL[2].bOnOff[i+iRotateStart];   //KenHsieh 20250107 : 需增加起始數量，避免顯示錯誤數值
                ...
                for(int i=0; i<COLOR_AMP_QTY; i++)                              //color //KenHsieh 20250107 : 數量修改為變數
                {
                    TMyTrayPtr[8]->SetCellNumber(i, 1, cSensorStatus[iStatus]);
                    TMyTrayPtr[8]->SetCellColorIndex(i, 1, iStatus);

                    iTemp=MyCCLink->CCLinkBoard->MyNCUL[2].iCurrValue[i+iColorStart];   //KenHsieh 20250107 : 需增加起始數量，避免顯示錯誤數值
                    TMyTrayPtr[8]->SetCellNumber(i, 2, iTemp);

                    iTemp=MyCCLink->CCLinkBoard->MyNCUL[2].iSetValue[i+iColorStart];    //KenHsieh 20250107 : 需增加起始數量，避免顯示錯誤數值
                    TMyTrayPtr[8]->SetCellNumber(i, 3, iTemp);

                    if(CUSTOMER_CODE==CC_TERAPOWER) //Sam 20180704 : 晶兆成 color sensor 軟硬體顯示與7系列一樣，亮起是紅色
                    {
                        LamePtr[i+32]->Value=!MyCCLink->CCLinkBoard->MyNCUL[2].bOnOff[i+iColorStart];   //KenHsieh 20250107 : 需增加起始數量，避免顯示錯誤數值
                    }
                    else
                    {
                        LamePtr[i+32]->Value=MyCCLink->CCLinkBoard->MyNCUL[2].bOnOff[i+iColorStart];    //KenHsieh 20250107 : 需增加起始數量，避免顯示錯誤數值
        ...
        else
        {
            ...
            {
                for(int i=0; i<SOCKET_AMP_QTY; i++)                             // 第3組  //kevin 20130420 socket sensor //KenHsieh 20250107 : 數量修改為變數
                {
                    TMyTrayPtr[6]->SetCellNumber(i, 1, cSensorStatus[iStatus]);
                    TMyTrayPtr[6]->SetCellColorIndex(i, 1, iStatus);
                ...
                for(int i=0; i<ROTATE_AMP_QTY; i++)                             // 第3組後面接第89 input rotate  10 11 output ROTATE SENSOR //kevin 20130420 socket sensor //KenHsieh 20250107 : 數量修改為變數
                {
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    ...
    sNu=Ptr->Tag/16;
    if(In_Shuttle_Auto_Latch==eInSHAutoLtc &&
       CANBUS_METHOD==1 && NUDN1_MACID11_AMP_QTY==2 &&
       Ptr->Tag<16)                                                             //KenHsieh 20251031 : In Shuttle sensor 只裝2個放大器
    {
        if(Ptr->Tag==8)
            sSen=1;
        else    //btIn2_8->Tag
    ...
    bSetData=true;                                                              //kevin 20220930 add Shuttle sensor set need save
    if(iInputMethod==1)
    {
        if(UseCanBusOrEtherCAT())                                               //Sam 20230707 : EtherCAT Shuttle sensor
        {
    ...
    if(UseSensorMod3())                                                         //Sam 20230707 : EtherCAT Shuttle sensor //kevin 20131115 設定cclink 偵測值     //Steven 20131008 : for HT9046AH
    {
        if(In_Shuttle_Auto_Latch==eInSHAutoLtc &&
           CANBUS_METHOD==1 && NUDN1_MACID11_AMP_QTY==2)                        //KenHsieh 20251031 : In Shuttle sensor 只裝2個放大器
        {
        ...
        {
            if(MachineTypeChoice==Type_HT9045 && UseSensorMod3())               //KenHsieh 20251126 : 12顆的Sensor 才需要減2    //Sam 20250821 Mark才對  //Ifor 20241216 Fix Index=2 In Shuttle 1 設定會位移兩格
              iIndex=2;
            else
              iIndex=0;
        }
...
void __fastcall TfCCLink::ReadSetting()
{
    ...
    if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                     //KenHsieh 20251128 : InSht Latch add percent for auto teach
    {
        AnsiString S=GetLastOpenFN();
        AnsiString szDir;
        szDir.sprintf("%s%s\\HandlerCondition.Data", DataPath, S);
        iInShtLtcPercent=ReadIniData(szDir, "Shuttle", "InShtLtcPercent", 80);
        edtInShtLtcPercent->Text=iInShtLtcPercent;
    }
    ...
    if(iInputMethod==1)
    {
        for(int i=0; i<20; i++)
        {
            ...
            if(UseCanBusOrEtherCAT())                                           //Sam 20230707 : EtherCAT Shuttle sensor
            {
                ...
                {
                    if(i==0 || i==8)
                        iValue=GetValue(0, i/8)*iPercentage/100;
                }
                else
                {
            ...
            else
            {
                ...
                {
                    if(i==0 || i==8)
                        iValue=MyCCLink->CCLinkBoard->MyNCUL[i/16].iCurrValue[i/8]*iPercentage/100;
                }
                else
                {
...
void __fastcall TfCCLink::SaveSetting()
{
    ...
    if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                     //KenHsieh 20251128 : InSht Latch add percent for auto teach
    {
        ...
        AnsiString szDir;
        szDir.sprintf("%s%s\\HandlerCondition.Data", DataPath, S);
        WriteIniData(szDir, "Shuttle", "InShtLtcPercent", edtInShtLtcPercent->Text);
    }
...
//        ledInSht1_8->Top=8;
    ...
    if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                     //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
    {
        ...
        ledInSht2_8->Top=iLedVisiblePos;
        edIn2_8->Visible=true;
        btIn2_8->Visible=true;
        ledInSht1_8->Top=iLedVisiblePos;
...
void __fastcall TfCCLink::btSaveClick(TObject *Sender)
{
    ...
    bSetData=false;                                                             //kevin 20230620 SAVE
}
//---------------------------------------------------------------------------
...
void TfCCLink::SetInitialOK(bool bInitailOK)
{
    if(fCCLink->MyCCLink!=NULL && (SHUTTLE_SENSOR_TYPE==eSensorCCLink || SHUTTLE_SENSOR_TYPE==eSensorCCLink3))          //Steven 20131008 : for HT9046AH
        fCCLink->MyCCLink->InitialOK=bInitailOK;                                                               //RogerYang 20250731 Fix
        ...
        fCCLink->DNM100UD->InitialOK=bInitailOK;                                                               //RogerYang 20250731 Fix
    else if(MyEtherCAT!=NULL && (SHUTTLE_SENSOR_TYPE==eSensorEtherCAT || SHUTTLE_SENSOR_TYPE==eSensorEtherCAT3))
        MyEtherCAT->InitialOK=bInitailOK;                                                                      //RogerYang 20250731 Fix
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void SingleSetValue(int itag, int iValue)                                       //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
{
    short sNu;
    int iIndex=0;
    ...
    else
    {
        ...
        if(UseSensorMod3() && sSen>=8 &&                                        //Sam 20230707 : EtherCAT Shuttle sensor //kevin 20131115 設定cclink 偵測值     //Steven 20131008 : for HT9046AH
           MachineTypeChoice==Type_HT9045)                                      //KenHsieh 20251126 : 12顆的Sensor 才需要減2)
        {
            iIndex=2;
        }
        else if(iSpecialSequence==1)
    ...
    if(UseCanBusOrEtherCAT())                                                   //Sam 20230707 : EtherCAT Shuttle sensor
    {
        fCCLink->SetIOValue(sNu, sSen-iIndex, iValue);                          //Sam 20230707 : EtherCAT Shuttle sensor
    }
    else
    {
        fCCLink->MyCCLink->DoSetValue(sNu, sSen-iIndex, iValue);
    }
}
//---------------------------------------------------------------------------
int SingleGetValue(int itag, int iPercen)                                       //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
{
    short sNu;
    int iValue=0;
    ...
    if(UseCanBusOrEtherCAT())                                                   //Sam 20230707 : EtherCAT Shuttle sensor
    {
        iValue=fCCLink->GetValue(sNu, sSen)*iPercen/100;                        //Sam 20230707 : EtherCAT Shuttle sensor
    }
    else
    {
        iValue=fCCLink->MyCCLink->CCLinkBoard->MyNCUL[sNu].iCurrValue[sSen]*iPercen/100;
    }

    return iValue;
}
//---------------------------------------------------------------------------
void __fastcall TfCCLink::btIn5_1MouseDown(TObject *Sender,
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    TButton *Ptr;
    Ptr=(TButton *)Sender;
    ...
    if(iInputMethod==1)
    {
        ...
        EditPtr[54+Ptr->Tag]->Text=AnsiString(iValue);
    }
    else
    {
        iValue=atoi(EditPtr[54+Ptr->Tag]->Text.c_str());
    }
