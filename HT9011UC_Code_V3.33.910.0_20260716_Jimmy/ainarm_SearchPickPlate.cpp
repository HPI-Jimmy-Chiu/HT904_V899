...
bool MoveInArmXYPickHotPlate_9045(bool bNeedDown)
{
    ...
    if(iInArmType==e9045_2x8_32 && HotPlateForm.XDivision==4)                   //Ifor 20260616 : 32-site跑4x8 HP X軸鎖列, 避免X軸soft limit error
    {
        iC=0;
        iPickPlateX[0]=0;
    }
    ...
    {
        if(iInArmType==e9045_2x8_32 && HotPlateForm.XDivision==4)               //Ifor 20260616 : 比照GetPlaceHotPlate_4_2x8_32 +2*XPitch基底
            iHPXPos=Prod.XInArm_Plate2_Pick[iInArmYBase][iInArmXBase]+2*Prod.HotPlateForm[0].iXPitch;
        else
    ...
    {
        if(iInArmType==e9045_2x8_32 && HotPlateForm.XDivision==4)               //Ifor 20260616 : 比照GetPlaceHotPlate_4_2x8_32 +2*XPitch基底
            iHPXPos=Prod.XInArm_Plate1_Pick[iInArmYBase][iInArmXBase]+2*Prod.HotPlateForm[0].iXPitch;
        else
...
bool DoInArmPickFromHotPlate_9045()
{
    ...
    switch(Task)
    {
        case 1:
            ...
            {
                if(MOT[MMPlate1].HasIC() || MOT[MMPlate2].HasIC())              //AI(ht9045-inarm-flow) 20260613 (RogerYang) : 佇列報空但盤實際有料=失步,記 log 對照
                {
                    MyDBIProcess("Exception",
                        AnsiString().sprintf("DoInArmPickFromHotPlate : No PickFromHPList but plate HasIC, group=%d",
                                             PickFromHPList->GetHPSuckGroupCount()));
                }
                ...
                if(InSHT1InLF())
                {
                    ...
                    else */
                    {
                        if(InArmSuck.iWhichKitPickFor32==0)
                        {
                            ...
                            {
                                if(USE_PICKER_COUNT==ep1Picker && FLCarryKit.UseSiteFullIC()==false)
                                {
                                    Task=190;
                                }
                                else
            ...
            else
            {
                ...
                if(InSHT2InLF())
                {
                    ...
                    if(InArmSuck.iWhichKitPickFor32==0)
                    {
                        ...
                        {
                            if(USE_PICKER_COUNT==ep1Picker && BLCarryKit.UseSiteFullIC()==false)
                            {
                                Task=190;
                            }
                            else
