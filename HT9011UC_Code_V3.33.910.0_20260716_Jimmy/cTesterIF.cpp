...
#include "fAOI.h"                                                               //Eastsun 20260710 Merge
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
__fastcall TFTestIF::TFTestIF(TComponent* Owner)
    : TForm(Owner)
{
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::InitcbDIOType(bool bAlarm)                            //Steven 20101008 : 連按三下會不見
{
    cbDIOType->Clear();
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::FormShow(TObject *Sender)
{
    AnsiString S;
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::SaveSetupFile(AnsiString szDir, AnsiString S)         //kevin 20150105
{
...
//==>
void __fastcall TFTestIF::CheckRs232StandardIni()
{
    int iTemp1, iTemp2;
    AnsiString szDir;
    bool bTemp;
...
//Sam 20181219 : Handler 與 Rs232Standard 設定同步
//---------------------------------------------------------------------------
void __fastcall TFTestIF::ReadTestIFFile()
{
    int iTemp;
    AnsiString S="";
    S=GetLastOpenFN();
    ...
    //    iTestBinCount=15;
        ...
        iTestBinCount=17;
            ...
            (TestIF_File.iGpibMode==InterfaceType_32Bin ||
             ...
             (FrmAOI->bCheckAOIFailBinUse() && CUSTOMER_CODE==CC_KYEC_LEE)))    //Eastsun 20260710 Merge
        iTestBinCount=33;
    else
        iTestBinCount=16;
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::DoIniDataToForm()
{
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::edMaxTestTimeKeyPress(TObject *Sender, char &Key)
{
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::Edit1KeyPress(TObject *Sender, char &Key)
{
    if(OnlyNumberInPut(Key)==false)
        Key=NULL;
}
//---------------------------------------------------------------------------
void __fastcall TFTestIF::rgInterfaceTypeClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::ShowPageControl2(int Index)
{
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::edMaxTestTimeMouseDown(TObject *Sender,
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    fQwertyKey->ShowQwertyKey((TEdit *)Sender, N_DOUBLE, 0, true, 0.0, 5000.0); //kevin 20200804 add min time
}
//---------------------------------------------------------------------------
void __fastcall TFTestIF::Edit1MouseDown(TObject *Sender,
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    fQwertyKey->ShowQwertyKey((TEdit*) Sender, N_INTEGER, 0, true, 100000, 1);
}
//---------------------------------------------------------------------------
void __fastcall TFTestIF::FormClose(TObject *Sender, TCloseAction &Action)
{
    ReadTestIFFile();
    fShow=false;
    bflag=false;
...
//---------------------------------------------------------------------------
//jou 980331
void __fastcall TFTestIF::ShowTTLState()
{
    lstTTL->Clear();
    AnsiString sString;
    char cStr[128]="";
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::cbDIOTypeChange(TObject *Sender)
{
    AnsiString S;
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::AntiSignalCBoxClick(TObject *Sender)
{
    TestIF.bAntiSignal = AntiSignalCBox->Checked;
}
//---------------------------------------------------------------------------
void __fastcall TFTestIF::cbRs232TypeChange(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::spbSaveClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::sbtExitClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::edMaxBinCountMouseDown(TObject *Sender,
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    fQwertyKey->ShowQwertyKey((TEdit*) Sender, N_INTEGER, 0, true, 15, 255);    //Steven 20121116 : RS232回傳的最大Bin數  //kevin 20140317 for 256 bin
}
//---------------------------------------------------------------------------
void __fastcall TFTestIF::edTempOffsetCountMouseDown(TObject *Sender,
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    fQwertyKey->ShowQwertyKey((TEdit*) Sender, N_INTEGER, 0, true, 1, 50);
}
//---------------------------------------------------------------------------
void __fastcall TFTestIF::edInitialMaxTestClick(TObject *Sender)
{
    ...
    {
        fQwertyKey->ShowQwertyKey((TEdit*) Sender, N_DOUBLE, 2, true, 0, 50000);//Jimmychiu 20260302 : Ben要求最大測試時間更改至50000
    }
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::edtInitialDelay_1Click(TObject *Sender)
{
    fQwertyKey->ShowQwertyKey((TEdit *)Sender, N_DOUBLE, 2, true, 0.0, 3000.0);
}
//---------------------------------------------------------------------------
void __fastcall TFTestIF::edtInitStartDelayClick(TObject *Sender)
{
    if(CosFunction.bHiSiliconFunction && CUSTOMER_CODE==CC_ASE_KaohSiung)       //kevin 20191227 add  Initial Start Delay Time 需卡下限, 最小30秒.  //JerryYang 20220330 : 只有高雄才卡
        fQwertyKey->ShowQwertyKey((TEdit *)Sender, N_DOUBLE, 2, true, 30.0, 3000.0);
    else
        fQwertyKey->ShowQwertyKey((TEdit*) Sender, N_DOUBLE, 2, false);
}
//---------------------------------------------------------------------------
void __fastcall TFTestIF::DoSetRPDefault(TWinControl *PCtrl, int iSection)
{
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::DoReplyDefaultToForm(TWinControl *PCtrl, int iSection)
{
    AnsiString aHintName;
...
//--------------------------------------------------------------------------
void __fastcall TFTestIF::SearchRecipeParameter(TWinControl *PCtrl, int iSection)
{
    AnsiString aHintName;
    AnsiString szDir="D:\\HT9045\\IniData\\RPDefault.ini";
    AnsiString aSection="";
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::btTesterTCPShowClick(TObject *Sender)
{
    fTesterTCP->Show();
}
//---------------------------------------------------------------------------
void __fastcall TFTestIF::FormShortCut(TWMKey &Msg, bool &Handled)
{
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::edTCPIP_PortClick(TObject *Sender)
{
    fQwertyKey->ShowQwertyKey((TEdit*)Sender, N_PORT);
}
//---------------------------------------------------------------------------
void __fastcall TFTestIF::edPowerSwitchDelayClick(TObject *Sender)
{
    fQwertyKey->ShowQwertyKey((TEdit *)Sender, N_DOUBLE, 2, true, 0, atof(edMaxTestTime->Text.c_str()));
}
//---------------------------------------------------------------------------
void __fastcall TFTestIF::edMaxBIOSWaitTimeClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TFTestIF::edMinTestTimeClick(TObject *Sender)
{
    fQwertyKey->ShowQwertyKey((TEdit *)Sender, N_DOUBLE, 2, true, 0, atof(edSLTMaxTestTime->Text.c_str())-1);
}
//---------------------------------------------------------------------------
void __fastcall TFTestIF::edtAutoOnecycleHomStartTimeClick(TObject *Sender)
{
    fQwertyKey->ShowQwertyKey((TEdit*)Sender, N_INTEGER, 0, true, 10000, 10);
}
//---------------------------------------------------------------------------
void __fastcall TFTestIF::edGPIBAddressClick(TObject *Sender)
{
    fQwertyKey->ShowQwertyKey((TEdit*)Sender, N_INTEGER, 0, true, 64, 1);
}
//---------------------------------------------------------------------------
void __fastcall TFTestIF::edtInitialDecCount1Click(TObject *Sender)
{
    fQwertyKey->ShowQwertyKey((TEdit *)Sender, N_INTEGER, 0, true, 0, 3000);
}
//---------------------------------------------------------------------------
void __fastcall TFTestIF::edPurgeAirClick(TObject *Sender)
{
