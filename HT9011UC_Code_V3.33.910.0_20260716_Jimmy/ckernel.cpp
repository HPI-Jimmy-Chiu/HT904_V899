...
#include "fAOI.h"                                                               //Eastsun 20260515 F023: E7 need FrmAOI pointer
//<==
//Austin 20190531.01 增加PLC_IO模組及安全相關IO.
#include "acarry.h"                                                             //AI(ht9045-staterecord-analysis) 20260525 (RogerYang) : for ResetShtMoveTimeoutWatchdog
//---------------------------------------------------------------------------
...
//        else
    ...
    {
        RunState=LED_Message;
        if(bPauseAlarmDelayActive && hPauseAlarmDelay.Off())                    //RogerYang 20260626 : Pause 逾時(>MaxTestTime)才響
        {
            bTesterPauseMusic=true;                                             //RogerYang 20260626 : 逾時點啟動蜂鳴器+訊息
            bPauseAlarmDelayActive=false;                                       //RogerYang 20260626 : latch,避免重複觸發
        }
    ...
    if(bN07AlarmActive)                                                         //RogerYang 20260625 : SECS disconnect alarm in ANY handler state (JSCC) -- buzzer + message regardless of run/stop/home
    {
        if(RunState!=LED_ErrJam && RunState!=LED_Homeing)                       //homing/jam 保留自己的聲音與優先權
        {                                                                       //homing/jam 不覆寫 RunState、不設 bAlarmBuzzer -> 播各自的音；N07 視覺(紅框/紅燈塔)仍持續
            RunState=LED_Message;
            if(!bN07BuzzerSilenced)
                bAlarmBuzzer=true;
        }
    }
    // 處理音樂
    ...
    else
    {
        ...
        }   //bN07AlarmActive else
    }   //bEnable_SECS_GEM PPSIGNALTOWER else
...
//        }
        ...
        if(Sen[SnFrontLeftEMG].IsOff())
            ...
            EmgStr="EMG 3";
        else if(IsSafePLCIOInstall() && Sen[SnAllEMG].IsOff())                  //KenHsieh 20250212 : 新增PLC 斷線可瞬間判斷EMG及安全門
            EmgStr="EMG & PLC";
        else
...
int ScanPannelKey()
{
    ...
    if(bFrontPadActive)
    {
        ...
        if(Sen[SnRKAlarmReset].IsOn() || bAseAlarmReset)
        {
            ...
            if(bTesterSendPause && TestIF.iMaxTime>0)                           //RogerYang 20260626 : Reset 後仍在 Pause→重設計時(C/D)
            {
                hPauseAlarmDelay.SetSecAndOn(TestIF.iMaxTime);                  //RogerYang 20260626 : Timer ReStart
                bPauseAlarmDelayActive=true;
            }
            if(bN07AlarmActive) bN07BuzzerSilenced=true;                        //Steven 20260603 : silence N07 buzzer on alarm reset
        }
