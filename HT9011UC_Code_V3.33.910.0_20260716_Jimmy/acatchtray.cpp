...
#include "cSortCT.h"
//#include "common.h"
#include "Magazine.h"                                                           //Eastsun 20260515 F011 AMR: SearchNeedAMRUnload/InitialDoMagazineAMRTrayFeed
...
bool bCoverTrayInProgress;                                                      //Eastsun 20260710整合
...
int WhichAutoNeedTray()
{
    ...
    else
    {
        ...
        if(fAGV->Use_AMR())                                                     //Steven 20251216 : for AMR
        {
            ...
            {
                if(bNeed1DCoverTray[i]==true)                                   //Need 1D Tray
                {
                    return  i+1;
                }
                else if(bNeedCoverTray[i]==true)                                //Need Cover Tray
                {
                    return  i+1;
                }
        ...
        else if(LastSet.iTester==OFF_LINE &&
                bPurgeOutAllDevice==true)                                       //JerryYang 20251020 : 渠梁半清機功能
        {
            for(int i=0; i<=iAutoRight; i++)
            {
        ...
        if(CUSTOMER_CODE==CC_KYEC_LEE && bDummyRunPosCheck==true)
        {
            if(bDummyRunPosCheckCatchTray[0]==false ||
               bDummyRunPosCheckCatchTray[1]==false ||
               bDummyRunPosCheckCatchTray[2]==false)
            {
                for(int i=0; i<3; i++)
                {
            ...
            else if(bDummyRunPosCheckFinish==true &&
               ...
               ((iRunStartMode==RT || iRunStartMode==RT_ART) && TrayForm.bFailAutoTrayManual_RT==true)))                 //Steven 20150116 : 手動移除Auto Fail Bin Tray
            {
                if(Prod.iIsPassT6[i]==1)                                        //Steven 20240105 : Prod.bIsPass --> Prod.iIsPassT6
                {
                    if(MOT[iMMAuto[i]].fHasTray==false || MOT[iMMAuto[i]].Tray.FullIC())
                    {
                        ...
                        if(LastSet.iRunStartMode == rsmContinuStart_ART && HasICUnderMachineART() == false &&
                           (MOT[iMMAuto[i]].fHasTray && MOT[iMMAuto[i]].FullIC() && BinSelect[FT_ART].bAutoRetest[i] == true) &&
                           CUSTOMER_CODE==CC_KYEC_LEE)
                        {
                            return 0;   //Eastsun ART 模式 最後一盤不要退 不需要夾空盤
                        }
...
int DoCatchFromLoader()
{
    ...
    static int iTrayXEncErrCT=0;                                                //RogerYang 20260617 : 增加到位保護
    ...
    switch(Task)
    {
        ...
        case 10:                                                                //Steven 20170922 (wei) : 加上保護
               ...
               InArmXYZSafe()==true)                                            //Steven 20250311 : 確認Tray能移動
            {
                ...
                int Rlt=TrayXMoveCheckEnc(Prod.iXTrayLoad);                     //AI(ht9045-motor-control) 20260617 (RogerYang) : 取代原 MotorMove(iXTrayLoad)
                ...
                else if(Rlt==2)                                                 //到位但位置錯 → 已停車
                {
                    if(++iTrayXEncErrCT >= 3)                                   //連續 3 次
                    {
                        iTrayXEncErrCT=0;
                        Cylinder[C_TrayX_UpDown].Off();                         //確保在上
                        ShowErrorMessage("JAM0616", K_HOME, MTrayX, false, "TrayX_EncPosErr_10");  //暫用 JAM0616 只給 HOME → 操作員按下後全機回原點
                    }
                }
            }
            else
        ...
        case 501:                                                               //kevin 20120718  Auto 2 Auto 3
               ...
               IniConfig.bC03UseCatchTray==false)
            {
                if(CatchTraySuck.Suck[0][0].Enable && CatchTraySuck.Suck[0][0].GetStatus()==false)
                {
                    ...
                    iCnt++;
                    //if(CUSTOMER_CODE!=CC_KYEC_LEE || iCnt>iTrayDetectCount)  //Eastsun 20260710整合
                    if((CUSTOMER_CODE!=CC_KYEC_LEE || iCnt>iTrayDetectCount) && bCoverTrayInProgress==false)  //Eastsun 20260710整合
                    {
                        iCnt=0;
...
bool IsOutArmSafe()
{
    ...
    {
        int iMargin;                                                            //RogerYang 20260626 : ATK發生雙排但還沒到位TrayArm就過來，調整讓位tolerance，暫時保守在這裡修正
        ...
        {
            iMargin=(CUSTOMER_CODE==CC_AMKOR_Korea)?500:3500;                   // 單排後排:維持原值 (ATK再保守一點)
        }
        else
        {
            iMargin = 500;                                                      // 單排前排 Row1
        }
...
bool DoPlaceTrayToAuto(int AutoTarget)
{
    ...
    switch(Task)
    {
        ...
        case 250:
            ...
            {
                iCnt++;
                //if(iCnt>100)                                                    //JerryYang 20220923 : add  //Eastsun 20260710整合
                if(iCnt>100 && bCoverTrayInProgress==false)                                                    //JerryYang 20220923 : add  //Eastsun 20260710整合
                {
                    iCnt=0;
            ...
            else
            {
                if(TrayArmMotorMove(Prod.iXTrayEmpty))
                {
                    ...
                    else
                    {
                        ...
                        else if(LastSet.iTester==OFF_LINE && bPurgeOutAllDevice==true)//JerryYang 20251020 : 渠梁半清機功能
   ...
   //kevin 20110601 整個函數修改
    switch(Task)
    {
        ...
        case 2100:
            ...
            {
                iCnt++;
                //if(iCnt>100)  //Eastsun 20260710整合
                if(iCnt>100 && bCoverTrayInProgress==false)  //Eastsun 20260710整合
                {
                    iCnt=0;
...
//                    if(TrayForm.LoaderToEmptyColor[iRunStartMode]==0)           //jou 2014-06-10 修正Catch Tray放置Color Tray時設置錯的Tray 位置
        ...
        case 3020:
            if(CheckCoverTrayID())
            {
                if(asColorCoverTrayID.Pos("ERROR")>0)                           //Eastsun 20260701//Eastsun 20260710整合
                {
                    ret=ShowErrorMessage("JAM1014", K_RETRY|K_SKIP, MTrayX, false, __FUNC__);
                    if(ret==K_RETRY)
                    {
                        Task=3015;
                        break;
...
void DoCatchTray()
{
    ...
    AnsiString str="";
    int iBinPos=0;
    static TQPF_Timer tCatchStuckLog;                                           //AI(ht9045-staterecord-analysis) 20260622 (RogerYang) : Catch-stuck 診斷 移動端vs檢查端 + enc/cmd 實值
    static bool bCatchStuckPrev=false;
...
//            iPosX=MOT[MInArmX].ReadPos();                                     //Steven 20241018 : 安全點不需要比較X軸
            ...
            if(MOT[MMTrayY].Tray.HasIC()==false &&                              //AI(ht9045-staterecord-analysis) 20260622 (RogerYang) : Catch-stuck 卡滿15s才開始記(期間不記)
               MOT[MMTrayY].fHasTray==true &&
               bPlaceToHotplate==false &&
               InArmXYZSafe()==false)
            {
                if(bCatchStuckPrev==false)                                      //剛開始卡: 起算15s, 期間不記
                {
                    bCatchStuckPrev=true;
                    tCatchStuckLog.SetSecAndOn(15);
                }
                else if(tCatchStuckLog.Off())                                   //連續卡滿15s才開始記, 之後每15s一組
                {
                    AnsiString s1, s2;
                    s1.sprintf("[CatchStuck] moveOK=%d waitPos=%d XYZSafe=0 LedX=%d LedY=%d encOKx=%d encOKy=%d cmdOKx=%d cmdOKy=%d p3Y=%d Zsafe=%d",
                        (int)bMoveInArm2XYToWaitOk, iInArmWaitPosition,
                        (int)MOT[MInArmX].Led[iInposLed], (int)MOT[MInArmY].Led[iInposLed],
                        MOT[MInArmX].CompareEncoderPos(Prod.iInArmSafeX, 9), MOT[MInArmY].CompareEncoderPos(Prod.iInArmSafeY, 9),
                        MOT[MInArmX].CompareCommandPos(Prod.iInArmSafeX, 2), MOT[MInArmY].CompareCommandPos(Prod.iInArmSafeY, 2),
                        (MOT[MInArmY].ReadPos() >= Prod.YInArm_Shuttle1_Place[iInArmYBase][iInArmXBase]-300) ? 1 : 0,
                        (int)InArmZInSafe());
                    s2.sprintf("[CatchStuck] encX=%d cmdX=%d safeX=%d | encY=%d cmdY=%d safeY=%d | Sht1Y=%d",
                        MOT[MInArmX].ReadEncoderPos(), MOT[MInArmX].ReadPos(), Prod.iInArmSafeX,
                        MOT[MInArmY].ReadEncoderPos(), MOT[MInArmY].ReadPos(), Prod.iInArmSafeY,
                        Prod.YInArm_Shuttle1_Place[iInArmYBase][iInArmXBase]);
                    RecordProcess(s1);
                    RecordProcess(s2);
                    tCatchStuckLog.SetSecAndOn(15);                             //之後每15s一組
                }
            }
            else
            {
                bCatchStuckPrev=false;                                          //條件一解除就重新計時(下次再卡重新從0算15s)
            }
...
{
    CoverTrayTask=1;
    bCoverTrayInProgress=false;                                                  //Eastsun 20260710整合
}
//------------------------------------------------------------------------------
...
//    }
    switch(Task)
    {
        ...
        case 1020:
            if(CheckCoverTrayID())
            {
                if(asColorCoverTrayID.Pos("ERROR")>0)                           //Eastsun 20260701  新增ID 異常
                {
                    ret=ShowErrorMessage("JAM1014", K_RETRY|K_SKIP, MTrayX, false, __FUNC__);
                    if(ret==K_RETRY)
                    {
            ...
            break;
        case 1500:
            bCoverTrayInProgress=true;                                          //Eastsun 20260710整合
        ...
        case 1700:
            ...
            //==>
            if(TrayForm.iTrayOrder[Target]==eNoUse || Target>=3)  //Eastsun 20260710整合
            {
