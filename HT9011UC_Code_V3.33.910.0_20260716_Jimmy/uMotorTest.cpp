      ...
      TScrollCode ScrollCode, int &ScrollPos)
{
    ...
    MOT[ActiveIndex].SetSpeed(scrlbrMotorSpeed->Position, true);                //RogerYang 20250729 Add for 9046AU
    ...
    if(btnLoopMove->Down)
    {
        for(int i=0; i<int(MotorTestClass.size()); i++)
            if(MotorTestClass[i]->Visible==true &&                              //Steven 20240822 : 沒用到的馬達不new
               MotorTestClass[i]->cbUsing->Checked)
                MOT[i].SetSpeed(scrlbrMotorSpeed->Position, true);              //RogerYang 20250729 Add for 9046AU
    }
}
//---------------------------------------------------------------------------
...
bool __fastcall TfMotorTest::IsMotorCanRun(bool attr)
{
    ...
    if(Sen[SnRearRightEMG].IsOff())
        flag=false;
    if(IsSafePLCIOInstall() && Sen[SnAllEMG].IsOff())                           //KenHsieh 20250212 : 新增PLC 斷線可瞬間判斷EMG及安全門
        flag=false;
