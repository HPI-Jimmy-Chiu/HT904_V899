...
bool CheckOutArmXYPitch_1x2_4(int *iX, int *iY, int iSht, int iMovePitchX, int iMovePitchY)
{
    ...
    if(USE_OUT_ARM_Y_PITCH==iXYPitch16Bd_Be)                                    //基準為第五隻吸嘴 //Ztex 2023.12.06 Add HT-1032  //JerryYang 20251218 : IN/OUT ARM支援不同模組
    {
    }
    ...
    switch(Task)
    {
        case 1:
            if(ArmSpeed[OutArm].dWaitOnSH!=0)                                   //kevin 20210525 out arm 起動在shuttle 上等待 時間
            {
                ...
                {
                    if(MoveOutArmToShuttleIncludeZ_9045_1x2_4(iSht, false))     //移到 Shuttle 上等待
                    {
                ...
                if(bHasErr)
                {
                    ...
                    if(ret==K_SKIP)
                    {
                        for(int j=0; j<OutArmSuck.iPickCol; j++)
                        {
                            ...
                            if(OutArmSuck.Suck[iSuckRow][j].Error)
                            {
                                PorcessJAM0201OutArmPickUpErrorSkip(iSht, 0, j, iSuckRow, j);                           //Steven 20161214 : 加上OutArm吸取異常Skip的ErrorLog
                            }
                        }
...
//  static bool FirstEnter=true, bCheckSpeed=false, bOutArmNeedMoveToSht=false;  //JerryYang 20170828 add bOutArmNeedMoveToSht
    static bool FirstEnter=true, bCheckSpeed=false;                               //JerryYang 20170828 (bOutArmNeedMoveToSht W8004 removed)
    int &Task=OutArmTask;
...
//#endif
        ...
        case 100:
            ...
            break;
        case 150:
            if(MoveOutArmXY_To_ESDSafePos())                                    //==> Eastsun 20260521 整合
            {
                if(WhichAutoNeedTray()==0)
                    Task=100;
            }
...
//#endif
        ...
        case 5100:                                                                                                      //判斷另一個Shuttle是否有IC
            if(iPickFinish==0x01)
            {
                ...
                {
                    iOutRotateFinish=0;                                                                                 //kevin 20130524 準備放到ROTATE   //JerryYang 20161007 修正Out Sht1 的Device沒有放到rotate
                    Task=3000;
                }
            }
