...
//                            else
                       ...
                       IniConfig.bN23_1_Enable2DIDCompare)                                                              //Steven 20200611 : for Murata, 2DID NG不測試
                    {
                           ...
                           TestSocket.b2DIDNG[i][j]==true)                                                              //Steven 20200611 : for Murata, 2DID NG不測試
                        {
                            ...
                            TestSocket.SetItemData(i, j, TEST_PASS+iTestBinCount);
                            TestSocket.iBinData[i][j]=iTestBinCount;
                            TestSocket.PordRec[i][j].AddTestResultRecord(iTestBinCount, "NonTestToRBin");
                        }
                    }
...
void DoTestHeadMotor()
{
    ...
    int ret, TorqueData, iIndexUpPos=0, ccRet, iContactZ=0;
    ...
    switch(Task)
    {
        case 1:                                                                 //確認Index Arm 吸嘴狀態
            //ReadWriteBinCountMode(true);                                      //JerryYang 20250120 : modify  //kevin 20210825 寫 Bin 1  Bin 2...資料記錄
...
//            bContSoftEpSwitch(1, true);                                         //ARM2 先浮動頭充氣  //kevin 20240816
        ...
        case 12101:
            ...
            {
                // JerryYang 20260629 : Index IC drop detection during Z1 DOWN - check both ARM1 & ARM2
                if(LastSet.iRealDummy==REALLY &&
                   MOT[MTestZ1].Gali_ReadPos() > (Prod.TestZ1_Test - Prod.TestZ1_Drop_Offset + 500))
                {
                    bool bDropF=CheckIndexAllSuckICFallDown(true, false);
                    bool bDropB=CheckIndexAllSuckICFallDown(false, true);
                    if(bDropF || bDropB)
                    {
                        MOT[MTestZ1].Gali_Command("ST", "DoTestHeadMotor 12101");
                        Task = 12500;
                        break;
                    }
                }
...
//                        ShowErrorMessage("WAR0322", K_RETRY, MTestZ1, false, str);//kevin 20130504 socket sensor
        ...
        case 14000:
            ...
            break;
        case 12500:                                                             //JerryYang 20260629 : Index IC Drop - move Z1 to safe
            if(MOT[MTestZ1].Gali_Two_ZAxis_Move(Prod.TestZ1_Safe, iSpeedSlow, "DoTestHeadMotor 12500"))
                Task=12501;
            break;
        case 12501:                                                             //JerryYang 20260629 : Index IC Drop - move Y to safe
            if(MOT[MTestY1].GalilTwoY_Move(Prod.TestY1_Front, Prod.TestY2_Rear, iSpeedY, "DoTestHeadMotor 12501"))
                Task=12502;
            break;
        case 12502:                                                             //JerryYang 20260629 : Index IC Drop - show alarm (ARM1 or ARM2)
        {
            bool bHasErrF=false;
            AnsiString ErrPartB="";
            bool bHasErrB=false;
            // Check ARM1 (FTestSuck)
            ErrPart="";
            for(int i=0; i<MAX_Index_Row; i++)
            {
                for(int j=0; j<NEW_MAX_Index_Col; j++)
                {
                       ...
                       FTestSuck.Item[i][j]!=NULL_IC)
                    {
                        ...
                        {
                            ErrPart+=IndexSuckName[i+IsNNMode()][j];
                            bHasErrF=true;
                        }
                    }
                }
            }
            // Check ARM2 (BTestSuck)
            for(int i=0; i<MAX_Index_Row; i++)
            {
                for(int j=0; j<NEW_MAX_Index_Col; j++)
                {
                       ...
                       BTestSuck.Item[i][j]!=NULL_IC)
                    {
                        ...
                        {
                            ErrPartB+=IndexSuckName[i][j];
                            bHasErrB=true;
                        }
                    }
                }
            }
            if(bHasErrF || bHasErrB)
            {
                if(CosFunction.bJAM0303NeedOpenChamberDoor)
                    bIsTestSitICFallDown=true;
                if(bHasErrF)
                {
                    ShowErrorMessage("JAM0303", K_SKIP, MTestZ1, false, ErrPart);  //JerryYang 20260629 : Device Drop Error ARM1
                    for(int i=0; i<MAX_Index_Row; i++)
                    {
                        for(int j=0; j<NEW_MAX_Index_Col; j++)
                        {
                ...
                {
                    ShowErrorMessage("JAM0304", K_SKIP, MTestZ2, false, ErrPartB);  //JerryYang 20260629 : Device Drop Error ARM2
                    for(int i=0; i<MAX_Index_Row; i++)
                    {
                        for(int j=0; j<NEW_MAX_Index_Col; j++)
                        {
        ...
        case 14101:
            ...
            {
                // JerryYang 20260629 : Index IC drop detection during Z2 DOWN - check both ARM1 & ARM2
                if(LastSet.iRealDummy==REALLY &&
                   MOT[MTestZ2].Gali_ReadPos() > (Prod.TestZ2_Test - Prod.TestZ2_Drop_Offset + 500))
                {
                    bool bDropF=CheckIndexAllSuckICFallDown(true, false);
                    bool bDropB=CheckIndexAllSuckICFallDown(false, true);
                    if(bDropF || bDropB)
                    {
                        MOT[MTestZ2].Gali_Command("ST", "DoTestHeadMotor 14101");
                        Task = 12500;
                        break;
                    }
                }
...
//                        ShowErrorMessage("WAR0322", K_RETRY, MTestZ2, false, str);//kevin 20130504 socket sensor
        ...
        case 600:
            ...
            break;
        case 15000:
            // JerryYang 20260629 : Check both ARM1 & ARM2 IC drop during Z1 UP to safe
            if(LastSet.iRealDummy==REALLY)
            {
                bool bDropF=CheckIndexAllSuckICFallDown(true, false);
                bool bDropB=CheckIndexAllSuckICFallDown(false, true);
            ...
            break;
        case 15100:
            // JerryYang 20260629 : Check both ARM1 & ARM2 IC drop before Y2 moves to socket position
            if(LastSet.iRealDummy==REALLY)
            {
                bool bDropF=CheckIndexAllSuckICFallDown(true, false);
                bool bDropB=CheckIndexAllSuckICFallDown(false, true);
                if(bDropF || bDropB)
                {
                    Task = 12500;                                               //unified: Z safe -> Y safe -> alarm -> Task=1
                    break;
                }
            }
            if(CosFunction.bSortingBy2DList==true &&
