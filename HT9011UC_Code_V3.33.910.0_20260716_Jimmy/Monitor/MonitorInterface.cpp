    ...
    : TForm(Owner)
{
    ...
    for(int i=0; i<4; i++)                                                      //AI(ht9045-v899) 20260422: initialize Greatek monitor Ch3 status slot
    {
        iStatus[i]=0;
    }
...
void __fastcall TfMonitor::MonitorTimerTimer(TObject *Sender)   //0.1
{
    ...
    if(MVCtrl->IsConnect()==true)
    {
        ...
        {
            for(int iNum=0;iNum<4;iNum++)                                       //AI(ht9045-v899) 20260422: reopen Greatek monitor Ch3 after reconnect
            {
                if(iStatus[iNum]==1)
                {
...
//                }
        ...
        if(bCleanOutClosedMonitorVideo)
        {
            ...
            if(iCleanOutCT>=iWaitCT*10)
            {
                ...
                StopMonitorVedio(0);
                if(CUSTOMER_CODE==CC_Greatek)                                   //AI(ht9045-v899) 20260422: sync Greatek monitor Ch3 close with Ch0
                    StopMonitorVedio(3);
            }
        }
