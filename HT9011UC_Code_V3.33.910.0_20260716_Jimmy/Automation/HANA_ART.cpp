...
{   //DEVON:16,85,0,CT01,2
    ...
    if(List->Count==4 ||                                                        //Steven 20250414 : HANA ART Function
       List->Count==5)
    {
        int    iSite=atof(List->Strings[0].c_str());
        double dTemp=atof(List->Strings[1].c_str());
        double dSoak=atof(List->Strings[2].c_str());
        AnsiString sBinConfig=List->Strings[3].Trim();
        ...
        {
            int iRTCount=atof(List->Strings[4].c_str());
            if(iRTCount>0)
            {
