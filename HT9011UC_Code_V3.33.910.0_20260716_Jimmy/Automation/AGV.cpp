...
bool TfAGV::IsSPIL_AMR()                                                        //JerryYang 20250521 : For AMR
{
    ...
    if(USE_COVER_TRAYID==tCID_Keyence &&
       ...
       CUSTOMER_CODE!=CC_XINYUN)                                                //RogerYang 20260630 : Add
    {
        bResult=true;
    }
    return bResult;
...
//==============================================================================
bool TfAGV::IsTFAMD_AMR()                                                        //Ifor 20260715 : TFAMD AMR flow (CC_AMD_M + A68 on + A65 off, no ID reader)
{
    bool bResult=false;
    if(CUSTOMER_CODE==CC_AMD_M &&                                               //Ifor 20260716 : only AMD(CC_AMD_M) can enter TFAMD AMR flow
       IniConfig.bA68_AutoLoadUnload==true &&
...
void TfAGV::ScanLoadPort()
{
    if((IniConfig.bA65_BundleIDList==true &&
       IniConfig.bSPILFunction==true) ||
       IsTFAMD_AMR()==true)                                                      //JerryYang 20250521 : For AMR  //Ifor 20260715 : TFAMD AMR also scan ports
    {
...
TQPF_Timer tTFAMDReleaseTimer[3];                                                //Ifor 20260715 : TFAMD AMR auto release timer
int iTFAMDReleaseStep[3]={0,0,0};                                                //Ifor 20260715 : TFAMD AMR auto release step
//==============================================================================
...
void bScanUnLoadPortState_SPIL(int iPos)                                        //JerryYang 20250521 : For AMR
{
    ...
    {
        //==> Ifor 20260715 : TFAMD AMR - auto release unload port after AMR took the bundle
        if(fAGV->IsTFAMD_AMR()==true &&
           bAskStopPort[ePortAuto1+iPos]==true &&
           bPortIsBusy[ePortAuto1+iPos]==false &&
           iPortStatus[ePortAuto1+iPos]==eReadyToUnload)
        {
            if(Sen[SnAutoTrayHasTray[iPos]].IsOff()==true)
            {
                if(iTFAMDReleaseStep[iPos]==0)
                {
                    iTFAMDReleaseStep[iPos]=1;
                    tTFAMDReleaseTimer[iPos].SetSecAndOn(3);
                }
                else if(tTFAMDReleaseTimer[iPos].Off()==true)
                {
                    iTFAMDReleaseStep[iPos]=0;
                    bAskStopPort[ePortAuto1+iPos]=false;
                    ...
                    RecordProcess("TFAMD AMR auto release unload port");
                }
            }
            else
            {
