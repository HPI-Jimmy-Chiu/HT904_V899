...
#include "ATC_Handler_Side.h"                                                   //Eastsun 20260522 整合: ATC_InterfaceForm
#include "INPUT.h"
#include "Password.h"
#include "TfFTP.h"
#include "TesterTCP.h"
...
void __fastcall TfFTPClient::LoadFileFormServer2(AnsiString FtpPath, AnsiString Source)
{
    ...
    try
    {
        ...
        else
        {
            ...
            if(CosFunction.bUseATCFileTransfer==true)                           //Eastsun 20260522 整合: ATC FileTransfer 用 .dat
            {
    ...
    try
    {
        ...
        if(bZip)                                                                //ChungHung 20140108 add FTP unload jam code
        {
            if(CUSTOMER_CODE==CC_TSMC_TAINAN)                                   //ChungHung 20150413 add for TSMC
            {
                ...
                if(CosFunction.bUseATCFileTransfer==true && ATC_SYSTEM==eNewATCSystem && Temperature.bATCActiveCooling==true && bPIDTransferErr==false)  //Eastsun 20260522 整合
                {
            ...
            else
            {
                ...
                //上傳ATC工作檔
                if((CosFunction.bUseFTPDownLoadATCRecipe==true || CosFunction.bUseATCFileTransfer==true) &&  //Eastsun 20260522 整合
                   ATC_SYSTEM==eNewATCSystem &&
                   Temperature.bATCActiveCooling==true && bPIDTransferErr==false)                         //Ifor 20191115 : add FTP DownLoad ATC Recipe
                {
                    if(CosFunction.bUseATCFileTransfer==true)  //Eastsun 20260522 整合: .dat 已於壓縮前直接複製，無需 FTP 上傳
                    {
                        ;
                    }
                    else
        ...
        else
        {
            if(CUSTOMER_CODE==CC_PTI && Source.Pos("JamAlarmLogTxt")>0)         //Sam 20170518 (wei) 增加路徑檢查 JamAlarmLogTxt //Sam 20170502 (wei) JamAlarm Data 格式修改 //RogerYang 20170417 for 力成 JamAlarm Data 格式
            {
                ...
                Source=Source.SubString(n+1 ,Source.Length());                  //刪除 Lot ID 路徑 "//LotId"     //RogerYang 20170508 (wei) 修改 n->n+1
                    ...
                    Source.Pos("_QA")>0))                                       //RogerYang 20260616 : 新增QA
            {
                int iPos=(Source.Pos("_FT")>0)?Source.AnsiPos("_FT"):Source.AnsiPos("_QA");       //RogerYang 20260616 : 新增QA
...
void __fastcall TfFTPClient::plSLoadClick(TObject *Sender)
{
    ...
    else
    {
        ...
        if(edtServerWaferName->Text!=lstServerFile->Items->Strings[0])          // Landam  選對名稱會一樣
        {
            ...
            bDownloadFTP=false;
            edtServerWaferName->Text="";
            bCanExit=true;   //Steven 20260618 fix: == -> = (P11 bug fix)                                                     //ChungHung 20150413 add for TSMC
            Close();
            return;
        }
    }
    ...
    if(CosFunction.bUseATCFileTransfer==true &&                                 //Eastsun 20260522 整合: 下載前清空 ATC FileTransfer 目錄
       ATC_SYSTEM==eNewATCSystem )
    {
    ...
    bCanExit=false;  //Steven 20260618 fix: == -> = (P11 bug fix)                                                            //ChungHung 20150413 add for TSMC
...
void __fastcall TfFTPClient::plUnloadClick(TObject *Sender)
{
    ...
    else if(CosFunction.bUseATCFileTransfer==true)                              //Eastsun 20260522 整合: ATC FileTransfer 模式，先觸發 ATC 端傳送 PID
    {
        if(bStartATCRun && ATC_InterfaceForm->IsConnect())
        {
            ATC_InterfaceForm->GET_ATC_Recipe();
            RecordProcess("FTP Upload PID File Transfer from ATC Start!!");
            memoFTP->Lines->Add("FTP Upload PID File Transfer from ATC Start!!");
            bPIDTransferErr=false;
            return;
        }
        else
        {
            if(CosFunction.bUseATCFileTransfer==true && ATC_SYSTEM==eNewATCSystem)  //Eastsun 20260522 整合
            {
...
void __fastcall TfFTPClient::NMFTP1Error(TComponent *Sender, WORD Errno,
      AnsiString Errmsg)
{
    ShowMyMessage("TfFTPClient:NMFTP1Error:"+Errmsg, "");          //Eastsun 20260518 : 新增錯誤位置代碼
}
//---------------------------------------------------------------------------
void __fastcall TfFTPClient::NMFTP1Status(TComponent *Sender,
      AnsiString Status)
{
    ShowMyMessage("TfFTPClient:NMFTP1Status:"+Status, "");          //Eastsun 20260518 : 新增錯誤位置代碼
}
//---------------------------------------------------------------------------
...
void TfFTPClient::DownloadWorkFileSettingFormServer()
{
    ...
    if(SystemStart || (fMain->cbRunStartMode->Enabled==false && LastSet.iRunStartMode!=rsmAutoSiteMap) ||  //JerryYang 20220215 : 機台內有IC不能download
       (HasICUnderMachine() || HasAnyICInMachine()))    //JerryYang 20250120 : modify
    {
        ShowMyMessage("Handler Must Clean Out");
        return;
    }
    ...
    catch(...)
    {
        ...
        bNeedBigMsg=true;
        ShowMyMessage("FTP Download Fail !! There is no file on server!", "FTP Download Fail !! Server上沒有檔案!");
    }
...
{
    AnsiString aBinPos[]={"AUTO1", "AUTO2", "AUTO3", "FIX1", "FIX2", "FIX3", "FIX4", "FIX5", "FIX6"};
    AnsiString aBinPos1[]={"AUTO 1", "AUTO 2", "AUTO 3", "FIX 1", "FIX 2", "FIX 3", "FIX 4", "FIX 5", "FIX 6"};
    AnsiString sMap[4][8];
    int iBin, ret, iMaxErrTray=0, iTempIfError=0, iTemp, iTempSoak;
    int tag=FT;
//    double dSitePitchX, dSitePitchY;

    AnsiString S="", S1="", S2="", S3="", S4="", S5="", S6="" , aBin="", aPF="", sSiteLayout="",sSitePitch, sSitePitchX="", sSitePitchY="", sSiteMap="", sConFail="", sbConFail="", siConFail="", sTemp="", sbACC="", siACC="";
...
//    int iSiteCnt=SiteData[iMode].Cnt;
    ...
    bool bHasChangeSoakTime=false, bHasChangeTemp=false;
...
//    char *cSiteMap[32];
    ...
    {
        bNeedBigMsg=true;
        ShowMyMessage("FTP Download Fail !! Has IC under machine!");
        return;
    }

    if(TestIF_File.iTestType==GPIB_MODE && TestIF_File.iGpibMode==InterfaceType_256Bin)  //JerryYang 20250430 : add
    {
        bNeedBigMsg=true;
        ShowMyMessage("FTP Download Fail !! 255 bin mode not support!");
        return;
    }
...
//        return;
    ...
    TStringList *sBinTraySetting=new TStringList();
    MyList->LoadFromFile(FileName.c_str());

    S1=ReadIniData(FileName, "Temperature", "Default", AnsiString("25"));
    S2=ReadIniData(FileName, "Temperature", "Soak Time", AnsiString("0"));

    iTemp=S1.ToIntDef(-1);
    iTempSoak=S2.ToIntDef(-1);

    S2="";

    if(iTemp==-1 || iTempSoak==-1)  //JerryYang 20250430 : add
    {
        bNeedBigMsg=true;
        ShowMyMessage("FTP Download Fail !! Temperature or Soak time data error!");
        return;
    }

    if(atoi(fMain->edWorkTemperBase->Text.c_str())!=iTemp ||
       atoi(fMain->edSoakTime->Text.c_str())!=iTempSoak)
    {
        if(atoi(fMain->edWorkTemperBase->Text.c_str())!=iTemp)
        {
            bHasChangeTemp=true;
        ...
        if(atoi(fMain->edWorkTemperBase->Text.c_str())!=iTemp)
        {
            bHasChangeSoakTime=true;
        }

        fMain->edWorkTemperBase->Text  =iTemp;
        fMain->edSoakTime->Text        =iTempSoak;
...
//    {
//        BinSelect[tag].iCategData[i]=ePosNoUse;
//    }
...
//        else if
        ...
        else
        {
            for(int j=0; j<9; j++)
            {
                S1=S.UpperCase();   //JerryYang 20250415 : 轉成大寫
                if(S1.Pos(aBinPos[j])!=0 || S1.Pos(aBinPos1[j])!=0)
                {
                    bHasData[6]=true;
                    aBin=S.SubString(1, S.Pos("="));
                    iBin=atoi(aBin.c_str());
                    if(iBin>=16)        //JerryYang 20250520 : Thoams說16bin以上不要SET
                    {
                    }
                    else if(iBin<0)
                    {
                        bNeedBigMsg=true;
                        ShowMyMessage("FTP Download Fail !! Bin setting error","FTP Download Fail !! Bin設定error");
                        return;
                    }
                    ...
                    {
                        bNeedBigMsg=true;
                        ShowMyMessage("FTP Download Fail !! Bin setting pass/fail error","FTP Download Fail !! Pass/fail內容錯誤");
                        return;
                    }
                    break;
                }
    ...
    for(int i=0; i<7; i++)
    {
        if(i==0 || i==1 || i==2)   //Site map, Site layout, site pitch不download
        {
            continue;
        }
        ...
        if(bHasData[i]==false)
        {
            AnsiString strData;
            bNeedBigMsg=true;
            strData.sprintf("FTP Download Fail !! Setup file缺少%s資料",sCheckData[i]);
            ShowMyMessage(strData);
            return;
        }
    }

    iTempIfError=ReadIniData(FileName, "Bin (FT)", "I/F Error Tray", ePosFix2);

    iTempIfError=5;     //JerryYang 20250411 : ASECL Thomas要求 Error bin強制固定在FIX 3
    ...
    if(iTempIfError<0 || iTempIfError>iMaxErrTray)    //Auto1=0, Auto2=1,Auto3=2,Fix1=3,Fix2=4,Fix3=5
    {
        bNeedBigMsg=true;
        ShowMyMessage("FTP Download Fail !!Auto download fail: Error tray setting over limit.","FTP Download Fail !! error bin設定超過極限值");
        return;
    }

    WriteIniData(BinFile, "I/F Error", "Bin", iTempIfError);
    ...
    for(int i=0; i<MyList->Count; i++)
    {
        ...
        for(int j=0; j<9; j++)
        {
            ...
            if(S1.Pos(aBinPos[j])!=0 || S1.Pos(aBinPos1[j])!=0)
            {
                ...
                {
                    bNeedBigMsg=true;
                    ShowMyMessage("FTP Download Fail !! Bin setting over max bin","FTP Download Fail !! bin別超過最大限制");
                ...
                WriteIniData(BinFile, s3TrayName[j], "Pass/Fail", atoi(aPF.c_str()));
                break;
            }
        }
    }
    //Bin
    //==<
    for(int i=0; i<iTestBinCount; i++)          //JerryYang 20250520
    {
        sBinTraySetting->Add(BinSelect[tag].iCatDataT3Pos[i]);
    }
    WriteIniData(BinFile, "Bin Func FT", "3617 BinTraySetting", sBinTraySetting->CommaText);
    fBinSel->ReadFile(false, false, "");
    fShowBinSelect->ShowBinSel();
    //<==
    //Bin
...
//    fCleaning->LoadAutoCleanData();
    ...
    MyList->Clear();
    sBinTraySetting->Clear();
    delete MyList;
    delete sBinTraySetting;
    ...
    if(bHasChangeSoakTime)
    {
        S2="Soak Time,";
        S4="預熱時間,";
    }

    S5=S1+S2+"Bin Location Has Been changed !!";
    S6=S3+S4+"Bin Location 已變更 !!";
...
void TfFTPClient::N25_UploadTempDataToFTP(int iType)                            //Steven 20210413 : 南茂的自動Start功能, 0:上傳十筆, 1:只記錄不上傳, 2:整日
{
    ...
    sContent1+=(bUT150Install[tcChamber] && bN25UploadChamberTemp)?(str.sprintf("%0.2f", UN150Read[tcChamber])):AnsiString("-");  //AI(ht9045-v899) 20260521: limit ChipMos Chamber upload value by selected heat mode
...
void __fastcall TfFTPClient::Download_2DSortingList(AnsiString FtpPath, AnsiString Source, int iTimeOut)  //RogerYang 20251217 Add for timeout    //Frank 20221122 : 2DID sorting for ATK
{
    ...
    try
    {
        ...
        AnsiString sDLFileName=Source;
        AnsiString sRmsPath="D:\\RMS\\";                                        //RogerYang 20260611 改用區域變數，不要碰全域 DataPath
...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
void TfFTPClient::N22_Upload_JHT_LOG()  //JerryYang 20250120 : modify
{
    if(IniConfig.bN23UploadJHT_Log==false)
        return ;

    if(IniConfig.sN22_1ASE_CL_FTPHost=="")
    {
        ShowMyMessage("FTP Server Host is Empty");
        return ;
    }

    if(IniConfig.sN22_1ASE_CL_FTPUserName=="")
    {
        ShowMyMessage("FTP Server User Name is Empty");
        return ;
    }

    if(IniConfig.sN22_1ASE_CL_FTPPassword=="")
    {
        ShowMyMessage("FTP Server Password is Empty");
        return ;
    }

    if(IniConfig.sN22_1ASE_CL_FTPUplaodPath=="")
    {
        ShowMyMessage("FTP Server File Path is Empty");
        return ;
    }
    ...
    AnsiString sFTPPath, sFTPFile, sUploadFile, sContent, sContent1, asFileName;
    AnsiString sFTPPathHour, sFTPFileHour, sUploadFileHour;
//    if(IniConfig.sN22_1ASE_CL_FTPUplaodPath.SubString(IniConfig.sN22_1ASE_CL_FTPUplaodPath.Length(), 1)!="/")
//    {
//        sFTPPath.sprintf("%s/%s/", IniConfig.sN22_1ASE_CL_FTPUplaodPath, IniConfig.SocketHandlerID);
//        sFTPFile.sprintf("%s/%s/HandlerTemperature.csv", IniConfig.sN22_1ASE_CL_FTPUplaodPath, IniConfig.SocketHandlerID, IniConfig.SocketHandlerID);
//        sFTPPathHour.sprintf("%s/%s/backup/", IniConfig.sN22_1ASE_CL_FTPUplaodPath, IniConfig.SocketHandlerID);
//        sFTPFileHour.sprintf("%s/%s/backup/TemperatureLog_%04d%02d%02d.csv", IniConfig.sN22_1ASE_CL_FTPUplaodPath, IniConfig.SocketHandlerID, SystemYear, SystemMonth, SystemDate);
//    }
//    else
    {
        sFTPPath.sprintf("%s/", IniConfig.sN22_1ASE_CL_FTPUplaodPath);
        sFTPFile.sprintf("%s%s/HandlerTemperature.csv", IniConfig.sN22_1ASE_CL_FTPUplaodPath, IniConfig.SocketHandlerID, IniConfig.SocketHandlerID);
        sFTPPathHour.sprintf("%s%s/backup/", IniConfig.sN22_1ASE_CL_FTPUplaodPath, IniConfig.SocketHandlerID);
        sFTPFileHour.sprintf("%s%s/backup/TemperatureLog_%04d%02d%02d.csv", IniConfig.sN22_1ASE_CL_FTPUplaodPath, IniConfig.SocketHandlerID, SystemYear, SystemMonth, SystemDate);
    }

    sFTPFile.sprintf("%s%s@WAFERLOT@%sXX@99@SWBIN@%s.txt", IniConfig.sN22_1ASE_CL_FTPUplaodPath, fLotInfo->edtASECL_LotID->Text, fLotInfo->edInsertion->Text, fLotInfo->edFlowID->Text);     //Steven 20170123 (Jou) : 修改檔案命名格式
//    asFileName.sprintf("%s\\%s@%s@%s.log", asTravelingLogPath, fLotInfo->edtASECL_LotID->Text, fLotInfo->edInsertion->Text, fLotInfo->edFlowID->Text);     //Steven 20170123 (Jou) : 修改檔案命名格式
...
//    sContent1.sprintf("%s,%s,%s,%s,%s", fMain->cbSetupFileName->Text, fMain->edWorkTemperBase->Text, fMain->edSoakTime->Text, IniConfig.sMachineType, IniConfig.SocketHandlerID);
//    sContent=slDailyTempLog->AddTextWithDateTime2(sContent1);
//    slDailyTempLog->MySaveToFile();
//    sUploadFileHour=slDailyTempLog->GetFileName();
//
//    sl10MinTempLog->AddTextWithDateTime2(sContent1);
//    sl10MinTempLog->MySaveToFile();
//    sUploadFile=sl10MinTempLog->GetFileName();
    if(fLotInfo->edtASECL_LotID->Text=="" || fLotInfo->edInsertion->Text=="" || fLotInfo->edFlowID->Text=="")
    {
        ShowMyMessage("Lot info is not complete, no upload summary.");
        return;
    }

    if(FileExists(asFileName)==false)
    {
        ShowMyMessage("JHT summary is missing!");
        return;
    }
    ...
    try
    {
        pFTP->Vendor    = NMOS_AUTO;
        pFTP->TimeOut   = 30000;
        pFTP->Host      = IniConfig.sN22_1ASE_CL_FTPHost;
        pFTP->UserID    = IniConfig.sN22_1ASE_CL_FTPUserName;
        pFTP->Password  = IniConfig.sN22_1ASE_CL_FTPPassword;
        pFTP->OnListItem= NMFTP1ListItem;
        pFTP->OnFailure = NMFTP1Failure;
        pFTP->Connect();
        ...
        pFTP->Upload(asFileName, sFTPFile);
...
//        pFTP->Upload(sUploadFileHour, sFTPFileHour);
    ...
    {
        MyDBIProcess("Exception", "TfFTPClient::N23_UploadTempDataToFTP");
        delete pFTP;
        ShowErrorMessage("WAR1685", 0, MMSystem, 0, sFTPFile);
        return ;
    }
}
//---------------------------------------------------------------------------
void TfFTPClient::N35_Upload_JHT_LOG()
{
    if(IniConfig.bN35UploadJHT_Log==false)
        return ;

    if(IniConfig.sN35_1ASE_CL_FTPHost=="")
    {
        ShowMyMessage("FTP Server Host is Empty");
        return ;
    }

    if(IniConfig.sN35_1ASE_CL_FTPUserName=="")
    {
        ShowMyMessage("FTP Server User Name is Empty");
        return ;
    }

    if(IniConfig.sN35_1ASE_CL_FTPPassword=="")
    {
        ShowMyMessage("FTP Server Password is Empty");
        return ;
    }

    if(IniConfig.sN35_1ASE_CL_FTPUplaodPath=="")
    {
        ShowMyMessage("FTP Server File Path is Empty");
        return ;
    }
...
//    else
    {
        sFTPPath.sprintf("%s/", IniConfig.sN35_1ASE_CL_FTPUplaodPath);
        sFTPFile.sprintf("%s%s/HandlerTemperature.csv", IniConfig.sN35_1ASE_CL_FTPUplaodPath, IniConfig.SocketHandlerID, IniConfig.SocketHandlerID);
        sFTPPathHour.sprintf("%s%s/backup/", IniConfig.sN35_1ASE_CL_FTPUplaodPath, IniConfig.SocketHandlerID);
        sFTPFileHour.sprintf("%s%s/backup/TemperatureLog_%04d%02d%02d.csv", IniConfig.sN35_1ASE_CL_FTPUplaodPath, IniConfig.SocketHandlerID, SystemYear, SystemMonth, SystemDate);
    }

    sFTPFile.sprintf("%s%s@WAFERLOT@%sXX@99@SWBIN@%s.txt", IniConfig.sN35_1ASE_CL_FTPUplaodPath, fLotInfo->edtASECL_LotID->Text, fLotInfo->edInsertion->Text, fLotInfo->edFlowID->Text);     //Steven 20170123 (Jou) : 修改檔案命名格式
//    asFileName.sprintf("%s\\%s@%s@%s.log", asTravelingLogPath, fLotInfo->edtASECL_LotID->Text, fLotInfo->edInsertion->Text, fLotInfo->edFlowID->Text);     //Steven 20170123 (Jou) : 修改檔案命名格式
    asFileName.sprintf("%s\\%s@WAFERLOT@%sXX@99@SWBIN@%s.txt", asTravelingLogPath, fLotInfo->edtASECL_LotID->Text, fLotInfo->edInsertion->Text, fLotInfo->edFlowID->Text);     //Steven 20170123 (Jou) : 修改檔案命名格式
...
//    sUploadFile=sl10MinTempLog->GetFileName();
    ...
    try
    {
        pFTP->Vendor    = NMOS_AUTO;
        pFTP->TimeOut   = 30000;
        pFTP->Host      = IniConfig.sN35_1ASE_CL_FTPHost;
        pFTP->UserID    = IniConfig.sN35_1ASE_CL_FTPUserName;
        pFTP->Password  = IniConfig.sN35_1ASE_CL_FTPPassword;
        pFTP->OnListItem= NMFTP1ListItem;
        pFTP->OnFailure = NMFTP1Failure;
        pFTP->Connect();
...
//        pFTP->Upload(sUploadFileHour, sFTPFileHour);
    ...
    {
        MyDBIProcess("Exception", "TfFTPClient::N35_UploadTempDataToFTP");
        delete pFTP;
        ShowErrorMessage("WAR1685", 0, MMSystem, 0, sFTPFile);
        return ;
    }
...
//---------------------------------------------------------------------------
void __fastcall TfFTPClient::Download_2DID_WhiteList(AnsiString FtpPath)      //JerryYang 20250616 : 2DID white list
{
    TNMFTP *NMFTP2;                                                             //Landam 用動態元件 才穩定
    NMFTP2=new TNMFTP(this);
    try
...
//        str01=DataPath+sDLFileName;
        ...
        str1.sprintf("%s_ASSY.xml", fLotInfo->edtASECL_LotID->Text);

        if(fLotInfo->cbRunModeASECL->Text=="CORR")
        {
            str1.sprintf("CORR.xml");
            str2.sprintf("D:\\HT9045_Log\\2DBarCode\\CORR.xml");
        }
        else if(fLotInfo->cbRunModeASECL->Text=="FT1" || fLotInfo->cbRunModeASECL->Text=="RT1")
        {
            str1.sprintf("%s_ASSY.xml",fLotInfo->edtASECL_LotID->Text);
            str2.sprintf("D:\\HT9045_Log\\2DBarCode\\%s_ASSY.xml", fLotInfo->edtASECL_LotID->Text);
        }
        else if(fLotInfo->cbRunModeASECL->Text=="FT2" || fLotInfo->cbRunModeASECL->Text=="QT1" || fLotInfo->cbRunModeASECL->Text=="RT2")
        {
            str1.sprintf("%s_FT1.xml", fLotInfo->edtASECL_LotID->Text);
            str2.sprintf("D:\\HT9045_Log\\2DBarCode\\%s_FT1.xml", fLotInfo->edtASECL_LotID->Text);
        }
        else if(fLotInfo->cbRunModeASECL->Text=="FT3" || fLotInfo->cbRunModeASECL->Text=="QT2" || fLotInfo->cbRunModeASECL->Text=="RT3")
        {
            str1.sprintf("%s_FT2.xml", fLotInfo->edtASECL_LotID->Text);
            str2.sprintf("D:\\HT9045_Log\\2DBarCode\\%s_FT2.xml", fLotInfo->edtASECL_LotID->Text);
        }
        else if(fLotInfo->cbRunModeASECL->Text=="FT4" || fLotInfo->cbRunModeASECL->Text=="QT3" || fLotInfo->cbRunModeASECL->Text=="RT4")
        {
            str1.sprintf("%s_FT3.xml", fLotInfo->edtASECL_LotID->Text);
            str2.sprintf("D:\\HT9045_Log\\2DBarCode\\%s_FT3.xml", fLotInfo->edtASECL_LotID->Text);
        }
        else if(fLotInfo->cbRunModeASECL->Text=="FT5" || fLotInfo->cbRunModeASECL->Text=="QT4" || fLotInfo->cbRunModeASECL->Text=="RT5")
        {
            str1.sprintf("%s_FT4.xml", fLotInfo->edtASECL_LotID->Text);
            str2.sprintf("D:\\HT9045_Log\\2DBarCode\\%s_FT4.xml", fLotInfo->edtASECL_LotID->Text);
        }
        else if(fLotInfo->cbRunModeASECL->Text=="QT5")
        {
            str1.sprintf("%s_FT5.xml", fLotInfo->edtASECL_LotID->Text);
            str2.sprintf("D:\\HT9045_Log\\2DBarCode\\%s_FT5.xml", fLotInfo->edtASECL_LotID->Text);
        }
...
//---------------------------------------------------------------------------
//Eastsun 20260520 整合//yunghsin 20221013 KYEC BU5 Handler Data BackUp Use Function   ==>
void TfFTPClient::BackUpDataToServerByFTP(AnsiString asDirPath,AnsiString sFileName)
{
    //Landam 用動態元件 才穩定
    AnsiString asDATA;
    AnsiString asFileName=asDirPath + sFileName;
    ...
    try
    {
        ...
        NMFTP2->TimeOut=20000; //Landam
        if(IniConfig.sN40_FTPHost!="")
            NMFTP2->Host = IniConfig.sN40_FTPHost;
        NMFTP2->UserID   = IniConfig.sN40_FTPUserName;
        NMFTP2->Password = IniConfig.sN40_FTPPassword ;
        ...
        //上傳檔案
        asDATA.sprintf("%s%s",IniConfig.sN40_FTPUploadPath, sFileName.c_str());

        NMFTP2->Upload(asFileName,asDATA);
...
//        WriteLogs(aDebugLog);                                                           //pig 20170105 RecordTryCatchLog
        ShowMyMessage("FTP Upload Fail");
        delete NMFTP2;
    }
}
