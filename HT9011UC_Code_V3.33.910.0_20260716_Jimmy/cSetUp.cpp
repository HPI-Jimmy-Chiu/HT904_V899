...
//    else
    ...
    cbUse1x3siteSLK         ->Visible=false;                                    //KevinCheng 20260109 : 1x2Site and 2x2 NN mode 使用1x3Site Kit
    ...
    cbPreventDropfunction   ->Visible=false;                                    //Frank 20260710 ADD//Eastsun 20260710整合
    switch(iCheckPos)                                                           //wei 20150314 ScrollBar1->Position改變時，會有問題。  iPos-->iCheckPos
    {
        case SingleSite:                                                        //1x1
            ...
            cbPreventDropfunction   ->Visible=true;                             //Frank 20260710 ADD//Eastsun 20260710整合
            break;
        case DualSite:                                                          //1x2
            gbArm1PickArm2Test->Visible=IniConfig.bD58UseArm1PickPlaceArm2Test; //ChungHung 20150528 add for 海思 _8Site1x4
            rgSelectSearchLast->Visible=false;
            ...
            if(INSTALL_DOUBLE_EP==DOUBLE_EP_INDIVIAL || INSTALL_DOUBLE_EP==DOUBLE_EP_MULTI)                    //AI(ht9045-v899) 20260430: extend cbIndSLK visibility to Multi EP half.
            {
...
//                    }
    ...
    if(TestIF_File.iTestMode==SingleSite)
    {
        ...
        TestIF_File.bPreventDropfunction=ReadIniData(szDir, "Configuration", "Enable Prevent Drop function", false);              //Frank 20260710 ADD//Eastsun 20260710整合
    }
    else
    {
        ...
        TestIF_File.bPreventDropfunction=false;                                 //Frank 20260710 ADD//Eastsun 20260710整合
    }
...
void __fastcall TfSetup::DoIniDataToForm()
{
    ...
    cbUse1x3siteSLK     ->Checked   = TestIF_File.bUse1x3SiteKit;               //KevinCheng 20260109 : 1x2Site and 2x2 NN mode 使用1x3Site Kit
    ...
    if(SPIL_FOR_QLE==1)                                                         //Kevincheng 20260525 : 渠梁常溫模式使用hotplate
    {
        cbEnablePreciserHotPlate    ->Checked   = TestIF_File.bEnablePreciserHotPlate;
    }
    else
    {
        cbEnablePreciserHotPlate    ->Visible   =false;
    }
    ...
    cbPreventDropfunction->Checked =TestIF_File.bPreventDropfunction;           //Frank 20260710 ADD//Eastsun 20260710整合
}
//------------------------------------------------------------------------------
...
//        TestIF_File.bEnableBottom2D==false)                                   //JerryYang 20250220 : 2DID硬體順序檢查功能
    ...
    if(cbSingleUseOtherSuck->Checked && cbSingleInArmUseOtherSuck->Checked)
    {
        ShowMyMessage("Two 1x1 nozzle modes selected!","已選擇兩種1x1吸嘴模式");
        return;
    }
...
void __fastcall TfSetup::SaveSetupFile(AnsiString sPath)
{
    ...
    WriteIniData(szDir, "Configuration", "Enable Prevent Drop function",   (cbPreventDropfunction->Checked)   ?1:0);        //Frank 20260710 ADD//Eastsun 20260710整合

    if(cbPreventDropfunction->Checked==true)                                                                                //Frank 20260710 ADD//Eastsun 20260710整合
        WriteIniData(sContact, "Mode", "Suck Shuttle Device After Tested",  1);
    ...
    WriteIniData(szDir, "Configuration", "Enable Preciser HotPlate",  (cbEnablePreciserHotPlate->Checked)   ?1:0);      //Kevincheng 20260525 : 渠梁常溫模式使用hotplate
    ...
    //==> Eastsun 20260526 #026-4.3 Ifor 20240618 :KYEC 特殊KIT 至小數點第三位
    if(CUSTOMER_CODE==CC_KYEC_LEE)
    {
        WriteIniData(szDir, "Configuration", "X Pitch",                 FormatFloat("0.000", XPitch->Text.ToDouble()));
        WriteIniData(szDir, "Configuration", "Y Pitch",                 FormatFloat("0.000", YPitch->Text.ToDouble()));
    }
    else
    {
    ...
    WriteIniData(szDir, "Configuration", "Use_1x3_Kit",             (cbUse1x3siteSLK->Checked)  ?1:0);                  //KevinCheng 20260109 : 1x2Site and 2x2 NN mode 使用1x3Site Kit
...
void __fastcall TfSetup::cbEnableRealTimeCCDClick(TObject *Sender)
{
    ...
    {
        if(IndexStatus!=Z1_Z2_Normal)                                           //JerryYang 20260526 : index arm 在安全位置才可以關閉RTC
        {
            lbShowMessage->Caption="* Turn off need Z1 Z2 up!";
    ...
    else
    {
        if(fMain->CheckCanChangeRealDummy()==false)                             //已經關閉real time
        {
            ...
            lbShowMessage->Caption="* Turn On need Clean-Out !";
            lbShowMessage->Visible=true;
        }
    }
}
