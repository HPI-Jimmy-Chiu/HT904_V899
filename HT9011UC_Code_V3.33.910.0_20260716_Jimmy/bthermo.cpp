...
//---------------------------------------------------------------------------
int iThermoTask=1;
int g_iHeaterTypeIdx_SendCmd = 0;                                               //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定
void DoThermo()
{
#if EN_HEATER_SHEET
    static bool s_bHeaterInsOptLoaded = false;
    if(!s_bHeaterInsOptLoaded)
    {
        s_bHeaterInsOptLoaded = true;
        extern void HeaterInsOpt_Read();
        HeaterInsOpt_Read();                                                    //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定
    }
#endif
    //if(IsAllValEqual_HeaterInsOpt(NoHeater, false))                             //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定  //Steven 20171227 (Wei) : Add for HT-9045L
...
//                          COM2->ATCFuntion_WriteSP1(Addr,Temp);
                ...
                else if(IsValEqual_HeaterInsOpt(Addr, TC401))                   //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定
                {
                    TMC401WriteTemp(Addr/4, Addr%4, Temp*10);                   //送溫度到溫控器
                }
                else if(IsValEqual_HeaterInsOpt(Addr, KT4H))                    //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定
                {
                    UT100WordWriteNoSucm(Addr, WriteCommand, Temp*10);          //送溫度到溫控器
                }
                else if(IsValEqual_HeaterInsOpt(Addr, E5DC))                    //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定  //Steven 20141030 : 新增OMRON E5DC溫控器
                {
                    E5DCWriteTemp(Addr, Temp*10);                                //Frank20141030
                }
                else if(IsValEqual_HeaterInsOpt(Addr, DTK4848))                 //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定  //KaiHuang 20190821 : 新增台達 DTK4848溫控器
                {
...
//jou 2012-03-14 ATC end
        case 300:
            ...
            g_iHeaterTypeIdx_SendCmd = Addr;   //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定
            Task=100;
            break;
        case 500:
            if(!IsValEqual_HeaterInsOpt(Addr, KT4H))                            //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定  //JerryYang 20190918 : 修正溫控器傳輸速度很慢的問題
            {
                Task=300;
                break;
            }
