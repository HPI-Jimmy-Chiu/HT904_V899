...
AnsiString gLastMsgBoxStep="(none)";                                            //AI(appexception-av-log-flood) 20260617 (RogerYang) : 定位 10ms timer 內 AV 元兇用麵包屑
//---------------------------------------------------------------------------
...
void __fastcall TMyMessageBox::FormShow(TObject *Sender)
{
    ...
    lblMainMsg->Font->Size=12;
    lblChineseMsg->Font->Size=12;
    lblMainMsg->Layout=tlTop;
    lblChineseMsg->Layout=tlTop;

    lblMainMsg->Align=alNone;
    lblChineseMsg->Align=alNone;

    lblMainMsg->Top=25;
    lblChineseMsg->Top=80;
    ...
    else if(bNeedBigMsg==true)                                                  //JerryYang 20250120 : add
    {
        Height=1002;
        Width=1288;
        Left=0;
        ...
        lblMainMsg->Align=alTop;
        lblChineseMsg->Align=alTop;

        lblMainMsg->Font->Size=22;
        lblChineseMsg->Font->Size=22;

        lblMainMsg->Layout=tlCenter;
        lblChineseMsg->Layout=tlCenter;
...
{
    bNeedBigMsg=false;                                                          //JerryYang 20250120 : add
    if(!iUnLoaderCount)                                                         //Jou 20150721 : 重新啟用功能
    {
...
void __fastcall TMyMessageBox::Timer1Timer(TObject *Sender)
{
    if(fShow==false)
        return;
                                                                                //AI(appexception-av-log-flood) 20260617 (RogerYang) : 麵包屑，AppException 可印出最後執行到哪
    gLastMsgBoxStep="fMain->Timer1Timer";        fMain->Timer1Timer(fMain);
    gLastMsgBoxStep="fMain->Timer7Timer";        fMain->Timer7Timer(fMain);
    gLastMsgBoxStep="fOmron->Main232";           fOmron->Main232();
    gLastMsgBoxStep="fOmron->Timer1Timer";       fOmron->Timer1Timer();
    gLastMsgBoxStep="ProcessStatrDigital";       ProcessStatrDigital();
    ...
    if(bSECSGEMAlarm==false)                                                    //wei 20150817 S10F3 按鍵不能按 //Ifor 20170616 (wei) add S10F3 後發生的Note Alarm 要恢復按鍵掃描解除Note後再鎖定按鍵
    {
        ...
        {
            gLastMsgBoxStep="fPadInterface->Main232";
            fPadInterface->Main232();
        }
        gLastMsgBoxStep="ScanPannelKey";
        ret=ScanPannelKey();
    ...
    gLastMsgBoxStep="DoSystemMessage";
    DoSystemMessage();
    if(bIsTestSitICFallDownResetHT9045)
    {
        gLastMsgBoxStep="CheckSafeDoor(ICFall)";
    ...
    {
        gLastMsgBoxStep="CheckSafeDoor(AutoClean)";
        CheckSafeDoorForICFallDown();
    ...
    {
        gLastMsgBoxStep="CheckSafeDoor(CleanPad)";
        CheckSafeDoorForICFallDown();
    ...
    {
        gLastMsgBoxStep="CheckSafeDoor(ContactCT)";
        CheckSafeDoorForICFallDown();
    ...
    {
        gLastMsgBoxStep="CheckSafeDoor(ContactForce)";
        CheckSafeDoorForICFallDown();
    ...
    {
        gLastMsgBoxStep="CheckSafeDoor(SocketSensor)";                          //kevin 20130504 socket sensor detect error
        CheckSafeDoorForICFallDown();
    ...
    {
        gLastMsgBoxStep="CheckTestBinData";
    ...
    gLastMsgBoxStep="DoAvoidIndexMotorFallDown";
    ...
    if(IniConfig.bEnableCCDUSETCPIP)                                            //Eliot 2010_1209 start
    {
        if(CCDInterfaceForm->iIdentificationStatus==2)                          //2:Fail
            ...
            Top=0;
        else if(bNeedBigMsg)                                                    //JerryYang 20250120 : add
            Top=0;
        else
            Top=243;
    }
    ...
    {
        gLastMsgBoxStep="ScanTrayStatus";
        ScanTrayStatus();
    }
    
    gLastMsgBoxStep="(timer-done)";
