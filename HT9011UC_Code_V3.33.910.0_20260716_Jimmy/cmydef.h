...
// ==========================
//  Modbus 功能碼定義表(FC:Function Code)
// ==========================
#define MODBUS_FC_READ_COILS                 0x01  // 讀取線圈
#define MODBUS_FC_READ_DISCRETE_INPUTS       0x02  // 讀取離散輸入
#define MODBUS_FC_READ_HOLDING_REGISTERS     0x03  // 讀取保持暫存器  //DXMR90-4K Support
#define MODBUS_FC_READ_INPUT_REGISTERS       0x04  // 讀取輸入暫存器
#define MODBUS_FC_WRITE_SINGLE_COIL          0x05  // 寫入單一線圈
#define MODBUS_FC_WRITE_SINGLE_REGISTER      0x06  // 寫入單一暫存器  //DXMR90-4K Support
#define MODBUS_FC_WRITE_MULTIPLE_COILS       0x0F  // 寫入多個線圈
#define MODBUS_FC_WRITE_MULTIPLE_REGISTERS   0x10  // 寫入多個暫存器  //DXMR90-4K Support

// 進階功能碼
#define MODBUS_FC_MASK_WRITE_REGISTER        0x16  // 遮罩寫入暫存器
#define MODBUS_FC_READ_WRITE_MULTIPLE_REGS   0x17  // 同時讀寫暫存器
#define MODBUS_FC_READ_FIFO_QUEUE            0x18  // 讀取FIFO佇列
#define MODBUS_FC_READ_FILE_RECORD           0x14  // 讀取檔案記錄
#define MODBUS_FC_WRITE_FILE_RECORD          0x15  // 寫入檔案記錄
#define MODBUS_FC_DIAGNOSTICS                0x08  // 通訊診斷
#define MODBUS_FC_GET_COMM_EVENT_COUNTER     0x0B  // 事件計數器
#define MODBUS_FC_GET_COMM_EVENT_LOG         0x0C  // 事件記錄
#define MODBUS_FC_REPORT_SLAVE_ID            0x11  // 從站識別

#define MODBUS_FC_ERROR_FLAG                 0x80  //錯誤回覆起始位 (bit7=1)
// 以下為範例
// | 原功能碼 | 錯誤回覆功能碼 |             意義                |
// | ------- | ------------- | ------------------------------- |
// | 0x03    |      0x83     | Read Holding Registers 發生錯誤 |
// | 0x06    |      0x86     | Write Single Register 發生錯誤  |

const unsigned short MODBUS_PORT         = 502;                         // Protocol
...
#define INVALID_DOUBLE_VAL_POS               9999.9
#define INVALID_DOUBLE_VAL_NEG               -9999.9
...
extern bool bN07AlarmActive;                           //Steven 20260603 : Secs_Gem disconnect alarm active flag (JSCC NetworkMonitor)
extern bool bN07BuzzerSilenced;                        //Steven 20260603 : N07 alarm buzzer silenced by alarm reset
...
extern int iResetStopMachine;                                                  //Eastsun 20260710整合
...
extern bool bPauseAlarmDelayActive;          //RogerYang 20260626 : Tester Pause 逾時計時啟用旗標
extern TQPF_Timer hPauseAlarmDelay;          //RogerYang 20260626 : Tester Pause 逾時(=MaxTestTime)計時器
...
//------------------------------------------------------------------------------
typedef enum E_MotExe_Type
{
    eMotExe_ServoOn = 0,
    eMotExe_ServoOff,
}EMotExe_Type;

typedef enum E_SafePLCIOType
{
    eSafePLCIOType_Uninstall = 0,
    eSafePLCIOType_InstallSchneider,                                            //既有研五PLC的版本為Schneider模組
    eSafePLCIOType_InstallReeR,                                                 //台積電SafePLC IO版本為ReeR模組

    eSafePLCIOType_Count,
}ESafePLCIOType;
extern ESafePLCIOType g_eSafePLCIOType;

bool IsSafePLCIOType_Schneider()
{
    if(eSafePLCIOType_InstallSchneider == g_eSafePLCIOType)
        return true;

    return false;
}

bool IsSafePLCIOType_ReeR()
{
    if(eSafePLCIOType_InstallReeR == g_eSafePLCIOType)
        return true;
...
{
    if(IsSafePLCIOType_Schneider() || IsSafePLCIOType_ReeR())
        return true;
    return false;
}
...
extern bool g_bEnRecPLCRecvData;
extern int g_iContTimes_SafeDoorSing;
extern int g_iContTimes_SafeDoorFlag;

#define _GET_ARY_SIZE_1(_ary_) (int)(sizeof(_ary_)         / sizeof((_ary_)[0]))
#define _GET_ARY_SIZE_2(_ary_) (int)(sizeof((_ary_)[0])    / sizeof((_ary_)[0][0]))
#define _GET_ARY_SIZE_3(_ary_) (int)(sizeof((_ary_)[0][0]) / sizeof((_ary_)[0][0][0]))
#define GET_ARY_SIZE(_ary_, dim) _GET_ARY_SIZE_##dim(_ary_)
//------------------------------------------------------------------------------
...
extern AnsiString asCheckContactOver[2];                                       //Eastsun 20260710整合
extern bool bCheckRunModeFlag;                                                 //Eastsun 20260710整合
#endif//Include Guard應該要在檔案的最末端!!!
