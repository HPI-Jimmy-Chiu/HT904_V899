...
__published:    // IDE-managed Components
    ...
    void __fastcall mtTrayItemMouseUp(TObject *Sender, TMouseButton Button,
          TShiftState Shift, int X, int Y);
    void __fastcall spbAOIBinClick(TObject *Sender);
...
//---------------
    ...
    TStringList *sAOIBinTraySetting[eBinTypeTotal];                             //Eastsun 20260316 : AOI 要分bin
    ...
    void __fastcall ARTBinCheck(int tag);                                       // Eastsun 20260508 整合: Ifor 20220426 add: KYEC 要求一定要有一個 ART Retest

    TStringList *sBinTrayLinked[eBinTypeTotal];
    TStringList *sBinLinked[eBinTypeTotal];
    TStringList *sMagazineSetup[eBinTypeTotal];

    std::vector<int> AOI_SplitToInt(int tag);     //Eastsun 20260316 : AOI 要分bin
};
//---------------------------------------------------------------------------
extern PACKAGE TfBinSel *fBinSel;
//extern const AnsiString sTrayName[eTrayCount];
//******************************************************************************
//
//  注意!! cBinSel為Handler Bin設定相關, 修改時要小心!!
//
