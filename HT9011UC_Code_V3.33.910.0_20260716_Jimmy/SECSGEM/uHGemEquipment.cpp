...
int __fastcall THGem::ShowSMLBinary(unsigned char *Ptr, int len)
{
    ...
    int ret=0;
    iSMLSpace=0;
    // Ifor 20260402: Always validate SML format for S9F7 detection (was only when checkbox=true)
    // ShowSMLSpaceBinaryString() internally guards binary output based on checkbox state
        ret=ProcessSMLBinary(Ptr, len, RunLength);
    if(GemCheckBoxShowBinary->Checked==true)
        StringBinaryOut("");
    return ret;
}
//---------------------------------------------------------------------------
...
void THGem::DoReportECDataChangeCheck()
{
    ...
    if(bOnLine==false)
        return;
    //==> Eastsun 20260526 #026-1.72 Ifor 20220125 add: KYEC ¤£°õ¦æEC Data Check
        ...
        do
        {
            ...
            for(int i=0; i<TimeLeft->Count; i++)
            {
                ...
                if(iT<=0)
                {
                    ...
                    S+="    T3   time out";
                    StringOut(S);
                    HSys.MyGem->S9F9_TransactionTimerTimeout(S);                //Ifor 20260402: S9F9 Transaction Timer Timeout
...
void __fastcall THGem::InitialHGem()
{
    ...
    if(OnLineOrOffLine!=NULL)
    {
        ...
        if(CUSTOMER_CODE==CC_KYEC_LEE) OnLineOrOffLine->ItemIndex=0;           //Eastsun 20260526 #026-1.73 Ifor 20210527 add:KYEC SECSGEM ±j¨îOnline Remote
    }
    ...
    if(RemoteOrLocal!=NULL)
    {
        ...
        if(CUSTOMER_CODE==CC_KYEC_LEE) RemoteOrLocal->ItemIndex=0;             //Eastsun 20260526 #026-1.73 Ifor 20210527 add:KYEC SECSGEM ±j¨îOnline Remote
    }
...
//------------------------------------------------------------------------------
void __fastcall THGem::ReportAlarm(AnsiString AlarmCode, bool bIsJam, int iDuplicateError, AnsiString SubMessage, bool bReleaseAlm, AnsiString sRecovery)       //JerryYang 20170504 (Steven) bReleaseAlm¬°trueªí¥Ü¸Ñ°£alarm»Ý¤W³ø0x00
{
...
void __fastcall THGem::FormShow(TObject *Sender)
{
    ...
    //==> Eastsun 20260526 #026-1.78 Ifor 20210527 add:KYEC SECSGEM ±j¨îOnline Remote, UIÂê©w
    if(CUSTOMER_CODE==CC_KYEC_LEE)
    {
...
void THGem::ProcessReceiceData()
{
    ...
    else
    {
        ...
        #ifndef SOFT_SIMULTE
        if(Remote.W_Bit==1 && Remote.DeviceID != Local.DeviceID && Remote.DeviceID != 0xFFFF)
        {
            AnsiString sDevIDErr;
            sDevIDErr.sprintf("Unrecognized Device ID: Remote=%d, Local=%d", Remote.DeviceID, Local.DeviceID);
        ...
        else if(Remote.MessageID_S==1 && Remote.MessageID_F==13)                // [S1F13] Connect Request
        {
...
void __fastcall THGem::ProcessSocketReceiveData()
{
    ...
    do
    {
        ...
        ret=ShowSMLBinary(ProcessBuffer, Value+4);
        if(ret==-2)
            bDataFormatOK=false;
        ...
        {
            AnsiString sExErr;
            sExErr.sprintf("Exception in ProcessReceiceData S%dF%d", Remote.MessageID_S, Remote.MessageID_F);
            StringOut(sExErr);
