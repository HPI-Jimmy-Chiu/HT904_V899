...
int HTGem::S2F34_DefineReportAcknowledgeSub()
{
    ...
    /*//Ifor 20260609 Mark:依 SEMI E5 規範，DATAID 只是 Host 端的關聯識別碼（correlation ID），可以是任意值，Equipment 不應對其內容做驗證。
            ...
            if(HGemPtr->stdGridReportID->Cells[0][y]==RptID)
            {
                ...
                for(x=2; x<HGemPtr->stdGridReportID->ColCount; x++)
                {
                    if(HGemPtr->stdGridReportID->Cells[x][y]!="")
                    {
                        ...
                        if(HGemPtr->SV_ID->IndexOf(SVID)>=0)                    //Ifor 20260402: fix IndexOf>=0 (was >0, would miss index 0)
                        {
                            ict++;
                        }
                        else
...
// ---------------------------------------------------------------------------
static void SendS9FxEchoHeader(THGem *p, int FCode, AnsiString LogMsg)			// Ifor 20260625: SEMI E5 conformant S9Fx sender.
{
    if(p==NULL)
        return;
    p->StringOut(LogMsg);
    p->InitLocalHead(9, FCode, 0);
    unsigned char hdr[10];
    hdr[0] = (unsigned char)((p->Remote.DeviceID >> 8) & 0xff);
    hdr[1] = (unsigned char)( p->Remote.DeviceID       & 0xff);
    hdr[2] = (unsigned char)(p->Remote.MessageID_S | (p->Remote.W_Bit ? 0x80 : 0));
    hdr[3] = (unsigned char) p->Remote.MessageID_F;
    hdr[4] = (unsigned char) p->Remote.PType;
    hdr[5] = (unsigned char) p->Remote.SType;
    hdr[6] = (unsigned char)((p->Remote.SystemByte >> 24) & 0xff);
    hdr[7] = (unsigned char)((p->Remote.SystemByte >> 16) & 0xff);
    hdr[8] = (unsigned char)((p->Remote.SystemByte >>  8) & 0xff);
    hdr[9] = (unsigned char)( p->Remote.SystemByte        & 0xff);
    p->DataItemOut(10, HType.BINARY_TYPE, hdr);
    p->SendLocalData();
}
// ---------------------------------------------------------------------------
// S9,F1 Unrecognized Device ID
// ---------------------------------------------------------------------------
void HTGem::S9F1_UnrecognizedDeviceID(AnsiString S)                             //Ifor 20260402: S9F1 Unrecognized Device ID
{
    SendS9FxEchoHeader(HGemPtr, 1, S);                                          //Ifor 20260625: body <B[10]>, echo Remote.SystemByte
}
// ---------------------------------------------------------------------------
...
{
    SendS9FxEchoHeader(HGemPtr, 3, S);                                          //Ifor 20260625: body <B[10]>, echo Remote.SystemByte
}
// ---------------------------------------------------------------------------
// S9,F5 Unrecognized Function Type
// ---------------------------------------------------------------------------
void HTGem::S9F5_UnrecognizedFunctionType(AnsiString S)                         //Ifor 20260402: S9F5 Unrecognized Function Type
{
    SendS9FxEchoHeader(HGemPtr, 5, S);                                          //Ifor 20260625: body <B[10]>, echo Remote.SystemByte
}
// ---------------------------------------------------------------------------
// V 1.1
// S9,F7 Illegal Data
...
{
    SendS9FxEchoHeader(HGemPtr, 7, S);                                          //Ifor 20260625: body <B[10]>, echo Remote.SystemByte
}
// ---------------------------------------------------------------------------
// S9,F9 Transaction Timer Timeout
// ---------------------------------------------------------------------------
void HTGem::S9F9_TransactionTimerTimeout(AnsiString S)                          //Ifor 20260402: S9F9 Transaction Timer Timeout
{
    SendS9FxEchoHeader(HGemPtr, 9, S);                                          //Ifor 20260625: body <B[10]>, echo Remote.SystemByte
}
// ---------------------------------------------------------------------------
// V 1.0
