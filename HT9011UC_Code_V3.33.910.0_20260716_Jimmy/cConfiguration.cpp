...
#include "cShowBinSelect.h"                                                     //EastSun 20260508 : A59 refresh main view fShowBinSelect
#include "fAOI.h"
//------------------------------------------------------------------------------
...
double dTemp=0.0;
bool bSave=false;
extern void SetTestRunMode();                                                   //EastSun 20260508 : A59 update iTestRunMode after FT/RT bin scheme change
...
__fastcall TfConfiguration::TfConfiguration(TComponent* Owner)
    : TForm(Owner)
{
    fConfiguration->ReadLockByFile();                                           //JerryYang 20260512 : add
    ...
    for(int i=0; i<tcTotalCount; i++)                                           //Steven 20200609 : 溫度用的元件改成動態產生
    {
        ...
        cbTempSelsct[i]->Left=iCbLeft+iCbColPitch*int((ChangeToFloatNonPcnt((double)(iCount), (double)(iCbColCount))));  //JerryYang 20260512 : add
...
//    if(CUSTOMER_CODE==CC_KYEC_LEE)   //EastSun 20260508 : A41 KYEC user manage FT/RT Bin Share (skip LotStart override)    //Eastsun 20260710整合
    if(CUSTOMER_CODE==CC_KYEC_CHEN || CUSTOMER_CODE==CC_KYEC_LEE || CUSTOMER_CODE==CC_KYEC_STM || CUSTOMER_CODE==CC_KYEC_XILINX)  //Eastsun 20260710整合
    {
    ...
    {
        elConfig->Add(cbA76, &IniConfig.bA76AfterHomeNeedAlm, ECBool, "Function", "bA76AfterHomeNeedAlm",              bShow, bEnable, bReadFromFile, 0);    
    ...
    {
        elConfig->Add(cbA76, &IniConfig.bA76AfterHomeNeedAlm, ECBool, "Function", "bA76AfterHomeNeedAlm",              bNoShow, bDisable, bFixedValue, 0);
...
//        elConfig->Add(edC05_Motor,  &IniConfig.iHaltTime_Motor,                 ECInteger,  "OutPowerSave", "HaltTime_Motor",           bShow, bEnable, bReadFromFile, 1,      false,  1,      200);            //kevin 20110328設定省電模式時間    //Eastsun 20260710整合
//        elConfig->Add(edC05_Temp,   &IniConfig.iHaltTime_Temp,                  ECInteger,  "OutPowerSave", "HaltTime_Temp",            bShow, bEnable, bReadFromFile, 1,      false,  1,      200);    //Eastsun 20260710整合
//        elConfig->Add(edC05_Vacuum, &IniConfig.iC05HaltTime_Vacuum,             ECInteger,  "OutPowerSave", "HaltTime_Vacuum",          bShow, bEnable, bReadFromFile, 1,      false,  1,      200);            //Steven 20221215 : Power saving for vacuum pump    //Eastsun 20260710整合
//        elConfig->Add(edC05_ATC,    &IniConfig.iHaltTime_ATC,                   ECInteger,  "OutPowerSave", "HaltTime_ATC",             bShow, bEnable, bReadFromFile, 1,      false,  1,      200);            //Ifor 20240401 : Power saving for ATC System    //Eastsun 20260710整合
        if(USE_BU5_Function==false)                                             //Eastsun 20260710整合
        {
            elConfig->Add(edC05_Motor,  &IniConfig.iHaltTime_Motor,    ECInteger, "OutPowerSave", "HaltTime_Motor",  bShow, bEnable, bReadFromFile, 1, false, 1, 200);
        ...
        {
            elConfig->Add(edC05_Motor,  &IniConfig.iHaltTime_Motor,    ECInteger, "OutPowerSave", "HaltTime_Motor",  bShow, bEnable, bReadFromFile, 1, false, 1, 400);
            elConfig->Add(edC05_Temp,   &IniConfig.iHaltTime_Temp,     ECInteger, "OutPowerSave", "HaltTime_Temp",   bShow, bEnable, bReadFromFile, 1, false, 1, 400);
            elConfig->Add(edC05_Vacuum, &IniConfig.iC05HaltTime_Vacuum,ECInteger, "OutPowerSave", "HaltTime_Vacuum", bShow, bEnable, bReadFromFile, 1, false, 1, 400);
            elConfig->Add(edC05_ATC,    &IniConfig.iHaltTime_ATC,      ECInteger, "OutPowerSave", "HaltTime_ATC",    bShow, bEnable, bReadFromFile, 1, false, 1, 400);
        }
    }
    else
    {
...
void TfConfiguration::InitConfigEdtList_ItemL()
{
    ...
    elConfig->Add(cbL49,        &IniConfig.bL49UseBySetupOffsetRange,           ECBool,     "Tempture", "bL49UseBySetupOffsetRange",       bShow, bEnable, bReadFromFile, 0);                       //Ifor 20231219 add Use By Setup Offset Range
    elConfig->Add(edL49,       &IniConfig.iL49UseBySetupOffsetRange,           ECInteger,  "Tempture", "iL49UseBySetupOffsetRange",       bShow, bEnable, bReadFromFile,  30, false, 1, 60);     //Ifor 20231219 add Use By Setup Offset Range
}
//------------------------------------------------------------------------------
...
void TfConfiguration::InitConfigEdtList_ItemN()
{
    ...
    if(CosFunction.bEnable_SECS_GEM)                                            //Steven 20141006 : SECS GEM使用Remote Start功能
    {
        #ifndef SOFT_SIMULTE
        if((IniConfig.bSPILFunction==true &&
            CUSTOMER_CODE!=CC_XINYUN) ||                                        //RogerYang 20260626 : 芯云要放開，但芯云用bSPILFunction
            CUSTOMER_CODE==CC_JCET)                                             //JerryYang20170329 (wei) 矽品世明要求強制開啟SECS GEM  //JerryYang 20210107 : Add JCET
        ...
        elConfig->Add(chkN07_3_2, &IniConfig.bN07_Alarm, ECBool, "SECS GEM", "SECS GEM Alarm",                                                          bShow, bEnable, bReadFromFile, 0);      //Steven 20260603 : Secs_Gem disconnect alarm
    ...
    else
    {
        ...
        elConfig->Add(cbN07_EnableHostStart,        &IniConfig.bRCMDStart,              ECBool, "SECS GEM", "Enable RCMD START",                        bNoShow, bDisable, bFixedValue, 0);
        elConfig->Add(cbN07_EnableSecsOneCycle,     &IniConfig.bSECS_GEM_OneCycle,      ECBool, "SECS GEM", "SECS GEM OneCycle",                        bNoShow, bDisable, bFixedValue, 0);
        elConfig->Add(chkN07_3_2,                   &IniConfig.bN07_Alarm,              ECBool, "SECS GEM", "SECS GEM Alarm",                           bNoShow, bDisable, bFixedValue, 0);      //Steven 20260603 : Secs_Gem disconnect alarm
        elConfig->Add(cbN07_EnableEmployeeCheak,    &IniConfig.bN07_EnableEmployeeIdCheak, ECBool, "SECS GEM", "Enable Employee ID Cheak",              bNoShow, bDisable, bFixedValue, 0);
        elConfig->Add(cbN07_EnableSecsLotCheck,     &IniConfig.bN07_EnableSecsLotCheck, ECBool, "Specific", "N07_SecsLotCheck",                         bNoShow, bDisable, bFixedValue, 0);
    }
...
//[N]-------------------------
    ...
    if(EnableRMSFunc())
    {
        ...
        cbN05_CheckSetupFile->Checked=IniConfig.bEnableRmsCheckSetupFile;       //Ifor 20230516 add: TFAMD 要求工作檔驗證
    }
...
//------------------------------------------------------------------------------
extern int g_iHeaterTypeIdx_SendCmd;
void TfConfiguration::UpdateUT150Comm()
{
    ...
    for(int i=0; i<tcTotalCount; i++)
    {
        if(bPollingUT150[i] && PauseUT150Time==0 && PageControl1->ActivePageIndex==ecp1TabSheet9) //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定
        {
            ret=i;
            g_iHeaterTypeIdx_SendCmd=i;                                         //RogerYang 20260619 : 移植創發2type溫控模組設定
            bPollingUT150[i]=false;
            PauseUT150Time=50;
            Com2Buffer="";                                                      //Steven 20111028 : 改成AnsiString
            if(bReadUT150==false)
            {
                if(IsValEqual_HeaterInsOpt(i, TC401))                           //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定
                {
                    ReadCommand=6;
                    TMC401WriteTemp(i/4, i%4, atof(edSetTemp->Text.c_str())*10);
                }
                else if(IsValEqual_HeaterInsOpt(i, KT4H))                       //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定
                {
                    UT100WordWriteNoSucm(i, 0x0001, atof(edSetTemp->Text.c_str())*10);  //Steven 20120831 : 6 -> 0x0001
                }
                else if(IsValEqual_HeaterInsOpt(i, E5DC))                       //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定  //Frank 20150909 : 新增OMRON E5DC溫控器
                {
                    E5DCWriteTemp(i, atof(edSetTemp->Text.c_str())*10);
                }
                else if(IsValEqual_HeaterInsOpt(i, DTK4848))                    //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定  //KaiHuang 20190821 : 新增台達 DTK4848溫控器
                {
            ...
            else
            {
                ...
                else if(IsValEqual_HeaterInsOpt(i, KT4H))                       //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定  //Steven 20141030 : 新增OMRON E5DC溫控器
                {
...
//            listHeaterMonitor->Items->Add(UT150SendBuffer);
    ...
    if(PauseUT150Polling)
    {
        ...
        if(IsValEqual_HeaterInsOpt(ret, TC401))                                 //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定
        {
        ...
        else if(IsValEqual_HeaterInsOpt(ret, KT4H))                             //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定  //Steven 20141030 : 新增OMRON E5DC溫控器
        {
        ...
        else if(IsValEqual_HeaterInsOpt(ret, E5DC))                             //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定  //Frank 20150909 : 新增OMRON E5DC溫控器
        {
            if(Com2Buffer.Length()>0)                                           //Steven 20111028 : 改成AnsiString
            {
                if(bReadUT150)
        ...
        else if(IsValEqual_HeaterInsOpt(ret, DTK4848))                          //AI(ht9045-heater-control) 20260618 (RogerYang) : 移植創發2type溫控模組設定  //KaiHuang 20190821 : 新增台達 DTK4848溫控器
        {
            if(Com2Buffer.Length()>0)                                           //Steven 20111028 : 改成AnsiString
            {
                if(bReadUT150)
...
void __fastcall TfConfiguration::FormClose(TObject *Sender, TCloseAction &Action)
{
    ...
    else
    {
        if(bUpdateAutomatically)                                                    //Sam 20220824 : FTP 自動下載安裝更新包
            ret=1;
        else if(CosFunction.bUseConfigSaveButton && bSave==false)    //JerryYang 20230215 : ASE-CL 尚智要求follow 7000軟體新增config存檔按鈕
        {
            ret=2;
        }
    ...
    //    if(IniConfig.bA15AutoDecayTest==false)
        ...
        fLotInfo->SetSelectionVisible();                                            //AI(safety-critical-change) 20260611 (RogerYang) : Config存檔後即時刷新下載項目權限
...
void TfConfiguration::SaveConfiguration()
{
    ...
    SetA73();                                                                   //Eastsun 20260515 RT FT Bin分開設定
...
//<<[D]-------------------------
    ...
    if(CUSTOMER_CODE==CC_ASE_CL)    //JerryYang 20220112 : 松諭要求回吸時間鎖定為0.2秒
    {
...
//    IniConfig.SocketHandlerID                               =edN04_ID->Text;  //Steven 20101019 Start
    ...
    if(EnableRMSFunc())
    {
        ...
        IniConfig.bEnableRmsCheckSetupFile                  =cbN05_CheckSetupFile->Checked;  //Ifor 20230516 add: TFAMD 要求工作檔驗證
    }
    else if(IniConfig.bShowLotInfo)
    {
        IniConfig.bEnableRms                                =cbN05_EnableRMS->Checked;
...
//---------------------------------------------------------------------------
void __fastcall TfConfiguration::btnSaveClick(TObject *Sender)
{
    bSave=true;         //JerryYang 20230215 : ASE-CL 尚智要求follow 7000軟體新增config存檔按鈕
    sbExitClick(btnSave);
}
//---------------------------------------------------------------------------
void TfConfiguration::SetA73()                                                  //Eastsun 20260515 : KYEC要求 RT FT Bin要分開設定 需要把 bFTBin2RTBin bDisableRTBinSet都設定成off
{
    // EastSun 20260508 : A73 apply runtime to memory vars (KYEC only) ; if state changed, reload Bin file so memory matches the chosen FT/RT scheme
//    if(CUSTOMER_CODE==CC_KYEC_LEE) //Eastsun 此方案只適用 bFTBin2RTBin bDisableRTBinSet都為true 的客戶    //Eastsun 20260710整合
    if(CUSTOMER_CODE==CC_KYEC_CHEN || CUSTOMER_CODE==CC_KYEC_LEE || CUSTOMER_CODE==CC_KYEC_STM || CUSTOMER_CODE==CC_KYEC_XILINX)  //Eastsun 20260710整合
    {
        bool bWantFT2RT = (IniConfig.bA73UserOverrideFTRTBin==false);   // A73 OFF => keep legacy (FT==RT) ; A73 ON => separate
        IniConfig.bFTBin2RTBin       = bWantFT2RT;
        CosFunction.bDisableRTBinSet = bWantFT2RT;
        //EastSun 20260508 : only reload when user actually toggled the A73 checkbox (avoid loading on every save)
        static int sLastA73 = -1;   // -1 = first call, force mismatch to sync once at startup
        bool bA73Changed = (sLastA73 != (int)IniConfig.bA73UserOverrideFTRTBin);
        sLastA73 = (int)IniConfig.bA73UserOverrideFTRTBin;
        if(bA73Changed && fBinSel!=NULL)
        {
            fBinSel->ReadParam();                  //EastSun 20260508 : reload bin settings
            fBinSel->ReadFile(false, false, "");   //EastSun 20260508 : reload bin file (FT/RT path picked by bFTBin2RTBin)
            SetTestRunMode();                      //EastSun 20260508 : refresh iTestRunMode FIRST so ShowBinSel picks the right BinSelect[]
            if(fShowBinSelect!=NULL)
            {
                fShowBinSelect->ShowBinSel();           //EastSun 20260508 : repaint main view BinSel label/panel
                fShowBinSelect->InitShowBinDigital();   //EastSun 20260508 : refresh hardware BinDisplay (7-seg / TFT) after FT/RT scheme change
            }
        }
    }
}
