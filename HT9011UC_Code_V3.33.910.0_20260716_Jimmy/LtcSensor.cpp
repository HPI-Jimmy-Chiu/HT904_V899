...
void __fastcall TfLtcSensor::FormShow(TObject *Sender)
{
    ...
    if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                     //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
    {
        palInSHAutoLtc->Visible=true;
        fLtcSensor->Width=979;
    }
    else
    {
        palInSHAutoLtc->Visible=false;
        fLtcSensor->Width=772;
    }
}
//------------------------------------------------------------------------------
...
void __fastcall TfLtcSensor::TimerLtcSensorTimer(TObject *Sender)
{
    ...
    if(MOTION_CARD_TYPE==1)                                                     //Steven 20140818 : 康泰克Out Sensor Y改用Latch
    {
        ...
        if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                 //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
        {
            ledIn1_2_Y->Value=MOT[MInShuttle1].Motor->GetLatchIOStatus(LatchInputNo8);
            ledIn2_2_Y->Value=MOT[MInShuttle2].Motor->GetLatchIOStatus(LatchInputNo9);
        }
    }
    ...
    if(USE_OUT_SORT_ARM!=eartUninstall)                                         //Steven 20240822 : For HT-9046AU
    {
        ...
        ledOut3_Y2->Value=MOT[MOutSortSht].Motor->GetLatchIOStatus(LatchInputNo3);
    }
}
//------------------------------------------------------------------------------
void TfLtcSensor::SetLtcSensor_SMC(int iShuttle)                                //JerryYang 20230223 : 清除latch函式拆成shuttle 1,2
{
    if(iShuttle==0)                                                             //shuttle 1
    {
        ...
        if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                 //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
        {
            MOT[MInShuttle1].Motor->SetFIFOLatchSrc(AxisNumShuttle1, 0x73, 0);      //先Disable
            MOT[MInShuttle1].Motor->SetFIFOLatchSrc(AxisNumShuttle1, 0x73 ,0x73);   //Enable 設定哪個Sensor要去偵測哪個Encoder
        }
        else
        {
    ...
    else if(iShuttle==1)                                                        //shuttle 2
    {
        ...
        if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                 //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
        {
            MOT[MInShuttle2].Motor->SetFIFOLatchSrc(AxisNumShuttle2, 0x73, 0);
            MOT[MInShuttle2].Motor->SetFIFOLatchSrc(AxisNumShuttle2, 0x73 ,0x73);
        }
        else
        {
    ...
    else if(iShuttle==2)                                                        //Steven 20240822 : For HT-9046AU
    {
        if(USE_OUT_SORT_ARM!=eartUninstall)
        {
            ...
            MOT[MOutSortSht].Motor->SetFIFOLatchSrc(AxisNumShuttle3, 0x0F, 0);  //RogerYang 20250729 Add for 9046AU
            MOT[MOutSortSht].Motor->SetFIFOLatchSrc(AxisNumShuttle3, 0x0F ,0x0F);
        }
    }
}
//------------------------------------------------------------------------------
...
void TfLtcSensor::GetLtcSensor_SMC(int iShuttle)
{
    ...
    if(iShuttle==0)
    {
        ...
        if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                 //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
            LatchDataCnt8 = MOT[MInShuttle1].Motor->GetLatchBuffer(LatchInputNo8 +1, &iGetAxisCounterNo8, LatchDataTable8);
        ...
        if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                 //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
            ABSLatchDataTable(LatchDataCnt8, LatchDataTable8);
    }
    else if(iShuttle==2)                                                        //Steven 20240822 : For HT-9046AU
    {
    ...
    else
    {
        ...
        if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                 //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
            LatchDataCnt9 = MOT[MInShuttle2].Motor->GetLatchBuffer(LatchInputNo9 +1, &iGetAxisCounterNo9, LatchDataTable9);
        ...
        if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                 //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
        {
            if(LatchDataCnt9<1024)
            {
                for(int i=0; i<LatchDataCnt9; i++)
                {
                    ltcTmp[i] = abs(LatchDataTable9[i]);
                    LatchDataTable9[i] = ltcTmp[i];
                }
            }
        }
    }
...
void __fastcall TfLtcSensor::GetLtcSensor(int iShuttle)
{
        ...
        if(SThreadPara.bExeShuttleThread==false ||
           In_Shuttle_Auto_Latch==eInSHAutoLtc)                                 //KenHsieh 20260108 : InSHLtc 有裝就要執行，OutLtc 外面有被卡掉了
        {
...
int TfLtcSensor::GetLtcDataCnt_SMC(int iShuttle)                                //Sam 20211115 : Latch 清除增加檢查
{
    ...
    {
        if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                 //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
            return (LatchDataCnt0+LatchDataCnt1+LatchDataCnt4+LatchDataCnt5+LatchDataCnt8);
        else
    ...
    {
        if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                 //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
            return (LatchDataCnt2+LatchDataCnt3+LatchDataCnt6+LatchDataCnt7+LatchDataCnt9);
        else
...
void __fastcall TfLtcSensor::btGetSh1LtcClick(TObject *Sender)
{
    ...
    else
    {
        ...
        if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                 //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
            labIn1_2_Y_Axis->Caption=iGetAxisCounterNo8;
    }
    ...
    if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                     //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
        labIn1_2_Y_Count->Caption=LatchDataCnt8;
    ...
    if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                     //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
    {
        Memo_In1_2Y->Lines->Clear();
        if(LatchDataCnt8<1024)
        {
            for(int i=0; i<LatchDataCnt8; i++)
            {
                Memo_In1_2Y->Lines->Add(AnsiString(abs(LatchDataTable8[i]*MOT[MInShuttle1].Motor->GearRatio)));
            }
        }
    }
}
...
void __fastcall TfLtcSensor::btGetSh2LtcClick(TObject *Sender)
{
    ...
    else
    {
        ...
        if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                 //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
            labIn2_2_Y_Axis->Caption=iGetAxisCounterNo9;
    }
    ...
    if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                     //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
        labIn2_2_Y_Count->Caption=LatchDataCnt9;
    ...
    if(In_Shuttle_Auto_Latch==eInSHAutoLtc)                                     //KenHsieh 20250722 : InSht sensor 改為2顆，並用Latch 判別疊料以及飛料
    {
        Memo_In2_2Y->Lines->Clear();
        if(LatchDataCnt9<1024)
        {
            for(int i=0; i<LatchDataCnt9; i++)
            {
                Memo_In2_2Y->Lines->Add(AnsiString(abs(LatchDataTable9[i]*MOT[MInShuttle1].Motor->GearRatio)));
            }
        }
    }
}
...
void __fastcall TfLtcSensor::btGetSh3LtcClick(TObject *Sender)
{
    ...
    labOut3_Y2_Axis->Caption =iGetSortCounter3;                                 //RogerYang 20250609 Add for 9046AU
    ...
    labOut3_Y2_Count->Caption =LatchDataCntSort3;                               //RogerYang 20250609 Add for 9046AU
    ...
    Memo_Out3Y2->Lines->Clear();                                                //RogerYang 20250609 Add for 9046AU
    if(LatchDataCntSort3<1024)
    {
        for(int i=0; i<LatchDataCntSort3; i++)
        {
            Memo_Out3Y2->Lines->Add(AnsiString(abs(LatchSortDataTable3[i]*MOT[MOutSortSht].Motor->GearRatio)));
        }
    }
}
//------------------------------------------------------------------------------
