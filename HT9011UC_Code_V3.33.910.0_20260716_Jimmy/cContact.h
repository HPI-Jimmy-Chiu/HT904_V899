...
extern const int VISUAL_DETECTION_TEST;                                         //Ifor 20260204 add: TFAMD 要求新增檢測流程
//---------------------------------------------------------------------------
...
__published:                                                                    // IDE-managed Components
    ...
    TLabel *lblTsetSec;
    TEdit *edTestSec;
    TCheckBox *chkDailyCorrelation;
    TLabel *labContactRelative;
    TEdit *edContactRelativeZ1;
    TEdit *edContactRelativeZ2;
    TCheckBox *cbCalibrateAboveHeight;
    TCheckBox *cbTeachInSHSen;
        TLabel *lblATCTempWait;
    TRadioButton *rbVisualDetectionTest;
...
public:                                                                         // User declarations
    ...
    bool __fastcall DoStepContactKitDevice(bool Reset=false);                   //KaiChen 20200525 ：Daily Correlation Function
    bool __fastcall DoStepContactUnKitDevice(bool Reset=false);                 //KaiChen 20200525 ：Daily Correlation Function
    ...
    int iStepContactTestKitTask;                                                //KaiChen 20200525 ：Daily Correlation Function
    int iStepContactTestUnKitTask;                                              //KaiChen 20200525 ：Daily Correlation Function
    bool bUseTwoArm;                                                            //KaiHuang 20200606 : For ASE-CL Daily Correlation
    int iRunWhichArm;                                                           //KaiHuang 20200606 : For ASE-CL Daily Correlation
    void UpdateContactRelative();
    int iTaskCaliAboveHeight;
    bool __fastcall DoCalibrateAboveHeightZ1(bool Reset=false);                 //KaiChen 20200525 ：Daily Correlation Function
    bool __fastcall DoCalibrateAboveHeightZ2(bool Reset=false);                 //KaiChen 20200525 ：Daily Correlation Function

    int  iAutoTeachInSHLtcStatus;                                               //KenHsieh 20250909 : AutoHigh 使用Auto teach In SH latch sensor
    enum eInSHLtcStatus
    {
        eTeachSHLtcNomal=0,
        eTeachSH1LtcAlarm,
    ...
    bool CheckInShuttleSensor_Latch_Contact();                                  //KenHsieh 20250811 : InSht sensor latch for contact
    void InitChkInSHLatchTask();                                                //KenHsieh 20250811 : InSht sensor latch for contact
    double CalcDeviceForce(double dPinCount, double dForcePerPinN, double dForcePerPinGf, bool bReturnKgf);  //AI(ht9045-contact-force) 20260624 (RogerYang) : Device接觸力單一公式 true=Kgf(走gf) / false=N(走N)
};
//---------------------------------------------------------------------------
