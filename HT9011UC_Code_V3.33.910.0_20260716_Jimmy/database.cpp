...
extern void GetPLCIOInfo(ESafePLCIOType eSafePLCIOType);
//---------------------------------------------------------------------------
...
//        WriteIniData(str, "Version", "Model", "9046_32GPIB");                 //jou 20200601 : GPIB 型號讀取失敗需Alarm,不應該回寫型號
    ...
    asLASER_COM[3]      =CheckAndReadIniDataGeneral("Laser",        "COM_Laser_OutArm",AnsiString("COM19"));            //==> Eastsun 20260525 laser 整合 MissF-4 : +asLASER_COM[3] COM_Laser_OutArm
...
//----------------------------------
    ...
    g_eSafePLCIOType            = (ESafePLCIOType)(CheckAndReadIniDataGeneral("System",  "SafePlcIO", eSafePLCIOType_Uninstall));//Jason 20230619 增加PLC_IO模組及安全相關IO.
    GetPLCIOInfo(g_eSafePLCIOType);    
...
void SYSTEM_MODULAR::LoadIoData()
{
    ...
    if(FileExists(IoTablePath))
    {
        ...
        mapIOTable.clear();
        for(int i=0; i<(int)IOTable.size(); i++)  //Steven 20260612 : Fix memory leak - delete TIODATA* before clear()
            delete IOTable[i];
        IOTable.clear();
        try
        {
...
TIODATA::TIODATA(AnsiString Str)
{
    ...
    if(SL->Count>=14)
    {
        ...
        if(SL->Strings[HSys.IoNo.eioISABase]==AnsiString(""))                   //KenHsieh 20260421 : ISABase 要移到前面
        {
            iISABase=eMotionNet;
        }
        else
