...
void MD5::update(const unsigned char input[], size_type length)
{
    ...
    if(length-i>0) //Ifor 20200826 : 修正MD5記憶體溢位問題
    {
...
{
    if(length>0) //Ifor 20200826 : 修正MD5記憶體溢位問題
    {
...
//    char* file_buffer;
    ...
    if(tsFileName->Count!=0)
    {
        ...
        {
            ifstream file(tsFileName->Strings[i].c_str(), ios::in|ios::binary|ios::ate);
            if(file.is_open())
            {
                size = file.tellg();
                memblock = new char [size];
...
int SearchFile(AnsiString sBeginPath, AnsiString sPattern, TStrings *slResult)
{
    ...
    if(DirectoryExists(sBeginPath))
    {
        ...
        //Steven 20101118 Start : 不要顯示資料夾以外的檔案
...
//    slResult->Clear();
    if(DirectoryExists(sBeginPath))
    {
        ...
        //Steven 20101118 Start : 不要顯示資料夾以外的檔案
        filehandle=FindFirstFile((sPath+"*").c_str(), &filedata);
//        filehandle=FindFirstFileEx((sPath+"*").c_str(), FindExInfoStandard, &filedata, FindExSearchNameMatch, NULL, 0);
        if(filehandle!=INVALID_HANDLE_VALUE)
        {
            do
            {
                ...
                {
                    sPath.sprintf("%s%s", sBeginPath, filedata.cFileName);
                    SearchFileAll(sPath, sPattern, slResult);
                }
                else
                {
                    if(CheckFilenameExtension(filedata.cFileName, sPattern)==true)
