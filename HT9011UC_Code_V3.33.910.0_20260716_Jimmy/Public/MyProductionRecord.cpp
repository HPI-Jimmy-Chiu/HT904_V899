...
void TMyProductionRecord::SaveRecord(AnsiString ErrorLog)
{
    ...
    //Ifor 20260610 add: Barcode CSV Compare - log every IC result to D:/BarcodeFile folder
    if(TestIF_File.bEnableBarCode==true &&
       TestIF_File.bEnableBarcodeCSVCompare==true)
    {
        fBarCode->AppendBarcodeCSVRecord(asBuffer->Strings[e2DCode], asBuffer->Strings[eArmTime], asBuffer->Strings[eTestCate]);
    }
    if(WriteProdLOG!=NULL)
        fclose(WriteProdLOG);
    InitialRecord();
...
void TMyProductionRecord::Save2DIDForJCET(AnsiString SubLOTID, AnsiString CustLOTID, AnsiString Modecode) //RogerYang 20251210 : JCET 2D FT1白名單/FT2比對功能
{
    ...
    if(asBuffer->Strings[eUnloadTime].Trim()=="")                               //AI(ht9045-barcode-flow) 20260611 (RogerYang) : 未實際放料不列入白名單
        return;
