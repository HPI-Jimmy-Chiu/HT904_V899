...
#include "AGV.h"
#include "AMR.h"                                                                //Spencerlin 20260202 : include AMR.h
...
//        slEventLog->bDupYMDFile = (IniConfig.iO15_SaveFilePeriod==8);           //AI(ht9045-config) 20260707 (RogerYang) : 月存(8)時才另存按天檔
    }
...
void ProcessLastSetIni_Index(bool bRead)
{
    ...
    if(CUSTOMER_CODE==CC_KYEC_LEE && USE_BU5_Function==true)                    //Frank 20260710 ADD//Eastsun 20260710整合
        IniConfig.GaliPosRange+=5;                                              //Eastsun 20260710整合
...
void CustomerFunctionSelect()
{
    ...
    if(IniConfig.bSPILFunction==true)                                           //JerryYang 20170328 (Jou) 矽品客戶碼統一用SPILFunction
    {
        ...
        if(CUSTOMER_CODE==CC_XINYUN)                                            //Steven 20230222 : 要可以拉動
        {
            ...
            CosFunction.bATC32UseTJMode=false;                                  //RogerYang 20260616 : Add for 芯云(偉測拖過去的機台沒這設定)
//            CosFunction.bTechComUseComboBox=true;                             //改登入方法，不再需要
            IniConfig.bFTBin2RTBin=true;
            CosFunction.bUseHeadContactCount=true;
            CosFunction.bLoginASECL=true;                                       //套用只輸入密碼的模式
            CosFunction.b2DUseSubJobFunction=false;
        }
    }
