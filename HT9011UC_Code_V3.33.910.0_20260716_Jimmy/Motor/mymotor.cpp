...
BYTE cData;
class TTrayMotor MOT[MAX_TRAY_MOTOR];                                           //JerryYang 20230515 : 215->300      //Steven 20150910 : Modify from 100 to 200
//設定馬達實際位置與螢幕位置之比例
//設定類別建構式
...
bool bPauseInMotor=false, bPauseOutMotor=false, bPauseSortMotor=false;          //RogerYang 20250510 Add for 9046AU

//jou 2011-04-03 start
int iPCIL112_SortArmXYMoveTask=1;                                               //RogerYang 20250512 Add for 9046AU
...
__fastcall TMyMotor::TMyMotor()
{
    ...
    fCMD=false;
    iOldPos=0;
    bSh1FloodgateOpenWaiting=false;                                             //Ifor 20260401 add: Floodgate open timeout
    bSh2FloodgateOpenWaiting=false;                                             //Ifor 20260401 add: Floodgate open timeout
    iCheckStatusCT=0;
    GaliScanServo=true;
    iEncoderCheckCT=100;
...
void TMyMotor::SetADCRate(int Scale)
{
    ...
    else
    {
        ...
        if(CardType=="PCI1203")                                                 //RogerYang 20250415 Add for PCI1203
        {
...
void TMyMotor::SetSpeed(double p, bool bSetJog)
{
       ...
       Motor->Enable)
    {
        ...
        else
        {
            ...
            Motor->SetSpeed(s, bSetJog);                                        //RogerYang 20250729 Add for 9046AU
...
void TTrayMotor::ClearTray(AnsiString Func)
{
    ...
    sTrayID="";
    Tray.ClearData();
    Tray.cTrayID="";                                                            //JerryYang 20250120 : modify
...
//------------------------------------------------------------------------------
bool g_bServoOnAllMOT_Already=false;
void ServoOnAllMOT()
{
    ...
    g_bServoOnAllMOT_Already=true;
}
//------------------------------------------------------------------------------
void BrakeOffAllMOT()//解除剎車
{
...
//------------------------------------------------------------------------------
void ServoAndBrakeAllMOT(EMotExe_Type eMotExe_Type)
{
    static long g_iMtrPosBeforeServoOff[TOTAL_MOTOR]={0};
//    long lCurPos = 0;

    if(eMotExe_ServoOn==eMotExe_Type)
    {
        //先ServoOn
        for(int iMtrIdx=0; iMtrIdx<TOTAL_MOTOR; iMtrIdx++)
        {
            //exclude index axis
            // if(iMtrIdx>=MTestY1 && iMtrIdx<=MTestY2)
            //     continue;
            if(MOT[iMtrIdx].Motor->Enable==false)
                continue;

            MOT[iMtrIdx].ServoOnOff(true);
            MySleep(2);
        }
    ...
    else if(eMotExe_ServoOff==eMotExe_Type)//仿照ServoOnAllMOT
    {
        //先把剎車啟動
        BrakeOnAllMOT();

        //再ServoOff
        memset(g_iMtrPosBeforeServoOff, 0, sizeof(g_iMtrPosBeforeServoOff));
        for(int iMtrIdx=0; iMtrIdx<TOTAL_MOTOR; iMtrIdx++)
        {
            //exclude index axis
            // if(iMtrIdx>=MTestY1 && iMtrIdx<=MTestY2)
            ...
            //順便記錄每個軸ServoOff前的位置
            g_iMtrPosBeforeServoOff[iMtrIdx]=MOT[iMtrIdx].Gali_ReadPos();

            MOT[iMtrIdx].ServoOnOff(false);
            MySleep(2);
        }

        g_bServoOnAllMOT_Already=false;
    }
}
//------------------------------------------------------------------------------
...
int PCIL112_InArmXYMove(int iXComPos, int iYComPos)                             //jou 2011-04-03
{
    ...
    {
        StrE.sprintf("In arm XY move, Z%s axis home sensor off alarm", MOT[iPos].NumberAlias);
        StrC.sprintf("In arm XY移動, Z%s軸home sensor異常警報", MOT[iPos].NumberAlias);
...
//------------------------------------------------------------------------------
extern bool MoveSortArmToAutoSafe();
...
void InitPCIL112_SortArmXYMoveTask()                                             //RogerYang 20250512 Add for 9046AU
{
    iPCIL112_SortArmXYMoveTask=1;
    MOT[MOutSortX].fCMD=false;
    MOT[MOutSortY].fCMD=false;
    MOT[MOutSortPitchX].fCMD=false;
}
//---------------------------------------------------------------------------
int PCIL112_SortArmXYMove(int iXComPos, int iYComPos)                            //RogerYang 20250512 Add for 9046AU
{
    static int iSortArmFlag[2]={0, 0};
    AnsiString StrE, StrC;
//    static int iYEncoder=0;
    static int iXPos=0, iYPos=0;
    int &iTask=iPCIL112_SortArmXYMoveTask;
    static int iCount=0;                                                        //Ifor 20220715 add:同一次移動中發生三次異常報警

    if(MOT[MOutSortX].CompareEncoderPos(iXComPos, 2)==1 &&                       //Steven 20231214 : CompareCommandPos --> CompareEncoderPos
       MOT[MOutSortY].CompareEncoderPos(iYComPos, 2)==1)
    {
        return PNP_DONE;
    }

    int iPos=SortArmZSafe(DETECT_SENSOR_FLAG);
    if(iPos!=-1)
    {
        StrE.sprintf("Sort arm XY move, %s axis home sensor off alarm", MOT[iPos].NumberAlias);
        StrC.sprintf("Sort arm XY移動, %s軸home sensor異常警報", MOT[iPos].NumberAlias);
        if(USE_ARM_PROTECTION==true)                                            //Steven 20220314 : In Our Arm Z Sensor保護加上開關
        {
            ShowMyMessage(StrE, StrC);
            return PNP_DOING;
...
//            if(IniConfig.bE77_OutamrCMotion)                                    //Steven 20240603 : [E77]改放到PCIL112_OutArmXYMove裡面
//            {
//                iYEncoder=MOT[MOutSortY].ReadEncoderPos();
//                if((iYEncoder>(Tech.iOutArmShuttle1Y-3000) && iYComPos<(Tech.iOutArmShuttle1Y-3000)) ||
//                   (iYEncoder<(Tech.iOutArmShuttle1Y-3000) && iYComPos>(Tech.iOutArmShuttle1Y-3000))) //Y 在FIX區, 要移動到AUTO區
//                {
//                    iTask=300;
...
//            }
        ...
        case 100:
            if(iSortArmFlag[0]==0)
                iSortArmFlag[0]=MOT[MOutSortX].MotorMove(iXComPos);
            if(iSortArmFlag[1]==0)
                iSortArmFlag[1]=MOT[MOutSortY].MotorMove(iYComPos);
            if(iSortArmFlag[0]>0 && iSortArmFlag[1]>0)
            {
                iCount=0;
                return PNP_DONE;
            }
...
//------------------------------------------------------------------------------
bool SortArmPitchMove(int Vari)                                                 //RogerYang 20250512 Add for 9046AU
{
//    static bool bMoveClose=false;
    static bool bFirstIn=true;
    static int  iFlag=0;
...
//    int iPos=0;
    #ifndef SOFT_SIMULTE
    long iCurrPos=MOT[MOutSortPitchX].Motor->ReadPos();

    //int iDevPos;
    //BYTE bDevNo=MOT[MOutSortPitchX].Motor->iPortID;
    #endif
...
//        if(bFirstIn)
//        {
//            iPos=MOT[MOutSortPitchX].ReadPos();
//            bMoveClose=(Vari<iPos);                                           //目前位置大於目標位置    //內縮
//        }
//        bFirstIn=false;
//        return 0;
//    }
    ...
    if(iFlag==0)
        iFlag=MOT[MOutSortPitchX].MotorMove(Vari);
    ...
    if(iResult==0)                                                              //沒Alarm就確認是不是走完了
    {
        ...
        #ifndef SOFT_SIMULTE
        if(abs(Vari-iCurrPos)>5)                                                //Fixed for pitch move
        {
            bFirstIn=true;
            return 0;
        }
...
//------------------------------------------------------------------------------
HTimer SortArmCylinderDelayTimer;                                               //RogerYang 20250512 Add for 9046AU
bool SortArmContinuousMove(int X, int Y, int Vari, bool ZDownSel[MAX_ARM_Row][MAX_ARM_Col], int iZPos[MAX_ARM_Row][MAX_ARM_Col], bool ZNeedDown, bool bLoader)
{
    AnsiString str=AnsiString("");
    AnsiString str1=AnsiString(""), str2=AnsiString("");
    int ret=PNP_DOING;
...
//    static int iXPosEncoer=0, iYPosEncoder=0;
    ...
    if(iLastPosX!=X || iLastPosY!=Y)
    {
        ...
        iSortArmAllZSafe=1;
        ZeroMemory(bFlag, sizeof(bFlag));
        if(MOT[MOutSortX].Motor->MotionDone()==false)
            MOT[MOutSortX].PCIL132_StopMotor();
        if(MOT[MOutSortY].Motor->MotionDone()==false)
            MOT[MOutSortY].PCIL132_StopMotor();
    }
    ...
    if(bFirstIn)
    {
        bFirstIn=false;
        ZeroMemory(bFlag, sizeof(bFlag));
        ZeroMemory(bSortArmZMove, sizeof(bSortArmZMove));
        SortArmPitchMove(Vari);
    }

    //_todo 未完成 ShowErrorMessage怎麼新增?
    if(iX>=MOT[MOutSortX].Motor->PSoftLimitP ||                                  //Jou 980313 safe protect
       iX<=MOT[MOutSortX].Motor->PSoftLimitN)
    {
        str.sprintf("X=%d", iX);
        ShowErrorMessage("WAR0254", 0, MOutSortX, 0, str);                       //"Out Arm X axis motor will out of limit !"
        return false;
    }

    if(iY>=MOT[MOutSortY].Motor->PSoftLimitP || iY<=MOT[MOutSortY].Motor->PSoftLimitN)
    {
        str.sprintf("Y=%d", iY);
        ShowErrorMessage("WAR0255", 0, MOutSortY, 0, str);                       //"Out Arm Y axis motor will out of limit !"
        return false;
    }

    if(MOT[MOutSortX].fCanMove==false || MOT[MOutSortY].fCanMove==false)
    {
        MOT[MOutSortX].PCIL132_StopMotor();
        MOT[MOutSortY].PCIL132_StopMotor();
        iSortArmAllZSafe=1;
    ...
    if(MOT[MOutSortX].Motor->Enable && MOT[MOutSortY].Motor->Enable)
    {
        if(MOT[MOutSortX].Motor->Direction==false)
            iX=-iX;
        if(MOT[MOutSortY].Motor->Direction==false)
            iY=-iY;
        ...
        if(bPauseSortMotor)
        {
            ...
            SortArmIdle.SetSecAndOn(iIDLETime);
        }
        ...
        switch(iSortArmAllZSafe)
        {
            case 1:
                if(MoveSortArmToAutoSafe())
                {
                    iPitchMoveOk=0;
                    SortArmPitchMove(Vari);
                    SortArmCylinderDelayTimer.SetMSAndOn(ArmSpeed_File[OutArm].dCylinderDelay*1000);  //蹭OutArm的速度
                    SortArmIdle.SetSecAndOn(iIDLETime);
            ...
            case 100:
                if(iPitchMoveOk==0)
                    iPitchMoveOk=SortArmPitchMove(Vari);
            ...
            case 150:
                if(bFlag[0]==false)
                    bFlag[0]=MOT[MOutSortX].MotorMove(X+100);
                if(bFlag[1]==false)
                    bFlag[1]=MOT[MOutSortY].MotorMove(Y+100);
                if(bFlag[0] && bFlag[1])
                {
                    bFlag[0]=false;
                    bFlag[1]=false;
            ...
            case 200:
                if(bXYMoveFinish)
                {
                    if(ZNeedDown)
                    {
                        MOT[MOutSortX].ScanMotorStatus();
                        MOT[MOutSortY].ScanMotorStatus();

                        if(MOT[MOutSortX].Led[iInposLed]==true ||
                           MOT[MOutSortY].Led[iInposLed]==true)                  //Z axis protection
                            return false;

                        bRet=SortArmZMoveDown(ZDownSel, iZPos, bLoader);
                        SortArmCylinderDelayTimer.SetMSAndOn(ArmSpeed_File[OutArm].dCylinderDelay*1000);  //蹭OutArm的速度
                        if(bRet==false)
                            return false;
                    }
                    ...
                    if(CheckSortArmZHomeSensor_2x8(ZNeedDown, ZDownSel)==false)
                    {
                        return false;
                    }

                    if(SortArmCylinderDelayTimer.Off())
                    {
                        iSortArmAllZSafe=1;
                        iPitchMoveOk=0;
                        return true;
        ...
        if(ret==PNP_DONE)
        {
            if(iPitchMoveOk>0)                                                  //Steven 20150718 : Fixed for Y-Pitch
            {
                ...
                if(ZNeedDown)
                {
                    ...
                    if(MOT[MOutSortX].Led[iInposLed]==true ||
                       MOT[MOutSortY].Led[iInposLed]==true)                      //Isaac 20170721 (wei) Z axis protection
                        return false;
    ...
    else
    {
        //動作模擬
        if(bFlag[0]==false)
            bFlag[0]=SortArmZMoveUp(ZSafePos, bLoader);
        ...
        if(bFlag[0])
        {
            ...
            if(bFlag[2]==false)
            {
                ret=PCIL112_SortArmXYMove(X, Y);                                 //Steven 20240603 : [E77]改放到PCIL112_OutArmXYMove裡面
                bFlag[2]=(ret==PNP_DONE);
            }

            if(bFlag[3]==false)
                bFlag[3]=SortArmPitchMove(Vari);
        }
        ...
        if(bFlag[0] && bFlag[1] && bFlag[2] && bFlag[3])
        {
            if(ZNeedDown)
                ...
                bRet=SortArmZMoveUp(ZSafePos, bLoader);
...
//------------------------------------------------------------------------------
bool SortArmZMoveDown(bool ZDownSel[MAX_ARM_Row][MAX_ARM_Col], int iZPos[MAX_ARM_Row][MAX_ARM_Col], bool bLoader, bool bPreOn)  //RogerYang 20250512 Add for 9046AU
{
    int iMot;
    bool bFlag=true;
    if(iSortArmZMoveTask==-1 || iSortArmZMoveTask==2)
    {
        ZeroMemory(bSortArmZMove, sizeof(bSortArmZMove));
        iSortArmZMoveTask=1;
    }
    ...
    for(int i=0; i<OutArm2Suck.iMotRow; i++)
    {
        for(int j=0; j<OutArm2Suck.iMotCol; j++)
        {
            ...
            if(i<OutArm2Suck.iPickRow && j<OutArm2Suck.iPickCol)              //有用到的才做判斷
            {
                if(ZDownSel[i][j])
                {
                    if(bPreOn)
                        OutArm2Suck.Suck[i][j].On();
                    if(bSortArmZMove[i][j]==false)
                        bSortArmZMove[i][j]=MOT[iMot].MotorMove2SpeedForPicker(iZPos[i][j], &ArmSpeed[OutArm], bLoader);     //蹭OutArm的速度
                }
                else
                {
                    bSortArmZMove[i][j]=true;
                }
            }
            else
            {
...
//------------------------------------------------------------------------------
bool SortArmZMoveUp(int iZPos, bool bLoader)                                    //RogerYang 20250512 Add for 9046AU
{
    int iMot;
    bool bFlag=true;
    if(iSortArmZMoveTask==-1 || iSortArmZMoveTask==1)
    {
        ZeroMemory(bSortArmZMove, sizeof(bSortArmZMove));
        iSortArmZMoveTask=2;
    }
    ...
    for(int i=0; i<OutArm2Suck.iMotRow; i++)
    {
        for(int j=0; j<OutArm2Suck.iMotCol; j++)
        {
            iMot=OutArm2Suck.Suck[i][j].iMotNo;
            if(bSortArmZMove[i][j]==false)
                bSortArmZMove[i][j]=MOT[iMot].MotorMove2SpeedForPicker(iZPos, &ArmSpeed[OutArm], bLoader);     //蹭OutArm的速度
