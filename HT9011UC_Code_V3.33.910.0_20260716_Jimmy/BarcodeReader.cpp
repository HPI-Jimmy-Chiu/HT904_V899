...
void __fastcall TFormBarcodeReader::TimerKeyInTimer(TObject *Sender)
{
            ...
            SPIL_FOR_QLE==0)
    {
        if(edtBarcodeNumber->Text.Length()<=8)
        {
            ...
            TestIF_File.b2DIDAllowList && SPIL_FOR_QLE==1)                      //KevinCheng 20250912 : 渠梁Lot ID輸入消失問題
    {
        if(edtBarcodeNumber->Text.Length()<8)
        {
            edtBarcodeNumber->Text="";
    ...
    else if(CUSTOMER_CODE==CC_HuaTian_NNAJING &&
        (_sInputType=="UserName" || _sInputType=="Password"))                   //RogerYang 20260630 : 華天帳號/密碼=「工號-密碼」,只在帳密輸入時認格式不卡長度
    {
        int iDashPos=edtBarcodeNumber->Text.Pos("-");
        if(iDashPos>1 && iDashPos<edtBarcodeNumber->Text.Length())              //有 "-" 且前(工號)後(密碼)都有字
        {
            iCount++;
            if(iCount>3)                                                        //去抖,確保整串掃完才送出
            {
                iCount=0;
                btnEnter->Click();
            }
