...
void __fastcall TfPassword::FormShow(TObject *Sender)
{
    ...
    if(CosFunction.bLoginASECL)                                                 //JerryYang 20250120 : add
    {
        lblUserName->Visible=false;
        edUserName->Visible=false;
        cbUserName->Visible=false;                                              //RogerYang 20260625 : Add
        edPassword->SetFocus();
    }
    else
    {
      ...
      TShiftState Shift)
{
    ...
    if(IniConfig.bVTESTFunction==true ||                                        //jou 20220912 : 增加VTEST不可以用鍵盤輸入
        CUSTOMER_CODE==CC_HuaTian_NNAJING)                                      //RogerYang 20260623 : 華天走偉測輸入模式
    {
        edUserName->Text="";
        Clipboard()->Clear();
      ...
      TShiftState Shift)
{
    ...
    if(CUSTOMER_CODE==CC_SCC ||
        IniConfig.bVTESTFunction==true ||                                       //Steven 20200302 : SCC楊恩民說只能用Barcode輸入  //jou 20220912 : 增加VTEST不可以用鍵盤輸入
        CUSTOMER_CODE==CC_HuaTian_NNAJING ||                                    //RogerYang 20260623 : 華天/芯云都走偉測輸入模式
        CUSTOMER_CODE==CC_XINYUN)
    {
        edPassword->Text="";
        Clipboard()->Clear();
        AnsiString sBarcodeID=InputBarcodeNumber("Input Password:", "Password");
        edPassword->Text=sBarcodeID;
        if(CUSTOMER_CODE==CC_XINYUN)                                            //RogerYang 20260625 : 芯云只輸入密碼,不檢查username
        {
            if(edPassword->Text!="")
                Close();
        }
        else if(edPassword->Text!="" && edUserName->Text!="")
            Close();
    }
    else
    #endif
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
    #ifndef SOFT_SIMULTE
    if(IniConfig.bVTESTFunction==true ||                                        //jou 20220912 : 增加VTEST不可以用鍵盤輸入
        CUSTOMER_CODE==CC_HuaTian_NNAJING)                                      //RogerYang 20260623 : 華天都走偉測輸入模式
    {
        edUserName->Text="";
        Clipboard()->Clear();
        edUserName->Text=InputBarcodeNumber("Input User Name:", "UserName");
