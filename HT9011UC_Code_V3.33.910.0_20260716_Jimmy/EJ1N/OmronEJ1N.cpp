    ...
    : TForm(Owner)
{
    ...
    bOmronCommOK=false;                                                         //AI(appexception-av-log-flood) 20260617 (RogerYang) : 尚未開埠前禁止寫入
...
{
    bool bFlag=true;
    bOmronCommOK=false;                                                         //RogerYang 20260617 : 開啟前先設 false
    ...
    {
        CommOmron->StartComm();
        bOmronCommOK=true;                                                      //RogerYang 20260617 : 開啟成功才允許寫入
    }
    catch(...)
    {
        bFlag=false;
        bOmronCommOK=false;                                                     //RogerYang : 開啟失敗，維持禁止
...
{
    bOmronCommOK=false;                                                         //RogerYang 20260617 : 關閉後禁止寫入
...
void TfOmron::SendCommand(AnsiString Command, AnsiString Func)                  //Rs232送碼位置
{
    ...
    if(bOmronCommOK==false)                                                     //RogerYang 20260617 : 未開啟成功不可寫，避免對無效 handle 寫入造成 ntdll Access violation
        return;
    ...
    try                                                                         //RogerYang 20260617 : 執行期裝置被拔除時 WriteCommData 會丟例外/AV，攔在本地並停止後續寫入，避免無限 AV 灌爆 log
    {
    ...
    catch(...)
    {
        bOmronCommOK=false;
        bResetCom=true;
        NewRecordProcess("", "TfOmron::SendCommand WriteCommData fail, CommOmron disabled", Func);
        return;
    }
...
void TfOmron::Timer1Timer()
{
    ...
    if(IsNoHeaterMachine()==false &&                                            //RogerYang 20260630 : Add NoHeater 判斷
    ...
    if(bOmronCommOK==false &&                                                   //RogerYang 2026017 : COM 失效且非重開流程時不跑送指令狀態機（bResetCom==true 時放行，讓 DoResetCom 能重開復原）
        bResetCom==false)
        return;
