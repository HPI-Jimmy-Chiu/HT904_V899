...
int GetInArmPitchY_9045(int w, int iOffsetPos)                                  //ChungHung 20131231 alter AutoYPitch
{
    ...
    m=((double)(Prod.iInArmY60Pitch-Prod.iInArmY15Pitch))/(6000-IN_OUT_ARM_Y_PITCH_MIN);
    ...
    else
    {
        if(iPickCount==4)
        {
            if(ArmXCanSuck4IC_9045()==false)
            {
                ...
                if((TestIF_File.iTestMode==TriSite1X3 || TestIF_File.iTestMode==_6Site2X3N) &&  //JerryYang 20251124 : fix 1x3
       ...
       Temperature.bUseInitialDelayAsSoakTime)
    {
        if(bFirstInput)
        {
               ...
               TestIF.iTestMode==_32Site4X8N)
            {
                ...
                {
                    InArmSuck.iWhichShtPickFor32=InArmSuck.iWhichSht;
...
bool DoInArmCheckShuttleFloating(int iSht, bool bPlaceOtherShuttle, bool bNeedCheck)
{
    ...
    switch(Task)
    {
        ...
        case 2100:
            ...
            Task=1411;                                                          //back to check shuttle sensor
            break;
...
int Find_InArm_PickerMaxUseCountOnTime(int iRowCT, int &isStart, int &iRow, int &iCol, bool bSearchLast)
{
    ...
    iInArmYStep=ChangeToFloatNonPcnt((double)(AutoCalculateInArmYClosePitch()), (double)(LoadForm->YPitch));
            ...
            if(bSearchLast && iCloseSiteState==0)
            {
                ...
                else
                {
                    if(iRowCT==1)
                    {
                        ...
                        if(bFrontHasIC)                                         //kevin 20181222 add吸嘴第一排有IC 第二排沒有IC 需回到第一排開始吸
...
bool SearchLoadTrayUpDown_9045(bool &bCanPick2ICAtOnceTime, int &iRow, int &iCol, int &NowRow, bool bNeedDown)
{
    ...
    while(CanMove==false)
    {
        ...
        if(CanMove==false)
        {
            ...
            {
                /*//AI(ht9045-v899) 20260407: add diagnostic info before WAR0152
                {
                    AnsiString sSucker = "";
                    for(int di=0; di<InArmSuck.iMaxRow; di++)
                        for(int dj=0; dj<InArmSuck.iMaxCol; dj++)
                            if(bInArmToLoaderUsage[di][dj])
                                sSucker += "[" + IntToStr(di) + "][" + IntToStr(dj) + "] ";
                    AnsiString sDiag = "WAR0152 Detail:";
                    sDiag += "\nSingle Sucker: " + sSucker;
                    sDiag += "\nTray Col=" + IntToStr(iCol) + " Row=" + IntToStr(iRow);
                    sDiag += "\nCol Limit: " + IntToStr(iCollimit) + " ~ " + IntToStr(LoadForm->XDivision-1);
                    sDiag += "\nRow Limit: 0 ~ " + IntToStr(LoadForm->YDivision-1);
                    sDiag += "\nX SoftLimit: " + IntToStr(MOT[MInArmX].Motor->PSoftLimitN) + " ~ " + IntToStr(MOT[MInArmX].Motor->PSoftLimitP);
                    sDiag += "\nY SoftLimit: " + IntToStr(MOT[MInArmY].Motor->PSoftLimitN) + " ~ " + IntToStr(MOT[MInArmY].Motor->PSoftLimitP);
                    MyDBIProcess("Message", sDiag);
                    ShowMyMessage(sDiag);
                }*/
...
{                                                                               //Steven 20141121 : 整合InArm吸取Loader異常Alarm
    ...
    {                                                                           //kevin 20150707
        if(iContinueAutoSkipAutoTrayEnd<=5)                                     //Frank 20160912 add
        {
            ret=K_SKIP;
            iContinueAutoSkipAutoTrayEnd++;
        }
