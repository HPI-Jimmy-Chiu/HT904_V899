...
bool bSHTOfsChangeRight[2];                                                     //Sam 20221205 : 修正 Shuttle 移動完成後又被修改 Offset 導致 Hang up
bool bSHTOfsChangeLeft[2];                                                      //Sam 20230202 : 修正 Shuttle Left 移動完成後又被修改 Offset 導致 Hang up
...
int  AutoSHT1Task=1,AutoSHT2Task=1;
...
bool ShowOutputShuttleICStatus_M204(int iShuttleNo, bool bAlarm)
{
    ...
    if(bHasErr==true && bAlarm==true)
    {
        ...
        if(CUSTOMER_CODE==CC_KYEC_LEE  || IniConfig.bF35OutShtLoseNeedSetErrBin)  //Ifor 20170803 (wei) add 京元強制按RESET
        {
            if(iShuttleNo==0)
        ...
        if(ret==K_SKIP || ret==K_RESET)                                         //Steven 20111222 : 改成可以RETRY
        {
            ...
            if(CUSTOMER_CODE==CC_KYEC_LEE || ret==K_RESET)                                  //Ifor 20170803 (wei) add 京元強制按RESET
            {
...
//                    bAutoCleanCheckOpenDoor=true;                             //JerryYang 20190625 mark掉       //kevin 20180725 (wei) add 掉料忽視需開門 Alarm 後需要開門確認
        ...
        case 400:
            ...
            {
                if(MOT[MInShuttle1].MotorMove(Prod.InSHT[0].iRight-1000))       //故意讓位置不到位，所以-1000
                {
                    if(iOldTask!=Task)
                    {
        ...
        case 200:
            ...
            else if(USE_LASER_DISTANCE &&
                    TestIF_File.bEnableShuttleLaser)                            //Steven 20140228 : 雷射測距功能
            {
                if(InShtInRT(0))
                {
                    Task=201;
        ...
        case 220:
            ...
            if(MOT[MInShuttle1].MotorMove(SThreadPara.iInShRotateCheck[0]))
            {
                RotShuttleDelay1.SetSecAndOn(IniConfig.dRoShCheckDelayTime);
                Task=225;
            }
        ...
        case 230:
            ...
            if(MOT[MInShuttle1].MotorMove(SThreadPara.iOShRotateToRightCheck[0]))
            {
                Task=235;
                RotShuttleDelay1.SetSecAndOn(IniConfig.dRoShCheckDelayTime);
            }
...
//                    break;
        ...
        case 300:
            ...
            iRetryCT=0;
            bCheckShuttleFlag=true;
            if(MOT[MInShuttle1].MotorMove(Prod.iBS1Right_Half))
            {
                if(iOldTask!=Task)
                {
                    fLtcSensor->ClearLtcSensor(0);                              //Sam 20221101 : Latch 清除都要確認是否清清乾淨
...
void Do_Auto_SHT2()
{
    ...
    switch(Task)
    {
        ...
        case 130:
            ...
            if(MOT[MInShuttle2].MotorMove(SThreadPara.iOShRotateToLeftCheck[1]))
            {
...
//                    bAutoCleanCheckOpenDoor=true;                             //JerryYang 20190625 mark掉  //kevin 20180725 (wei) add 掉料忽視需開門 Alarm 後需要開門確認
        ...
        case 400:
            ...
            if(MOT[MInShuttle2].IsCanMove() &&
               ...
               (iIndex2ZPos>=Prod.TestZ2_Safe-3 ||
                bflag))
            {
                if(MOT[MInShuttle2].MotorMove(Prod.InSHT[1].iRight-1000))       //故意讓位置不到位，所以-1000
                {
                    if(iOldTask!=Task)
                    {
        ...
        case 200:
            ...
            else if(USE_LASER_DISTANCE && TestIF_File.bEnableShuttleLaser)      //Steven 20140228 : 雷射測距功能
            {
        ...
        case 220:
            ...
            if(MOT[MInShuttle2].MotorMove(SThreadPara.iInShRotateCheck[1]))
            {
                RotShuttleDelay2.SetSecAndOn(IniConfig.dRoShCheckDelayTime);
                Task=225;
            }
        ...
        case 230:
            ...
            if(MOT[MInShuttle2].MotorMove(SThreadPara.iOShRotateToRightCheck[1]))
            {
                RotShuttleDelay2.SetSecAndOn(IniConfig.dRoShCheckDelayTime);
                Task=235;
            }
...
//                break;
        ...
        case 300:
            ...
            iRetryCT=0;
            bCheckShuttleFlag=true;
            if(MOT[MInShuttle2].MotorMove(Prod.iBS2Right_Half))
            {
                if(iOldTask!=Task)
                {
