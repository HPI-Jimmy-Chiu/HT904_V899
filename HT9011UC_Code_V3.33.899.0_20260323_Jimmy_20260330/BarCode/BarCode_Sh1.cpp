...
//---------------------------------------------------------------------------
void TfBarCode::InitialBarcodeScanInShuttle1(bool bClear2DID)                   //Steven 20160823 : 歸零後也要清空蝦頭的2DID
{
    AnsiString Log;
...
bool TfBarCode::DoBarcodeCCDInShuttle_1(bool bVerify)                           //Steven 20160106 : 改用CCD拍完就跑的方式
{
    ...
    switch(Task)
    {
        ...
        case 1170:
            if(bCCDBarcodeExposureOK[iBarCodeRowA]==false)
            {
                ...
                {
                    bNeesScanCode[0][iNowMoveStep]=false;
                    SendSE0(iBarCodeRowA, false, iNowMoveStep, iNowMoveStepMulti2D, "exposure 1170: ");   //Steven 20200908 : 整合2DID SE0
                }
                else
                {
                    bNeesScanCode[0][iNowMoveStep]=true;
                    SendSE0(iBarCodeRowA, true, iNowMoveStep, iNowMoveStepMulti2D, "exposure 1170: ");   //Steven 20200908 : 整合2DID SE0
                }
            }
            ...
            if(InArmSuck.iShtRow==2)
            {
                if(bCCDBarcodeExposureOK[iBarCodeRowB]==false)
                {
                    ...
                    {
                        bNeesScanCode[1][iNowMoveStep]=false;
                        SendSE0(iBarCodeRowB, false, iNowMoveStep, iNowMoveStepMulti2D, "exposure 1170: ");   //Steven 20200908 : 整合2DID SE0
                    }
                    else
                    {
                        bNeesScanCode[1][iNowMoveStep]=true;
                        SendSE0(iBarCodeRowB, true, iNowMoveStep, iNowMoveStepMulti2D, "exposure 1170: ");   //Steven 20200908 : 整合2DID SE0
                    }
                }
            }
            else if(TestIF_File.bEnableMulti2D &&                               //Steven 20240612 : for 對角2D
        ...
        case 1300:
            ...
            if(InArmSuck.iShtRow==2)
            {
                ...
                SendCCDCommand(iBarCodeRowB, "get result 1300: ", sCCDCommand);
            }
        ...
        case 1350:
            ...
            {
                CCDBarcode_SortingForMulti2D(0);
                sErrorPart="";
                for(int i=0; i<InArmSuck.iShtCol; i++)                          //清空畫面
                {
                        ...
                        FLCarryKit.cDeviceInf[0][i]==""))                       //wei 20160318 Barcode Error依客戶設定
                    {
                        ...
                        if(CUSTOMER_CODE==CC_ASE_CL)                            //KaiChen 20191121 ：中壢日月光 2D Check Sum
                        {
                            ...
                            (FLCarryKit.cDeviceInf[0][i].AnsiPos(asBarCodeErrorCheckSum)!=0 ||
                             FLCarryKit.cDeviceInf[0][i]==""))                  //KaiChen 20191121 ：中壢日月光 2D Check Sum
                    {
                        bHasError=true;
                        if(IsNNMode()==NN_2Row)                                 //Steven 20220315 : for 2did nn mode alarm
                        {
                        ...
                        if(CUSTOMER_CODE==CC_ASE_CL)                            //KaiChen 20191121 ：中壢日月光 2D Check Sum
                        {
                            ...
                            ((fBarCode->RunCheckBarcodeByServerData()==true) &&
                             (fMesSystem->IsMatchServerData(FLCarryKit.cDeviceInf[0][i])==false)))      //Jimmychiu 20230925 : read 2did in json file
                    {
                        bHasError=true;
                        if(IsNNMode()==NN_2Row)                                 //Steven 20220315 : for 2did nn mode alarm
                        {
                    ...
                    if(bNeesScanCode[1][i]==true &&
                       ...
                       (FLCarryKit.cDeviceInf[1][i].AnsiPos(asBarCodeErrorSend)!=0 ||
                        FLCarryKit.cDeviceInf[1][i]==""))                       //wei 20160318 Barcode Error依客戶設定
                    {
                        bHasError=true;
                        if(IsNNMode()==NN_2Row)                                 //Steven 20220315 : for 2did nn mode alarm
                        {
                        ...
                        if(CUSTOMER_CODE==CC_ASE_CL)                            //KaiChen 20191121 ：中壢日月光 2D Check Sum
                        {
                            ...
                            (FLCarryKit.cDeviceInf[1][i].AnsiPos(asBarCodeErrorCheckSum) ||
                             FLCarryKit.cDeviceInf[1][i]==""))                  //KaiChen 20191121 ：中壢日月光 2D Check Sum
                    {
                        bHasError=true;
                        if(IsNNMode()==NN_2Row)                                 //Steven 20220315 : for 2did nn mode alarm
                        {
                        ...
                        if(CUSTOMER_CODE==CC_ASE_CL)                            //KaiChen 20191121 ：中壢日月光 2D Check Sum
                        {
                            ...
                            (FLCarryKit.Item[1][i]!=HAS_NULL_IC &&
                             FLCarryKit.Item[1][i]!=NULL_IC) &&
                            ((fBarCode->RunCheckBarcodeByServerData()==true) &&
                             (fMesSystem->IsMatchServerData(FLCarryKit.cDeviceInf[1][i])==false) ))      //Jimmychiu 20230925 : read 2did in json file
                    {
                        bHasError=true;
                        if(IsNNMode()==NN_2Row)                                 //Steven 20220315 : for 2did nn mode alarm
                        {
        ...
        case 2600:
            ...
            AddCCDCommunicationLog(iBarCodeRowA, Log);
            if(TestIF_File.bCheckCodeByLot &&                                   //Alick 20170124 add Retry時bRetryOffsetMove=true先退出去
               iRetryCount<=TestIF_File.iBarcodeRetryCount)                     //Alick 20170315 (wei) modify bRetryOffsetMove=>bCheckCodeByLot
            {
                bHasError=false;
                iRetryCount++;
            ...
            else
            {
                ...
                {
                    ret=ShowErrorMessage("JAM0495", K_RETRY, MSht, false, sErrorPart);      //In Shuttle1 Barcode Error
                }
                else
                {
                    ret=ShowErrorMessage("JAM0460", K_RETRY|K_SKIP, MSht, false, sErrorPart);      //In Shuttle1 Barcode Error
                }
        ...
        case 3000:                                                              //例外處理
            ...
            for(int i=0; i<FLCarryKit.iMaxCol; i++)                             //全部重新掃描
            {
                if(bVerify==true ||
                   (FLCarryKit.Item[0][i]!=HAS_NULL_IC &&
                    FLCarryKit.Item[0][i]!=NULL_IC))
...
//                        bCCDBarcodeExposureOK[iBarCodeRowB]=true;             //KenHsieh 20250716 : Mark 避免到下一步直接通過，使得Step減1，導致exposure error
                    ...
                    {
                        SendSE0(iBarCodeRowB, true, iNowCheckStep, iNowMoveStepMulti2DX, "exposure 3300: ");    //JerryYang 20210120 : iBarCodeRowA -> iBarCodeRowB, 修正2DID fail時使用錯誤CCD取像的問題   //Steven 20200908 : 整合2DID SE0
...
//                        bCCDBarcodeExposureOK[iBarCodeRowA]=true;             //KenHsieh 20250716 : Mark 避免到下一步直接通過，使得Step減1，導致exposure error
//                        bCCDBarcodeExposureOK[iBarCodeRowB]=true;             //KenHsieh 20250716 : Mark 避免到下一步直接通過，使得Step減1，導致exposure error
                        SendSE0(iBarCodeRowA, false, iNowCheckStep, iNowMoveStepMulti2DX, "exposure 3300: ");
                        SendSE0(iBarCodeRowB, false, iNowCheckStep, iNowMoveStepMulti2DX, "exposure 3300: ");
                    }
                    else
                    {
                        if(TestIF_File.iMulti2DMap[0][iNowMoveStepMulti2D]>0)
                            SendSE0(iBarCodeRowA, true, iNowCheckStep, iNowMoveStepMulti2DX, "exposure 3300: ");
                        else
                            SendSE0(iBarCodeRowA, false, iNowCheckStep, iNowMoveStepMulti2DX, "exposure 3300: ");

                        if(TestIF_File.iMulti2DMap[1][iNowMoveStepMulti2D]>0)
                            SendSE0(iBarCodeRowB, true, iNowCheckStep, iNowMoveStepMulti2DX, "exposure 3300: ");
                        else
                            SendSE0(iBarCodeRowB, false, iNowCheckStep, iNowMoveStepMulti2DX, "exposure 3300: ");
                        FLCarryKit.cDeviceInf[0][iNowCheckStep]="";
                    }
        ...
        case 3400:                                                              //拍照讀取2D Code
               ...
               bStepError[iBarCodeRowB]==true)                                  //Steven 20160503 : 確認移動位置與拍照位置相同
            {
                ...
                if(IsNNMode()==NN_2Row)                                         //Steven 20220315 : for 2did nn mode alarm
                {
                    ...
                    if(bStepError[iBarCodeRowB])
                        ErrPart+=IndexSuckName[3][iNowCheckStep];               //Frank 20171011 (Steven) iNowMoveStep-->iNowCheckStep
                }
                else if(IsNNMode()==NN_1Row)
                {
                    if(bStepError[iBarCodeRowA] ||
...
//                        bCCDBarcodeExposureOK[iBarCodeRowB]=true;             //KenHsieh 20250716 : Mark 需等待回復
        ...
        case 3460:
            ...
            {
                ret=ShowErrorMessage("WAR0463", K_RETRY|K_SKIP, MSht);          //2D function CCD 1 or 2 Exposure time out!
                if(ret==K_SKIP)
                {
                    Task=3100;
                }
        ...
        case 3470:
                     ...
                     TestIF_File.iMulti2DType==e2x2In2CCD))
            {
                if(bCCDBarcodeExposureOK[iBarCodeRowA]==false)
                {
                    bNeesScanCode[0][iNowCheckStep]=true;                       //KenHsieh 20260114 : iNowMoveStep -> iNowCheckStep 修正取錯變數導致死當問題
                    if(TestIF_File.iMulti2DMap[0][iNowMoveStepMulti2D]>0)
                        SendSE0(iBarCodeRowA, true, iNowCheckStep, iNowMoveStepMulti2DX, "exposure 3470: ");
                    else
                ...
                if(bCCDBarcodeExposureOK[iBarCodeRowB]==false)
                {
                    bNeesScanCode[0][iNowCheckStep]=true;                       //KenHsieh 20260114 : iNowMoveStep -> iNowCheckStep 修正取錯變數導致死當問題
                    if(TestIF_File.iMulti2DMap[1][iNowMoveStepMulti2D]>0)
                        SendSE0(iBarCodeRowB, true, iNowCheckStep, iNowMoveStepMulti2DX, "exposure 3470: ");
                    else
                        SendSE0(iBarCodeRowB, false, iNowCheckStep, iNowMoveStepMulti2DX, "exposure 3470: ");
                }
            }
            else
            {
                        ...
                        for(int i=0; i<InArmSuck.iShtCol; i++)
                        {
                               ...
                               FLCarryKit.cDeviceInf[0][i]!="")
                            {
                                ...
                                if(!fBarCode->list2DByLot->Find(Str2, iIndex) &&
                                   LastSet.iTester==OFF_LINE)                   //Frank 20170611 (Steven) add Alex 多種Dummy IC 需要同時辨識
                                {
                                    bLotCheckError=true;
                                    bHasErr[0][i]=true;
                                }
        ...
        case 3600:
            if(MOT[MSht].MotorMove(Prod.InSHT[iSht].iLeft))                     //Steven 20160503 : 先移出來才Alarm
            {
                ...
                for(int i=0; i<InArmSuck.iShtRow; i++)
                {
                    ...
                    {
                        fNote->t2DCode->SetCellNumber(i, j, FLCarryKit.cDeviceInf[i][j].c_str());
                        if(bHasErr[i][j]==true)
                ...
                {
                    ret=ShowErrorMessage("WAR0471", K_RETRY, MInShuttle1, false);      //In Shuttle1 Barcode Error  //Alick 20170202 remove skip
                }
        ...
        case 4000:                                                              //Steven 20160428 : 檢查蝦頭的2D重複碼
            if(TestIF_File.bCheckCodeByShuttle ||
               TestIF_File.bCheckCodeByLot)                                     //Steven 20171128 (Wei) : check lot之前一定要先確認蝦頭內沒有重複的
            {
                    ...
                    if(InArmSuck.iShtRow==2)
                    {
                           ...
                           FLCarryKit.cDeviceInf[1][i]!=asBarCodeErrorCheckSum) //KaiHuang 20200826 : Add CheckSum
                        {
                            ...
                            else
                            {
                                ...
                                ret=atoi(map2DShuttleList[iBarCodeRowB][FLCarryKit.cDeviceInf[1][i]].c_str());   //Steven 20160503 : 重複的Code要設定為Error
                                if(IsNNMode()==NN_2Row)                         //Steven 20220315 : for 2did nn mode alarm
                                {
        ...
        case 4050:
            if(MOT[MSht].MotorMove(Prod.InSHT[iSht].iLeft))                     //Steven 20160503 : 先移出來才Alarm
            {
                   ...
                   TestIF_File.iShtDuplicateRetryCnt>=iShtDupilcateFailCnt[iSht])       //Steven 20160823 : 蝦頭重複碼要可以自動Retry
                {
                    ...
                    ret=ShowErrorMessage("WAR0465", K_RETRY|K_SKIP, MSht, false, ErrPart);  //In shuttle 1 check have duplicate barcode error!
                }
                else
                {
                    ...
                    for(int i=0; i<InArmSuck.iShtRow; i++)
                    {
                        for(int j=0; j<InArmSuck.iShtCol; j++)
                        {
                               ...
                               Prod.bF18InshuttleDetect==true)                  //JerryYang 20241009 : 渠梁陳勇要求重複2DID ALARM要求人員手動取出DEVICE
                            {
                                if(InArmSuck.iShtRow==2)
                                {
                                    if(bHasErr[0][j]==true || bHasErr[1][j]==true)
                                    {
                                        ...
                                        FLCarryKit.PordRec[i][j].AddTestResultRecord(iTestBinCount, "2DID_Duplicate");
                            ...
                            else
                            {
                                ...
                                {
                                    FLCarryKit.cDeviceInf[i][j]=asBarCodeErrorSend;
                                    FLCarryKit.PordRec[i][j].AddErrorRecordNoSave("Duplicate 2DID error");  //JerryYang 20230322 : Lot summary要計算各類型ERR的數量
                                }
                            }
                        }
                    }
        ...
        case 4150:
            if(MOT[MSht].MotorMove(Prod.InSHT[iSht].iLeft))                     //Steven 20160503 : 先移出來才Alarm
            {
                ...
                AddCCDCommunicationLog(iBarCodeRowA, Log);
                ret=ShowErrorMessage("WAR0467", K_RETRY|K_SKIP, MSht, false, ErrPart);  //In shuttle 1 check have duplicate barcode in lot error!
                if(ret==K_SKIP)
                {
                    for(int i=0; i<InArmSuck.iShtRow; i++)
                    {
            ...
            break;
        case 4200:
            if(TestIF_File.bCheckCodeByLot &&                                   //jou 20191008 : Barcode duplicate file by out arm
               ...
               fContact->IsRun2DCheck()==false)                                 //JerryYang 20250220 : 2DID硬體順序檢查功能
            {
                ...
                for(int i=0; i<InArmSuck.iShtCol; i++)
                {
                       ...
                       FLCarryKit.cDeviceInf[0][i]!=asBarCodeErrorCheckSum)     //KaiHuang 20200826 : Add CheckSum
                    {
                        ...
                        list2DByLot->Add(FLCarryKit.cDeviceInf[0][i]);
                    }
                    ...
                    if(InArmSuck.iShtRow==2)
                    {
                           ...
                           FLCarryKit.cDeviceInf[1][i]!=asBarCodeErrorCheckSum) //KaiHuang 20200826 : Add CheckSum
                        {
                            ...
                            list2DByLot->Add(FLCarryKit.cDeviceInf[1][i]);
                        }
                    }
                }
            ...
            if(TestIF_File.bEnableBarCode==true &&                              //JerryYang 20241104 : 支援2DID白名單功能
               ...
               fContact->IsRun2DCheck()==false)                                 //JerryYang 20250220 : 2DID硬體順序檢查功能
            {
                ...
                for(int i=0; i<InArmSuck.iShtCol; i++)
                {
                       ...
                       IniConfig.iN23DownloadMethod==2)                         //JerryYang 20250320 : 2DID白名單功能
                    {
                        ...
                        {
                            if(fBarCode->bSHT_2DIDIsInsideList(0, 0, i, &iIndex)==0)
                            {
                                bHasErr[0][i]=true;
                                if(IsNNMode()==NN_2Row)                         //Steven 20220315 : for 2did nn mode alarm
                                {
                                ...
                                else if(IsNNMode()==NN_1Row)                    //Steven 20220425 : 2X3NN Mode
                                {
                                    ErrPart+=IndexSuckName[1][i];
                                }
                                else
                                ...
                                bAlarm=true;
                                sLog.sprintf("%s_%s_%s_Fail", FLCarryKit.cDeviceInf[0][i], sWhiteListLotID, sWhiteListProcess);
                            ...
                            else
                            {
                                if(iIndex>=0)
                                {
                                    fBarCode->list2DWhitleTemp->DelimitedText="_";
                                    fBarCode->list2DWhitleTemp->Text=fBarCode->list2DWhitleResult->Strings[iIndex];
                                    fBarCode->list2DWhitleTemp->DelimitedText="_";

                                    sLog.sprintf("%s_%s_%s_Pass", FLCarryKit.cDeviceInf[0][i], sWhiteListLotID, sWhiteListProcess);
                                    WriteDataToFile(as2DWhiteListLog, sLog);
                                }
                            }
                        }
                        ...
                        if(InArmSuck.iShtRow==2)
                        {
                            if(FLCarryKit.Item[1][i]!=HAS_NULL_IC && FLCarryKit.Item[1][i]!=NULL_IC &&
                               ...
                               FLCarryKit.cDeviceInf[1][i]!=asBarCodeErrorCheckSum)     //KaiHuang 20200826 : Add CheckSum
                            {
                                if(fBarCode->bSHT_2DIDIsInsideList(0, 1, i, &iIndex)==0)
                                {
                                    bHasErr[1][i]=true;
                                    if(IsNNMode()==NN_2Row)                     //Steven 20220315 : for 2did nn mode alarm
                                        ErrPart+=IndexSuckName[3][i];
                                        ...
                                        ErrPart+=IndexSuckName[1][i];
                                    bAlarm=true;
                                    sLog.sprintf("%s_%s_%s_Fail", FLCarryKit.cDeviceInf[1][i], sWhiteListLotID, sWhiteListProcess);
                                    WriteDataToFile(as2DWhiteListLog, sLog);
                                }
                                else
                                {
                                    if(iIndex>=0)
                                    {
                                        ...
                                        sLog.sprintf("%s_%s_%s_Pass", FLCarryKit.cDeviceInf[1][i], sWhiteListLotID, sWhiteListProcess);
                                        WriteDataToFile(as2DWhiteListLog, sLog);
                                    }
                                }
                            }
                    ...
                    else
                    {
                        if(FLCarryKit.Item[0][i]!=HAS_NULL_IC && FLCarryKit.Item[0][i]!=NULL_IC &&
                           ...
                           FLCarryKit.cDeviceInf[0][i]!=asBarCodeErrorCheckSum)
                        {
                            sLog.sprintf("%s_%s_%s_NA", FLCarryKit.cDeviceInf[0][i], sWhiteListLotID, sWhiteListProcess);
                            WriteDataToFile(as2DWhiteListLog, sLog);
                        }
                        ...
                        if(InArmSuck.iShtRow==2)
                        {
                            if(FLCarryKit.Item[1][i]!=HAS_NULL_IC && FLCarryKit.Item[1][i]!=NULL_IC &&
                               ...
                               FLCarryKit.cDeviceInf[1][i]!=asBarCodeErrorCheckSum)     //KaiHuang 20200826 : Add CheckSum
                            {
                                sLog.sprintf("%s_%s_%s_NA", FLCarryKit.cDeviceInf[1][i], sWhiteListLotID, sWhiteListProcess);
                                WriteDataToFile(as2DWhiteListLog, sLog);
                            }
                        }
                    }
        ...
        case 4300:
            if(MOT[MSht].MotorMove(Prod.InSHT[iSht].iLeft))
            {
                ...
                AddCCDCommunicationLog(iBarCodeRowA, Log);
                ret=ShowErrorMessage("WAR0485", K_SKIP, MSht, false, ErrPart);  //In shuttle 1 check have duplicate barcode in lot error!
                        ...
                        for(int j=0; j<InArmSuck.iShtCol; j++)
                        {
                               ...
                               Prod.bF18InshuttleDetect==true)                  //JerryYang 20241009 : 渠梁陳勇要求重複2DID ALARM要求人員手動取出DEVICE
                            {
                                if(InArmSuck.iShtRow==2 && Prod.bF18InshuttleDetect)
                                {
                                    ...
                                    {
                                        FLCarryKit.SetItemData(i, j, HAS_NULL_IC);
                                        FLCarryKit.PordRec[i][j].AddTestResultRecord(iTestBinCount, "2DIDNotInAllowList");
                                    }
                                }
                                else
                                {
                            ...
                            else
                            {
                                ...
                                {
                                    FLCarryKit.cDeviceInf[i][j]=asBarCodeErrorSend;
                                    FLCarryKit.PordRec[i][j].AddErrorRecordNoSave("2DIDNotInAllowList 2DID error");  //JerryYang 20230322 : Lot summary要計算各類型ERR的數量
                                }
                            }
                        }
                    }
            ...
            break;
        case 5000:
            if(fBarCode->DoCheckSHT2DIDStatus(0)==true)                         //Steven 20200909 : 將2DID all site fail變成選項
            {
                    ...
                    for(int i=0; i<InArmSuck.iShtCol; i++)
                    {
                           ...
                           FLCarryKit.Item[0][i]!=NULL_IC)
                        {
                            if(FLCarryKit.cDeviceInf[0][i]==asBarCodeErrorSend ||
                               FLCarryKit.cDeviceInf[0][i]=="" ||
                               FLCarryKit.cDeviceInf[0][i]==asBarCodeErrorCheckSum)       //KaiChen 20191121 ：中壢日月光 2D Check Sum
                            {
                                FLCarryKit.SetItemData(0, i, TEST_PASS+iTestBinCount);
                                FLCarryKit.iBinData[0][i]=iTestBinCount;
                                FLCarryKit.PordRec[0][i].AddTestResultRecord(iTestBinCount, FLCarryKit.cSBin[0][i], "NonTestToRBin");
                            }
                        }
                        ...
                        if(InArmSuck.iShtRow==2)
                        {
                               ...
                               FLCarryKit.Item[1][i]!=NULL_IC)
                            {
                                if(FLCarryKit.cDeviceInf[1][i]==asBarCodeErrorSend ||
                                   ...
                                   FLCarryKit.cDeviceInf[1][i]==asBarCodeErrorCheckSum)  //KaiChen 20191121 ：中壢日月光 2D Check Sum
                                {
                                    FLCarryKit.SetItemData(1, i, TEST_PASS+iTestBinCount);
                                    FLCarryKit.iBinData[1][i]=iTestBinCount;
                                    FLCarryKit.PordRec[1][i].AddTestResultRecord(iTestBinCount, FLCarryKit.cSBin[1][i], "NonTestToRBin");
                                }
                            }
                        }
                    }
        ...
        case 5050:
            for(int i=0; i<InArmSuck.iShtCol; i++)
            {
                   ...
                   FLCarryKit.Item[0][i]!=NULL_IC)
                {
                    ...
                    FLCarryKit.PordRec[0][i].Add2DIDRecord(FLCarryKit.cDeviceInf[0][i]);
                }

                if(InArmSuck.iShtRow==2)
                {
                    if(FLCarryKit.Item[1][i]!=HAS_NULL_IC &&                    //RogerYang 20181219 修正沒放IC還是會增加計數問題
                       FLCarryKit.Item[1][i]!=NULL_IC)
                    {
                    ...
                    FLCarryKit.PordRec[1][i].Add2DIDRecord(FLCarryKit.cDeviceInf[1][i]);
                }
            }
...
//------------------------------------------------------------------------------
bool TfBarCode::DoBarcodeTriggerInShuttle_1()                                   //Steven 20151225 : 改用拍完就跑的方式
{
    static int i2DIDRetryCTSH1=0;                                               //Isaac 20200206 : 2DID All site fail recheck twice and alarm
    static int iRetryCount=0, iStepCount;
    ...
    switch(Task)
    {
        case 1:
            ...
            for(int i=0; i<FLCarryKit.iMaxCol; i++)                             //清空畫面
            {
                mtBarcodeInSh->SetCellNumber(iBarCode1_1, i+1, "");
                mtBarcodeInSh->SetCellNumber(iBarCode1_2, i+1, "");

                if(TestIF_File.i2DIDFormat==eAMD)                               //JerryYang 20200422 2DID format選項改用下拉選單
                {
                    fLotInfo->mtBarcodeInShLotInfo->SetCellNumber(0, i+1, "");
                }
            }
        ...
        case 1000:
            ...
            {
                pos=Prod.iInSHBarCodeDetectPos1x2[0][iNowMoveStep];
            }
            else if(InArmSuck.iShtCol==3)                                       //ChungHung 20140115 add for 2x3_6
            {
                pos=Prod.iInSHBarCodeDetectPos2x3[0][iNowMoveStep];
            }
            else if(InArmSuck.iShtCol==4)
            {
                pos=Prod.iInSHBarCodeDetectPos1x4[0][iNowMoveStep];
            }
            else if(InArmSuck.iShtCol==5)                                       //Steven 20221027 : Add for 2x5
            {
                pos=Prod.iInSHBarCodeDetectPos2x5[0][iNowMoveStep];
            }
            else if(InArmSuck.iShtCol==6)                                       //Steven 20170111 : fixed for 2x6
            {
                pos=Prod.iInSHBarCodeDetectPos2x6[0][iNowMoveStep];
            }
            else if(InArmSuck.iShtCol==8)
            {
                pos=Prod.iInSHBarCodeDetectPos2x8[0][iNowMoveStep];
            }
            else
            {
                ShowMyMessage("The mode is not support!!", "Please save state record and provide to HonPrec software engineer", "DoBarcodeTriggerInShuttle_1");
            }
        ...
        case 1120:
            if(BarcodePosDelay[iSht].Off())                                     //到位Delay
            {
                ...
                if(bBarcodeNum[iBarCode1_1]==false)
                {
                    ...
                    Barcode_StartScan_In(iBarCode1_1, iNowCheckStep, FLCarryKit.cDeviceInf[0][iNowCheckStep], cLastString);
                }
                ...
                if(bBarcodeNum[iBarCode1_2]==false)
                {
                    ...
                    Barcode_StartScan_In(iBarCode1_2, iNowCheckStep, FLCarryKit.cDeviceInf[1][iNowCheckStep], cLastString);
                }
        ...
        case 1350:
            if(bBarcodeNum[iBarCode1_1]==false)
            {
                ...
                bBarcodeNum[iBarCode1_1]=Barcode_StartScan_In(iBarCode1_1, iNowCheckStep, FLCarryKit.cDeviceInf[0][iNowCheckStep], cLastString);
            }
            ...
            if(bBarcodeNum[iBarCode1_2]==false)
            {
                ...
                bBarcodeNum[iBarCode1_2]=Barcode_StartScan_In(iBarCode1_2, iNowCheckStep, FLCarryKit.cDeviceInf[1][iNowCheckStep], cLastString);
            }
            ...
            else if(BarcodeDelay[iSht].Off())                                   //讀取超時
            {
                ...
                {
                    bHasError=true;
                    TurnOffInspection(iBarCode1_1);
                    FLCarryKit.cDeviceInf[0][iNowCheckStep]=asBarCodeErrorSend; //wei 20160318 Barcode Error依客戶設定
                    mtBarcodeInSh->SetCellNumber(iBarCode1_1, iNowCheckStep+1, asBarCodeErrorSend.c_str());
                    if(TestIF_File.i2DIDFormat==eAMD)                           //JerryYang 20200422 2DID format選項改用下拉選單
                    {
                        fLotInfo->mtBarcodeInShLotInfo->SetCellNumber(0, iNowCheckStep+1, asBarCodeErrorSend.c_str());
                    }
                }
                if(bBarcodeNum[iBarCode1_2]==false)
                {
                    bHasError=true;
                    TurnOffInspection(iBarCode1_2);
                    FLCarryKit.cDeviceInf[1][iNowCheckStep]=asBarCodeErrorSend; //wei 20160318 Barcode Error依客戶設定
                    mtBarcodeInSh->SetCellNumber(iBarCode1_2, iNowCheckStep+1, asBarCodeErrorSend.c_str());
                }
                Task=2000;
            }
            break;
        case 2000:
            ...
            if(iNowMoveStep==-1)
            {
                ...
                else
                {
                    ...
                    {
                        if(fBarCode->DoCheckSHT2DIDStatus(0)==true)
                        {
                            Task=5000;                                          //例外，要重掃
                            break;
                        }
        ...
        case 3200:                                                              //重置參數
            if(FLCarryKit.cDeviceInf[0][iNowCheckStep]==asBarCodeErrorSend)     //wei 20160318 Barcode Error依客戶設定
            {
                mtBarcodeInSh->SetCellNumber(iBarCode1_1, iNowCheckStep+1, "");
                if(TestIF_File.i2DIDFormat==eAMD)                               //JerryYang 20200422 2DID format選項改用下拉選單
                {
                    fLotInfo->mtBarcodeInShLotInfo->SetCellNumber(0, iNowCheckStep+1, "");
                }
                FLCarryKit.cDeviceInf[0][iNowCheckStep]="";
                bBarcodeNum[iBarCode1_1]=false;
                bBarcodeStartDelay[iBarCode1_1]=true;
            ...
            {
                mtBarcodeInSh->SetCellNumber(iBarCode1_2, iNowCheckStep+1, "");
                FLCarryKit.cDeviceInf[1][iNowCheckStep]="";
                bBarcodeNum[iBarCode1_2]=false;
                bBarcodeStartDelay[iBarCode1_2]=true;
            }
...
//------------------------------------------------------------------------------
bool TfBarCode::DoBarcodeScanInShuttle_1()
{
    ...
    switch(Task)
    {
        case 1:
            ...
            bBarcodeStartDelay[iOCRMap[iBarCode1_1]]=true;
            if(InArmSuck.iShtRow==2)
            {
                bBarcodeStartDelay[iOCRMap[iBarCode1_2]]=true;
            }
            ...
            bBarcodeNum[iOCRMap[iBarCode1_1]]=false;
            bBarcodeNum[iOCRMap[iBarCode1_2]]=false;
            bBarcodeFirstAutoRetry[iOCRMap[iBarCode1_1]]=false;
            bBarcodeFirstAutoRetry[iOCRMap[iBarCode1_2]]=false;
            iRetryCount=0;
            if(bErrorRetry)
            {
                Task=1100;
        ...
        case 1000:
            ...
            {
                if(bBarcodeFirstAutoRetry[iOCRMap[iBarCode1_1]]==false)
                    iNeedBarcodeCount[iOCRMap[iBarCode1_1]]++;
            }

            if(FLCarryKit.Item[1][iNowCheckStep]!=HAS_NULL_IC &&
               FLCarryKit.Item[1][iNowCheckStep]!=NULL_IC)                      //wei 20160516 修改需讀取barcode顆數累計
            {
                if(bBarcodeFirstAutoRetry[iOCRMap[iBarCode1_2]]==false)
                    iNeedBarcodeCount[iOCRMap[iBarCode1_2]]++;
            }
            Task=1100;
        case 1100:
            if(bBarcodeNum[iOCRMap[iBarCode1_1]]==false)
            {
                if(BAR_CODE_INSTALL!=ebcUseOCR)
                    FLCarryKit.cDeviceInf[0][iNowCheckStep]="";
                else
                    FLCarryKit.cDeviceInf[1][iNowCheckStep]="";
                mtBarcodeInSh->SetCellNumber(iOCRMap[iBarCode1_1], iNowCheckStep+1, "");
            ...
            if(bBarcodeNum[iOCRMap[iBarCode1_2]]==false)
            {
                if(BAR_CODE_INSTALL!=ebcUseOCR)
                    FLCarryKit.cDeviceInf[1][iNowCheckStep]="";
                else
                    FLCarryKit.cDeviceInf[0][iNowCheckStep]="";
                mtBarcodeInSh->SetCellNumber(iOCRMap[iBarCode1_2], iNowCheckStep+1, "");
            }
            ...
            if(iNowCheckStep>=0 && iNowCheckStep<InArmSuck.iShtCol)
            {
                ...
                {
                    ShowMyMessage("The mode is not support!!", "Please save state record and provide to HonPrec software engineer", "DoBarcodeScanInShuttle_1");
                }
        ...
        case 1150:
            if(FLCarryKit.Item[0][iNowCheckStep]==HAS_NULL_IC ||                //Shuttle 1, Row A使用第1隻CCD, Row B使用第2隻CCD
               FLCarryKit.Item[0][iNowCheckStep]==NULL_IC)
            {
                bBarcodeNum[iOCRMap[iBarCode1_1]]=true;
            }
            else if(bBarcodeNum[iOCRMap[iBarCode1_1]]==false)
            {
                if(BAR_CODE_INSTALL!=ebcUseOCR)
                    iSH1_1BarcodePosition=iNowCheckStep;
                else
                ...
                if(Barcode_StartScan_In(iOCRMap[iBarCode1_1], iNowCheckStep, FLCarryKit.cDeviceInf[0][iNowCheckStep], cLastString)==true)
                {
                    bBarcodeStartDelay[iOCRMap[iBarCode1_1]]=true;
                    bBarcodeDataSaveReady[iOCRMap[iBarCode1_1]]=false;
                    bBarcodeNum[iOCRMap[iBarCode1_1]]=true;
                }
                ...
                {
                    if(bCCDBarcodeWaitReply[iOCRMap[iBarCode1_1]]==false)
                    {
                        BarcodeWaitReplyTimeOut[iOCRMap[iBarCode1_1]].SetMSAndOn(TestIF_File.i2DHandShakeTimeOut);
                        bCCDBarcodeWaitReply[iOCRMap[iBarCode1_1]]=true;
                    }
                }
            }
            ...
            if(InArmSuck.iShtRow==2)
            {
                if(FLCarryKit.Item[1][iNowCheckStep]==HAS_NULL_IC ||
                   FLCarryKit.Item[1][iNowCheckStep]==NULL_IC)
                {
                    bBarcodeNum[iOCRMap[iBarCode1_2]]=true;
                }
                else if(bBarcodeNum[iOCRMap[iBarCode1_2]]==false)
                {
                    if(BAR_CODE_INSTALL!=ebcUseOCR)
                        iSH1_2BarcodePosition=iNowCheckStep;
                    else
                    ...
                    if(Barcode_StartScan_In(iOCRMap[iBarCode1_2], iNowCheckStep, FLCarryKit.cDeviceInf[1][iNowCheckStep], cLastString)==true)
                    {
                        bBarcodeStartDelay[iOCRMap[iBarCode1_2]]=true;
                        bBarcodeDataSaveReady[iOCRMap[iBarCode1_2]]=false;
                        bBarcodeNum[iOCRMap[iBarCode1_2]]=true;
                    }
                    ...
                    if(BAR_CODE_INSTALL==ebctEtherNetCCD &&                     //Ifor 20190225 :add Bar Code Use HandShake Communication
                       ...
                       TestIF_File.bUseHandShakeCommunication==true)
                    {
                        if(bCCDBarcodeWaitReply[iOCRMap[iBarCode1_2]]==false)
                        {
                            BarcodeWaitReplyTimeOut[iOCRMap[iBarCode1_2]].SetMSAndOn(TestIF_File.i2DHandShakeTimeOut);
                            bCCDBarcodeWaitReply[iOCRMap[iBarCode1_2]]=true;
                        }
                    }
                }
            }
            ...
            if(bBarcodeNum[iOCRMap[iBarCode1_1]]==true &&
               bBarcodeNum[iOCRMap[iBarCode1_2]]==true)
            {
                Task=1200;
                bCCDBarcodeWaitReply[iOCRMap[iBarCode1_1]]=false;
                bCCDBarcodeWaitReply[iOCRMap[iBarCode1_2]]=false;
            }
            else if(BAR_CODE_INSTALL==ebctEtherNetCCD &&
                    CosFunction.bUseHandShakeCommunication &&
                    TestIF_File.bUseHandShakeCommunication==true)
            {
                if(BarcodeWaitReplyTimeOut[iOCRMap[iBarCode1_1]].Off())
                {
                    Task=1180;
                    bCCDBarcodeWaitReply[iOCRMap[iBarCode1_1]]=false;
                ...
                if(InArmSuck.iShtRow==2 &&
                   BarcodeWaitReplyTimeOut[iOCRMap[iBarCode1_2]].Off())
                {
                    Task=1180;
                    bCCDBarcodeWaitReply[iOCRMap[iBarCode1_2]]=false;
                    bBarcodeHandShakeTimeOut=true;
            ...
            else if(BarcodeDelay[iSht].Off())
            {
                if(iRetryCount<TestIF_File.iBarcodeRetryCount)                  //Steven 20151221 : 讀不到Code時,自動Retry的次數
                {
                    if(iRetryCount==0)
                    {
                        if(bBarcodeNum[iOCRMap[iBarCode1_1]]==false &&
                           bBarcodeFirstAutoRetry[iOCRMap[iBarCode1_1]]==false)
                        {
                            iBarcodeAutoRetry[iOCRMap[iBarCode1_1]]++;
                            bBarcodeFirstAutoRetry[iOCRMap[iBarCode1_1]]=true;
                        }

                        if(bBarcodeNum[iOCRMap[iBarCode1_2]]==false &&
                           bBarcodeFirstAutoRetry[iOCRMap[iBarCode1_2]]==false)
                        {
                            iBarcodeAutoRetry[iOCRMap[iBarCode1_2]]++;
                            bBarcodeFirstAutoRetry[iOCRMap[iBarCode1_2]]=true;
                        }
                    }
                ...
                else if(bBarcodeAutoSkip[iOCRMap[iBarCode1_1]]==true ||
                        bBarcodeAutoSkip[iOCRMap[iBarCode1_2]]==true)           //wei Barcode Lot 錯誤取出IC，需重新讀取確認是否取出IC
                {
                    ...
                    bBarcodeAutoSkip[iOCRMap[iBarCode1_1]]=false;

                    if(bBarcodeAutoSkip[iOCRMap[iBarCode1_2]]==true)
                    {
                        if(BAR_CODE_INSTALL!=ebcUseOCR)
                            FLCarryKit.SetItemData(1, iNowCheckStep, HAS_NULL_IC);
                        else
                    ...
                    bBarcodeAutoSkip[iOCRMap[iBarCode1_2]]=false;
                    InitSht2DCodeComPort(iSht);                                 //Ifor 20160322 Retry 時重置
                    iSH1_1BarcodePosition=iNowCheckStep;
                    iSH1_2BarcodePosition=iNowCheckStep;
                    CleanBarcodeError(1);
        ...
        case 1170:
            ...
            break;
        case 1175:
            if(bImageSaveflag[iOCRMap[iBarCode1_1]]==false &&
               bImageSaveflag[iOCRMap[iBarCode1_2]]==false)
            {
                Task=1180;
            }
            else
        ...
        case 1180:
            ...
            if(bMoveOK==true)
            {
                TurnOffInspection(iOCRMap[iBarCode1_1]);
                TurnOffInspection(iOCRMap[iBarCode1_2]);
                #ifdef SOFT_SIMULTE
                ret=K_SKIP;
                #else
                if(LastSet.iRealDummy==DUMMY)
                ...
                else
                {
                    ...
                    else if(bBarcodeAutoSkipError[iOCRMap[iBarCode1_1]]==true ||
                            bBarcodeAutoSkipError[iOCRMap[iBarCode1_2]]==true)  //wei Barcode Lot 錯誤取出IC，需重新讀取確認是否取出IC
                    {
                        sCheckErrorPart="";
                        if(BAR_CODE_INSTALL!=ebcUseOCR)
                        {
                            if(bBarcodeAutoSkipError[iOCRMap[iBarCode1_1]])     //wei 20160505 Barcode 比對Lot
                            {
                                if(IsNNMode()==NN_2Row)                         //Steven 20220315 : for 2did nn mode alarm
                            ...
                            if(bBarcodeAutoSkipError[iOCRMap[iBarCode1_2]])     //wei 20160505 Barcode 比對Lot
                            {
                                if(IsNNMode()==NN_2Row)                         //Steven 20220315 : for 2did nn mode alarm
                        ...
                        ret=ShowErrorMessage("WAR0476", K_SKIP, MInShuttle1, false, sCheckErrorPart);      //In Shuttle1 Barcode Error
                            ...
                            (bCheckCodeError[iOCRMap[iBarCode1_1]] ||
                             bCheckCodeError[iOCRMap[iBarCode1_2]]))
                    {
                        sCheckErrorPart="";
                        if(BAR_CODE_INSTALL!=ebcUseOCR)
                        {
                            if(bCheckCodeError[iOCRMap[iBarCode1_1]])           //wei 20160505 Barcode 比對Lot
                            {
                                if(IsNNMode()==NN_2Row)                         //Steven 20220315 : for 2did nn mode alarm
                                    sCheckErrorPart+=IndexSuckName[2][iNowCheckStep];
                                else if(IsNNMode()==NN_1Row)
                            ...
                            if(bCheckCodeError[iOCRMap[iBarCode1_2]])           //wei 20160505 Barcode 比對Lot
                            {
                                if(IsNNMode()==NN_2Row)                         //Steven 20220315 : for 2did nn mode alarm
                                    sCheckErrorPart+=IndexSuckName[3][iNowCheckStep];
                                else
                        ...
                        ret=ShowErrorMessage("WAR0467", K_RETRY|K_SKIP, MInShuttle1, false, sCheckErrorPart);      //In Shuttle1 Barcode Error  //Alick 20170126 (wei) 拿掉SKIP，避免跑錯LOT //Steven 20250808 : OCR add K_SKIP for WAR0467
                            ...
                            (bCheckLotError[iOCRMap[iBarCode1_1]] ||
                             bCheckLotError[iOCRMap[iBarCode1_2]]))
                    {
                        sCheckErrorPart="";
                        if(BAR_CODE_INSTALL!=ebcUseOCR)
                        {
                            if(bCheckLotError[iOCRMap[iBarCode1_1]])            //wei 20160505 Barcode 比對Lot
                            {
                                if(IsNNMode()==NN_2Row)                         //Steven 20220315 : for 2did nn mode alarm
                                    sCheckErrorPart+=IndexSuckName[2][iNowCheckStep];
                                else if(IsNNMode()==NN_1Row)
                            ...
                            if(bCheckLotError[iOCRMap[iBarCode1_2]])            //wei 20160505 Barcode 比對Lot
                            {
                                if(IsNNMode()==NN_2Row)                         //Steven 20220315 : for 2did nn mode alarm
                                    sCheckErrorPart+=IndexSuckName[3][iNowCheckStep];
                                else
                ...
                else
                {
                    ...
                    if(TestIF_File.bCheckCodeByLot && (bCheckCodeError[iOCRMap[iBarCode1_1]] || bCheckCodeError[iOCRMap[iBarCode1_2]]))
                    {
                        if(bCheckCodeError[iOCRMap[iBarCode1_1]])
                        {
                            ...
                            FLCarryKit.cDeviceInf[0][iNowCheckStep]=asBarCodeErrorSend;         //wei 20160318 Barcode Error依客戶設定
                            mtBarcodeInSh->SetCellNumber(iOCRMap[iBarCode1_1], iNowCheckStep+1, asBarCodeErrorSend.c_str());
                            if(TestIF_File.i2DIDFormat==eAMD)                                   //JerryYang 20200422 2DID format選項改用下拉選單
                            {
                                fLotInfo->mtBarcodeInShLotInfo->SetCellNumber(0, iNowCheckStep+1, asBarCodeErrorSend.c_str());
                            }
                            iBarcodeErrorCount[iOCRMap[iBarCode1_1]]++;
                            bBarcodeNum[iOCRMap[iBarCode1_1]]=true;
                            bCheckCodeError[iOCRMap[iBarCode1_1]]=false;                        //Steven 20250808 : for OCR
                        }
                        ...
                        if(bCheckCodeError[iOCRMap[iBarCode1_2]])
                        {
                            ...
                            WriteDataToFile(Path.c_str() , DataStr.c_str());
                            FLCarryKit.cDeviceInf[0][iNowCheckStep]=asBarCodeErrorSend;         //wei 20160318 Barcode Error依客戶設定
                            mtBarcodeInSh->SetCellNumber(iOCRMap[iBarCode1_2], iNowCheckStep+1, asBarCodeErrorSend.c_str());
                            iBarcodeErrorCount[iOCRMap[iBarCode1_2]]++;
                            bBarcodeNum[iOCRMap[iBarCode1_2]]=true;
                            bCheckCodeError[iOCRMap[iBarCode1_1]]=false;                        //Steven 20250808 : for OCR
                        }
                           ...
                           bBarcodeNum[iOCRMap[iBarCode1_2]]==false)
                        {
                            ...
                            (bCheckLotError[iOCRMap[iBarCode1_1]] || bCheckLotError[iOCRMap[iBarCode1_2]])) ||
                            (bBarcodeAutoSkipError[iOCRMap[iBarCode1_1]] || bBarcodeAutoSkipError[iOCRMap[iBarCode1_2]]))    //wei Barcode Lot 錯誤取出IC，需重新讀取確認是否取出IC
                    {
                        if(bCheckLotError[iOCRMap[iBarCode1_1]] ||
                           bBarcodeAutoSkipError[iOCRMap[iBarCode1_1]])
                        {
                            DataStr.sprintf("%s InShuttle1-1 %d Site%d : %s", Data.c_str(), iSH1_1BarcodePosition, TestIF_File.iSiteMap[0][iSH1_1BarcodePosition], asBarCodeErrorSend.c_str()); //wei 20160325 Barcode 紀錄位置
                            WriteDataToFile(Path.c_str() , DataStr.c_str());
                            FLCarryKit.cDeviceInf[0][iNowCheckStep]=asBarCodeErrorSend;            //wei 20160318 Barcode Error依客戶設定
                            ...
                            iBarcodeErrorCount[iOCRMap[iBarCode1_1]]++;
                            bBarcodeAutoSkip[iOCRMap[iBarCode1_1]]=true;
                        }
                        bBarcodeNum[iOCRMap[iBarCode1_1]]=false;
                        bBarcodeAutoSkipError[iOCRMap[iBarCode1_1]]=false;

                        if(bCheckLotError[iOCRMap[iBarCode1_2]] ||
                           bBarcodeAutoSkipError[iOCRMap[iBarCode1_2]])
                        {
                            DataStr.sprintf("%s InShuttle1-1 %d Site%d : %s", Data.c_str(), iSH1_1BarcodePosition, TestIF_File.iSiteMap[0][iSH1_1BarcodePosition], asBarCodeErrorSend.c_str()); //wei 20160325 Barcode 紀錄位置
                            WriteDataToFile(Path.c_str() , DataStr.c_str());
                            FLCarryKit.cDeviceInf[0][iNowCheckStep]=asBarCodeErrorSend;            //wei 20160318 Barcode Error依客戶設定
                            mtBarcodeInSh->SetCellNumber(iOCRMap[iBarCode1_2], iNowCheckStep+1, asBarCodeErrorSend.c_str());
                            iBarcodeErrorCount[iOCRMap[iBarCode1_2]]++;
                            bBarcodeAutoSkip[iOCRMap[iBarCode1_2]]=true;
                        }
                        bBarcodeNum[iOCRMap[iBarCode1_2]]=false;
                        bBarcodeAutoSkipError[iOCRMap[iBarCode1_2]]=false;
        ...
        case 1200:
            for(int i=0; i<InArmSuck.iShtRow; i++)                              //20161123 Frank 同一個Shuttle上有兩個一樣的ID的時候
            {
                for(int j=0; j<InArmSuck.iShtCol; j++)
                {
                    asString=mtBarcodeInSh->GetCellText(i, j+1);
            ...
            if(fBarCode->RunCheckBarcodeByServerData()==true &&                 //Jimmychiu 20230925 : read 2did in json file
               fMesSystem->IsMatchServerData(FLCarryKit.cDeviceInf[0][iNowCheckStep])==false)
            {
                bCheckCodeError[iOCRMap[iBarCode1_1]]=true;
            }

            if(fBarCode->RunCheckBarcodeByServerData()==true &&                 //Jimmychiu 20230925 : read 2did in json file
               fMesSystem->IsMatchServerData(FLCarryKit.cDeviceInf[1][iNowCheckStep])==false)
            {
                bCheckCodeError[iOCRMap[iBarCode1_2]]=true;
            }
            #endif
           if(bCheckCodeError[iOCRMap[iBarCode1_1]] || bCheckCodeError[iOCRMap[iBarCode1_2]])
            {
                Task=1180;
            }
            else
...
//---------------------------------------------------------------------------
bool TfBarCode::DoBarcodeScanOutShuttle_1()
{
    static int iNowCheckStep=0;
    static bool bBarcodeNum[BAR_CODE_COUNT]={false};
...
//            break;
        case 1000:
            if(iNowCheckStep>=0 && iNowCheckStep<InArmSuck.iShtCol)
            {
                if(InArmSuck.iShtCol==1)
                {
                    pos=Prod.iOutSHBarCodeDetectPos1x1[0];
                }
                else if(InArmSuck.iShtCol==2)
                {
                    pos=Prod.iOutSHBarCodeDetectPos1x2[0][iNowCheckStep];
                }
                else if(InArmSuck.iShtCol==3)                                   //ChungHung 20140115 add for 2x3_6
                {
                    pos=Prod.iOutSHBarCodeDetectPos2x3[0][iNowCheckStep];
                }
                else if(InArmSuck.iShtCol==4)
                {
                    pos=Prod.iOutSHBarCodeDetectPos1x4[0][iNowCheckStep];
                }
                else if(InArmSuck.iShtCol==5)                                   //Steven 20221027 : Add for 2x5
                {
                ...
                {
                    pos=Prod.iOutSHBarCodeDetectPos2x6[0][iNowCheckStep];
                }
                else if(InArmSuck.iShtCol==8)
                {
                    pos=Prod.iOutSHBarCodeDetectPos2x8[0][iNowCheckStep];
                }
                else
                {
                    ShowMyMessage("The mode is not support!!", "Please save state record and provide to HonPrec software engineer", "DoBarcodeScanOutShuttle_1");
                }
        ...
        case 1100:
            ...
            if(FRCarryKit.Item[0][iNowCheckStep]==HAS_NULL_IC || FRCarryKit.Item[0][iNowCheckStep]==NULL_IC)
                bBarcodeNum[iBarCode1_2]=true;
            else if(bBarcodeNum[iBarCode1_2]==false && Barcode_1StartScan(0, MOutShuttle1)==true)        // 2012.06.01 Q_Q BarCode Mode
            {
                bBarcodeStartDelay[iBarCode1_2]=true;
                bBarcodeDataSaveReady[iBarCode1_2]=false;
                bBarcodeNum[iBarCode1_2]=true;
            ...
            if(InArmSuck.iShtRow==2)
            {
                if(FRCarryKit.Item[1][iNowCheckStep]==HAS_NULL_IC || FRCarryKit.Item[1][iNowCheckStep]==NULL_IC)
                    bBarcodeNum[iBarCode1_1]=true;
                else if(bBarcodeNum[iBarCode1_1]==false && Barcode_1StartScan(1, MOutShuttle1)==true)
                {
                    bBarcodeStartDelay[iBarCode1_1]=true;
                    bBarcodeDataSaveReady[iBarCode1_1]=false;
                    bBarcodeNum[iBarCode1_1]=true;
...
//---------------------------------------------------------------------------
void TfBarCode::InitialShuttleFloatCheck1()                                     //Steven 20160920 : IC置偏檢查
{
    iShuttleFloatCheck1Task=1;
    bCCDcheckShuttle[0]=true;                                                   //wei 20161102 IC置偏檢查
}
//---------------------------------------------------------------------------
bool TfBarCode::DoShuttleFloatCheck_1()                                         //Steven 20160920 : IC置偏檢查
{
    ...
    switch(Task)
    {
        ...
        case 2600:
            ...
            AddCCDCommunicationLog(iSFCRowA, Log);
            ret=ShowErrorMessage("JAM0478", K_RETRY, MSht, false, sErrorPart);  //In Shuttle1 Barcode Error
            iRetryCount=0;
            bHasError=false;
            Task=1;
            break;
...
//---------------------------------------------------------------------------
void TfBarCode::InitialSFCAutoTune1(bool bHasIC)
{
    if(bHasIC==false)
        iSFCAutoTune1Task=1;
    else
...
//---------------------------------------------------------------------------
bool TfBarCode::DoSFCAutoTune_1(bool bHasIC)
{
    static int iNowMoveStep=0;
    static int iRetryCount=0;
    static int iRetryCountExpTimo=0;
        ...
        case 1150:
            if(BAR_CODE_INSTALL==ebcUseOCR)//Ifor 20211129 OCR Shuttle1 Row A 使用第2隻CCD Row B使用第1隻CCD
            {
                if(bBarcodeNum[iBarCodeRowB]==false)
                {
                    ...
                    if(Barcode_StartScan_In(iBarCodeRowB, iNowCheckStep, FLCarryKit.cDeviceInf[0][iNowCheckStep], cLastString)==true)
                    {
                        bBarcodeNum[iBarCodeRowB]=true;
                        mtBarcodeInSh->SetCellNumber(iBarCodeRowB, iNowCheckStep+1, FLCarryKit.cDeviceInf[0][iNowCheckStep]);
                    }
                }
                ...
                if(InArmSuck.iShtRow==2)
                {
                    if(bBarcodeNum[iBarCodeRowA]==false)
                    {
                        ...
                        if(Barcode_StartScan_In(iBarCodeRowA, iNowCheckStep, FLCarryKit.cDeviceInf[1][iNowCheckStep], cLastString)==true)
                        {
                            bBarcodeNum[iBarCodeRowA]=true;
                            mtBarcodeInSh->SetCellNumber(iBarCodeRowA, iNowCheckStep+1, FLCarryKit.cDeviceInf[1][iNowCheckStep]);
                        }
                    }
                }
                else
            ...
            else    //Shuttle 1, Row A使用第1隻CCD, Row B使用第2隻CCD
            {
                if(bBarcodeNum[iBarCodeRowA]==false)
                {
                    ...
                    if(Barcode_StartScan_In(iBarCodeRowA, iNowCheckStep, FLCarryKit.cDeviceInf[0][iNowCheckStep], cLastString)==true)
                    {
                        bBarcodeNum[iBarCodeRowA]=true;
                        mtBarcodeInSh->SetCellNumber(iBarCodeRowA, iNowCheckStep+1, FLCarryKit.cDeviceInf[0][iNowCheckStep]);
                    }
                }
                ...
                if(InArmSuck.iShtRow==2)
                {
                    if(bBarcodeNum[iBarCodeRowB]==false)
                    {
                        ...
                        if(Barcode_StartScan_In(iBarCodeRowB, iNowCheckStep, FLCarryKit.cDeviceInf[1][iNowCheckStep], cLastString)==true)
                        {
                            bBarcodeNum[iBarCodeRowB]=true;
                            mtBarcodeInSh->SetCellNumber(iBarCodeRowA, iNowCheckStep+1, FLCarryKit.cDeviceInf[1][iNowCheckStep]);
                        }
