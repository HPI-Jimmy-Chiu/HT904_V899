...
double KpaTransferKG(int fInputMPA, bool bDualForce)                            //Ifor 20150826 :新增 Kpa 轉 公斤 Function  //JerryYang 20171023 (wei) int -> double
{
    ...
    if(bDualForce==true && INSTALL_DOUBLE_EP==2)                                //Ifor 20191003 : add Die Force 可以自定義Kit直徑
    {
        if(fDiameter==6.0)
        {
...
//        }
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    if(INSTALL_DOUBLE_EP==2  && TestIF_File.bIndEPSLK==true)
                    {
        ...
        for(int i=0; i<fContactForce->slSLKTypeInd->Count; i++)                 //JerryYang 20210413 : 讀取獨立EP offset
        {
            for(int j=0; j<16; j++)
            {
                if(fContactForce->SLKIndClass[i*16+j]->dDiameter==fDiameter*10)
                {
...
void ADAM_DirectWriteData(WORD data, int Num, int iAdd)                         //Ifor 20150709 ：加入設備位置 //Ifor 20190104 : add 選擇 ADAM 輸出 CH
{
    ...
    else
    {
        ...
        if(INSTALL_DOUBLE_EP==2 && TestIF_File.bIndEPSLK)
        {
            if(Num==0)
            {
                APAX_WriteData(true, data);
            }
        }
        else
        {
