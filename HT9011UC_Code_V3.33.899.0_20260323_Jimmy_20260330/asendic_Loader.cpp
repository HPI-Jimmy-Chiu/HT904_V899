...
bool DoSupplyNewICTray()                                                        //Supply IC from Loader car
{
    ...
    //AI(ht9045-v899) 20260409: DIAG LOG - DoSupplyNewICTray startup sensor states (temp)
    { static bool _once=false; if(!_once) { AnsiString _d3; _d3.sprintf("[DIAG-SupplyNew] TrackDetect_En=%d ColorSen=%d bLastNoIn=%d bForKyecBu3=%d",
        Sen[SnLoaderTrackDetect].Enable, fTrayForm->IsEnableColorSensor(), IniConfig.bLastLoaderNoInSide, bForKyecBu3RunART);
        RecordProcess(_d3); _once=true; } }
    switch(Task)
    {
        case 1:
        ...
        case 200:
            if(CylinderMiddle(C_Load_Up))
            {
                        ...
                        IniConfig.bLastLoaderNoInSide)                          //jou 2010-01-24 start : 如果Loader是最後一盤，不再load進去。
                {
                    ...
                    {
                        //AI(ht9045-v899) 20260409: DIAG LOG - last tray detected (temp)
                        { AnsiString _d; _d.sprintf("[DIAG-LTZLTW-200] bLoadHasTray=1 SnHasTray=%d SnART_En=%d SnART=%d CatchTask=%d bAutoFeed=%d",
                            Sen[SnLoaderTrayHasTray].IsOff(), Sen[SnLoaderTrayHasTray_ART].Enable, Sen[SnLoaderTrayHasTray_ART].IsOff(),
                            iCatchFromLoaderTask, TrayForm.bAutoFeed);
                          RecordProcess(_d); }
                        ...
                        //AI(ht9045-v899) 20260409: interlock guard - wait for TrayArm DoCatchFromLoader to finish before clear-empty sequence
                        if(iCatchFromLoaderTask != 1) { AnsiString _d2; _d2.sprintf("[DIAG-LTZLTW-200] INTERLOCK: CatchTask=%d, wait...", iCatchFromLoaderTask); RecordProcess(_d2); break; }
                        Task=1000;
                        break;
                    }
                }
...
void DoLoad()
{
    ...
    switch(Task)
    {
        case 1:
                ...
                if(USE_LdUldCassetteMode==1)                                    //_todo 20260203 RogerYang 這裡沒有判斷裡面有沒有tray就直接入料
                {
                    bLoadHasDuplicateError=false;                               //Steven 20120220 : Load Tray重複Alarm Flag
                    Task=300;
                }
                    ...
                    if(CosFunction.bUseSCKART)                                  //Steven 20161201 (wei) : For SCK 93K ART
                    {
                           ...
                           fSCKART->CheckLoadingCount()==0)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    ret=ShowErrorMessage("WAR0119", K_RETRY, MInArmX, "DoLoad");
                                }
                                else
                                {
                                    ret=ShowErrorMessage("WAR0119", K_RETRY|K_CLEAN_OUT|K_SKIP, MInArmX, "DoLoad");
                                }
