...
bool ArmFinishLoader_2x4_16() //check Arm finish and  can go shuttle
{
    ...
    for(int i=0; i<InArmSuck.iPickRow; i++)
    {
        for(int j=0; j<InArmSuck.iPickCol; j++)
        {
            ...
            else if(Prod.fInArmSuckUse_TwoArm32Site[iSht+i][iKit+j] && InArmSuck.Item[i][j]==NULL_IC)
            {
                return false;
            }
        }
...
bool DoArmPickFromLoadStage_9045_2x4_16()
{
    ...
    switch(Task)
    {
        case 1:
            ...
            if(MoveInArmZToPlateSafe(Task))
            {
                ...
                for(int i=0; i<InArmSuck.iPickRow; i++)                         //ChungHung 20111230 add 增加保護並解決bP17InArmFullPickFromLoader 所產生的bug InArmSuck上已全部有IC
                {
                    for(int j=0; j<InArmSuck.iPickCol; j++)
                    {
                        if(Prod.fInArmSuckUse_TwoArm32Site[i][j] && InArmSuck.Item[i][j]==NULL_IC)
                        {
                            bflag=true;
                        }
                    }
        ...
        case 4000:
            ...
            break;
        case 5000:
            if(ProcessTrayMapDataError())         //wei 20170302 (Steven) Tray map data error
            {
...
//            break;
        ...
        case 100:
            ...
            Task=110;
        case 110:
            if(MoveInArmXYPickHotPlate_9045())
            {
...
//            }
        ...
        case 1200:
            ...
            //AI(ht9045-v899) 20260407: align with 2x4_8 baseline — add CheckInArmDestroyICFail for SHT1
...
void DoInArm_9045_2x4_16_SuckerMap()
{
    ...
    else
    {
        for(int i=0; i<InArmSuck.iShtRow; i++)
        {
            for(int j=0; j<InArmSuck.iShtCol; j++)
            {
                ...
                Prod.fInArmSuckUse_TwoArm32Site[i+2][j1]=LastSet.bUseTestSocket[0][i][j];
                Prod.fInArmSuckUse_TwoArm32Site[i+0][j1]=LastSet.bUseTestSocket[1][i][j];
                Prod.fInArmSuckUse_TwoArm32Site[i+2][j2]=LastSet.bUseTestSocket[0][i][j];
                Prod.fInArmSuckUse_TwoArm32Site[i+0][j2]=LastSet.bUseTestSocket[1][i][j];
            }
        }
    }
    ...
    for(int i=0; i<InArmSuck.iMaxRow; i++)
    {
        for(int j=0; j<InArmSuck.iMaxCol; j++)
        {
            Prod.fInArmSuckUse[0][i][j]=Prod.fInArmSuckUse_TwoArm32Site[i+2][j];
            Prod.fInArmSuckUse[1][i][j]=Prod.fInArmSuckUse_TwoArm32Site[i+0][j];
        }
    }
}
//==============================================================================
    ...
    switch(Task)
    {
        ...
        case 50:
               ...
               IsInArmCleanOutFinish()==false)                                  //Steven 20230309 : fixed for NN mode hang up
            {
                ...
                if((MOT[MMPlate1].HasIC() || MOT[MMPlate2].HasIC()) && HasHotReadyIC_9045(false))
                {
        ...
        case 300:
            ...
            else
            {
                for(int i=0; i<InArmSuck.iPickRow; i++)
                {
                    for(int j=0; j<InArmSuck.iPickCol; j++)
                    {
                        if(Prod.fInArmSuckUse_TwoArm32Site[iSht+i][iKit+j]==false && InArmSuck.Suck[i][j].GetStatus())
                        {
                            bHasDuplicateErr=true;
...
//            bPickFromHotplate=true;
        ...
        case 1600:
            ...
            for(int i=0; i<InArmSuck.iPickRow; i++)
            {
                for(int j=0; j<InArmSuck.iPickCol; j++)
                {
                    if(Prod.fInArmSuckUse_TwoArm32Site[iShtHP+i][iKitHP+j]==false &&
