...
bool CheckZ2IsDown()
{
        ...
        {
            if(MOT[MTestZ2].Gali_ReadEncoderPos()>Prod.iTestDown_Z2)
            {
...
bool DoRearTestDestroyIC(bool bCheckZ1)
{
    ...
    switch(Task)
    {
        ...
        case 750:
            ...
            {
                flag=false;
                if(ArmSpeed_File[IndexArm].bDevicConfirm && TotalErrPart!="")   //kevin 20190629 add error show
                {
                   flag=true;
    ...
    switch(Task)
    {
        ...
        case 301:                                                               //確認Z軸位置 位置在下->吸取IC->Z軸到位後->Task=310
            ...
            if(BLCarryKit.UseSiteHasIC() && bCheckShuttle2Flag==false)
            {
                ...
                if(CheckZ1IsDown())
                {
                    ...
                    else
                    {
                        ...
                        if(CosFunction.bSortingBy2DList==true &&                //jou Index更換Offset會hang up? 2010-04-13
                           LastSet.iTester==_2D_SORT &&
                           TestIF_File.bSortingBy2DIDList==true)                //Frank 20221122 : 2DID sorting for ATK
                        {
                    ...
                    BLCarryKit.UseSiteNoIC())
            {
                ...
                {
                    ZPos=MOT[MTestZ1].Gali_ReadPos();
                    if(IsNNMode()!=NN_2Row)                                     //jou Index更換Offset會hang up? 2010-04-13
                    {
                        if(ZPos>=(Prod.TestZ1_Test-Prod.TestZ1_Drop_Offset+500))//2008/06/24
                        {
                            if(ZPos!=iBackUpZ1DownPosition)
...
bool DoTestYRear()
{
    ...
    switch(Task)
    {
        ...
        case 61:
            ...
            {
                if(REAL_TIME_CCD==true &&                                       //----- by dell ccd realtime-------------
                   COM2->bRealTimeCom_ReceiveOK[COM2->rtAlarHasIC])
                {
