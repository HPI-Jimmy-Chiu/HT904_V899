...
bool bUseCKPP = false;  //AI(ht9045-v899) 20260416: disable CKPP - pick order wrong (top-half instead of right-half) and pad LifeTime stuck at 1 per Gigas feedback
...
//------------------------------------------------------------------------------
//AI(ht9045-v899) 20260410: Y position calculation mirroring MoveInArmXYPickCleanKit logic
int cCleanKitPickPlan::CalcY(int iKitRow)
{
    int iMovePitchY = GetYPitchOfCleanKit();
    int iBaseY = Prod.YInArm_AutoClean_Pick[iInArmYBase][iInArmXBase];
    int iYPos = 0;

    if(USE_IN_Y_IS_AUTO_PITCH == true)
    {
        if(m_iSuckRow == 0)
            iYPos = iBaseY - iKitRow * (int)TestIF_File.dAutoClean_YPitch - iMovePitchY;
        else
            iYPos = iBaseY - iKitRow * (int)TestIF_File.dAutoClean_YPitch;
    }
    else
    {
        if(bUse8Picker == false)
            iYPos = iBaseY - iKitRow * (int)TestIF_File.dAutoClean_YPitch + iMovePitchY;
        else if(m_iSuckRow == 1)
            iYPos = iBaseY - iKitRow * (int)TestIF_File.dAutoClean_YPitch + iMovePitchY;
        else
            iYPos = iBaseY - iKitRow * (int)TestIF_File.dAutoClean_YPitch;
    }
    iYPos += HotplatlYOffset;
    return iYPos;
}
//------------------------------------------------------------------------------
//AI(ht9045-v899) 20260410: cross-verify motor actual position vs plan-calculated position
bool cCleanKitPickPlan::VerifyPosition(int iSlotIdx, int iTolerance, AnsiString &sDetail)
{
    if(!m_bFound) return true;
    if(iSlotIdx < 0 || iSlotIdx >= m_Best.iSlotCount) return true;

    const TPickSlot &slot = m_Best.Slots[iSlotIdx];
    if(slot.iPhysical < 0 || !slot.bActive) return true;

    int iExpectX = slot.iXPos;
    int iExpectY = CalcY(m_Best.iKitRow);
    int iActualX = MOT[MInArmX].ReadPos();
    int iActualY = MOT[MInArmY].ReadPos();
    int iDeltaX  = iActualX - iExpectX;
    int iDeltaY  = iActualY - iExpectY;
    if(iDeltaX < 0) iDeltaX = -iDeltaX;
    if(iDeltaY < 0) iDeltaY = -iDeltaY;

    if(iDeltaX <= iTolerance && iDeltaY <= iTolerance)
    {
        sDetail.sprintf("AC_VERIFY slot=%d suck[%d][%d] expect(%d,%d) actual(%d,%d) D(%d,%d) PASS",
            iSlotIdx, m_iRealRow, slot.iPhysical, iExpectX, iExpectY, iActualX, iActualY, iDeltaX, iDeltaY);
        RecordProcess(sDetail);
        return true;
    }

    // check if offset is the main contributor
    int iDxNoOfs = iDeltaX;
    int iDyNoOfs = iDeltaY;
    if(m_iHPXOfs != 0)
    {
        int iXNoOfs = iExpectX - m_iHPXOfs;
        int d = iActualX - iXNoOfs;
        if(d < 0) d = -d;
        iDxNoOfs = d;
    }
    int iHPYOfs = HotplatlYOffset;
    if(iHPYOfs != 0)
    {
        int iYNoOfs = iExpectY - iHPYOfs;
        int d = iActualY - iYNoOfs;
        if(d < 0) d = -d;
        iDyNoOfs = d;
    }

    if(iDxNoOfs <= 50 && iDyNoOfs <= 50)
    {
        sDetail.sprintf("AC_VERIFY slot=%d suck[%d][%d] D(%d,%d) OFFSET_DRIFT HPXOfs=%d HPYOfs=%d recommend re-teach",
            iSlotIdx, m_iRealRow, slot.iPhysical, iDeltaX, iDeltaY, m_iHPXOfs, iHPYOfs);
    }
    else
    {
        sDetail.sprintf("AC_VERIFY slot=%d suck[%d][%d] expect(%d,%d) actual(%d,%d) D(%d,%d) POSITION_ERROR",
            iSlotIdx, m_iRealRow, slot.iPhysical, iExpectX, iExpectY, iActualX, iActualY, iDeltaX, iDeltaY);
    }
    RecordProcess(sDetail);
    return false;
...
//------------------------------------------------------------------------------
//AI(ht9045-v899) 20260410: verify all active slots in one call, returns false if any fails
bool cCleanKitPickPlan::VerifyAllSlots(int iTolerance)
{
    if(!m_bFound) return true;
    bool bAllPass = true;
    AnsiString sDetail;
    ...
    {
        if(!m_Best.Slots[i].bActive) continue;
        if(!VerifyPosition(i, iTolerance, sDetail))
            bAllPass = false;
    }
    return bAllPass;
}
...
//------------------------------------------------------------------------------
void cCleanKitPickPlan::EvaluatePlan(int startL, int kCol, int kRow)
{
    TPickPlan plan;
    ZeroMemory(&plan, sizeof(plan));
    plan.iStartLogical = startL;
    plan.iKitCol = kCol;
    plan.iKitRow = kRow;
    plan.iSlotCount = m_iPickerCount;
    plan.iActiveCount = 0;

    for(int i = 0; i < m_iPickerCount; i++)
    {
        TPickSlot &s = plan.Slots[i];
        s.iLogical  = i;
        s.iPhysical = GetAutoCleanPickStep(i);
        s.iKitCol   = kCol + m_iXPitchStep * (i - startL);
        s.iXPos     = (s.iPhysical >= 0) ? CalcX(s.iPhysical, s.iKitCol) : 0;
        s.bReachable = (s.iPhysical >= 0 &&
                        s.iXPos >= m_iSLimN && s.iXPos <= m_iSLimP);
        s.bHasPad    = IsPadAt(s.iKitCol, kRow);
        s.bDemanded  = (s.iPhysical >= 0 && m_bDemand[s.iPhysical]);
        s.bActive    = (s.bReachable && s.bHasPad && s.bDemanded);

        if(s.bActive) plan.iActiveCount++;
    }
...
{
    m_bFound = false;
    m_Best.iActiveCount = 0;

    for(int kRow = 0; kRow < m_iYItem; kRow++)
    {
        for(int startL = 0; startL < m_iPickerCount; startL++)
        {
            int iLeadPhys = GetAutoCleanPickStep(startL);
            if(iLeadPhys == -1) continue;

            for(int kCol = 0; kCol < m_iXItem; kCol++)
            {
                if(!IsPadAt(kCol, kRow)) continue;

                int iLeadX = CalcX(iLeadPhys, kCol);
                if(iLeadX < m_iSLimN || iLeadX > m_iSLimP) continue;

                EvaluatePlan(startL, kCol, kRow);
            }
        }
    }
...
void SearchCleanKitUpDown(int iRow, eWhichShuttle iSht)
{
    ...
    for(int j=0; j<InArmSuck.iMaxCol; j++)
    {
        iSuckCol=GetAutoCleanPickStep(j);
        if(iSuckCol==-1)
        {
            continue;
        }
        ...
        iShtRow=iSuckRow;
        iShtCol=iSuckCol+iKitStep;
        iSiteCol=GetShuttleCol(iShtRow, iShtCol);
        if(IsNNMode()==NN_2Row)
        {
            iSiteRow=(iSht==0)?2:0;
        }
        ...
        if(i1x2_4UseACEGPicker==1)
        {
            ...
            {
                bFlag[iSuckCol]=(Prod.bInSuckUse[iSht][iShtRow][iShtCol] &&
                                 InArmSuck.Item[iRealRow][iSuckCol]==NULL_IC);
        ...
        {
            {
                ...
                {
                    bFlag[iSuckCol]=(Prod.fInArmSuck4x8[iSht][iShtRow][iShtCol] &&
                                     InArmSuck.Item[iRealRow][iSuckCol]==NULL_IC);
    ...
    for(int j=0; j<InArmSuck.iMaxCol; j++)
    {
        ...
        {
            if((bFlag[iSuckCol] ||
                Prod.bInSuckUse[iSht][iShtRow][iShtCol]) &&
               InArmSuck.Item[iRealRow][iSuckCol]==NULL_IC)
            {
                iAutoCleanStart=j;
                break;
    ...
    RecordProcess(AnsiString().sprintf("AC_SEARCH [OLD] iAutoCleanStart=%d iRealRow=%d bRowHasIC=%d iKitStep=%d iSuckRow=%d XItem=%d",
        iAutoCleanStart, iRealRow, (int)bRowHasIC, iKitStep, iSuckRow, MOT[MMAutoCleanKit].Tray.XItem));
    int iKRow=0,iKCol=0;
    if(SearchCleanKitRowCol(iKRow,iKCol))
    {
        iAutoCleanPickPlateX=iKCol;
        iAutoCleanPickPlateY=iKRow;
        RecordProcess(AnsiString().sprintf("AC_KITRC [OLD] iAutoCleanPickPlateX=%d Y=%d iAutoCleanStart=%d XPitch=%d",
            iAutoCleanPickPlateX, iAutoCleanPickPlateY, iAutoCleanStart, iAutoCleanUseXPitch));
...
//    else
    ...
    if(iPick==bAutoPick)
    {
        //AI(ht9045-v899) 20260410: plan-driven coordinate - use plan's lead slot directly
        if(bUseCKPP && g_CKPlan.Found())
        {
            const TPickPlan &plan = g_CKPlan.GetPlan();
            iSuckRow = g_CKPlan.GetRealRow();
            iKitRow  = plan.iKitRow;
            iKitCol  = plan.iKitCol;
            iSuckCol = GetAutoCleanPickStep(plan.iStartLogical);
            if(iSuckCol == -1)
                return true;
        }
        else
...
//        PlaceToCleanList->GetHPFirstTeamMotUse(bCleanZMotToPlace);
    ...
    //AI(ht9045-v899) 20260407: AC debug log - X position breakdown
    RecordProcess(AnsiString().sprintf("AC_XPOS iXPos=%d Base=%d dMovePitchX=%.1f iInArmXBase=%d iSuckCol=%d iKitCol=%d dXPitch=%.1f HPXOfs=%d SLimP=%d SLimN=%d iPick=%d",
        iXPos, Prod.XInArm_AutoClean_Pick[iInArmYBase][iInArmXBase], dMovePitchX, iInArmXBase, iSuckCol, iKitCol,
        TestIF_File.dAutoClean_XPitch, HotplatlXOffset, MOT[MInArmX].Motor->PSoftLimitP, MOT[MInArmX].Motor->PSoftLimitN, iPick));
...
//    int iPickCount=GetAutoCleanPickCount();
    ...
    //AI(ht9045-v899) 20260410: anti-hang guard - cap search-retry loops (case 3100->10 cycle)
    static int iSearchRetry=0;
...
//    static int iShuttleRowKit=0;
    ...
    if(Restart)
    {
        iAutoCleanNum=1;
        Task=1;
        iSearchRetry=0;                                                         //AI(ht9045-v899) 20260410: reset anti-hang counter on restart
        #ifdef DEBUG_AUTO_CLEAN
            iOldTask=1;
    ...
    switch(Task)
    {
        ...
        case 31:
            ...
            break;
        case 200:
            //AI(ht9045-v899) 20260410: enhanced position cross-verification (200-condition tolerance)
            if(bUseCKPP && g_CKPlan.Found())
            {
                if(!g_CKPlan.VerifyAllSlots(200))
                {
                    RecordProcess("AC_PICK200 VerifyAllSlots FAIL -> re-move XY");
                    Task=21;
                    break;
                }
            }
        ...
        case 3000:
            if(MoveInArmZToPlateSafe(Task))
            {
                ...
                //AI(ht9045-v899) 20260410: post-pick position consistency check
                if(bUseCKPP && g_CKPlan.Found())
                {
                    g_CKPlan.VerifyAllSlots(200);
                }
        ...
        case 3100:
            ...
            //AI(ht9045-v899) 20260410: anti-hang guard - if search retried too many times, force finish pick
            if(Task == 10)
            {
                iSearchRetry++;
                if(iSearchRetry > 3)
                {
                    RecordProcess(AnsiString().sprintf("AC_HANGUP_GUARD iSearchRetry=%d forced finish iSht=%d", iSearchRetry, (int)iSht));
                    PlaceToCleanList->AddHPSuckGroup();
                    iSearchRetry = 0;
                    Task = 3300;
                }
        ...
        case 3300:
            ...
            {
                Task=1;
                iSearchRetry=0;                                                 //AI(ht9045-v899) 20260410: reset anti-hang on normal finish
            #ifdef DEBUG_AUTO_CLEAN
...
bool DoAutoCleanPlaceToCleanKit(bool Reset)
{
    ...
    switch(Task)
    {
        ...
        case 100:
            ...
            {
                //AI(ht9045-v899) 20260410: cross-verify place position before destroy
                if(bUseCKPP && g_CKPlan.Found())
                {
                    g_CKPlan.VerifyAllSlots(200);
                }
...
//            }
        ...
        case 300:
            ...
            if(flag==1)
            {
                ...
                if(InArmSuck.HasIC()==false)
                {
                    //AI(ht9045-v899) 20260407: break outer loop when clean kit is empty
                    if(TrayHasCleanIC()==false && FLCarryKit.HasDefineIC(HAS_CLEAN_IC))
                    {
                        RecordProcess("AC_NOPAD_OUTER Kit empty, proceed with partial shuttle");
                        Task=530;
                    }
                    else
                    {
        ...
        case 400:
            ...
            if(DoPlaceToShuttle(euShuttle1))
            {
                ...
                //AI(ht9045-v899) 20260408: skip all remaining groups with no sucker demand
                if(bUseCKPP && iShuttleRowKit != 0)
                {
                    g_CKPlan.Init((int)euShuttle1, iShuttleRowKit);
                    if(!g_CKPlan.HasDemand())
                    {
                        RecordProcess(AnsiString().sprintf("AC_SKIP_NODEMAND iShuttleRowKit=%d -> force 0", iShuttleRowKit));
                        iShuttleRowKit = 0;
                    }
                }
                ...
                else
                {
                    if(iShuttleRowKit!=0)
                    {
                        //AI(ht9045-v899) 20260407: recycle CLEAN_FINISH_IC pads before re-entry to prevent hang
                        bool bKitHasPad = true;
                        if(TestIF_File.iAutoClean_Tray==eCKPos_CleanKit)
                        {
                            bKitHasPad = false;
                            for(int Y=0; Y<TestIF_File.iAutoClean_YDivision; Y++)
                                for(int X=0; X<TestIF_File.iAutoClean_XDivision; X++)
                                {
                                    if(MOT[MMAutoCleanKit].Tray.Data[X][Y]==CLEAN_FINISH_IC)
                                        MOT[MMAutoCleanKit].SetTraySingleData(X, Y, HAS_CLEAN_IC);
                                    if(MOT[MMAutoCleanKit].Tray.Data[X][Y]==HAS_CLEAN_IC)
                                        bKitHasPad = true;
                                }
                        }
                        RecordProcess(AnsiString().sprintf("AC_REENTRY Task400->200 iShuttleRowKit=%d InArmHasIC=%d FLHasClean=%d bKitHasPad=%d",
                            iShuttleRowKit, (int)InArmSuck.HasIC(), (int)FLCarryKit.HasDefineIC(HAS_CLEAN_IC), (int)bKitHasPad));
                        if(bKitHasPad)
                            Task=200;
                        else
                            Task=530;
        ...
        case 2400:
            ...
            if(DoPlaceToShuttle(euShuttle2))
            {
                ...
                else
                {
                    ...
                    if(iShuttleRowKit!=0)
                    {
                        //AI(ht9045-v899) 20260407: recycle CLEAN_FINISH_IC pads before re-entry (ARM2)
                        bool bKitHasPad = true;
                        if(TestIF_File.iAutoClean_Tray==eCKPos_CleanKit)
                        {
                            bKitHasPad = false;
                        ...
                        RecordProcess(AnsiString().sprintf("AC_REENTRY Task2400->2200 iShuttleRowKit=%d bKitHasPad=%d",
                            iShuttleRowKit, (int)bKitHasPad));
                        if(bKitHasPad)
                            Task=2200;
                        else
                            Task=2530;
