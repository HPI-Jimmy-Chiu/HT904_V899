...
#define ATC_READ_NetWork_VALVE      1072
...
#define ATC_AIRMACHINE_STATUS2      1091                                        //Ztex 2023.04.19 Add HT-1032 TriTemp Function
...
#define ATC_DewPoint                1099                                        //Ztex 2023.04.19 Add HT-1032 TriTemp Function
...
#define ATC_MultiSensorOfs          1129                                        //Ifor 20251203 add: ATC MultiSensor
#define ATC_MultiSensorEnabled      1131                                        //Ifor 20251203 add: ATC MultiSensor
...
void __fastcall TATC_InterfaceForm::SetCommandString()
{
    ...
    asCommString[ATC_READ_NetWork_VALVE     -1000]="ATC_READ_NetWork_VALVE(1072)";      //kevin 20200409
    ...
    asCommString[ATC_AIRMACHINE_STATUS2     -1000]="ATC_AIRMACHINE_STATUS2(1091)";      //Ztex 2023.04.19 Add HT-1032 TriTemp Function
    asCommString[ATC_DewPoint               -1000]="ATC_DewPoint(1099)";                //Ztex 2023.04.19 Add HT-1032 TriTemp Function
    ...
    asCommString[ATC_DewPoint               -1000]="ATC_DewPoint(1099)";                //Ztex 2023.04.19 Add HT-1032 TriTemp Function
    asCommString[ATC_51_Set_TJ_WATCHDOG     -1000]="ATC_5.1_SET_TJ_WATCHDOG(1100)";
    ...
    asCommString[ATC_Set_TC_WATER_VALVE     -1000]="ATC Set TC Water value(1125)";
    ...
    asCommString[ATC_MultiSensorOfs         -1000]="ATC_MultiSensorOfs(1129)";      //Ifor 20251203 add: ATC MultiSensor
    asCommString[ATC_MultiSensorEnabled     -1000]="ATC_MultiSensorEnabled(1131)";  //Ifor 20251203 add: ATC MultiSensor
}
//---------------------------------------------------------------------------
...
void __fastcall TATC_InterfaceForm::SendCommand(int iCommIndex)
{
    ...
    switch (iCommIndex)
    {
        ...
        case ATC_READ_NetWork_VALVE:                                            //1072
        ...
        case ATC_MultiSensorEnabled:                                            //Ifor 20251203 add: ATC MultiSensor
...
//            sATC_COMMAND.sprintf("@%d,4,%d,%d,%s,%s#",iCommIndex,iSendData[0],iSendData[1],asSendData_PF[0],asSendData_PF[1]);
            break;
        case ATC_AIRMACHINE_STATUS2:                                            //Ztex 2023.04.19 Add HT-1032 TriTemp Function
        ...
        case ATC_Set_TC_WATER_VALVE:
            ...
            break;
        case ATC_MultiSensorOfs:                                                //Ifor 20251203 add: ATC MultiSensor
            for(int i=0; i<(iDataCount); i++)
            {
                if(i==iDataCount-1)
                {
...
void __fastcall TATC_InterfaceForm::ProcessReceiveString_ATC()
{
    ...
    int iCheckATCVer=0;
//    double fValue=0;
    AnsiString asResult, asValue, asTemp, str, sBuffer;
...
/*        if(Tri_Temp_Machine==1)                                                 //Ztex 2023.04.19 Add HT-1032 TriTemp Function
        ...
        if(bCommFlag[ATC_STATE-1000]==true)
        {
            iSendData[0]=1;
            SendCommand(ATC_STATE);                                             //ChungHung 20150826 add
            bCommFlag[ATC_STATE-1000]=false;
            iCommCount[ATC_STATE-1000]=0;
        }
        iCount++;

        if(iCount%9==0)                                                         //Steven 20251101 : ATC分段送命令
        {
            SendCommand(ATC_READ_SETTEMP);                                      //ChungHung 20150826 add
            SendCommand(ATC_READ_OFFSET);                                       //Eliot 2015_1124
        }
        else if(iCount%11==0)
        {
        ...
        {
            iCount=0;
            SendCommand(ATC_TIME_SYNC);                                         //Eliot 2015_1017
        }
...
void __fastcall TATC_InterfaceForm::AskATCDateToHandle()
{
    ...
    else
    {
        ...
        SendCommand(ATC_READ_NetWork_VALVE);                                    //kevin 20221011 add 讀取 ATC 資料給 eKeep 判斷        
    }
}
//------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void __fastcall TATC_InterfaceForm::Send_ATC_AIRMACHINE_STATUS2(int iEnableAirMachine,double dTemp)//Add ATC_AIRMACHINE_STATUS2 Command //Ztex 2023.04.19 Add HT-1032 TriTemp Function
{
    bool bArm1=false,bArm2=false;
    bool bUse = false;

    if(iATC_MODE_TYPE!=61)
         return;
    if(Temperature.bEnableArm_1_Air==true)
        bArm1=true;
    else
        bArm1=false;

    if(Temperature.bEnableArm_2_Air==true)
        bArm2=true;
    else
        bArm2=false;
    bUse = Temperature.bEnableArm_1_Air || Temperature.bEnableArm_2_Air || Temperature.bEnableSocket_Air;
    if(bCloseAirMachine==true || bDefrostKeepATCTemp==true)
        bUse=false;
//    iSendData[0] =iEnableAirMachine;      Temperature.bEnableAirMachineIndex
//    iSendData[0]=Temperature.bEnableAirMachineIndex;
    iSendData[0]=bUse;
    iSendData[1]=bArm1;
    iSendData[2]=bArm2;
    iSendData[3]=dTemp;
//    SendCommand(ATC_AIRMACHINE_STATUS2);
}
//-----------------------------------------------------------------------------
...
//        if(iDataCount>32)      //kevin 20160507
    ...
    SendCommand(ATC_MultiSensorOfs);
}
//---------------------------------------------------------------------------
void __fastcall TATC_InterfaceForm::EnablesMultiSensor(bool *Enabled)
{
    ...
    SendCommand(ATC_MultiSensorEnabled);
}
//---------------------------------------------------------------------------