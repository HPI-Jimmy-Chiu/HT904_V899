...
//---------------------------------------------------------------------------
bool TfProductionInfo::OEE_StartLot()
{
    bN14_21_UsedGDK04=false;
    sLocalMOFullFileName="";                                                    //Sam 20190227 : Try Catch Mo Download SetupFile
    ...
    else if(IniConfig.bN14_10_DownFileByMO)    //Sam 20190227 : Try Catch Mo Download SetupFile
    {
        ...
        if(FileExists(sLocalMOSetupFileFullFileName) == true)
        {
            ...
            {
                ShowMyMessage("Auto Download Setup File by MO LineString Error in LoadMOInformation");
                return false;
            }
        }
        else
        {
            ShowMyMessage("Auto Download Setup File by MO File Error in LoadMOInformation");
            return false;
        }
...
bool TfProductionInfo::CheckMOInformation()
{
    ...
    AnsiString sTempErrorDetail=""; 

    bTempError=false;
    if(sLoadMO_TemperatureMode=="Hot")
    {
        if(LastSet.iTemperature!=Tempture_Hot)
        {
            bTempError=true;
            sTempErrorDetail.sprintf("Mode Mismatch: MO requires [Hot], System is [%s]",
                (LastSet.iTemperature==Tempture_Ambient ? "Ambient" : "Unknown"));
        }
        else if(iNowTemperature!=Temperature.fWorkTemperBase)
        {
            bTempError=true;
            sTempErrorDetail.sprintf("Temperature Mismatch: MO requires [%d C], System is [%.0f C]",
                iNowTemperature, Temperature.fWorkTemperBase);
        }
    }
    else
    {
        ...
        {
            bTempError=true;
            sTempErrorDetail.sprintf("Mode Mismatch: MO Temp [%d C] <= Ambient Limit [%d C], requires [Ambient], System is [Hot]",
                iNowTemperature, IniConfig.iN14_17_AmbientULTemp);
        }
        else if(iNowTemperature>IniConfig.iN14_17_AmbientULTemp)
        {
            if(LastSet.iTemperature!=Tempture_Hot)
            {
                bTempError=true;
                sTempErrorDetail.sprintf("Mode Mismatch: MO Temp [%d C] > Ambient Limit [%d C], requires [Hot], System is [Ambient]",
                    iNowTemperature, IniConfig.iN14_17_AmbientULTemp);
            }
            else if(iNowTemperature!=Temperature.fWorkTemperBase)
            {
    ...
    if(bTempError)
    {
        AnsiString sMsg="Check MO Information Fail!#Temperature Error: "+sTempErrorDetail;
        ShowMyMessage(sMsg);
        return false;
    }
    ...
    {
        AnsiString sMsg;
        sMsg.sprintf("Check MO Information Fail!#Soak Time Mismatch: MO requires [%d sec], System is [%.0f sec]",
            iNowSoakTime, Temperature.fSoakTime);
        ShowMyMessage(sMsg);
        return false;
    }
...
void TfProductionInfo::UploadBinQtyReport(bool bSaveNow)        //Steven 20190621 : OEE Function
{
    ...
    if(bBinQtyCycleTimeUp==true)
    {
        ...
        {
            MyDBIProcess("Exception", "TfProductionInfo::UploadBinQtyReport: " + sFileName);
...
void TfProductionInfo::DoRunExecutFile(AnsiString sExecutFullFileName, AnsiString sParameters) //Mylin 20180911 Add Lead Scan Function Tray Report Defect Code
{
    ...
    {
        MyDBIProcess("Exception", "TfProductionInfo::DoRunExecutFile: " + sExecutFullFileName);
    }
}
//---------------------------------------------------------------------------
...
AnsiString TfProductionInfo::ACM_ReadFlag(AnsiString sName)
{
    ...
    if(FileExists(sFullFileName)==true)
    {
        ...
        {
            MyDBIProcess("Exception", "TfProductionInfo::ACM_ReadFlag: " + sFullFileName);
        }
        if(INIFile!=NULL)
        {
            INIFile->UpdateFile();
...
void __fastcall TfProductionInfo::tm_IPSCControlTimer(TObject *Sender)
{
    ...
    {
        MyDBIProcess("Exception", "TfProductionInfo::tm_IPSCControlTimer");
    }
    bIsRunning=false;
}
//---------------------------------------------------------------------------
    ...
    for(int i=0; i<TestSocket.iShtRow; i++)
    {
        for(int j=0; j<TestSocket.iShtCol; j++)
        {
            if(LastSet.bUseTestSocket[0][i][j])
            {
                if(bfirstTime)
                {
                    sTemp+=IntToStr(j+1+(i*TestSocket.iShtCol));
...
bool TfProductionInfo::CopyFolder(AnsiString asSource, AnsiString asTarget)
{
    ...
    if(FindFirst(asSource+"*.*", faAnyFile, sr)==0)
    {
        do
        {
            ...
            {
                MyDBIProcess("Exception", "TfProductionInfo::CopyFolder: " + asSource + " -> " + asTarget);
                return false;
            }
        }while(FindNext(sr)==0);
        FindClose(sr);
