...
int GetNowSiteKitMode_2x8_8(int iSht, int iKit, bool bPlace)
{
    ...
    if(bPlace==true)
    {
        ...
        {
            if(iCloseSiteModeFor2x8==e2x8_STMMode)                              //KevinC 20251031 : 新增條件 避免特殊關Site時Auto Clean的HAS_NULL_CLEAN_IC資料丟不過去    //JerryYang 20191122 STM 8 site Auto clean支援前後排一起丟
            {                                                                   //KevinC 20251110 : 新增條件 避免特殊關Site時的HAS_NULL_IC資料丟不過去
            ...
            else if(iCloseSiteModeFor2x8==e2x8_TW153Mode)                       //KevinC 20251031 : 新增條件 避免特殊關Site時Auto Clean的HAS_NULL_CLEAN_IC資料丟不過去    //Steven 20220816 : Add for TW153TK spec
            {                                                                   //KevinC 20251110 : 新增條件 避免特殊關Site時的HAS_NULL_IC資料丟不過去
...
void InArmZNeedDown_2x8_8(int iSht, int iKit, bool bPlace)
{
    ...
    if(iCloseSiteModeFor2x8==e2x8_STMMode)                                      //JerryYang 20191122 STM 8 site Auto clean支援前後排一起丟
    {
        ...
        else
        {
            if(iModeCol==300)
            {                                                                   //吸嘴       ==>  蝦頭
                SetInArmNeedDestory(bPlace, 0, 1, 0, 0);                        // O X O X        X O X O X X X X       //KevinC 20251031 : 2 改 1
            ...
            else
            {
                if(TestIF.iAutoClean_Function && bRunAutoClean)
                {
                    ...
                    else if(iShuttleRowKit==1)                                  //20251031 KevinC : 在STMMode下 Auto Clean pick from shuttle 不會進iShuttleRowKit3與4
                    {
                        SetInArmNeedDestory(bPlace, 0, 5, 0, 1);
                        SetInArmNeedDestory(bPlace, 0, 7, 0, 3);
                        SetInArmNeedDestory(bPlace, 1, 4, 1, 0);
        ...
        else
        {
            ...
            else
            {
                if(TestIF.iAutoClean_Function && bRunAutoClean)
                {
                    ...
                    else if(iShuttleRowKit==1)                                  //20251031 KevinC : 在TW153Mode下 Auto Clean pick from shuttle 不會進iShuttleRowKit3與4
                    {
                        SetInArmNeedDestory(bPlace, 0, 4, 0, 0);
                        SetInArmNeedDestory(bPlace, 0, 6, 0, 2);
                        SetInArmNeedDestory(bPlace, 1, 5, 1, 1);
