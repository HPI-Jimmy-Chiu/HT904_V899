...
//==============================================================================
//2012-05-28    Add
bool TMyMN200Motor::ResetPos(int Pulse=0)
{
    long iR1, iR2;
    ...
    if(IO_CARD_TYPE==0 || MOTION_CARD_TYPE==0)                                  //SYN-TEK開卡
    {
        ...
        for(int i=0; i<2; i++)
        {
            if(bfirst)
            {
                ...
                ret =_mnet_save_ring_active_table(i, FilePath.c_str());         //ChungHung 20111101 add 24v壓降
            }
            else
            {
                _mnet_get_slave_error_table(i, ErrorTable[i]);                  //ChungHung 20111101 add 24v壓降
                _mnet_clear_slave_error_flag(i, ErrorTable[i]);
                ret = _mnet_load_ring_active_table(i, DevTable[i], FilePath.c_str());
            }
